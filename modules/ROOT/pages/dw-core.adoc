...
| xref:dw-core-functions-flatten.adoc[flatten] | Helper function that makes `flatten` work with a `null` value.
...
| xref:dw-core-functions-keysof.adoc[keysOf] | Returns an array of keys from the given object.
...
by the `divisor`).
| xref:dw-core-functions-namesof.adoc[namesOf] | Returns an array of strings with the names of all the keys from the given object.
