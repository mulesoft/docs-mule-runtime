...
| xref:dw-core-functions-filterobject.adoc[filterObject] | Iterates over an array of objects and applies an expression that returns only
matching objects, filtering out the rest from the output.
| xref:dw-core-functions-find.adoc[find] | Lists indices where the specified characters of a string are present.
| xref:dw-core-functions-flatmap.adoc[flatMap] | Iterates over each item in an array and flattens the results.
| xref:dw-core-functions-flatten.adoc[flatten] | Flattens an array of arrays into a single, simple array.
...
| xref:dw-core-functions-mapobject.adoc[mapObject] | Helper function that allows `mapObject` to work with null values.
| xref:dw-core-functions-match.adoc[match] | Uses a regular expression (regex) to match a string and then separates it into
...
| xref:dw-core-functions-max.adoc[max] | Returns the highest numeric value in an array.
...
| xref:dw-core-functions-min.adoc[min] | Returns the lowest numeric value in an array.
