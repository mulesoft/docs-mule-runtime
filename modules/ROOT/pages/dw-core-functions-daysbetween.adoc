...
| `to` | To date (a `Date` type). If the `to` date is _earlier_ than the `from` date, the function returns a negative number equal to the number of days between the two dates.
|===
