...
One instance of Mule runtime engine can run multiple applications when deployed on-premises, enabling you to include the same namespaces within different applications that neither collide nor share information, which allows additional advantages such as:
...
When you deploy applications on-premises, every application is deployed together within a *domain*. By default, your application references the `default` domain, and no further configuration is required. You can see that the domain is deployed with the Mule app in the console:
...
Mule application deployment consist of two main aspects:

* The Mule runtime engine instance running in your infrastructure
* The Mule applications deployed to that Mule instance

When you deploy applications on-premises, you are responsible of the installation, execution and management of your Mule runtime engine instances. +
When you deploy applications to CloudHub, the Mule instance is handled in the background by the CloudHub workers. +
When you deploy applications to Anypoint Runtime Fabric, the Mule instance is handled by the container service.

== On-Premises Deployments

All applications in Mule are unpacked at runtime and the original `jar` file is removed. For example, dropping a `jar` file into the `/apps` directory of your Mule instance creates a new folder with the same name (without the `jar` extension) and deletes the `jar`.
