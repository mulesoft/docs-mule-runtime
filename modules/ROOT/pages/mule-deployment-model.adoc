...
The following characteristics are specific of Mule runtime engine instances, they only apply for on-premises deployments where you have complete control of the Mule instance.
...
Mule instances run independently from the Management pane to execute integration logic and serve API requests. This architecture enables you to deploy Mule runtime engine strategically and ensures it is not a bottleneck for any communications. +
