...
| `deferred` | `Boolean` | `false` | When set to `true`, DataWeave passes an output data stream to the next message processor in the flow. Valid values are `true` or `false`.
