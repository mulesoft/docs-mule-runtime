...
|`duplicateKeyAsArray`  |`Boolean`|`false`|If duplicate keys are detected in an object, the writer will change the value to an array with all those values. Valid values are `true` or `false`. Valid Options: `true` or `false`.
...
|`skipNullOn`  |`String`|`null`|Skips `null` values in the specified data structure. By default, it does not skip the values. Valid values are `arrays`, `objects`, or `everywhere`. Valid Options: `arrays` or `objects` or `everywhere`.
|`writeAttributes`  |`Boolean`|`false`|Indicates whether to add attributes of a key to children of the key. The new attribute key name will start with &#64;. Valid options are `true` or `false`. Valid Options: `true` or `false`.
|===
