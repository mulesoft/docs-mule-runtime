...
* Mule 4.9.17 or later (for earlier versions of Mule runtime, use xref:fips-140-2-compliance-support.adoc[FIPS 140-2 Compliance Support])
* Java 17 or later
...
----
+
. Configure the security properties in `java.security`.
+
** Security providers:
+
----
...
[NOTE]
--
On ARM64 (AArch64) architectures, register the `SUN` provider. BC-FJA 2.1.2 doesn't include a `SecureRandom` implementation for AArch64 (its native cryptographic library targets x86_64 only). The JVM's `securerandom.strongAlgorithms` property defaults to `NativePRNGBlocking:SUN` and `DRBG:SUN`. Without a provider that supplies these algorithms, the JVM fails during early bootstrap, before Mule logs initialize.
...
--
+
. Set the key manager and trust manager algorithms:
+
----
...
----
+
For more information about clustering in FIPS mode, see xref:on-premises::mule-high-availability-ha-clusters.adoc#cluster_fips[Cluster reference].
+
