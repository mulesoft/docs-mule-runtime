...
This document assumes you are familiar with Mule ESB and the xref:6.x@studio::index.adoc[Studio interface]. Further, it assumes you have read the xref:mule-message-structure.adoc[Mule Message Structure document] to understand the contents of a Mule message. To increase your familiarity with Studio, consider reading xref:6.x@studio::index.adoc[Anypoint Studio Essentials].
...
|image:set-payload-bb.png[Set-payload-bb] |image:metadata2.png[] |The message leaves the inbound connector with a set of inbound properties that are set by the message source (HTTP) and one outbound property reflecting the default encoding type.
|image:me-5.png[] |image:metadata4.png[] a|The previous message processor set a payload of type List, which DataSense Explorer now reflects as the message arrives at the following message processor.
.3+|image:timestamp.png[]
.3+|image:me-6.png[] |In the previous step, the Variable Transformer set a flow variable called timestamp, which is now visible in the DataSense explorer as the message reaches this flow reference building block. Note that you can expand the timestamp variable to view all the components accessible via the #[server.dateTime] expression used for this variable:
|image:metadata5.png[]
|image:logger1-1.png[] |image:metadata6.png[]
|image:enricherscope.png[]
|Thexref:flow-reference-component-reference.adoc[flow reference] in the previous step sent the message to a new flow.
|image:metadata8.png[] |image:enricher-configuration.png[]
...
|image:logger3.png[] |image:metadata9.png[] |Observe that the flow variable colorchoice, added by the Enricher, now appears in the DataSense Explorer.
...
* xref:5.x@studio::studio-visual-debugger.adoc[Debug mode]
