...
|`com.mulesoft.dw.multipart.defaultContentType`|`String`|`application/octet-stream`|Sets the default Content-Type to use on parts of the `multipart/*` format when a `Content-Type` is not specified.
|===
