...
|`com.mulesoft.dw.multipart.defaultContentType`|`String`|`application/octet-stream`|Sets the default Content-Type used on parts of the `multipart/*` format where a `Content-Type` is not specified.
|===
