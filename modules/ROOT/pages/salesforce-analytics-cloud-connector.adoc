...
NOTE:  Starting with v2.4.0, a retry mechanism was added to the following operations: *Upload External Data*, *Upload External Data Into New Data Set And Start Processing* and *Create Data Set*. The *Number of retries* parameter was added to each operation into the Advanced tab with a default value of 5.
...
. Create a dataset and upload data into it by processing the data in several chunks.
. Add sfdc-analytics namespace to teh Mule element as follows:
+
[source,text]
----
xmlns:sfdc-analytics="http://www.mulesoft.org/schema/mule/sfdc-analytics"
