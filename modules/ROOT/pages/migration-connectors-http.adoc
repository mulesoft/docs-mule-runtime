...
For outbound traffic, that meant that if attachments were present, then each
attachment would be a separate part of a `multipart/form-data` body.
For inbound traffic, it meant that
...
Note that the content type header is autogenerated correctly. For details, see
xref:dataweave-formats#format_form_data[Multipart (Form-Data)].
