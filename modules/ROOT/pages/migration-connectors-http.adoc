...
In Mule 4 to avoid a malformed URI when you make an HTTP request to the HTTP Listener, as recommended by in xref:https://tools.ietf.org/html/rfc1738#page-3[RFC] encode characters such as `{` and `}` that you set in the HTTP request.

Encode these characters both when you configure the request URL in the *Path* field of the HTTP Listener in Studio, and when you send the curl request command. In the following example, you encode the `{` and `}` characters of the URL `path=/profiles/{profileid}/credit`:
...
xref:dataweave-formats-multipart.adoc[Multipart Format (Form-Data)].
== See Also
