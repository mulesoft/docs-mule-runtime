...
In Mule 4 to avoid a malformed URI when you make an HTTP request to the HTTP Listener, encode characters such as `{` and `}` that you set in the HTTP request as recommended in https://datatracker.ietf.org/doc/html/rfc1738[RFC^].
