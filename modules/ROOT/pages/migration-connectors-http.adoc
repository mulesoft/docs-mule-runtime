...
Encode these characters both when you configure request URL in the *Path* field of the HTTP Listener in Studio, and when you send the curl request command. In the following example, you encode the `{` and `}` characters of the URL `path=/profiles/{profileid}/credit`:
