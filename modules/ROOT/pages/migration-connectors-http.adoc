...
In Mule 4 to avoid a malformed URI when you make an HTTP request to the HTTP Listener, as recommended by the RFC encode characters such as `{` and `}` that you set in the HTTP request.

Encode these characters both when you configure request URL in the *Path* field of the HTTP Listener in Studio, and when you send the curl request command. In the following example, you encode the `{` and `}` characters of the URL:

`+http://localhost:8084/test?path=/profiles/{profileid}/credit+`
