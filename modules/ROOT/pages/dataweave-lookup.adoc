...
Which takes two or three parameters:

* The name of the flow that must be called
* The payload to send to this flow
* The timeout, in milliseconds, for the execution of the target flow. If the lookup takes more time than this, an error will be raised. This parameter is optional, with a default value of 2 seconds or 1 minute, depending on the thread that is executing.
