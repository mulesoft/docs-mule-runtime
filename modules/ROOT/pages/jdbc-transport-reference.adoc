...
http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_jdbc/namespace-overview.html[SchemaDoc] |image:check.png[] |image:check.png[] |image:check.png[] |image:check.png[](local,XA) |image:error.png[] |image:check.png[] |one-way, request-response |one-way |org.mule.transport:mule-transport-jdbc
|===
...
Note that stored procedures are only supported on outbound endpoints. If you want to set up a service that calls a stored procedure at a regular interval, you can define a xref:quartz-transport-reference.adoc[Quartz] inbound endpoint and then define the stored procedure call in the outbound endpoint.
...
[source,xml,linenums]
----
<connector name="C3p0Connector" className="org.mule.providers.jdbc.JdbcConnector">   <properties>     <container-property name="dataSource" reference="c3p0DataSource"/>       <map name="queries">         <property name="test1" value="select * from Table1"/>         <property name="test2" value="call testd(1)"/>       </map>   </properties> </connector>
----
