...
|JDBC |https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/transport/jdbc/package-summary.html[JavaDoc] |image:check.png[(tick)] |image:check.png[(tick)] |image:check.png[(tick)] |image:check.png[(tick)](local,XA) |image:error.png[(error)] |image:check.png[(tick)] |one-way, request-response |one-way |org.mule.transport:mule-transport-jdbc
