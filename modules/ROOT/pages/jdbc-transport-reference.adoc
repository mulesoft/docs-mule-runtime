...
|JDBC |[JavaDoc] |check |check |check |check (local XM) |image:error-1.png[] |check |one-way request response |one-way |`org.mule.transport:mule-transport-jdbc`
|===
...
|[Flexible Data Source Configuration] |Support for configuration of data source through JNDI, XAPool, or Spring. |x |x
...
You can reference SQL statement or call a stored procedure on outbound endpoints. Statements on the outbound endpoint get invoked synchronously. SQL select statements or stored procedures may return output that is handled by the ResultSetHandler and then attached to the message as the payload.
...
|sqlCommandRetryPolicyFactory |0..1 |The factory that creates the retry policies which detail if a SQL statements must be re-executed in case of errors.
|===
...
* Put you database connection and credential information in a separate properties file. This allows your port configuration file to different environments. See [Example Configurations] for an example on how this is done.
...
|===
|Attribute |Description
|*create* |If `True` the database will be created upon first access. See the [Derby Documentation] for details.
|*database* |Name of the database to connect to. This attribute cannot be use together with `url` attribute.
|*name* |Unique identifier of the datasource. Use this name to reference the data source from the JDBC connector.
...
|*url* |JDBC URL to use when connection to the database. This attribute cannot be used together with the `database`, `host`, or `port` attribute.
