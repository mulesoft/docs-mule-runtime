...
http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_jdbc/namespace-overview.html[SchemaDoc] |image:check.png[(tick)] |image:check.png[(tick)] |image:check.png[(tick)] |image:check.png[(tick)](local,XA) |image:error.png[(error)] |image:check.png[(tick)] |one-way, request-response |one-way |org.mule.transport:mule-transport-jdbc
|===
