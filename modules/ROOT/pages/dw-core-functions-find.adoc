...
Returns the indices in the text that match the specified regular expression (followed by the capture groups).


The first element in each resulting sub-array is the index in the text that
matched the regular expression (regex), and the next ones are the capture groups in the regex
(if present).
