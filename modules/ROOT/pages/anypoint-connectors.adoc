...
Anypoint Connectors are automatically bundled with Anypoint Studio, and there is a large library of additional connectors available for download in https://anypoint.mulesoft.com/exchange/?type=connector[Anypoint Exchange]. You can also create a xref:3.7@publishing-and-consuming-apis-with-mule.adoc[REST or SOAP API connector], or create your own connectors using xref:3.8@connector-devkit::index.adoc[Anypoint Connector DevKit]. For more information on integration solutions, see the http://www.mulesoft.com/platform/cloud-connectors[Anypoint Connectors] product site.
...
* Contact MuleSoft and the community using the http://help.mulesoft.com[MuleSoft Help Center] – either MuleSoft or a member of the community might be in the process of building the connector you need or can point you to a community-built connector hosted elsewhere.
