...
Using a type parameter, you can write a function or data type generically
to handle values identically, without depending on the type of the values.
...
With `firstNumber` in the body, the script returns the number `1`. If you replace `firstNumber` with `firstString`, the script returns the string `"DataWeave Rules!!!"`.

==== Bind Type Parameters

Type parameters can also be bound to limit the kind of values that can be used.
For example we can append a `{name: "DataWeave"}`
...
Last Type Parameters can be used to Parametrize type definitions

For example we can define a data structure that models file content. One for text files and other for binary files.
The types parameters are going to be used to construct new types that later can be used to dispatch to the correct implementation.
