...
[NOTE]
Although the use of constraints is optional, they can be useful in big scripts or multiple scripts. The type system helps you to avoid bugs in your DataWeave logic because it causes the type-checking algorithm to run. Also, when you write your code in an IDE and define these constraints, the tooling can help you find type checking errors while writing your script, instead of when the script is running and fails.
...
A common pattern is to use `Null` type with a `Union` type to specify that a type accepts `null` as a value. In the following example, the function `parseEmail` allows inputs of type String or Null. This could be the case where you have an object with a optional field `email` as the payload and you call the function with `payload.email`.
...
In the following example, the variable accepts only a value of the literal types `404` and `500`. The type system ensures before runtime that the variable could be only one of those literal values:
