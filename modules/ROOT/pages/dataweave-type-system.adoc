...
DataWeave 2.0 introduced a new type system on the language. Learn how to describe constraint expressions for variables and functions to take advantage of the type checking that the type system executes. To understand how values for these types are defined see xref:dataweave-types.adoc[Value Constructs for Types] documentation.
...
The constraints to call the function `toUser` are that the first argument has to be of type Number, the second one of type String (or to be coercible to those types). Another constraint here is that the result of the function has to be of type String.(this constraint isn't used in the call for `toUser`).
...
Though you would expect the second call to fail, autocoercions allow it to be successful and both of them work. In the case of the second function call, there is an autocoercion of the String `"123"` to the Number value `123`, and the Boolean `true` to the String value `"true"` to being able to call the function. +
...
Note that the use of constraints is optional but this can be useful in big scripts or multiple scripts where the type system could help you avoid bugs in your DataWeave logic by running the type checking algorithm.
...
Simple types represent values such as string, Booleans, etc. These values are atomic, they are not composed by other values. These are the simple types:
...
For example, the syntax to define a variable with the type Array of `Number` is:
...
In order to define an object type there are two ways, either `Object` or `{}`, they both define an open object and doesn't specify any constraint to their key value pairs. Open and close objects are later defined in this section.
...
Open object only put constraints on the fields that are declared in the type, and if they are all verified, then the type accepts the value. The type accept objects that have other fields apart of the ones mentioned in the declaration. All object types are open unless specified.
