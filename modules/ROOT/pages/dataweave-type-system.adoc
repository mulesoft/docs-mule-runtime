...
So if you call the function map with a null (`null map (item, index) -&gt; item`) this does not throw an exception because map is defined for null as its first parameter. The type system chooses the correct function definition with the function dispatching algorithm.
