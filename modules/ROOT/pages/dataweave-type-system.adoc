...
Using Type Parameter, a function or a data type can be written generically 
so that it can handle values identically without depending on their type.

For example we can define a function that returns the first element of an array without loosing the type information.
