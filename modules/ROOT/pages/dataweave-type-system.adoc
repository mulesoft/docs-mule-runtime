...
Simple types represent values such as String, Boolean, etc. These values are atomic, they are not composed by other values. These are the simple types:
...
Null is a type of only one value, which means that `null` cannot be assigned to the type String or any other type except for the type Null.
