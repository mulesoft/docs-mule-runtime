...
By default, this value is set to match the Runtime Manager configuration of OSv2. | No
...
By default, it is set to `false`. | No
| `disableCloudHubLogs` | When set to `true`, the plugin instructs CloudHub to disable CloudHub logging and rely on an application provided log4j2.xml. +
By default, it is set to `false`. | No
|===
