...
By default, it is set to `false`. | No
| `disableCloudHubLogs` | When set to `true`, the plugin instructs CloudHub to disable the CloudHub logging and rely on an application provided by the `log4j2.xml` file. +
By default, it is set to `false`. | No
|===
