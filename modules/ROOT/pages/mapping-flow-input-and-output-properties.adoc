...
NOTE: All data types expose an input property Error Policy, that is used to specify how to handle bad input data in the mapping. For the semantics of Error Policy, see xref:5@studio::datamapper-input-error-policy-for-bad-input-data.adoc[DataMapper Input Error Policy for Bad Input Data].
...
|Error Policy |Input |How to handle bad input data. See xref:5@studio::datamapper-input-error-policy-for-bad-input-data.adoc[DataMapper Input Error Policy for Bad Input Data] for details.
|===
