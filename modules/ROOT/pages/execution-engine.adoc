...
According to this design pattern,  all tasks are classified in categories which correspond to each of our thread pools, and each task is submitted for execution to its corresponding thread pool.
...
In any case, MuleSoft recommends to consult with Support before going into production with customized settings.
