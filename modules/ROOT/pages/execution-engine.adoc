...
the environment where it is running. The default configuration provided has 
been established by doing performance testing to find optimal values for most 
scenarios.
...
The pools can be configured through the well-commented `schedulers.conf` file in the `conf` folder of the standalone Mule EE 
installation. For example, you can modify the formulas for the different kinds 
of pool sizes, however this is not recommended. Note that the configuration is 
global and affects the entire Mule Runtime instance. It is strongly recommended
to perform load and stress testing with all applications involved in real-life 
scenarios to validate any change in the threading configurations and to have an
understanding of how the thread pools work in Mule 4.
...
source. For instance, an `http:listeenr` may return a `503` error code, while 
a message broker listener might give the option to either wait for resources to
be available or drop the message. On some particular case, a source may even 
stop its connection to a remote system to avoid getting more data it won't be 
able to process, to reconnect after some time when the server state is 
normalized.

See the documentation of each connector for details about how 
