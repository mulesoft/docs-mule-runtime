...
Although the thread pool is automatically configured by Mule at startup, applying formulas that consider available resources such as CPU and memory, you can modify these formulas in the `MULE_HOME/conf/schedulers-pools.conf` file. This configuration file contains comments that document each of the properties, but depending on your pool strategy, you can configure specific properties.
...
* If tests confirm that custom configuration is still necessary, contact MuleSoft Support. +
Use a custom configuration only after Support has validated the root cause of the problem. Failing to follow this best practice can lead to inefficient use of resources and might hide underlying issues.
