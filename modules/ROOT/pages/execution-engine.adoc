...
The following table describes the configurable properties and includes the XML attribute for the field.
...
| Keep Alive | `keepAlive` | When the number of threads in the pool is greater than the indicated core pool size, this is the maximum time (in milliseconds) that excess idle threads will wait for new tasks before terminating.
|===
