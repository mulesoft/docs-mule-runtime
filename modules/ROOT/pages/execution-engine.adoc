...
* If tests confirm that a custom configuration is still necessary, it might indicate that there is a problem with the resource handling in the application, one of the application's dependencies or the runtime. 
Note that using a custom configuration can lead to inefficient use of resources and might hide underlying issues.
