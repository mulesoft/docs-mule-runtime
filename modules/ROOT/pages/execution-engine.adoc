...
:page-aliases: intro-engine.adoc
...
details on that mechanism, refer to the xref:mule-sdk::index.adoc[Mule SDK documentation].
...
This single thread pool allows Mule to be efficient, requiring significantly fewer threads (and their inherent memory footprint) to process a given workload when compared to previous versions.

[NOTE]
If you are upgrading from earlier Mule versions, see xref:execution-engine.adoc#threading[Threading in Mule 4.2] to understand the differences applied to the threading model in Mule 4.3.
...
image::mruntime-thread-profiler.png["A table listing thread groups in a profiling tool"]
