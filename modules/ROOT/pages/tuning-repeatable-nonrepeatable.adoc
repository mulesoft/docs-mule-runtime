...
Another parameter that could be set is `eager read`: Defines if the buffer should be read immediately when available (true) or wait for it to be filled (false). Default value is false.
[source,xml,linenums]
----
<file:read path="bigFile.json">
  <repeatable-file-store-stream
...
* `eagerRead` +
Defines if the buffer should be read immediately when available (true) or wait for it to be filled (false). Default value is false.
