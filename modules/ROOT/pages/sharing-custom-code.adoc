...
* Custom transformers - performing operations that none of the Mule stock transformers can perform (see xref:creating-custom-transformers.adoc[here]), +
* Custom components - typically Mule-aware or non-business oriented components, as business components are usually simple POJOs coming from pre-existing projects (see: xref:developing-components.adoc[here]), +
* Custom expression evaluators - for the cases when a specific message extraction capacity is needed in several places of an application (see: xref:creating-non-mel-expression-evaluators.adoc[here]).
