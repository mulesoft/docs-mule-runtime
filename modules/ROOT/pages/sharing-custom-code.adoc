...
* Custom transformers - performing operations that none of the Mule stock transformers can perform (see xref:3.6@creating-custom-transformers.adoc[here]).
* Custom components - typically Mule-aware or non-business oriented components, as business components are usually simple POJOs coming from pre-existing projects (see: xref:3.7@developing-components.adoc[here]).
