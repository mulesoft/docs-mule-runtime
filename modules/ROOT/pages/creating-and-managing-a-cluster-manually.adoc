...
* Some relational databases have certain constraints regarding the name length of tables. To leap over this, use the `mule.cluster.jdbcstoretableNametransformerstrategy` property. +
...
...
----
The table names will be hashed using MD5 and a prefix to identify that these are Mule tables. Hashing table names guarantee that the length constraint is honored.

Optionally, you can create a custom transformer strategy by implementing the interface
`com.mulesoft.mule.cluster.hazelcast.persistence.query.TableNameTransformerStrategy`
