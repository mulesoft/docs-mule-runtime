...
For example, the default value for `maxIdleTime` is 0, which means that idle connections never expire and are not removed from the pool, they are not closed and remain connected to the database in an idle state. +
