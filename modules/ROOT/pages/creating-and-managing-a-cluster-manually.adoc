...
For example, if you set the following property in the `mule-cluster.properties` file:
[source]
----
...
----
Mule hashes table names using MD5 and a prefix to identify that these are Mule tables. Hashing table names guarantees that the length constraint is honored.

Optionally, you can create a custom transformer strategy by implementing the `com.mulesoft.mule.runtime.module.cluster.api.persistence.query.TableNameTransformerStrategy` interface.
