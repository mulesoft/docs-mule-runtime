...
You can persistently store JDBC data in a centralized system, accessible from all cluster nodes when using when running Mule runtime engine on premises. Object store persistence is not currently supported for Mule applications deployed on Runtime Fabric. 
