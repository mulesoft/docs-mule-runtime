...
[NOTE]
====
All nodes in a cluster must have the same Mule runtime engine and Runtime Manager agent version. If you are using a cumulative patch release, such as 3.9.5-20210322, all instances of Mule must be the same cumulative patch version. 
====
...
[CAUTION]
--
Oracle database support is a xref:release-notes::mule-runtime/mule-3.8.0-release-notes.adoc#enterprise-edition-known-issues-and-limitations[known limitation] for Mule 3.8.x and it is planned for a future release.
--
