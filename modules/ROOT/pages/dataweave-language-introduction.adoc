...
+
Valid values: xref:dataweave-formats.adoc[Data Formats Supported by DataWeave].
+
...
you are using in an input string. The output string only escapes characters
in the string that the output format (such as `appliation/json`,
`appliation/xml`, or `application/csv`) requires you to escape, as the
examples show.
...
The JSON output also escapes these quotation marks:
...
In the following XML example, the output does not escape the double quotation
marks.
...
The DataWeave script shows all three escape scenarios:
