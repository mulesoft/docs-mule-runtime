...
Reserved.
* `case`: +
Reserved for `case` statements, for example, to perform xref:dataweave-pattern-matching[pattern matching]
or with the xref:dw-operators.adoc#update-operator[`update` operator].
* `default`: +
...
* `else`: +
Reserved for use in if-else and else-if statements. See xref:dataweave-flow-control.adoc[Flow Control in DataWeave].
* `enum`: +
Reserved.
* `false`: +
...
Reserved.
* `fun`: +
Reserved for function definitions in the <<dw_header>>.
* `if`: +
Reserved for if-else and else-if statements. See xref:dataweave-flow-control.adoc[Flow Control in DataWeave].
* `import`: +
Reserved for the `import` directive in the <<dw_header>>.
* `input`: +
Reserved.
* `is`: +
Reserved.
* `ns`: +
Reserved for namespace definitions in the <<dw_header>>.
* `null`: +
Reserved as the value `null`. See dataweave-types.adoc#dw_type_null[Null (dw::Core Type)].
* `or`: +
Reserved as a xref:dw-operators.adoc#logical_operators[logical operator].
* `output`: +
Reserved for the `output` directive in the <<dw_header>>.
* `private`: +
Reserved.
* `throw`: +
Reserved.
...
Reserved as a Boolean value.
* `type`: +
Reserved for type definitions in the <<dw_header>>.
* `unless`: +
Reserved.
* `using`: +
Reserved for backwards compability and replaced by `do`.
See dw-operators.adoc#scope-and-flow-control-operators[Scope and Flow Control Operators].
* `var`: +
Reserved for type definitions in the <<dw_header>>.
* `yield`: +
Reserved.
