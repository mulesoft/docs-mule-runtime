...
Reserved for internal purposes.
* `fun`: +
Used for namespace definitions in the <<dw_header>>.
* `if`: +
Used in if-else and else-if statements. See xref:dataweave-flow-control.adoc[Flow Control in DataWeave].
* `import`: +
Used for the `import` directive in the <<dw_header>>.
* `input`: +
Reserved for internal purposes.
* `is`: +
Reserved for internal purposes.
...
Reserved as a xref:dw-operators.adoc#logical_operators[logical operator].
* `output`: +
Used for the `output` directive in the <<dw_header>>.
* `private`: +
Reserved for internal purposes.
* `throw`: +
Reserved for internal purposes.
...
Reserved as a Boolean value.
* `type`: +
Used for type definitions in the <<dw_header>>.
* `unless`: +
Reserved for internal purposes.
* `using`: +
