...
2.0), you need to escape any backtick that is part of the string, for example,
``abc\`def``.
...
* It must begin with a letter of the alphabet (a-Z).
* After the first letter, it can contain any combination of letters, numbers and underscores (`_`).
* Cannot match any DataWeave reserved keyword (see <<reserved-keywords>> for a complete list).
