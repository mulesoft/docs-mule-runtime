...
[WARNING]
Some components that accept DataWeave code might cause an error in Anypoint Studio if the first line of the DataWeave script's body is a single-line comment. To avoid this, you can either use multi-line comments or move the single-line comment to a different code line after the first line of the DW script's body.
