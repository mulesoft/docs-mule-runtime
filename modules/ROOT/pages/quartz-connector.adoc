...
Note that you also have the option to use a Poll scope at the beginning of a flow, which also supports Cron expressions and watermarks. Read more about the xref:3.6@poll-reference.adoc[Poll scope].
...
You can install a connector in Anypoint Studio using the instructions in xref:3.8@installing-connectors.adoc[Installing a Connector from Anypoint Exchange].
...
|*Enable default events tracking* |Enable default xref:3.6@business-events.adoc[business event] tracking for this endpoint.
|===
...
* See the xref:quartz-transport-reference.adoc[Quartz Transport Reference] for details on setting the properties for a Quartz endpoint using an XML editor.
...
* Read about the xref:poll-reference.adoc[Poll scope].