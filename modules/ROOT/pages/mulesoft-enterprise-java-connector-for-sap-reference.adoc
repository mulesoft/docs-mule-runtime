...
If `evaluateFunctionResponse` is set to *true*, then the SAP outbound endpoint throws an exception when the return type is *A*, *E*, or exceptions are present.
====
...
[source,text,linenums]
----
String tid = destination.createTID();try{    JCoContext.begin(destination, tid);    function1.execute(destination, tid);    function2.execute(destination, tid);}finally{    JCoContext.end(destination);}
----
...
[source,text,linenums]
----
String tid = destination.createTID();try{    JCoContext.begin(destination, tid);    function1.execute(destination, tid, queueName1);    function2.execute(destination, tid, queueName2);}finally{    JCoContext.end(destination);}
----
