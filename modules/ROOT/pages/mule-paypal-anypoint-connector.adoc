...
This document assumes you are familiar with Mule, xref:3.7@connector-devkit::index.adoc[Anypoint Connectors] and xref:5@studio::index.adoc[Anypoint Studio Essentials]. To increase your familiarity with Studio, consider completing the xref:general:getting-started:anypoint-connector.adoc[Anypoint Connector quickstart]. See also xref:elements-in-a-mule-flow.adoc[Elements in a Mule Flow] and xref:global-elements.adoc[Global Elements].
...
To use the PayPal Connector in your Mule application, you must configure a global PayPal element that can be used by all the PayPal connectors in the application (read more about xref:global-elements.adoc[global elements]).
...
The placeholder values refer to a configuration file placed in the *src* folder of your project. Learn how to xref:configuring-properties.adoc[configure properties]. You can either enter your credentials into the global configuration properties, or reference a configuration file that contains these values. For simpler maintenance and better re-usability of your project, Mule recommends that you use a configuration file. Keeping these values in a separate file is useful if you need to deploy to different environments, such as production, development, and QA, where your access credentials differ. See xref:deploying-to-multiple-environments.adoc[Deploying to Multiple Environments] for instructions on how to manage this.
+
