...
image:javascript-transformer-24x16.png[] |JavaScript |Implements a JavaScript transformer backed by a JavaScript script engine |xref:3.7@script-transformer-reference.adoc[Script Transformer Reference]


image:ruby-transformer-24x16.png[] |Ruby |Implements a script transformer backed by a Ruby script engine |xref:3.7@script-transformer-reference.adoc[Script Transformer Reference]
...
|image:java-transformer-24x16.png[] |Java |Transforms the data from one format to another |xref:3.6@java-transformer-reference.adoc[Java Transformer Reference]
...
|image:transformer-24x16.png[Transformer-24x16] |Object-to-XML |Converts a Java Object into XML code using XStream |xref:3.7@object-to-xml-transformer-reference.adoc[Object-to-XML Transformer Reference]
...
|  |Content +
Transformer |Description |Documentation
|image:transformer-24x16.png[Transformer-24x16] |Append string |Appends a string to a message payload |xref:3.6@append-string-transformer-reference.adoc[Append String Transformer Reference]


|image:transformer-24x16.png[Transformer-24x16] |Expression |Evaluates one or more expressions within the message, then transforms the message according to the results of its evaluation |xref:3.7@expression-transformer-reference.adoc[Expression Transformer Reference]


|image:transformer-24x16.png[Transformer-24x16] |XSLT |Transforms XML using XSLT |xref:3.7@xslt-transformer-reference.adoc[XSLT Transformer Reference]
|===
...
|  |SAP-Specific +
 Transformer |Description |Documentation
|image:transformer-24x16.png[Transformer-24x16] |SAP-Object-to-XML *Enterprise Edition* |Transforms a SAP object representing a JCo function or IDoc document into its XML representation |xref:3.6@sap-connector.adoc[SAP Connector]


image:transformer-24x16.png[Transformer-24x16] |XML-to-Function (BAPI) *Enterprise Edition* |Reads the XML representing a JCo function from java.io.InputStream, java.lang.String or byte[] to build the SAP object expected by the SAP transport |xref:3.6@sap-connector.adoc[SAP Connector]
...
|image:transformer-24x16.png[Transformer-24x16] |Attachment |In contrast to the *Message Enricher Scope* or the *Append String Transformer*, the *Attachment Transformer* does not add to the string that typically composes the main data payload. Instead, this transformer specifies an attachment to append to each message being processed through the flow. If the name or the value of the attachment is defined through an expression, the exact identity (and content) of the attachment can be calculated at run-time, with the possibility that each message will receive a different payload. Typically, this attachment is treated as a separate, secondary part of the outbound payload. |xref:3.7@attachment-transformer-reference.adoc[Attachment Transformer Reference]


|image:transformer-24x16.png[Transformer-24x16] |Property |This transformer allows you to specify a property, which is typically applied to the message header. The "life span" of such a property extends from the moment it is created until the message is passed to an outbound endpoint. |xref:3.6@property-transformer-reference.adoc[Property Transformer Reference]


|image:transformer-24x16.png[Transformer-24x16] |Session Variable |This transformer resembles the Variable transformer, except the Session Variable set by this transformer persists as long as the associated message remains within the Mule application, even though the message may be processed through multiple flows. |xref:3.6@session-variable-transformer-reference.adoc[Session Variable Transformer Reference]
|===


=== Custom Transformers

For detailed information on configuring standard and custom Transformers with an XML editor, see xref:3.6@using-transformers.adoc[Using Transformers].

=== Deprecated HTTP Related Transformers

The transformers below are all related to the deprecated xref:3.6@http-connector-deprecated.adoc[endpoint-based HTTP Connector]. They have all been made unnecessary, as the new xref:3.6@http-connector.adoc[HTTP Connector]'s features cover the use cases these were built to serve.
