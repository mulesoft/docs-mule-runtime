...
The DataWeave script outputs the JSON object in the `application/dw` format
and MIME type.
...
Notice that only the keys of the JSON input and the `application/dw` output differ. The JSON keys are surrounded by quotation marks. The DataWeave (dw)
keys are not.
...
The following examples show uses of the Text Plain format.
...
The DataWeave script outputs the Text Plain data in the DataWeave (dw) format. It returns a `String`.
...
The DataWeave script outputs the `text/x-java-properties` content the the DataWeave (dw) format.
