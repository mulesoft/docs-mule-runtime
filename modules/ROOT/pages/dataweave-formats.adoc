...
Scripts that use the `read` or `readUrl` methods can also specify reader properties through the `readerProperties` parameter. See examples in xref:dw-core-functions-read.adoc[read].
