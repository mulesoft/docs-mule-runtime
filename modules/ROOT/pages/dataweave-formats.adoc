...
You can specify whether this setting generates an outbound message that contains fields
null values or if these fields are ignored entirely. This can be set
through an attribute in the output directive named `skipNullOn`, which can be
set to three different values: `arrays`, `objects`, or `everywhere`.

When set to:

* `arrays`: Ignore null values in an array. The output omits the `null` value.
* `objects`: Ignore an object that has a null value. The output contains an empty object (`{}`) in place of the object with the null value.
