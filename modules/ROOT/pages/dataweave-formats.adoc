...
To demonstrate a use of the `streaming` property, the following example shows a Mule flow that streams a CSV file and transforms it to JSON.
...
* The example configures the Read operation (`<file:read/>`) to stream the CSV input
by setting `outputMimeType="application/csv; streaming=true"`. The input CSV file
is located in the project directory, `src/main/resources`, which is the location of
`${app.home}`.
...
When defining an input of type dw, there are a few optional parameters that can be set.

//TODO: COPYEDIT TABLE
[cols="2,1,1,2", options="header"]
|===
|Parameter |Type |Default|Description
//TODO QUESTION: onlyData: What's "this DataWeave" mean? This DW script?
|`onlyData`|Boolean|false|Indicates that this DataWeave will have only data. Valid Options are `true` or `false`. |`externalResources`|Boolean|false|Allow to read external entities on readUrl. *Valid Options*: true or false
...
| `indent` | String | *TODO* | The string to use for indentation.
...
The following YAML snippet serves as an input payload for the DataWeave script
in this example.
...
The output shows how the YAML input is represented by the DataWeave (dw) format.
