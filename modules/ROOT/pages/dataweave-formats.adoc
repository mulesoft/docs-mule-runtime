...
* It configures the HTTP listener to stream the XLSX input
...
select the values of each cell in the row (using `row.a`, `row.b`).
...
| `streaming` | Boolean | `false` a| Introduced in Mule 4.2.2: Indicates whether
to read the input Excel file as a stream of data. Streaming is intended for
processing a large file in which case the reader accesses each row sequentially,
keeping one row in memory at a time, instead of making all data available at
once. So, streaming does not permit random access to rows in the file.
