...
By default, the CSV reader stores input data in a file in-memory, but for large
files, you can instead set a `streaming` property to improve performance. To
illustrate streaming, the next example streams a CSV file and transforms it
to JSON:

* It configures the HTTP listener to stream the CSV input
by setting `outputMimeType="application/csv; streaming=true"`.
In the Studio UI, you can use the *MIME Type* on the listener to `application/csv`
and the *Parameters* for the MIME Type to *Key* `streaming` with *Value* `true`.
It also sets `header=true` for illustration purposes, though this setting is the
default.
* It uses a DataWeave script with the `map` function in the *Transform Message*
component to iterate over each row in the CSV payload and selects the value of
each field in the `zip` column by using `row.zip`.
...
Note that example indicates that the input file is located in Studio project
directory `src/main/resources`, which is the location of `${app.home}`.

The structure of the CSV input looks something like this, though a streamed file
is typically much longer because you usually stream to enhance performance
...
The Write operation in the example returns a file `output.json` that
...
See the <<stream_csv, streaming example>> above, and see
xref:dataweave-languag-introduction#dw_readers_writers[DataWeave Readers and Writers].
|===
...
To illustrate streaming, the following example streams a JSON file, which reads
each element in an array one at a time, and transforms the JSON to XML:

* It configures the HTTP listener to stream the JSON inputi
by setting `outputMimeType="application/json; streaming=true"`.
In the Studio UI, you can use the *MIME Type* on the listener to `application/json`
and the *Parameters* for the MIME Type to *Key* `streaming` and *Value* `true`.
* It uses a DataWeave script in the *Transform Message* component to iterate
over the JSON payload and transform it to XML.
...
The JSON input array something like this, though a streamed file is
typically much longer because you usually stream to enhance performance
when reading a large file.

.CSV Input for Streaming Example (truncated):
[source,json,linenums]
----
TODO
----
