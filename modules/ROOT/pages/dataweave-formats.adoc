...
By default, the CSV reader stores input data from an entire file in-memory as
it reads the file, but for large files, you can improve the performance of the
reader by set a `streaming` property to `true`. To demonstrate the use of
...
The Write operation in the streaming example returns a file, `output.json`,
...
xref:dataweave-language-introduction.adoc#dw_readers_writers[DataWeave Readers and Writers].
|===
...
* It configures the HTTP listener to stream the JSON input
...
* It uses a DataWeave script in the *Transform Message* component to
select and iterate over the array in the JSON payload.
...
The Write operation in the streaming example returns a file, `output.json`,
which contains the content of the transformation. The Logger in the JSON
example prints same output.
