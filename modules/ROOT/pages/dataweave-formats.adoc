...
In many cases, the DataWeave writer and the specified output formats are the same. In such cases, DataWeave uses the default writer for the output format, such as the `json` writer for `application/json` and the `xml` writer for `application/xml` output. 
