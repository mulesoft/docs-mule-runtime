...
== Specify a DataWeave Writer Using `with`

In many cases, the DataWeave writer and the specified output formats are the same. In such cases, DataWeave uses the default writer for the output format, such as the `json` writer for `application/json` and the `xml` writer for `application/xml` output. In these cases, you can specify the directive `output application/json` or `output application/xml` in the header of your DataWeave script, or you can specify `output json` or `output xml`, as decribed in <<set_mime_types>>.

Starting in Mule 4.3.0, DataWeave enables you to use the `with` keyword to separate the output format from the DataWeave writer for the script's output. For example, you can specify 'output application/csv with binary' to output the `application/binary` format when using the `binary` writer. This setting is necessary in cases shown in xref:dataweave-cookbook-set-reader-writer-props.adoc#ex04[Use a Mule Variable as a Configuration Value] and xref:dataweave-cookbook-set-reader-writer-props.adoc#use-reader-and-writer-properties-in-dataweave-functions[Use Reader and Writer Properties in DataWeave Functions]. 

Another use of `with` outputs a format that is similar to the format that the writer recognizes, such as `output application/problem+json with json`, as described in <<reader_writer_properties>>.
