...
Scripts that use the `read` or `readUrl` methods can also specify reader properties through the `readerProperties` parameter. See examples in xref:dw-core-functions-read.adoc[read]. Use of reader properties is more common in message sources, such as the HTTP listener, as shown in the <<example_csv_reader, CSV reader example>>. 
