...
Starting in Mule 4.3.0, the `output` can be set using just the format ID instead
of it's specific MIME type. For example, you might set the `output` header directive
of an expression in the Transform Message component or a Write operation to
`output json` or `output csv`. This can also be used to indicate that, while the
data should be written using a certain format, the MIME type should be different.
For example, you could write JSON data but customize the MIME type to
 `application/problem+json` saying `output application/problem+json with json`.
 See xref:dataweave-cookbook-change-script-output-mime.adoc[Change Your Script Output Mime Type]
 for examples on how to customize the mime type output of a script.
