...
The following DataWeave script processes the input data.
...
For more on streaming in DataWeave, see <<dw_readers_writers>>.
|===
...
The DataWeave script outputs the Excel input payload in the DataWeave (dw) format.
...
//TODO: VERIFY THAT FIXED WIDTH SHARES SAME CONFIG PROPERTIES AS FLAT FILE
...
//TODO: DISCREPANCIES BETWEEN GENERATED TABLE AND SOSNOSKY
[cols="1,1,1,3a", options="header"]
|===
| Parameter | Type | Default | Description
...
//TODO: CHECK ON NEW trimValues, whether valid or not.
//TODO: need to list all new properties.
...
TODO: Json Data Format values are mapped one to one with DataWeave values.
Json supports `String`, `Boolean`, `Number`, `Null`, `Object` and `Array`
and DataWeave supports each of this values natively.

There are three parsing strategies for Json

* in memory: This strategy is being used when data that is being parsed is consider small
* indexed: This strategy is being used when data that is being parsed is consider big.
In this the document is going to be indexed so that random access is being supported without running out of memory.
* streaming: This strategy needs to be enable through reader properties but allows to access the document in a secuential without the need
of indexing the entire document.
...
TODO: This example shows how a JSON values are represented in DataWeave
...
TODO: Xml encodes collections using repeated elements (UNBOUNDED elements) this is represented in DataWeave by repeating the same key.
This example shows how to convert a Xml repeated elements to Json
...
TODO: This example shows how a Text Plain is represented in DataWeave
...
TODO: This format parses any java properties file. This format is being used to represent simple
key value pairs and they are represented in DataWeave as an `Object` whose values are all `String` type.
...
TODO: This example shows how properties file are being represented in DataWeave
...
TODO: Url Encoded data represents a collection key value pairs. This are being represented in DataWeave as an Object where each value is of type `String`
...
The following examples show uses of the `x-www-form-urlencoded` format.

=== Example

TODO

This example shows how a simple form-urlencoded data is being represented in DataWeave
...
The following examples show uses of the YAML format.

===== Example: Represent YAML value in DataWeave

This example shows how a YAML values are represented in DataWeave.
...
The following DataWeave script sets the outputs the YAML payload in the `application/dw` format and MIME type.
