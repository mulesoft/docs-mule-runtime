...
The `schemaUrl` property in the header of this DataWeave script passes a schema (`schema.json`). The DataWeave writer uses the schema to structure content from the body of the script and output the results in Avro format.
...
You can customize the parsing instructions by appending optional parameters to the `output` directive's value.
...
You can set the following property to help the DataWeave reader parse the input data correctly.
...
You can set the following property to help the DataWeave reader parse the input data correctly. For details, see <<reader_properties_flat_file>>.
...
You can customize the parsing instructions by appending optional parameters to the `output` directive's value. For details, see <<writer_properties_flat_file>>.
...
Note that you can define a CSV type by using one of the following methods:

* By providing a sample CSV file.
* Through the graphical editor, which enables you to specify each field manually.
+
...
The example shows how DataWeave represents CSV data.
...
You can set the following properties to help the DataWeave reader parse the input data correctly.
...
=== Writer Properties (for DataWeave)
...
| `deferred` | Boolean | `false` | Property for deferred output.
  Valid values are `true` or `false`.
| `ignoreSchema` | Boolean | `false` | Indicates whether the writer will
ignore the schema. Valid values are `true` or `false`.
...
| `streaming` | Boolean | `false` | Introduced in Mule 4.2.2: Streaming is intended for
...
| `zipBombCheck` | Boolean | `true` | If set to `false`, the zip bomb check is
turned off. Valid values are `true` or `false`.
|===
...
line. Valid values are `true` or `false`.
| `tableOffset` | String | None | The position of the first cell in the table
...
You can customize the parsing instructions by appending optional parameters to the `output` directive's value. For details. see <<writer_properties_flat_file>>. All of the properties are optional.
...
| `schemaPath` | String | None | Schema definition. Location in your local disk
...
|===
|Parameter |Type |Default|Description
| `maxEntityCount` | Number | `1` | The maximum number of entity expansions. The limit is provided to avoid the Billion Laughs attacks.
|===
...
| `deferred` | Boolean | `false` | Property for deferred output.
  Valid values are `true` or `false`.
| `skipNullOn` | String | None | Skips null values in the specified data
structure. By default it does not skip. Valid values are
