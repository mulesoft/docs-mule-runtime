...
Starting in Mule 4.3.0, you can set the `output` using the format ID alone
instead of using its full MIME type. For example, you might set the `output` header directive
of an expression in the Transform Message component or a Write operation to
`output json` or `output csv`. You can also use the format ID when you want to
...
For example, you can write JSON data but customize the MIME type to
 `application/problem+json` with the DataWeave directive `output application/problem+json with json`.
