...
////
REMOVE TABLE AND REPLACE WITH LINK TO COPYBOOK PROPERTIES:
[%header]
|===
|Parameter |Type |Default|Description
...
////
REMOVE AND REPLACE WITH LINK ABOVE
[%header]
|===
|Parameter |Type |Default|Description
...
| `enforceRequires` | Boolean | `false` a| Error if required value missing.
  Valid options: `true` or `false`. Mule 3 support: Version 3.9.3 and later.
...
select that one through this field. Mule 3 support: Version 3.9.3 and later.
...
|===
| Parameter | Type | Default | Description
| `bufferSize` | Number | `8192` a| Size of the buffer writer. Mule 3 support: Version 3.9.3 and later.
| `deferred` | Boolean | `false` a| For deferred output. Mule 3 support: Version 3.9.3 and later.
...
| `recordTerminator` | String | None a| Record separator line break. In Mule
3.8.4 and earlier, this is only used as a separator when there are multiple
records. Mule 3 support: Version 3.9.3 and later.
...
`false`.  Mule 3 support: Version 3.9.3 and later.
|===
