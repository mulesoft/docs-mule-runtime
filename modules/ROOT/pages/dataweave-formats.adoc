...
* USAGE of DISPLAY, BINARY (COMP), COMP-5, and PACKED-DECIMAL (COMP-3) (see
character encoding restrictions in section below)
...
REDEFINES allows dynamic interpretation of data in a record. When you import a
copybook with REDEFINES present, the generated schema uses a special grouping
with the name '*' (or '*1', '*2', etc. if multiple REDEFINES groupings are present
at the same level) to combine all the different interpretations. You use this
special grouping name in your DataWeave expressions just as you would any other
grouping name.

Use of REDEFINES groupings has higher overhead than normal copybook groupings,
so it's recommended that you remove REDEFINES from your copybooks where possible
before you import them in Studio.
...
these usages (since they're not single-byte), and ASCII is also not supported
(since it doesn't use the entire range). Supported character encodings include
ISO-8859-1 (an extension of ASCII to full 8 bits) and other 8859 variations and
EBCDIC (IBM037).

REDEFINES requires you use a single-byte-per-character character encoding for
...
| `segmentIdent` | String | None a| Segment identifier in schema for fixed width or copybook schemas (only needed when parsing a single segment/record definition, and if the schema includes multiple segment definitions).
...
* `zeroes`: Fill numeric fields with '0' characters and
character fields with space characters
...
| `segmentIdent` | String | None a| Segment identifier in schema for fixed width or copybook schemas (only needed when writing a single segment/record definition, and if the schema includes multiple segment definitions).
| `structureIdent` | String | None a| Structure identifier in schema for flatfile schemas (only needed when writing a structure definition, and if the schema includes multiple structure definitions)| `trimValues` | Boolean | `false` a| Trim string values longer than field
