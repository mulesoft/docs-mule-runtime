...
[[stream_csv]]
To illustrate streaming, the next example streams an CSV file and
...
over each row CSV payload and select the values of each cell in the row (using
`row.a`, `row.b`).
...
is typically much longer because you usually only stream to enhance performance
...
See the <<stream_csv, streaming example>>.
|===
...
To illustrate streaming, the next example streams an JSON file and
