...
You can use `skipNullOn` to specify whether JSON output contains 
null values or if it omits null values from arrays, objects, or both.

When set to:

* `arrays`: Ignore and omit `null` values.
...
* `everywhere`: Apply `skipNullOn` to arrays and objects.
