...
CSV reader doesn't keep all the input in memory, if the input is larger than the set threshold (1.5 MB) it writes it to disk. 
...
of the reader by set a `streaming` property to `true`. To demonstrate the use of
