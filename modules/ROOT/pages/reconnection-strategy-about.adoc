...
[NOTE]
Operation executions don't trigger the reconnection strategy configured at the FTP connector when failing with connectivity errors. In this example, the reconnection strategy only applies to the connection that the `ftp:listener` needs to establish with the FTP server to query for new or updated files.
...
image::mruntime-reconnection-strategy-ftp.png["An FTP configuration with reconnection strategy"]
