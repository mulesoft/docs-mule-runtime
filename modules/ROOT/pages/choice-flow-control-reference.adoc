...
In Studio, drag the *Choice* icon from the Studio palette to the canvas, positioning it within the sequence of xref:elements-in-a-mule-flow.adoc[building blocks] that form the flow (below).
...
+
image::hola-amigos.png[HolaAmigos]
...
+
image::var-set-lang-to-env.png[VarSetLangToEnv]
