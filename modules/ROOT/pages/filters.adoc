...
|image:filter-24x16.png[Filter-24x16]  |Custom |References a user-implemented filter class. |xref:custom-filter.adoc[Customer Filter Reference]
...
|image:filter-24x16.png[Filter-24x16]  |Schema Validation |Uses the JAXP libraries, to validate a message against a schema. |xref:schema-validation-filter.adoc[Schema Validation Reference]
...
This section covers only those configuration activities common to all filters. For configuration activities that apply only to individual filters, click one of the links in the Reference column of the table xref:filters.adoc[Bundled Filters].
