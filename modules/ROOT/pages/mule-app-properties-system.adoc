...
For example, to set Mule's encoding, add the following line to the Wrapper configuration file:
...
+
If you add `wrapper.java.additional.<n>` in an include file, configure a high `<n>` value to ensure that they are outside the standard sequence. Also, ensure that the `wrapper.ignore_sequence_gaps` property is set to `true`. For example:
+
```
