...
* *In memory*: Content being stored is just in memory. If the runtime goes down you lose your data. 
* *Persistent*: Object Store data goes to disk, therefore it is possible to restart the Mule runtime and content persists.
* *Shared*: If a cluster is running, the content of the Object Store persists across cluster nodes. When a new node spins up, it will have access to the already existing content.
* *External OS implementation*: A user can use external implementation as a backend for Object Store. Examples include MySQL database or Redis.
...
* *Persistent*: Content is stored in an outside persistence layer and data survives runtime restarts or application redeployments. The outside persistence layer can be one of the following: Cloud OSv1 (Cloudhub-managed MySQL database) or Cloud OSv2 (API service built on top of AWS DynamoDB).
* *Shared*: Mule workers in CloudHub are not in a cluster, therefore they do not share memory the same way as on-premises. However, if the application is configured to use OSv2, then the object store data is stored in the cloud and the data is shared across workers.
* *External OS implementation*: This is the same as on-premises - a user can use external implementation as a backend for Object Store. Examples include MySQL database or Redis.
