...
To start validating your application's performance, first understand the prerequisites to testing, follow the recommended best practices, and then execute the performance testing.


== Prerequisites to Perform Test Validations

Consider the following points before testing the performance:
...
The components used for testing make a big difference for your use case:

* Choose the most suitable load injector and a backend service configured with the same setup as your customer environment to get real performance data.  +
...
* After you configure the tools, stick to them for consistent comparisons.
...
When you test the performance of your application, it is important that you mimic real-world customer use cases. Therefore, to obtain customer scenarios, analyze:
...
* Make sure to create different scenarios with a repeatable, common, and high load in order to stress Mule, but not too high that pushes Mule to the limit and causes test failure.
...
The combination of these factors may affect the performance of your Mule app.

== Execute Performance Test Validations

Considering the best practices from the previous section, execute the testing:
...
To avoid affecting the throughput calculation results remove the ramp-up, tear-down, and error percentage.
