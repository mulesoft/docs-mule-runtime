...
then provide and save the following sample content to the `myJSON.json` tab:
+
[source,json,linenums]
----
{
...
+
* By adding a `myInput` variable that supplies an input data the script can read.
* By changing the `payload` variable in the body of the script to the `myInput`
variable.
+
[source,DataWeave, linenums]
----
