...
sample data. It describes a _limited use_ of the Transform Message component 
to help you get started with the language (as opposed to 
xref:dataweave-quickstart.adoc[Mule app development]) and does not attempt 
to cover every DataWeave use case. For more information, 
see xref:transform-component-about.adoc[Transform Message], and note that 
many Mule connectors, modules, and components beyond Transform Message
(see xref:mule-application-about.adoc[Mule Applications]) accept short 
DataWeave expressions in their *fx* fields. 
