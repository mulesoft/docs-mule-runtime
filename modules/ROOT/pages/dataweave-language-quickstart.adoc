...
(`myJson`) to get averages of two sets of numbers.
+
...
+
Learn more about the xref:dw-core-functions-contains.adoc[contains] function and about the built-in `payload` variable in xref:dataweave-variables-context.adoc[Mule Runtime variables].
+
...
the scope of this tutorial to describe that process. To get started with 
Mule app development and data mapping, see the 
xref:dataweave-quickstart.adoc[Mule App Tutorial]. And note that some 
useful Mule components for creating sample data in a running Mule app 
that does not depend on external data sources are 
xref:set-payload-transformer-reference.adoc[Set Payload] to create
a payload, xref:variable-transformer-reference.adoc[Set Variable]
to create a Mule variable, and a xref:scheduler-concept[Scheduler]
to trigger the regular generation of a Mule events to hold data
