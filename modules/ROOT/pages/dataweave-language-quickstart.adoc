...
of named parameters in anonymous functions (such as,
`(item) -> { author : item.author }`); see xref:dataweave-functions-lambdas.html#_use_named_parameters_in_a_lambda[Use Named Parameters in a Lambda].
+
.. Preview the output:
+
[source,json,linenums]
