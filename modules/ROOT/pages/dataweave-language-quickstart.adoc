...
not attempt to cover every use of DataWeave. For more complete information about the component, see xref:transform-component-about.adoc[Transform Message].)
...
----
+
image::dw-payload-example.png[Script Without Input Payload]
+
[[modify_payload]]
...
+
Note that some examples require you to provide your own input data, which you can do through a variable (`var`) in the header of your DataWeave script (as shown <<modify_example, the previous step>>) or through a file that you read, as shown in the `readUrl` example. You might also need to change the
built-in `payload` variable in some scripts into variable name.
+
