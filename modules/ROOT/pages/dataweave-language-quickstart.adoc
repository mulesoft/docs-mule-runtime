...
to cover every DataWeave use case. Also note that beyond Transform Message,
many Mule connectors, modules, and Core components accept short DataWeave 
expressions in their *fx* fields. To learn about them, you can start with 
...
+
. Now try this simple DataWeave script, which outputs the value of the DataWeave
variable `myJson`.
...
----
+
Learn more with xref:dataweave-language-introduction.adoc[DataWeave Scripts]
...
you can see in the *Preview* pane. Learn more about the 
xref:dw-core-functions-avg.adoc[avg] function.
+
.. Preview the output:
+
[source,json,linenums]
...
----
+
Learn more about the xref:dw-core-functions-read.adoc[read] function,
...
----
+
Learn more about the xref:dw-core-functions-zip.adoc[zip] and
...
----
+
Learn more about the xref:dw-core-functions-map.adoc[map] and
xref:dw-core-functions-filter.adoc[filter] functions,
xref:dataweave-types-coercion.adoc[Type Coercion with DataWeave] (for
`as Number`), and 
xref:dataweave-variables#example_local_variable[Examples: Local DataWeave Variables] 
(for `using (id = item.bookId)`). Also notice the
...
+
* Add a `myInput` DataWeave variable that supplies an input data the script can read.
...
Find many more examples to try in Studio here:
...
NOTE: To modify examples that use the built-in Mule variable, `payload`, you 
can provide sample input data through a DataWeave variable (`var`) in the header 
of your DataWeave script (as shown <<modify_payload, the last step>>) or from a
file that you read into a DataWeave variable, as shown in the <<readurl, readUrl>>
example, then replace `payload` with the DataWeave variable name. 
+
If you instead want to create an example that uses `payload`, you need to 
create a Mule app that can generate the payload in a Mule message. It is 
beyond the scope of this tutorial to describe that process. To get started 
with Mule app development and data mapping, see the 
xref:dataweave-quickstart.adoc[Mule App Tutorial]. Also note that when you
create your own Mule apps, some useful Mule components for trying out 
sample data that do not depend on external data sources are 
xref:set-payload-transformer-reference.adoc[Set Payload] to 
provide content for the payload of a Mule event, 
xref:variable-transformer-reference.adoc[Set Variable]
to create content in a Mule event variable, and a 
xref:scheduler-concept[Scheduler] to trigger the regular generation 
of a Mule events to hold data from those components.
