...
the scope of this tutorial to describe that process. However, note that
some useful Mule components for creating sample data, without depending 
on external data sources, are 
xref:set-payload-transformer-reference.adoc[Set Payload] to create
a payload and xref:variable-transformer-reference.adoc[Set Variable]
to create a Mule variable, along with a xref:scheduler-concept[Scheduler]
to trigger the regular generation of a Mule events to hold that data. 
