...
scripts and only describes a _limited use case_ for the component. It does
not cover every use DataWeave, such as uses of the built-in
xref:dataweave-variables-context.adoc[Mule Runtime variables], like `payload`,
that DataWeave can handle. For more complete information about the component,
see xref:transform-component-about.adoc[Transform Message].)
...
+
Note that some of the examples require you to provide your own input examples,
which you can do through a variable (`var`) in the script or through a file
that you read wit the `readUrl` function. You might also need to change the
