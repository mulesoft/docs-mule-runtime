...
[NOTE]
====
For an example of using *Until Successful* in Anypoint Studio see the <<xmlexample,XML Example>> and the canvas flow thereafter.
====
...
|===
|Attribute |Description
|`maxBufferSize` |Determines how many requests are queued when the pool is at maximum usage capacity and the `poolExhaustedAction` is `WAIT`. The buffer is used as an overflow.
...
Type: Integer +
Required: No
|`threadWaitTimeout` |How long to wait in milliseconds when the `poolExhaustedAction` is `WAIT`. If the value is negative, it waits indefinitely.
