...
Every execution of the UntilSucessful scope will start with the same variables and values as before the execution of the block. New variables or modifications of already existing ones while processing one element will not be visible in the next execution (in case there is an error). In case the execution finishes correctly, then the variables (and payload) will be propagated to the rest of the flow. 
