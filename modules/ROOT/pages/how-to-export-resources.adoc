...
NOTE: A common case for exporting resource files is a DataWeave script (`.dwl` file) referenced from a Transform Message component through the `resource` attribute. Starting in Mule 4.5, the runtime validates the accessibility of these scripts, so export any referenced `.dwl` file that the runtime needs. For more information, see xref:transform-dataweave-xml-reference.adoc#export-referenced-dwl-files[Exporting Referenced DataWeave Files].

== See Also

* xref:transform-dataweave-xml-reference.adoc#adding-dataweave-scripts-to-the-transform-component[Adding DataWeave Scripts to the Transform Component]
