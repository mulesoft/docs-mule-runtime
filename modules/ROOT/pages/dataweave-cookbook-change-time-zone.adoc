...
For daylight saving you can also use the time zone ID values. The following example also uses the `>>` operator and the time zone ID `America/Los_Angeles` to change the time zone.

.DataWeave Script:
[source,dataweave,linenums]
----
include::partial$cookbook-dw/change-time-zone-ex2/transform.dwl[]
----

.Output XML:
[source,xml,linenums]
----
include::partial$cookbook-dw/change-time-zone-ex2/out.json[]
