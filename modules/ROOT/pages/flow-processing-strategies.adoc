...
[%header,cols="5*"]
|===
|Flow Processing Strategy link:#FlowProcessingStrategies-HowMuleSelectsaProcessingStrategy[implicitly +
...
One thread is always used for the the polling operation of the asynchronous processing strategy. As a consequence of this, if you set `maxThreads` attribute to 1, no work will be scheduled. In case this configuration is set a warning will be logged for you to verify the threading profile config.
====
...
The following components can be included in the flow with "Non-Blocking" strategy, but not all of them can support the non-blocking processing operations.
...
* xref:async-scope-reference.adoc[Async Scope Reference] (how to embed an asynchronous processing block in an otherwise synchronous flow)
* xref:flows-and-subflows.adoc[Flows and Subflows]
* xref:reliability-patterns.adoc[Reliability Patterns]
* Find out how to xref:tuning-performance.adoc[Tuning Performance] (by analyzing and configuring threading profiles)
* https://blogs.mulesoft.com/dev/mule-dev/cheat-sheet-asynchronous-message-processing/[Asynchronous Message Cheat Sheet] (MuleSoft blog)
