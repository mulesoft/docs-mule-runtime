...
Reference information for all connectors bundled automatically in Anypoint Studio is available via the Reference column in xref:anypoint-connectors.adoc[this table.]

== Assumptions

This document assumes that you are familiar with Mule and the xref:6.x@studio::index.adoc[Anypoint Studio interface]. To increase your familiarity with Studio, consider completing one or more xref:6.x@studio::basic-studio-tutorial.adoc[Anypoint Studio Tutorials]. Further, this example assumes that you have a basic understanding of xref:3.7@mule-concepts.adoc[Mule flows], xref:3.6@global-elements.adoc[Global Elements], and xref:3.7@anypoint-connectors.adoc[Anypoint Connectors].
...
|*Transaction* |Lets you select the element to use for a transaction. Use the image:add.png[(plus)] button to add Mule transactions.
...
Selected global connector configurations can also be defined as *shared resources* for a domain, then referenced by all applications that reference that same domain. For more information, see xref:3.7@shared-resources.adoc[Shared Resources].
...
* Return to the xref:3.7@anypoint-connectors.adoc[Anypoint Connectors] main page.