...
Connectors group together components (also called "operations") that were created to facilitate integration of MuleSoft applications with any external resource, such as third-party APIs or standard integration protocols. They enable connectivity to SaaS and on-premises applications, systems, and services. 
...
Modules group together components (also called "operations") that were created to add flexibility to your apps by allowing you to aggregate values, compress data, use Java features, use extra features for processing JSON, and more. The features that modules offer are more at the level of traditional application coding.

For example, the XML module provides components that can process and extract data from an XML document.

.The XML module and its operations listed in Anypoint Studio
image::about-mule-apps-xml-module.png[The XML module and its operations, as listed in Anypoint Studio]
