...
There is a known issue that causes some incorrect expressions to work during an evaluation without causing any compiler errors. The issue occurs when an unexpected character is affixed at the end of an escaped property in an expression that is part of an evaluation, for example:

```
<when expression="#[message.inboundProperties.'http.request.path'] == '/some_path']">
```

The previous expression is incorrect, because a `]` character is incorrectly affixed to the `'http.request.path'` property. However, Mule ignores this extra character and is able to compile the expression without errors.
