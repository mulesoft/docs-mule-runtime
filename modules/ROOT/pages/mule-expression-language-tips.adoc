...
You can carry out powerful complex data transformations by including xref:mel-dataweave-functions.adoc[MEL DataWeave Functions] that use xref:dataweave-language-introduction.adoc[DataWeave Language code]. You can include this code via two different functions in MEL: 'dw()' and 'split()'.
...
=== Boolean Operations Troubleshooting
...
A known issue causes certain incorrect expressions to produce results during an evaluation without causing any compiler errors. The issue occurs when an unexpected character is affixed at the end of an escaped property in an expression that is part of an evaluation, for example:

```
<when expression="#[message.inboundProperties.'http.request.path'] == '/some_path']">
```

Although the `]` character is incorrectly affixed to the `'http.request.path'` property in this example, Mule ignores this extra character and is able to compile the expression without errors.

Keep in mind this parsing issue while troubleshooting incorrect expressions or if you experience unexpected behaviors as a result of evaluating an expression. This behavior is present in all Mule 3 versions, in any component that evaluates a MEL expression.

== See Also

* For the complete MEL reference, including lists of operators, imported Java classes, context objects, and so on, see xref:mule-expression-language-reference.adoc[Mule Expression Language Reference].
