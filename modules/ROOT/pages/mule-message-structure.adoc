...
A Mule message is created as part of a xref:about-mule-event.adoc[Mule event] when a Message source in a Mule flow (such as an HTTP listener or Scheduler component) receives a request from a client. For example, when an HTTP listener in a Mule app receives a request from a client, it creates a Mule event with a Mule message that contains the content of the response as its payload and the metadata associated with that content, such as HTTP headers, as it attributes. This request also starts (or triggers) the Mule flow. Message processors in the flow (such as Core components, File read operations, or the HTTP request operations) can then retrieve, set, and process Mule message data that resides in the Mule event according to their configurations.

Note that in Anypoint Studio, the
xref:7.1@studio::datasense-explorer.adoc[DataSense Explorer] shows the
...
content of a file that you retrieve through a Read operation to the File or FTP connector.

The payload content typically changes as it travels through a flow as message processors in a Mule flow set it, enrich it, transform it into a new format, extract information from it, or even store it in a Mule event variable.
...
+`https://jsonplaceholder.typicode.com/users`+ might look something like this (shortened for readability) in the Studio console:
...
A Logger component set to display the `payload` for the output of a File read operation for a simple JSON file might look something like this in the Studio
console:
...
use it to change the format from XML to JSON. You might exclude a field that contains personal identifiers or private data for a customer, such as the social security or medical record number, or you might add a new field to the output.
...
message. The specific attributes of a message depend on the protocol (such as HTTP, FTP, File) associated with the message. Metadata can consist of headers
and properties received or returned by a connector, as well as other metadata that is populated by the connector or through a Core component, such as Transform Message.

You can select attributes of a Mule message through a DataWeave expression
that uses the xref:dataweave-variables-context.adoc[Mule Runtime variable],
`attributes`. For example, when using the `attributes` variable to display HTTP response metadata through a Logger component, the Studio console output looks something like this:
...
(`=`). You access attributes of a message through the keyword `attributes`, a
Mule runtime variable. For example, when using a connector or component (such
as the Logger) in your flow, you might access attributes of an HTTP response or
a file that you are reading:
...
For File metadata the attributes are different. For example, when using the `attributes` variable to display File metadata through a Logger component, the Studio console output looks something like this:
