...
In some cases when an issue occurs in a Message source, the component does not produce a Mule event. For example, invalid or incorrect paths prevent the creation of a Mule event. The Mule application logs such issues in ERROR, WARN, or INFO messages.
