...
In some cases when an issue occurs in a Message source, the component does not produce a Mule event. For example, invalid or incorrect paths in an HTTP listener prevents the creation of a Mule event. Connectors log such issues in ERROR, WARN, or INFO messages.
