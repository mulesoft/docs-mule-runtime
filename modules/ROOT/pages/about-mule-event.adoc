...
[NOTE]
====
Mule initializes listener configurations before building and executing flows. For this reason, message sources don't support expressions that depend on values created during flow execution. Configuring a source with expressions results in an error such as `The source: 'listener' is using expressions, which are not allowed on message sources.`
====
