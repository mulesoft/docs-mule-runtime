...
Mule batch processing is designed to handle very large data sets and to perform "near real-time" data integration that recovers from crashes and continues processing a job from a point of failure. However, _verbose_ logs for issues that occur can become enormous and severely impact performance. 
