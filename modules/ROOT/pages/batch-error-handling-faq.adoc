...
NOTE: From 4.11 onwards, the batch error handling relies on BatchError objects instead of exceptions.
...
| `#[Batch::isSuccessfulRecord()]` | A boolean function that returns true if the current record has not presented errors in any prior step.
...
There are also bindings to work with the old Batch error handling mechanism, which relied in exceptions rather than `BatchError`s. These bindings won't work for apps with Min Mule Version 4.12 and higher.
...
Now, assume that as a third step, you need to be able to write into a JMS dead-letter queue per each record that has failed. To keep it simple, let’s say that the message will be the error itself. This requirement holds a trick: each record could have failed in both steps, which means that the same record would translate into two JMS messages.
...
The set-payload processor in this step can be configured to use the `Batch::getStepErrors()` function.

image::batch-error-handling-faq-6269e.png["A Set Payload component with a value expression related to batch errors in Anypoint Studio"]

As stated above, this function returns a map with all errors found in all steps. And since our goal is to send the errors through JMS and we don’t care about the steps, we can use a foreach scope to iterate over the map’s values (the errors) and send them through a JMS outbound endpoint:

image::batch-error-handling-faq-b822e.png["A For Each component processing error in Anypoint Studio"]
