...
Handling errors inside a batch job is complicated by the enormous amount of data this scope is designed to handle. For example, logs from processing 1 million records become huge and unreadable, and the processing required for the logs causes major performance issues. On the other hand, logging too little would make it impossible for you to know what went wrong. And considering that if out of those 1 million records, 30 thousand of them failed, not knowing what’s wrong with them can turn your debugging task very problematic. This trade-off is not simple to overcome.
