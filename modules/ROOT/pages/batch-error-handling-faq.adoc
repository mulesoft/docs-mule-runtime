...
Verbose logs for issues that occur when a batch job processes very large chunks of data are likely to impact performance negatively. 

To limit this impact, Mule uses INFO-level log messaging by default, as described in <<default_logging>>. For cases in which you require more verbose log messages, you can change the mode to DEBUG. This mode is helpful not only for debugging but for some cases that involve smaller data sets. For example, batch processing recovers from crashes and continues processing the job from the point it "left off" when performing "near real-time" data integration. 
