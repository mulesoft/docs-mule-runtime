...
Verbose log messages for issues that occur when a batch job processes very large chunks of data can impact performance negatively. 

To limit this impact, Mule uses INFO-level log messaging, as described in <<default_logging>>. For cases in which you require more verbose log messages, you can change the mode to DEBUG. This mode is helpful not only for debugging but, for example, for cases in which you must recover from a crash and continue processing a batch job from the point at which it failed during a "near real-time" data integration. 
