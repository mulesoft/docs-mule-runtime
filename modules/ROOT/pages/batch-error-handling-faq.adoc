...
Handling errors inside a batch job is complicated by the enormous amount of data this scope is designed to handle. For example, logs from processing 1 million records become huge and unreadable, and the processing required for the logs causes major performance issues. However, logs are necessary to understand issues that occur. Without logs, debugging the failure of 30,000 records is not practical. 

== Logs of Failing Records Inside a Batch Step

When processing a batch job instance, a processor inside a batch step may fail or raise an error, for example, because of corrupted or incomplete record data. When such an issue occurs, Mule logs the stack traces following this logic:

. Mule gets the exception's full stack trace.
. Mule strips that stack trace from error messages. +
