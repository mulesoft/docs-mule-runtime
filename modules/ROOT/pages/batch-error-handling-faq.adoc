...
When processing a batch job instance, a processor inside a batch step may fail or raise an error. When this occurs, for example, because of corrupted or incomplete record data, Mule logs the stack traces following this logic:
