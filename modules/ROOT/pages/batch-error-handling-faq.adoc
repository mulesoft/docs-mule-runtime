...
Batch processing performs "near real-time" data integration, which recovers from crashes and continues processing a job from a point of failure. However, when a batch job processes very large chunks of data, _verbose_ logs for issues that occur can become enormous and severely impact performance. 

To limit this impact, Mule uses INFO-level logging by default, as described in <<default_logging>>. For cases in which you require more verbose log messages, you can change the mode to DEBUG. This mode is helpful not only for debugging but is feasible for cases that involve smaller data sets. 
...
When processing a batch job instance, a processor inside a batch step can fail or raise an error, for example, because of corrupted or incomplete record data. By default, Mule logs the stack traces when such an issue occurs by following this logic:
