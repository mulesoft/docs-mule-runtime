...
[NOTE]
You can use MDC Logging with Anypoint Runtime Fabric or CloudHub 2.0. However, if you are using CloudHub to deploy your Mule applications, MDC logging isn't supported.

To use the MDC Logging operations, complete the following tasks:

* Install the Mule Tracing module in your application.
* Change the pattern layouts in the `log4j2.xml` file to `MDC`. (This step doesn't apply for Anypoint Runtime Fabric or CloudHub 2.0)
