...
[NOTE]
You can use MDC Logging with Anypoint Runtime Fabric or CloudHub 2.0. However, if you are using CloudHub 1.0 to deploy your Mule applications, MDC logging isn't supported. For more information, see https://help.salesforce.com/s/articleView?id=001120207&type=1[MDC Logging Module support in CloudHub].
...
!===
| `clear-logging-variables` | Removes all the logging variables. This option doesn't remove the processor path or the correlation ID.
| None
|===
...
image::mruntime-mdc-example.png["Logger and variable setting components in a Mule error handling flow"]
