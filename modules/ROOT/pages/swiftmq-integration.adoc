...
The following example shows you how to modify the xref:loan-broker-example.adoc[Loan Broker example] to use SwiftMQ. The only change necessary is to modify the JMS connector in both example configuration files. With a SwiftMQ Router running on the local host, the connector would look like this:
