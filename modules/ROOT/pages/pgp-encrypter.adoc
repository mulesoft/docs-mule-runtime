...
Just in the same way you configure the *Encryption* operation, you can use mule to attomically *encrypt & sign* some part of you message. 
For this, you need to configure the an encryption transformer.
This requires the private key of the signer (usually the sender) to be present in the configured private key-ring. Also, the configured secret passphrase has to correspond to the latter private key.

*Anypoint Studio Visual Editor*

. Follow the steps in the previous (Encryption) section to configure the *encryption* stage, but selecting as operation *Encrypt and sign*.
. Select the *Singer principal* (A). This corresponds to the user whose *private key* will be used for signing the message. This will require the recipient of the message to posses A's *public key*, in order to validate the attached signature. Requires that the private configures contains that user's *private key* and *secret passphrase*.
+
...
. Follow the steps in the previous (Encryption) section to configure the *encryption* stage, but using changing the global element's name to *encryption:encrypt-and-sign*, as shown below. The attributes and child elements are the same as in the *Encryption* operation, but adding the one described in the table below.
[source,xml,linenums]
----
<encryption:encrypt-and-sign config-ref="Encryption_PGP" doc:name="Encryption" using="PGP_ENCRYPTER" input-ref="#[message.payload]" signerPrincipal="Alice &lt;alice@example.com&gt;">
        <encryption:pgp-encrypter principal="fernando.martinez &lt;fernando.martinez@mulesoft.com&gt;" />
</encryption:encrypt>
----
...
|*Verify Signature If Found* |*DEFAULT: False*. If a signature of the message is found while decrupting, attemp to verify the decrypted contents against it. Notice that if this option is *enabled*, and the validation fails, or the signer *public key* is missing, the operation will fail.
|===
