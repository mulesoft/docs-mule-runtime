...
The Encryption module does not support ASCII-armored keyrings. For more details see the xref:pgp-encrypter.adoc#using-ascii-armored-keys[Using ASCII-armored Keys] section below.
...
|*verifySignatureIfFound* |  |*DEFAULT: False*. If a signature of the message is found while decrypting, attemp to verify the decrypted contents against it. Notice that if this option is *enabled*, and the validation fails, or the signer *public key* is missing, the operation will fail.
|===
+
[%header%autowidth.spread]
|===
