...
To atomically ecrypt and sign in the Anypoint Studio Visual Editor:

. Follow the steps in <<encryption_studio>> to configure the *encryption* stage, but select the *Encrypt and sign* operation.
...
. Follow the steps in <<encryption_xml>> to configure the *encryption* stage, but change the global element's name to `encryption:encrypt-and-sign`. 
