...
Just like you configure the Encryption operation, you can use Mule to atomically encrypt and sign a part of your message using an encryption transformer. This process requires the private key of the signer (usually the sender) to be present in the configured private key ring. Also, the configured secret passphrase must correspond to the latter private key.
...
. Select the *Singer principal* (A). This corresponds to the user whose *private key* will be used for signing the message. This will require the recipient of the message to possess A's *public key*, in order to validate the attached signature. Requires that the private configuration contains that user's private key and secret passphrase.
+
