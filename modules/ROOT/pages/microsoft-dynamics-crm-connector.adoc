...
Use the instructions in xref:installing-connectors.adoc[To Install a Connector from Anypoint Exchange].
...
|Property |Usage
|*Query* a|
DataSense Query Language: The DSQL to run. The query is translated by the connector to a Fetch XML. For more information, see xref:datasense-query-language.adoc[DataSense Query Language].
