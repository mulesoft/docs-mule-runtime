...
This document assumes that you have xref:installing-anypoint-enterprise-security.adoc[installed the Anypoint Enterprise Security] extensions.

It also assumes you are familiar with the xref:6.x@studio::index.adoc[Anypoint Studio] interface.
...
Make sure to right click on any of the xref:mule-message-encryption-processor.adoc[Mule Message Encryption Processors] and select `Add Encryption libraries to the project`.

image::add-encryption-libraries.png[]
...
* For more information on encryption in Mule, see xref:mule-message-encryption-processor.adoc[Mule Message Encryption Processor].
* For more information on signatures in Mule, see xref:mule-digital-signature-processor.adoc[Mule Digital Signature Processor].
