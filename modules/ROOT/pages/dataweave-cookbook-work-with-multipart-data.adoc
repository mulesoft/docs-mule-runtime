...
The example uses the following function:

* `mapObject` iterates over the parts and returns `name` key and index of each part.
...
For additional examples, see xref:dataweave-formats-multipart.adoc#examples[Multipart Format (Form Data)] examples and functions referenced in xref:dw-multipart.adoc[Multipart function].
