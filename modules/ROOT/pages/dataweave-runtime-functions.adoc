...
variables, and any error, but it only passes in a Java payload without any
...
`flow2` and passes the object `{test:'hello '}` as its payload to `flow2`. That 
object is treated as a Java object. The Set Payload component  (`<set-payload/>`) in 
`flow2` then concatenates the value of `{test:'hello '}` with the string `world` 
