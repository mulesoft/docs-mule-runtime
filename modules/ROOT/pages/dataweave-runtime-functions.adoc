...
In addition to the built-in DataWeave functions, Mule Runtime and some Core
components and operations in modules and connectors provide functions that
you can use in DataWeave scripts. The functions are specific to the module,
...
[NOTE]
====
MuleSoft recommends that you start using Mule Runtime functions with the
`Mule` namespace in all Mule apps that are running on _Mule Runtime 4.1.4_ and
later. You simply append the namespace to the function name, for example:
`Mule::p`, `Mule::lookup`, `Mule::causedBy`

The `Mule` namespace is for the Mule function module, which enables you to 
use the Mule Runtime functions not only in your DataWeave scripts and mappings 
but also in your xref:dataweave-create-module.adoc[custom modules]. Mule apps 
running on Mule Runtime versions _prior_ to version 4.1.4 cannot use `Mule` 
namespace or use Mule Runtime functions in custom modules. 
====
...
variables, and error. It only replaces the payload for the first parameter,
which must be a Java type.
...
* Optional: The timeout (in milliseconds) for the execution of the target flow.
+
