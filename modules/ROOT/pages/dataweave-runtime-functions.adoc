...
[NOTE]
====
MuleSoft recommends that you start using the Mule Runtime functions with the
`Mule` namespace in all Mule apps that are running on Mule Runtime 4.1.4 and
later. You simply append the `Mule` namespace before the function
(for example, `Mule::p`, `Mule::lookup`, `Mule::causedBy`).

For Mule apps running on Mule Runtime versions _prior_ to version 4.1.4,
you cannot use Mule Runtime functions in your
xref:dataweave-create-module.adoc[custom modules].

For Mule apps running on Mule Runtime 4.1.4 and later, you can use them in
custom modules if you use the `Mule` namespace.
====
