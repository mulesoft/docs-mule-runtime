...
To the Mule flow that involves an API, add a xref:3.6@cxf-component-reference.adoc[*CXF Component*] to publish, consume, or proxy an API. Complete the following steps to configure WS-security on the CXF component.
...
=== Adding a Token in Studio Visual Editor
...
+
[TIP]
If you have not yet created any beans, click the image:add.png[(plus)] button to open a new properties panel in which you can create and configure a new bean. The bean imports the Java class you have built to specify the custom validator's override behavior.
+
...
=== Adding a Token in Studio XML Editor or Standalone
