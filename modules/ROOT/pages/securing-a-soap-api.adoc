...
+
image::cxf-security-tab.png[CXFsecuritytab]
+
. Click the
...
|Value |Enter a WS Constant (a class to define the kind of access the server allows) or a WSHandlerConstant (a class to specify the names, actions, and other strings for data deployment of the WSS handler). For example, enter `UsernameToken` in the value field.
...
. To your CXF element (that is, your SOAP component), add a child element for `cxf:ws-security`.
...
|value |Specify a WS Constant (a class to define the kind of access the server allows) or a WSHandlerConstant (a class to specify the names, actions, and other strings for data deployment of the WSS handler). For example, enter `UsernameToken` in the value field.
...
|SAML 1 |Checks messages against SAML 1.1 assertion statements to approve or reject access to the API.
|SAML 2 |Checks messages against SAML 2.0 assertion statements to approve or reject access to the API.
...
. If you have not created any beans, click the
...
|cxf:saml1-token-validator |Checks messages against SAML 1.1 assertion statements in order to approve or reject access to the API.
|cxf:saml2-token-validator |Checks messages against SAML 2.0 assertion statements in order to approve or reject access to the API.
