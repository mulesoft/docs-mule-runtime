...
+
image::mruntime-quickstart-json.png["A JSON response example in a test environment showing data"]
+
...
image::mruntime-quickstart-package-explorer.png["A Mule project with directories for main, test, and resources"]
...
+
image::mruntime-quickstart-listener.png["A palette menu showing components including Listener and Transform Message"]
+
...
+
image::mruntime-quickstart-listener-properties.png["A Listener configuration panel in Mule showing required settings for connector configuration"]
...
+
image::mruntime-quickstart-path.png["A configuration panel for a Listener component showing settings for path and connector configuration"]
...
+
image::mruntime-quickstart-add-db1.png["A palette menu in Mule interface showing available modules like Database, HTTP, and APIKit for SOAP"]
...
+
image::mruntime-quickstart-add-db2.png["A Mule flow configuration interface showing elements like Listener and Error handling"]
...
** **Database**: `training`
+
image::mruntime-quickstart-db-values.png["A database configuration panel for MySQL connection with fields for host, port, user, and database"]
...
+
image::mruntime-quickstart-db-driver.png["A database configuration panel showing MySQL connection settings and required JDBC driver information"]
...
+
image::mruntime-quickstart-select-query.png["A database query setup with SQL text"]
...
+
image::mruntime-quickstart-object.png["An error message indicating a server error due to unsupported source type in a transformation process"]
...
+
image::mruntime-quickstart-add-transform.png["A palette menu showing components like Database, HTTP, and Batch"]
. Drag and drop the Transform Message to the right of the Select operation in the canvas.
+
image::mruntime-quickstart-canvas.png["A visual editor in Mule showing components of a flight service workflow including Listener and Transform Message"]
. Click the Transform Message component to display the graphical view and source code view.
+
image::mruntime-quickstart-dw-blank-palette.png["An empty data transformation interface with options to define input and output metadata"]
+
...
+
image::mruntime-quickstart-dw-palette.png["A DataWeave transformation interface with fields for defining JSON output structure"]
+
...
+
image::mruntime-quickstart-json1.png["An HTTP request setup showing method, URL, and headers"]
+
...
+
image::mruntime-quickstart-empty-type.png["An interface for selecting metadata type with options for simple and user-defined types"]
+
...
+
image::mruntime-quickstart-json-example2.png["Dialog box for selecting a metadata type with options for JSON and user-defined types"]
. Choose **Select** to save your change. Now you see the input and output data structures in the DataWeave interface.
+
image::mruntime-quickstart-input-output.png["A DataWeave editor showing transformation setup with input and output structures"]
...
+
image::mruntime-quickstart-same-name.png["A DataWeave editor showing transformation data into JSON format with fields including ID and code"]
...
+
image::mruntime-quickstart-sample-data1.png["A DataWeave script showing mappings for fields including ID and code"]
...
+
image::mruntime-quickstart-transformed.png["A successful GET request with response details"]
