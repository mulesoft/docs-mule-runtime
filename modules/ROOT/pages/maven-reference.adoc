...
To add the MuleSoft Enterprise repository to your project, you need to meet the following prerequisites:

* Own an Enterprise license
+
If you need instructions to acquire or install an Enterprise License, see xref:installing-an-enterprise-license.adoc[Installing an Enterprise License]
* Have valid credentials to access the https://repository.mulesoft.org/nexus-ee/content/repositories/releases-ee/[MuleSoft Enterprise repository]
+
If you are a MuleSoft customer and do not have access to the repository, contact Support and request Nexus enterprise credentials.

=== Configure MuleSoft Enterprise Repository

To configure Maven to access the MuleSoft Enterprise Repository:

. Inside `<USER_HOME>/.m2/settings.xml`, open the `settings.xml` config file on all workstations that require access.
