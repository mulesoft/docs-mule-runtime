...
This page summarizes reference information that helps work with Maven and Mule. Refer to xref:using-maven-with-mule.adoc[Maven Support in Mule] for an introduction and overview.
...
The `settings.xml` file may reside in two locations:

* In a system-wide settings folder:
+
In this case, it is a _global_ settings file which defines the settings for all Maven installations on the system, regardless of user. For example:
+
----
...
+
In this case, the file is a _user_ settings file only relevant for a specific user's Maven installation. The default location is the `.m2` directory in the user's home directory:
+
----
