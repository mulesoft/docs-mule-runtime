...
For the `-Durl` parameter, replace `your_repository_ip` with the correct IP address. Also, the `-DrepositoryId` should match with the credentials defined on the settings.xml file.
...
* https://github.com/mulesoft/mule-maven-plugin[Mule Maven Plugin]
