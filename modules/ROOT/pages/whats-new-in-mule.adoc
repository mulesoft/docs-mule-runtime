...
Common subexpression elimination is included to optimize scripts. Now, DataWeave optimizes scripts by defining internal variables to represent subexpressions that are repeated, which replaces the repeated executions of that subexpression with a single execution.
