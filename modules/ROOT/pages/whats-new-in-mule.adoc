...
Mule 4.3 incorporates several changes from earlier Mule 4.x versions. If you are migrating from a 4.x version to 4.3, ensure that you read each of the relevant topics in this article to make the necessary changes to your applications and also understand what has changed in the Mule runtime engine.

[[threading_model]]
== Threading Model
Mule 4.3 handles threads differently than Mule 4.2 and 4.1.
In earlier versions, Mule owned and managed three different thread pools, each dedicated to each Processing Type (CPU_LIGHT, IO, CPU_INTENSIVE). Tasks were submitted to these pools using the Proactor design pattern.
...
The JSON reader now supports streaming beyond arrays. The entire JSON document is now streamable.
+
See xref:dataweave-formats-json.adoc[JSON Format] for details.

=== Literal Types
This feature adds DataWeave support for literal types. A literal type is a type that represents exactly one value, which means, for example, that DataWeave can represent the `String` value `hello` with the type `hello`. Combined with union types, this feature enables DataWeave to represent an enumeration of literal values as a type, for example:
...
* xref:dw-objects.adoc[Objects] (`dw::core::Objects`) module: `everyEntry`, `someEntry`, and `takeWhile`; `dropWhile` also includes a new function that iterates over and ignores an items in an array until a condition becomes true.
...
DataWeave introduces the `Intersection` type. This type intersects `Object` types. In this case, the intersection works as the concatenation (`++`) of object types.
...
Common subexpression elimination has been included to optimize scripts. Now, DataWeave will optimize the scripts by defining internal variables to represent subexpressions that are repeated, therefore replacing the repeated execution of that subexpression for only one.
...
* On-Error error handler components: +
On-Error error handlers such as On Error Continue support `when` filtering for a given error type. In the following example, every error that matches the when expression is handled by on-error-continue. You can also add a restrictive type such as type="HTTP:CONNECTIVITY" so that the error handler handles only the errors that match the specified type and condition.
+
[source,xml,linenums]
----
<on-error-continue
...
----
+
For complete documentation, see xref:on-error-scope-concept.adoc.adoc[On-Error Error Handlers].
