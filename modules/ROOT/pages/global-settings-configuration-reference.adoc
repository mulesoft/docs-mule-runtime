...
This page provides details on the global settings you configure at the root level of a Mule configuration. Some of this information is pulled directly from `mule.xsd` and is cached. If the information appears to be out of date, refresh the page. For more information on configuration, see xref:about-mule-configuration.adoc[About Mule Configuration]. For information on threading profiles, see xref:tuning-performance.adoc[Tuning Performance].
