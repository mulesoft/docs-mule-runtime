...
This document assumes that you are familiar with Mule, the xref:6@studio::index.adoc[Anypoint Studio interface], and xref:global-elements.adoc[Global Elements]. Further, it is assumed that you are familiar with the Microsoft .NET Framework.
...
* Anypoint Studio, the graphical interface to Mule. See xref:6@studio::download-and-launch-anypoint-studio.adoc[Download and Launch Anypoint Studio] for details
...
|*xref:http-connector.adoc[HTTP Connector]* |Enables the example application to receive and send HTTP requests.
|*xref:set-payload-transformer-reference.adoc[Set Payload Transformer]* |The set payload transformer sets the payload to whatever you define. The payload can be a literal string or a Mule Expression.
|*xref:dataweave.adoc[DataWeave]* |DataWeave is a Mule transformer that delivers simple, yet powerful, visual design of complex data transformations for use in Mule flows.
|===
