...
he HTTP connector sends and receives HTTP or HTTPS requests using a host, port, and address. Find the two important use cases for this component below.
...
* Send xref:authentication-in-http-requests.adoc[Authenticated Requests], via *Basic Authentication, Digest and OAuth*
...
[TIP]
To migrate a project that uses the old xref:deprecated-http-transport-reference.adoc[HTTP endpoint-based connector] to the new HTTP operation-based connector, see xref:migrating-to-the-new-http-connector.adoc[Migrating to the New HTTP Connector]. The old HTTP endpoint-based connector still works with Mule runtime 3.6, but it's deprecated and will eventually be removed.

[TIP]
You can edit the log4j2 configuration file to make the logging of the HTTP connector's activity a lot more verbose, if you need to. See xref:logging-in-mule.adoc[Logging in Mule] for instructions.

== Debugging

Gaining visibility into HTTP inbound and outbound behavior can be achieved by enabling underlying library loggers with log4j2. This section assumes you're comfortable adjusting log levels with log4j2. If you have not adjusted logging levels in the past, read xref:logging-in-mule.adoc#configuring-custom-logging-settings[configuring custom logging settings] before continuing.
...
This means that whenever a message is pending a response from an external source, the message processor will still be free to process other messages that keep arriving to it. Read more about this in xref:flow-processing-strategies.adoc#non-blocking-processing-strategy[Non-Blocking Processing Strategy].
...
[NOTE]
Note that xref:flow-processing-strategies.adoc#supported-non-blocking-components[not all Mule components] currently support the non-blocking processing strategy, if there are any unsupported components in a flow, they will cause the flow to fall back to synchronous processing.
