...
* HTTP - Send or receive HTTPS requests using xref:tls-configuration.adoc[TLS] encryption.
...
[TIP]
You can edit the log4j2 configuration file to make logging of the HTTP connector's activity more verbose, if you need to. See xref:logging-in-mule.adoc[Logging in Mule] for instructions.
...
This means that whenever a message is pending a response from an external source, the message processor is free to process other messages that keep arriving to it. Read more about this in xref:flow-processing-strategies.adoc#non-blocking-processing-strategy[Non-Blocking Processing Strategy].
...
[NOTE]
Note that xref:flow-processing-strategies.adoc#supported-non-blocking-components[not all Mule components] support the non-blocking processing strategy, if there are any unsupported components in a flow, they cause the flow to fall back to synchronous processing.
