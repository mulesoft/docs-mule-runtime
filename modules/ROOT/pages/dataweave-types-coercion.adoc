...
[[modeExamples]]
The following examples show uses of the `mode` property. Notice that while `LENIENT` and `SMART` return valid dates when they receive an invalid date such as `02/31/2020`, but the resulting dates differ. `STRICT` returns an error on an invalid date. It uses `uuuu` to represent the year instead of `yyyy`.
...
Using `badDateWithStrict: '02/31/2020' as Date {mode: "STRICT", format: 'MM/dd/uuuu'}` returns the following error:

.Use of STRICT on invalid date:
----
