...
== Using a Java Bridge
////
In the case you need to coerce a Java type that is not supported automatically
by DataWeave, you can use a Java bridge. For example, the following script will 
coerce  a `java.time.Duration` payload to a `String`.
...
FROM MDA, NEEDS EDITING:

DataWeave allows to call any Java static function or constructor using the java bridge. 
This is usefull to reuse some business logic writen in java or to instanciate any java object that doesn't have a public empty constructor.
In order to do this we need to transform a java full qualified name to a DataWeave one. This is done by adding the prefix
`!java` and to replace the dots with `::`
When invoking the java function DW will transform each argument to the expected type in the function parameter.
This example shows how to create a new instance of a java.lang.NullPointerException
[source,dataweave,linenums]
----
%dw 2.0
output application/json
...
----
This example invokes the function java.lang.String.valueOf
[source,dataweave,linenums]
----
%dw 2.0
output text/plain
