...
The following example converts a string to double. First, you transform the string value `"22"` to `Number` and then to `String` again because `format` add zeros when converting from a number to a string. If the example string data `"22"` would have been a number you wouldn't need the first `Number` conversion.
The latter `Number` conversion makes the output as a float. Otherwise you would get a string, formatted according to the hosts locale.
