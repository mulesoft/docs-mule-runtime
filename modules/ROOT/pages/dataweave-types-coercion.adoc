...
The following example converts two strings into date format and concatenates the result. The first string represents the date, the second string represents the time. The transformation uses `as` to coerce the first string to `LocalDateTime`, and then to a `String` with the desired format. The transformation also uses `++` to concatenate the result.
...
%dw 2.0
output application/json
var s1= 20201228 // (yyyymmdd),
var s2= 1608 //(hhmm)
---
(s1 ++ s2)
as LocalDateTime {format:"yyyyMMddHHmm"}
as String {format:"MM-dd-yyyy HH:mm:ss"}
----
...
The following example is similar to the previous one and converts a string into date format. First the transformation uses `as` to coerce the string into `LocalDateTime`, and then coerces it again to `String` with the desired output format.
