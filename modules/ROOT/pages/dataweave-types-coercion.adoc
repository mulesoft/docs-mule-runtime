...
Note that `LENIENT` AND `SMART` auto-correct invalid dates in different ways, but `STRICT` returns an error on the date. See <<modeExamples, examples>> that use `mode`.
...
[[modeExamples]]
The following examples show uses of the `mode` property. Notice that `LENIENT` and `SMART` return valid dates when they receive an invalid date such as `02/31/2020`, but the resulting dates differ. `STRICT` returns an error on an invalid date. It uses `uuuu` to represent the year instead of `yyyy`.
...
Using STRICT on a invalid date returns an error. The following example is an error returned by `badDateWithStrict: '02/31/2020' as Date {mode: "STRICT", format: 'MM/dd/uuuu'}`:
