...
[[modeExamples]]
The following examples show uses of the `mode` property. Notice that while `LENIENT` and `SMART` return valid dates, the resulting dates differ. `STRICT` returns an error on the invalid date. It uses `uuuu` to represent the year instead of `yyyy`.
