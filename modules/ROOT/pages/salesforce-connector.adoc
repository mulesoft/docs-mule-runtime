...
* *Consumer key and Secret* - If you are using the Salesforce connector to access an *OAuth API*, you also need a consumer key and secret. Refer to the detailed documentation on how to xref:using-a-connector-to-access-an-oauth-api.adoc[use the Salesforce connector to access an OAuth API].
...
[IMPORTANT]
Starting with v.7.0.0, the Salesforce connector is licensed commercially with Anypoint Platform as are other xref:anypoint-connectors.adoc#connector-categories[_Select_] connectors.  Prior versions will remain freely available to the community.
...
To use the Salesforce connector in your Mule application, you must configure a global Salesforce connector element that can be used by all the Salesforce connectors in the application (read more about xref:global-elements.adoc[Global Elements]).
...
[NOTE]
*Basic authentication* is the easiest to implement. All you need to do is provide your credentials in a global configuration (see xref:configuring-properties.adoc#global-properties[Global Properties]), then reference the global configuration file in any Salesforce connector in your application. Basic authentication is generally recommended for internal applications.
...
The sections below offer information about how to use the Salesforce connector in your application. Beyond these basics, you can access documentation that describes how to xref:salesforce-connector-authentication.adoc[secure your connection to Salesforce] (via basic authentication or OAuth authentication), or access http://mulesoft.github.io/mule-salesforce-connector[full reference documentation] for the connector.

== Using the Salesforce Connector

To see all possible operations, expected attributes and returned data for the connector, see the list of http://mulesoft.github.io/mule-salesforce-connector/[Technical Reference APIdocs].
...
[NOTE]
====
You can also use a Salesforce connector in a xref:batch-processing.adoc[batch process] to push data to Salesforce in batches (see image below, bottom).
====
...
Use the connector in conjunction with a xref:poll-reference.adoc[Poll Scope] in a flow to pull data from Salesforce into your application. To use the connector in this capacity, you must first place a *Poll scope* element at the beginning of your flow, then place a Salesforce connector within the poll scope (see image directly below).
...
+
[NOTE]
You can also use a poll-wrapped Salesforce connector at the beginning of a xref:batch-processing.adoc[Batch Process] to extract data from Salesforce, then batch process the content in Mule.
...
.. Learn more about using the xref:poll-reference.adoc[Poll Scope] to regularly poll Salesforce for new data for your application to process.

Depending upon its function in your application (streaming data, polling for data, pushing data, etc.), the operation you select, and the sObject you are acting upon, Studio makes different Salesforce connector fields available for configuration.  The objectives of this document do not include the exhaustive exploration of all combinations of operations and objects in an application. However, you can access http://mulesoft.github.io/mule-salesforce-connector[full reference documentation] to learn about how to configure the connector for all operations.
...
* Read more about xref:anypoint-connectors.adoc[Anypoint Connectors].
* Read more about xref:batch-processing.adoc[Batch Processing] in Mule.
* Read more about the xref:poll-reference.adoc[Poll Scope].
