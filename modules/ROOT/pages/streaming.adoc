...
When the stream gets to the Transform Message component, the stream content is available in memory, so it is possible for Transform Message component to consume it. After Transform Message component consumes the content, the second Logger receives an empty stream.
+
