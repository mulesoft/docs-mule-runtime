...
log the resulting payload after the Transform Message component because the Logger component consumes the stream.
When the stream gets to the Transform Message component, the stream content is available in memory, so it is possible for Transform Message component to consume it. After the Transform Message component consumes the content, the second Logger receives an empty stream.
+
