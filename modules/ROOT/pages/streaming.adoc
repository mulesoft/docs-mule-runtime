...
+
In the following example, the flow writes the first file correctly, while the second file is created with empty content because each component that consumes a stream expects to receive a new stream. After the first File Write operation consumes the stream, the second File Write operation receives an empty stream. Thereby, the second operation has no content to write to a file.
+
.Mule 3 Streaming: Writing a File
image::mruntime-streaming-about-1.png[]
+
In the following example, something similar happens when you try to log the payload before and
after a DataWeave transformation. The flow logs the payload before the Transform Message component, but does not
log the resulting payload after the Transform Message component because the Logger component consumes
and loads the stream into memory.
Because the stream content is available in memory, the Transform Message component consumes the content and the second Logger component receives an empty stream.
+
.Mule 3 Streaming: Logging a Payload
image::mruntime-streaming-about-5f1b3.png[]
+
...
and simultaneously log and write the payload to a file. The application fails because different processor chains can not process the data stream content simultaneously.
+
.Mule 3 Streaming: Consuming Data Streams
image::mruntime-streaming-about-6af9b.png[]
+
