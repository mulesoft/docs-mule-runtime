...
[source,bash]
----
➜  mule-enterprise-standalone-4.9.21 ./tools/diag help
...
[NOTE]
====
Watch mode and the `--logsmaxage` and `--logsmaxtotalsize` options are available in Mule runtime 4.9.21 and later.
====
...
[NOTE]
====
In Mule runtime patches earlier than 4.9.19, this section also reports `OS Name`, `OS Version`, `OS Arch`, `load.system`, and `load.systemAverage`. Starting with Mule 4.9.19, the operating system details (`host.os.*`) and the host CPU load (`host.cpu.load.*`) are reported in the <<diaf-system-info,System Info>> section instead.
====
...
[NOTE]
====
Available in Mule runtime 4.9.19 and later.
====
...
[NOTE]
====
Available in Mule runtime 4.9.21 and later.
====
...
[NOTE]
====
Available in Mule runtime 4.9.20 and later.
====
...
[NOTE]
====
This section applies to Enterprise Edition only and is available in Mule runtime 4.9.19 and later.
====
...
[NOTE]
====
Available in Mule runtime 4.9.20 and later. This section is available only in Enterprise Edition runtimes that have the Mule agent installed.
====
...
[NOTE]
====
Available in Mule runtime 4.9.20 and later. This operation runs on demand only. Because it performs outbound network connections, it isn't included in the default diagnostic dump. Run it explicitly by name with `./diag platformConnectivity`.
====
