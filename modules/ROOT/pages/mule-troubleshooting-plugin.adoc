...
This section lists the JVM agents attached at JVM startup through the `-javaagent`, `-agentpath`, or `-agentlib` arguments. Use this information to identify third-party, APM, and observability agents such as profilers, monitoring, security, and bytecode instrumentation agents, that can add overhead, change runtime behavior, or interfere with the Mule runtime.
The report doesn't include agents attached dynamically after startup by using the Attach API. If no agents were attached at startup, the report shows `No JVM agents detected.` instead.
...
| `Options`
| Arguments passed to the agent after the JAR path, if any.

| Manifest Headers
a| Selected `META-INF/MANIFEST.MF` headers read from the agent JAR. If no manifest metadata is available, the report shows `(no manifest metadata available)`. The headers reported include:
...
| `Agent Name`
| Agent name, shown with the flag it was attached with (`-agentpath` or `-agentlib`).

| `Options`
| Arguments passed to the agent, if any.

| `File exists`
| (`-agentpath` only) Indicates whether the agent library file exists at the given path.
|===
