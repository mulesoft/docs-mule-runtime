...
Counter metrics include cumulative totals and rolling counts for the last 1, 5, 15, and 60 minutes (shown next to the total as `(rolling) 1m: ...  5m: ...  15m: ...  60m: ...`).
...
a| Aggregated processing-time figures for rolling windows of the last 1, 5, 15, and 60 minutes. Each window reports `min`, `max`, `avg`, and `count` (number of processed events that contributed to the window). When `count` is `0`, `min`, `max`, and `avg` are reported as `0`.
|===
