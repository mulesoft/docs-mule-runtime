...
This section lists the agents attached to the JVM at startup through the `-javaagent`, `-agentpath`, and `-agentlib` arguments. JVM agents instrument the runtime and can intercept class loading, redefine classes, and add overhead. Common examples include application performance monitoring (APM) and observability agents, profilers, and the Java Debug Wire Protocol (JDWP) debug agent.

Agents attached dynamically through the Attach API after the JVM starts can't be enumerated through standard JVM APIs and aren't listed in this section. The report includes a note stating this, so the absence of an agent here doesn't guarantee that no agent is loaded.

If no startup agent arguments are present, the section shows `No JVM agents detected.`

Option values that look like secrets (for example, license keys, tokens, and passwords) are masked as `****` in both the report and the debug logs.

==== Java Agents

Agents attached through `-javaagent` are jar-based. For each agent, the report shows the jar path, the option string, and metadata read from the jar's `META-INF/MANIFEST.MF`. If the jar is missing or has no usable manifest, the report shows `(no manifest metadata available)`.
...
| Options
| Option string passed to the agent. Shows `(none)` if no options are given or `(empty)` if an empty option string is given. Sensitive values are masked.
...
| Whether the agent can redefine classes.

| `Can-Retransform-Classes`
| Whether the agent can retransform classes.

| `Can-Set-Native-Method-Prefix`
| Whether the agent can set a native method prefix.
...
| Flag
| Argument used to attach the agent: `-agentpath` or `-agentlib`.
