...
This section shows the status and metadata of artifacts currently deployed in the Mule runtime instance, including applications, domains, and policies. It also lists undeployed artifacts that remain in memory, which can indicate possible leaks.
...
| `State`
a| Current state of the artifact:
...
An `UNDEPLOYED` artifact remains listed if its descriptor is still in memory, which can indicate a memory leak.

| `Plugins`
| List of all plugins available in the deployment, showing each plugin name and version.

| `Time Since Last State Update`
| Time since the last state update, formatted as HH:mm:ss. Long durations for states other than `STARTED` can indicate an issue.

| `Feature Flags`
| List of all enabled feature flags for the deployment, sorted alphabetically. Shown only for `DEPLOYED`, `STARTED`, and `STOPPED` artifacts.
|===

.Example of a deployment section

[source,text]
----
"[Artifact Name]" - type: [Artifact Type], state: [State]
