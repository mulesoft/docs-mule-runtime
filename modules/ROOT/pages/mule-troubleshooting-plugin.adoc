...
[NOTE]
====
In Mule runtime patches earlier than 4.11.6, this section also reports `OS Name`, `OS Version`, `OS Arch`, `load.system`, and `load.systemAverage`. Starting with Mule 4.11.6, the operating system details (`host.os.*`) and the host CPU load (`host.cpu.load.*`) are reported in the <<diaf-system-info,System Info>> section instead.
====
...
[NOTE]
====
Available in Mule runtime 4.11.6 and later.
====
...
[NOTE]
====
This section applies to Enterprise Edition only and is available in Mule runtime 4.11.6 and later.
====
