...
| If OpenTelemetry export queues fill up, the runtime either drops the exceeding resources (spans or log records) or blocks the thread that tries to add them to the export queue until space becomes available. Dropping spans or log records results in missing data at the export destination, while blocking threads can cause performance issues in the Mule runtime. The context shows the signaled resources and the action taken either block until space becomes available or drop the resources.
|===
