...
[NOTE]
====
Watch mode and the `--logsmaxage` and `--logsmaxtotalsize` options are available in Mule runtime 4.6.34 and later.
====
...
[NOTE]
====
In Mule runtime patches earlier than 4.6.32, this section also reports `OS Name`, `OS Version`, `OS Arch`, `load.system`, and `load.systemAverage`. Starting with Mule 4.6.32, the operating system details (`host.os.*`) and the host CPU load (`host.cpu.load.*`) are reported in the <<diaf-system-info,System Info>> section instead.
====
...
[NOTE]
====
Available in Mule runtime 4.6.32 and later.
====
...
[NOTE]
====
Available in Mule runtime 4.6.34 and later.
====
...
[NOTE]
====
Available in Mule runtime 4.6.33 and later.
====
...
[NOTE]
====
This section applies to Enterprise Edition only and is available in Mule runtime 4.6.32 and later.
====
...
[NOTE]
====
Available in Mule runtime 4.6.33 and later. This section is available only in Enterprise Edition runtimes that have the Mule agent installed.
====
...
[NOTE]
====
Available in Mule runtime 4.6.33 and later. This operation runs on demand only. Because it performs outbound network connections, it isn't included in the default diagnostic dump. Run it explicitly by name with `./diag platformConnectivity`.
====
