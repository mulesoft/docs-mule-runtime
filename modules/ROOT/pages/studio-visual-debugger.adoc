...
The left pane displays the message payload (in this case, an HTTP GET request received via an xref:http-endpoint-reference.adoc[HTTP inbound endpoint]). The right pane displays various metadata associated with the message:
...
|11
|image:eval-mule-exp-icon.png[eval.mule.exp.icon] |*Evaluate Mule Expression –* Click to evaluate a Mule expression. a|Mac:
...
|14
|image:f8-resume-icon.png[f8.resume.icon] |*Resume -* Click to run the application until the next breakpoint, in any. |`F8`
|===
...
A conditional breakpoint is activated only if a user-defined condition evaluates to _true_ when Studio executes the building block with the breakpoint. You define the condition using a xref:mule-expression-language-mel.adoc[Mule Expression].
...
When you start Mule ESB in debug mode, _all_ applications that you launch in that server run in debug mode. Remote debugging does not work with xref:mule-high-availability-ha-clusters.adoc[clusters].
...
Before accessing and debugging your application on a remote server, you must first xref:3.4@mule-management-console::deploying-applications.adoc[export and deploy] your application to the desired Mule ESB server.
...
* xref:studio-in-eclipse.adoc[Eclipse plugin]
