...
+
Based on how your exiting Mule 3 application is configured, carefully review the prerequisites and determine how to plan the migration.
. <<determine-migration-approach,Determine the migration approach>>
+
Based on several factors, such as whether the migration requires a downtime, removing applied policies, or required migrating contracts, you use the existing API instance or use a new API instance for migration.
. <<perform-migration,Perform the migration>>
+
Based on whether you want to create an API or use an existing one, you can appropriately migrate your application.
. <<migrate-contracts,Migrate Contracts>>
+
If the API your are migrating has contracts, you must migrate them to your new Mule 4 environment.


[[prepare-migration]]
== Prepare to Migrate Your Mule 3 Applications

Before you migrate your existing Mule 3 applications that are managed by API Manager, to Mule 4 carefully review the following guidelines and determine:
...
You migrate your Mule 3 application that is managed by API Manager using an existing API instance or by creating a new API instance. Before you decide the option, carefully consider the following migration factors to determine the approach that best suits your requirements:
...
| Requires API metrics continuity on API Instance | Yes | No
|===

[[perform-migration]]
== Perform the Migration

You can migrate your Mule 3 application using an existing API instance or by creating an API instance:

* <<migrate-with-existing-api,Migrate Using an Existing API Instance>>
* <<migrate-with-new-api,Migrate Using a New API Instance>>
...
Before you migrate you use the existing API Instance to migrate your Mule 3 application, determine whether your API raises security issues if policies are inactive. 

* API with no security concerns
+
This scenario is for APIs that allow a small window of time in which their policies can be removed. For example, an API that has the Rate Limiting SLA-based policy applied may not be critical for business operation. If your API can support inactive policies for a short period of time, skip step 1 of the <<migrate-with-existing-api,procedure>>. 
* API with security concerns
+ This scenario is for APIs that need to have their policies active at all times for security reasons. For example, APIs that use API Manager to apply authentication policies. If your API cannot support inactive policies even for a short period of time, perform step 1 of the <<migrate-with-existing-api,procedure>>. 
...
[TIP]
---
You can remove the application from the runtime or stop the server that is running the application.
---
...
+
image:mule-version-migrate-mule3.png[]
...
.. If your application is an integration application, start the Mule 4 server that contains the migrated application. If your Mule 3 server was still running, turn it off.
.. If your application is an out-of-the-box proxy, download the new proxy and place it on your Mule 4 server. 
+
Alternatively, redeploy thr proxy to your CloudHub or Runtime Fabric runtime instance. If your Mule 3 server was still running, turn it off.

//Tomas--Is turning off Mule 3 the same as undeploying? I think not, right? We ask them to do this for a new instance, not here?
//Don't they need to migrate contracts in this case?

[[migrate-with-new-api]]
=== Migrate Using a New API Instance

You can use the high-level proposal diagram illustrated in the following diagram to use a new API instance for configuring an independent Mule runtime instance on Mule 4:
...
. If you completed fully configuring your API, start your Mule 4 application and start sending traffic to it. 
. After successfully validating your application, undeploy your Mule 3 application.

[[migrate-contracts]]
=== Migrate Contracts

If the API your are migrating has contracts, migrate them to your new Mule 4 environment.
...
.. From *API Manager*, approve yourself the access. 
...
== Migrate to the Latest Mule 3 Version
