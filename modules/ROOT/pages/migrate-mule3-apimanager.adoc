...
+
This scenario is for APIs that allow a small window of time in which their policies can be removed. For example, an API that has the Rate Limiting SLA-based policy applied may not be critical for business operations. If your API can support inactive policies for a short period of time, skip <<step1,step 1>> of the following procedure. 
