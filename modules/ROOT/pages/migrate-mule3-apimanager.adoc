...
+
Based on how your existing Mule 3 application is configured, carefully review the prerequisites and determine how to plan the migration.
. <<determine-migration-approach,Determine the migration approach>>
+
Based on several factors, such as whether the migration requires a downtime, requires removing applied policies, or requires migrating contracts, you use the existing API instance or create an instance for migration.
...
You migrate your Mule 3 application that is managed by API Manager using an existing API instance or by creating an instance. Before you decide the option, carefully consider the following migration factors to determine the approach that best suits your requirements:
...
You can migrate your Mule 3 application:

* <<migrate-with-existing-api,Using an Existing API Instance>>
* <<migrate-with-new-api,Using a New API Instance>>
