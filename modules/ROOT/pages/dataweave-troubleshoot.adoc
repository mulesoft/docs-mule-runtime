...
== Troubleshoot a Failing Script

One challenge to reproducing an error is having the same inputs the script had
...
. Set the following system property to enable the dump feature: `-M-Dcom.mulesoft.dw.dump_files=true`.
. [Optional] Specify the path in which to generate the dump files: `-M-Dcom.mulesoft.dw.dump_folder=<path_to_folder>`
+
