...
To create a global settings configuration:
...
The following example defines a global error handler, and then a global configuration which specifies a default transaction timeout and references the global error handler as the default one:
