...
+
image::mruntime-global-settings-create.png["A configuration panel for creating global settings with fields for HTTP listener configuration"]
. Select *Global Configurations* > *Configuration*:
+
image:mruntime-global-configurations.png["A menu for choosing types of global configurations including caching and scheduler settings"]
...
For configuration details, see: xref:on-premises::mule-high-availability-ha-clusters.adoc#cluster-high-performance[Clustering for High Performance].
|===
...
* xref:on-premises::mule-high-availability-ha-clusters.adoc[Mule High Availability Cluster]
