...
Since 4.2.2 there is an additional option `com.mulesoft.dw.directbuffer.disable` that allows to control weather DW is going to use off heap or heap memory. By default DW uses off heap memory for internal buffering but this can cause problems on machine with small amount of memory.
