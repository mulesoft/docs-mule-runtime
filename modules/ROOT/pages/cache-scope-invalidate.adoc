...
[WARNING]
Do not specify an Invalidate Key or Invalidate Cache component inside a Cache scope if they reference the same caching strategy unless the caching strategy disables synchronized access. Otherwise, the application will deadlock while trying to invalidate the same cache that it is trying to calculate.  
