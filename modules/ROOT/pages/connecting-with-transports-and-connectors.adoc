...
How to use each connector depends on the specific connector. You can find a list of available connectors in https://anypoint.mulesoft.com/exchange/?type=connector[Anypoint Exchange].
...
* **Request-Response Only**: Connectors are designed as request-response message processors. So every operation inside a message processor must have a request to generate a response. You cannot use endpoints with them.
