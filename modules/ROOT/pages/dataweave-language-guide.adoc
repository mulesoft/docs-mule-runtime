...
The following example uses the function `pluck` to map an object into an array, and then the resulting array serves as input for the function `filter`, which selects the elements in the array that have a `Role` equal to `ADMIN`. Also notice that during the mapping process, the function `upper` is used to transform one of the values:
...
To illustrate better, next is the same exact expression but using prefix notation.
This syntax helps visualize how `filter` takes as the first argument the result of `pluck`.
