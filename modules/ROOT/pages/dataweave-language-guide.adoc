...
Data operators are symbols that enable you to perform operations on data to produce a result. DataWeave supports different types of operators including mathematical, relational, and logical operators. The operator notation varies depending on the operator. For example, mathematical and relational operators use infix notation: (2 `+` 2 `==` 4). Logical operators `and` and `or` also use infix notation, but logical operators `!` and `not` use prefix notation.

See xref:dw-operators.adoc[DataWeave Operators] for a complete list of supported operators and their usage.
