...
For example, `upper` is a built-in function defined in the String module. You can call this function to convert a text value to uppercase. In addition, you can use a selector to retrieve a value from the payload, and then execute the `upper()` function call with this value as the input parameter `upper(payload.someKey)`.
...
//Option 1
The `Key` value is formed by `Name`, which is composed of a `String` value as the local name and an optional `Namespace`, and `Attributes`, which is a collection of zero or more `Name:value` pairs, where `value` can be any possible value.
The `value` content can be any supported type, including another object, a function call, or an array.
...
DataWeave follows the core principles of functional programming, including pure functions, first-class functions, high-order functions, function composition, lambda expressions, and immutable variables.

=== Pure Functions

Pure functions in programming are similar to pure functions in mathematics in that, given a certain input, they always return the same output. In addition, executing a function has no impact on other parts of a program, other than producing a result.
All functions in DataWeave are pure; they are not affected by external values that could possibly change the result of the execution.
...
For example, to make changes to a list, you use the `map` function to apply changes to the elements in the collection. Additionally, you can use `filter` to remove specific elements from a list, or `reduce` to collapse all values from an array into one single value.
