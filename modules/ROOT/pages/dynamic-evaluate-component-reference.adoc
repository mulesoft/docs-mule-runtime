...
The Dynamic Evaluate component evaluates an expression to select a DataWeave script, and then executes the new script to generate a final result. This behavior enables you to dynamically select the script, instead of hardcoding it into the xref:transform-component-about.adoc[Transform Message Component].
...
| Expression | DataWeave expression | Specifies the expression to select a DataWeave script, and then Mule executes this script to produce a result. |
