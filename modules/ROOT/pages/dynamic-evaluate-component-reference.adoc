...
* A File Read operation uses the locale query parameter name received in the HTTP request to search locally the DataWeave script that matches the parameter name.
...
+
. In the *Advanced* tab, set the *Target Variable* field with `dwScript`. +
...
. Send a POST request to `+http://localhost:8081/test+` appending any of the following query parameters to the URL `locale=en-US` or `locale=es-AR`, for example: +
`+http://localhost:8081/test?locale=en-US+`
+
Additionally, in the POST request also send the following payload:
+
