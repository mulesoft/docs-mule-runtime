...
== Evaluate DataWeave Script from a File System Example

The following example shows that based on a POST request made to the Mule application, the Dynamic Evaluate Component selects and executes a DataWeave script stored locally in the file system. The application's flow behavior is as follows:

* An HTTP Listener source listens to a POST URL request that contains a locale query parameter and a payload.
* A File Read operation uses the locale query parameter name received in the HTTP request to search locally the DataWeave script that matches the parameter name. Then, stores the content of the script into a variable.
...
The DataWeave scripts stored locally can be stored in a Database instead of locally, and the app can be modified to pick the files from there instead of the file system. This enables you to replace any of the `.dw` files while the app is deployed and running, changing the DataWeave script to execute without having to modify or redeploy the application.

To accomplish this example you must create two DataWeave scripts and stored them locally, create the Mule application, run and test the application.
...
To create and store the DataWeave scripts locally, follow the next steps:

. Create a new Mule project in Studio.
. In the *Package Explorer* window, under your Mule project right-click the *src/main/resources* folder and select *New>File*.
...
To create the Mule application in Studio, follow the next steps:

. In the *Package Explorer* window, under the *src/main/mule (Flows)* folder navigate to your flow `.xml` file.
...
. On the *General* tab, set the *Working Directory* field to the main resources folder where your Studio project and previous DataWeave scripts live, for example:
+
`/Users/username/AnypointStudio/studio-workspace/dynamicevaluate/src/main/resources`
+
. Click *OK*.
. In the *File path* field set the following path `#["dw_" ++ attributes.queryParams.locale ++ ".dwl"]`. +
The connector reads the locally stored DataWeave script files names that start with `dw_` and contains the locale query parameter (`attributes.queryParams.locale`) received in the HTTP request.
+
...
. Set the *Target value* to `payload`.
...
. Send a POST request using payload:
+
...
----
+
Also append any of the following query parameters to the POST URL `locale=en-US` or `locale=es-AR`, for example: +
`http://localhost:8081/test?locale=en-US`
+
. In the Studio Console, scroll through the logs to see the result message, for example:
+
