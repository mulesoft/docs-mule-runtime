...
* addSeqNo:  Adds a sequence number to the target filename making the filename unique, starting with 1 and incrementing the number until a unique filename is found. The default behavior is to throw an exception. This feature does not support concurrency, concurrent executions of flows that make use of this feature on the same set of files might result in duplicate file naming.
