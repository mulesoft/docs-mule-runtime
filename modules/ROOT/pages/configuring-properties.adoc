...
Also, a property can derive its value from properties that are higher in the hierarchy. Therefore, an application property can derive its value from environment, system, and/or deployment properties. A deployment property can derive its value from an environment and/or system property, and so on. For example, if there is a system property named 'env', an application configuration property can have the value 'file.$github.xml'.
