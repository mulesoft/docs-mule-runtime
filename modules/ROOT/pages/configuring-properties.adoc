...
So, for example, if a configuration property is defined in a System Property, and there is also an application configuration property, the value for that application is the one with the most precedence (in this case, the System Property).

Also, an application property could depend its value on Environment, System and/or Deployment properties. Deployment Properties could depend on System Properties, and so on. For example, if there is a System Property named `env`, a Configuration Property could have its value `file.${env}.xml`. 
