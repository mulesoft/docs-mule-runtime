...
[NOTE]
For the above example to work, you need to have the Mule repository defined in your `settings.xml` or your `pom.xml`. For details, see xref:using-maven-with-mule.adoc[Using Maven with Mule] and xref:configuring-maven-to-work-with-mule-esb.adoc[Configuring Maven to Work with Mule].
...
* For more information about developing Mule applications with Maven, see the overview in xref:using-maven-with-mule.adoc[Using Maven with Mule], and the guides at  xref:6.x@studio::building-a-mule-application-with-maven-in-studio.adoc[Building a Mule Application with Maven in Studio].
* Learn how to xref:6.x@studio::importing-a-maven-project-into-studio.adoc[import an existing Maven project into Anypoint Studio].
* Access additional Maven xref:maven-reference.adoc[reference].