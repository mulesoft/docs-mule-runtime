...
[[export-referenced-dwl-files]]
[NOTE]
====
Starting in Mule 4.5, the runtime validates that referenced `.dwl` files are accessible. If the `mule-artifact.json` file of your app declares an `exportedResources` array, or if the script comes from a dependency or shared library, export the `.dwl` file there so that the runtime can load it. Otherwise, the app fails to deploy with an error such as `Error loading referenced file for parameter 'resource'`. For more information, see xref:how-to-export-resources.adoc#how-to-export-resource-files[How to Export Resource Files].
====
