= Profiling Mule
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:keywords: anypoint studio, profiling, yourkit, monitoring, performance, memory, cpu, tuning

Profiling Mule runtime engine can help you identify memory leaks in your custom Mule extensions.

To profile Mule, you need to load a Java Profiler into the Mule instance. +
Although there are multiple Java profiler options available, MuleSoft recommends using https://www.yourkit.com/[YourKit], a commonly used Java profiler for analyzing JVM performance.
If you're not using YourKit, visit your respective profiler's documentation to understand how to hook into the Mule-Java process.

There are two main methods to profile a Mule instance:

* *Configuring Mule to point to the YourKit Agent*: For this method, you download the YourKit profiler, you configure it via the `wrapper.conf` file and then restart your Mule. To finalize the process, you connect the YourKit UI to your Local/Remote Mule. YourKit will be hooked up to your Mule instance.

* *Running the Profiler Agent in Attach Mode*: For this method, you download the YourKit profiler and configure its agent to run in attach mode. To finalize the process you run the profiler. Running in attach mode allows you to profile a running Mule instance without restarting it.

== Configuring Mule to point to the YourKit Agent

Configuring YourKit from the Mule `wrapper.conf` file requires you to restart your Mule instance.

In order to use your own YourKit profiler, you need to:
[start=1]
. https://www.yourkit.com/java/profiler/download/[Download] the YourKit build to your Mule server. You can download older build versions from the previous Profiler download releases page.
. Expand the YourKit package content and find the agent inside the `/bin/` folder.
. Enable the YourKit agent to interact with your Mule instance by adding the following line to the `wrapper.conf` file located in the `$MULE_HOME/conf` path:

----
wrapper.java.additional.<n>=-agentpath:<absolute-path-to-the-agent>=delay=10000
----

[NOTE]
The `<absolute-path-to-the-agent>` value varies depending on the configuration and OS of your machine. +
The `<n>` component of the Java Service Wrapper needs to be in sequence with the rest of the `wrapper.java.additional` components in your wrapper.conf file.

Assuming you downloaded `yjp-2013-build-13062` for Linux, for example, the entry to add in the `wrapper.conf` should look like this:
----
wrapper.java.additional.<n>=-agentpath:/yjp-2013-build-13062/bin/linux-x86-64/libyjpagent.so=delay=10000
----

By default, YourKit agent scans for an internet facing port. To set a specific desired port, you can add the following line: +
`wrapper.java.additional.<n>=-agentpath:<absolute-path-to-the-agent>=delay=10000,port=<port>`.

[start=4]
. Restart Mule and set the YourKit profiler UI to connect to the host, with a port (if the port was previously specified).

Check the YourKit documentation for a detailed explanation on how to properly launch the agent based on your operating system, and to get a full list of start up parameters.

=== Connect the YourKit UI to a Local Mule

. Launch YourKit on your local machine.
. In Monitor Local Applications, double click the `MuleContainerBootstrap` process
+
image::profiling-mule-286a5.png[width=500]
+
. YourKit should now be hooked up to your Mule instance.
+
image::yourkit-hooked-up.png[width=1000]

=== Connect the YourKit UI to a Remote Mule

. Launch YourKit on your local machine.
. In Monitor Remote Applications, click *Connect to remote application*.
+
image::monitor-remote-apps.png[width=500]
+
. Set up your server username and password.
. Configure the SSH authentication.
+
image::yourkit-auth.png[width=500]
+
. Click *OK*.
. After scanning for available applications, YourKit should be hooked up to your Mule instance.
+
image::yourkit-hooked-up.png[width=1000]

== Running the Profiler Agent in Attach Mode

Running the profiler in Attach Mode allows you to profile a running Mule instance without restarting it.

. https://www.yourkit.com/java/profiler/download/[Download] the profiler to the Mule server.
. Unpackage the profiler.
. Determine the PID of Mule by running `jps`.
+
[source,console]
----
jps
----
+
. Capture the PID of the `MuleContainerBootstrap`.
+
----
2838 MuleContainerBootstrap
21311 Jps
----
+
. Within the YourKit package, open the `bin` directory.
+
[source,console]
----
cd yjp-2015-build-15086/bin
----
+
. Run `yph.sh` including the `-attach` flag with the PID of `MuleContainerBootstrap`.
+
[source,console]
----
./yph.sh -attach 2838
----
+
. Verify the agent is attached
+
[source,console]
----
Attaching to process 2838 using default options
The profiler agent has attached. Waiting while it initializes...
The agent is loaded and is listening on port 10001.
You can connect to it from the profiler UI.
----

[NOTE]
The previous steps can vary in a Windows environment. Check the YourKit documentation if the previous steps cannot be translated.

=== Running the Profiler

To run the profiler, you run Mule with the *-profile* switch, for example:

----
./mule -profile
----

You can add any extra YourKit startup options with multiple parameters separated by commas, for example, *-profile onlylocal,onexit=memory*. This integration pack automatically takes care of configuration differences for Java 1.4.x and 5.x/6.x.


== See Also
https://www.yourkit.com/[YourKit]
