...
* *Configuring Mule to point to the YourKit Agent*: For this method, you download the YourKit profiler, you configure it in the `wrapper.conf` file, and then restart your Mule instance. To finalize the process, you connect the YourKit UI to your local or remote Mule instance.

* *Running the Profiler Agent in Attach Mode*: For this method, you download the YourKit profiler and configure its agent to run in attach mode. To finalize the process you run the profiler. Running in attach mode enables you to profile a running Mule instance without restarting it.
