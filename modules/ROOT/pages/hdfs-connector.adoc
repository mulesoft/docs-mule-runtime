...
The Anypoint Connector for the Hadoop Distributed File System (HDFS) is used as a bi-directional gateway between applications. Its source is stored at the http://mulesoft.github.io/mule-hadoop-connector[HDFS Connector GitHub site].
...
To use the HDFS connector in your Mule application, you must configure a global HDFS element that can be used by all the HDFS connectors in the application (read more about xref:global-elements.adoc[global elements]).
...
HDFS connector is an operation-based connector, which means that when you add the connector to your flow, you need to configure a specific operation for the connector to perform. The HDFS connector currently supports the http://mulesoft.github.io/mule-hadoop-connector[following list of operations]:
...
* Learn how to xref:using-maven-with-mule.adoc[use Maven with Mule].