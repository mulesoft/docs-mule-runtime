...
The Jetty transport provides support for exposing applications over HTTP by embedding a light-weight Jetty server. The xref:3.4@jetty-ssl-transport.adoc[Jetty SSL Transport] works the same way but over SSL. You can only define inbound endpoints with this transport.
...
Jetty endpoints are configured the same way as xref:3.4@https-transport-reference.adoc[HTTP endpoints]. Note that the Jetty transport can only be used for inbound endpoints.
