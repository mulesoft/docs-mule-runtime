...
* Avoid using an <async> scope to log messages

Enclosing a logger into an <async> scope doesn't provide improved parallelism, because the logging is already implemented in an asynchrnous way by the backend Log4j2 library, however it increases the resource usage and duplicates the numbers of threads required. Using this pattern in a repeated way in an application can cause thread exhaustion, low performance and backpressure.
