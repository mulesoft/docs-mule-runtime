= Migrating API Gateways
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

// authors: Federico Balbi and Nahuel Dalla Vecchia (assigned by Eva)

////
Here's an old Gateways migration guide example: https://docs.mulesoft.com/release-notes/api-gateway-runtime-to-mule-3.8.0-migration-guide
////

// Explain generally how and why things changed between Mule 3 and Mule 4.
This section covers the migration of API Gateway related features. Following topics will be explained:

* xref:migration-api-gateways-autodiscovery.adoc[Migrating API Gateways: Autodiscovery]
* xref:migrate-mule3-apps-managedby-apimanager.adoc[Migrating Mule 3 Applications from API Manager]
* xref:migration-api-gateways-policies.adoc[Migrating API Gateways: Policies]
* xref:migration-api-gateways-proxies.adoc[Migrating API Gateways: Proxies]
* xref:migration-api-gateways-runtime-config.adoc[Migrating API Gateways: Mule Runtime Configuration]

== See Also

xref:migration-core.adoc[Core Components Migration]