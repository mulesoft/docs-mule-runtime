...
Mule applications are configured to run in the Mule runtime engine. A request
...
the Validation, Java, Spring, OAuth modules, provide modules that act on
data in a Mule application without providing a direct connection to an endpoint.
+
* xref:about-components.adoc[Core Components]: Core components support
...
To break up processing into more manageable units, Mule applications often contain
...
Once a flow is triggered through a source component, subsequent components process
...
* Components from modules and connectors. Examples include operations that read
