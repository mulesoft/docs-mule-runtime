...
== Steps to Test the Examples Locally

If you want to test these examples on your end, ensure you follow these steps to create and configure your example Mule application:

. Create a new *Mule Project* in Anypoint Studio.
. Copy the XML code of the application you want to test into the *Configuration XML* view of your project.
. xref:studio::add-modules-in-studio-to.adoc[Add the file connector module] to your project.
. Create a xref:global-elements.adoc[global configuration element] for the File connector.
.. Set the value for *Working Directory* (`workingDir`) as `#[mule.home ++ "/apps/<your_app_name/]"`
...
. Configure a xref:configuring-properties.adoc[properties file] for your project.
+
Set the content of the properties file with the following values:
+
----
...
. Create a file named `flatFileJson.json` in the `src/main.resources` folder of your Mule application and set its content with the <<json-transformation-output, JSON transformation output>> from the Example of Reading a Flat File.
