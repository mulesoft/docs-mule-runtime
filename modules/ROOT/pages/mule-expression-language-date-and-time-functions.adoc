...
This reference page provides lists and brief explanations of the date and time functions in MEL. For explanatory information, see xref:mule-expression-language-mel.adoc[Mule Expression Language], xref:mule-expression-language-basic-syntax.adoc[MEL Basic Syntax], and the MEL examples.
...
Equivalent to: http://docs.oracle.com/javase/7/docs/api/java/util/Calendar.html#add(int,%20int)[calendar.add(Calendar.SECOND, add);]
 ...
 |as above
|plusMinutes(int add) |Returns the dateTime with the given amount of days added (or subtracted if it is a negative value). +
