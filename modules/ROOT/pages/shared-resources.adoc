...
Apps can only be associated with one domain at a time. To associate an existing app with a domain from Studio:
...
To associate an app with a domain outside Studio, just add the domain dependency to the `pom.xml` file as shown in the previous example.
