...
[[assoc_apps_domain]]
=== Associating Apps with the Domain

Apps can only be associated with one domain at a time.

==== Using Studio

To associate an existing app with a domain from Studio:
...
To associate an app with a domain outside Studio, you have several options:

. If you don't set any domain dependency, the application will use the `default` domain.
. Just add the domain dependency to the `pom.xml` file as shown in the previous example.
. Since Mule 4.2.2, the version honors semmantic versioning, so if you set the version `1.0.1`, a domain with version `1.0.2` and greater will work too, but `1.0.0` will not.

[NOTE]
====
Since Mule 4.2.2, if there are several domains deployed with the same group id, artifact id, and version, the pom reference may be ambiguous. In order to avoid the ambiguity, you need to add the domain name in the application `mule-artifact.json` file:
