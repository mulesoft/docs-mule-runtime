...
This creates a `.jar` file that you can deploy to a standalone Mule runtime engine.
+
image::mruntime-export-studio.png[]
+
. Copy the exported domain's JAR file to `MULE_HOME/domains`.
. Copy the JAR file for each application that references this domain into the `MULE_HOME/apps` folder.
