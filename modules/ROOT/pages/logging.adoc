...
You can configure what gets logged, where it gets logged, and how by editing a configuration file from your Studio project or in a folder in your standalone Mule server's `$MULE_HOME` directory. For instructions on how to modify the file, see xref:logging-in-mule.adoc[Configuring Logging].
