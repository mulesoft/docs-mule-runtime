...
You can specify the namespace on any XML-based functionality in Mule, including the JXPath filter, Jaxen filter, XPath filter, filter-based splitter, expression splitter, round-robin splitter, JXPath extractor transformer, and XPath expression transformer in the xref:xml-module-reference.adoc[XML Module Reference], xref:xpath-annotation.adoc[XPath Annotation], as well as the SXC filter and filtering router in the xref:sxc-module-reference.adoc[SXC Module Reference].