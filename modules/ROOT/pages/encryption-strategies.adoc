...
The Security Manager can be configured with one or more encryption strategies that can then be used by encryption transformers, security filters, or secure transports such as xref:3.7@ssl-and-tls-transports-reference.adoc[SSL] or xref:3.6@http-connector.adoc[HTTPS]. These encryption strategies can greatly simplify configuration for secure messaging as they can be shared across components.
