...
The examples use Parse Template to load data dynamically into HTML.

=== Use Parse Template to Process a DataBase Query

This example uses Parse Template in a Mule application that loads employee data from a database into an HTML template. The flow returns the template-built payload to the caller.

image::parse-template-flow.png[width="55%"]

==== Template

The template selects fields containing name, department, job title, start date, and employee type values from the payload.
...
In the Mule configuration XML for the example, Parse Template loads the template file through the setting, `location="src/main/resources/responseHtml.template"` The flow returns a template-built payload.
...
To trigger the template through, you must send an HTTP request with a URL that includes an ID query parameter, such as `+http://localhost:8081/getEmployee?id=12345+`.

=== Dynamically Generate Data in HTML Rows

The following example uses Parse Template on employee records that are represented as elements in a JSON array.

image::parse-template-flow2.png[width="75%"]

==== Input to Parse Template

Assume that Parse Template component receives the following payload.
...
The following template uses a DataWeave script to iterate over the input array and generates an HTML row for each element. The script uses the DataWeave xref:dw-core-functions-map.adoc[map] function with xref:dw-core-functions-joinby.adoc[joinBy].
...
In the Mule configuration XML for the example, Parse Template loads the template file through the setting, `location="/path/to/my.template"` The flow returns a template-built payload.
...
To use the application in this example, send an HTTP request with a URL, `+http://localhost:8081/parseit+`.

==== Output of the Template

The output payload of the <<template, template example>> produces HTML with the dynamically produced rows and data from the input array.
