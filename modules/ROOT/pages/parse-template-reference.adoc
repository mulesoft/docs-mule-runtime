...
Parse Template supports the use of expressions within a template and the use of literals within these expressions. The template recognizes certain special characters, such as the sequence `\#[`, quotations marks (`'` or `"`), apostrophes (`'`), and `$`. You can escape such characters to treat them as regular characters.
