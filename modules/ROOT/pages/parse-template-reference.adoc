...
Parse Template supports the use of nested expressions and literals inside expressions. For example, the component can process an expression such as `'hello ++ upper("world")'`

The following characters are for special uses:

* An expression, which begins with the sequence `+#[+` can contain nested expressions. Inside an expression, the character `[` defines the start of an expression. The character `[` does not need a preceding `+#+`. The character `[` is treated as a special character inside an expression.
* Inside an expression, single quotes are used both to quote text and to treat text as a literal. For example, the expression `+#['[']+` is a valid expression that evaluates to `[`. When used outside of expressions, single quotes are treated as literals.
* To avoid receiving an error for apostrophes and quotation marks within a string that is part of an expression, add a backslash to escape these characters. For example, `+#['this isn\'t an expression']+` escapes the apostrophe within the string, which is surrounded by single quotation marks.
* It is not necessary to wrap the characters (`#`, `[`, and `]`) in quotation marks. For example, you can write `+\#[ the-expression-to-resolve ]+`. For example, the component transforms `<td>#[upper("mulesoft")]</td>` to `<td>MULESOFT</td>`. The DataWeave expression `upper("mulesoft")` is the value of the `<td/>` element.
