...
* Escape character (`\`):
+
Parse Template uses the character sequence `\#[` to identify where an expression begins. To avoid this behavior and treat that sequence as literal characters, you must escape it with `\`, for example, `<td>\\#[</td>`.
+
