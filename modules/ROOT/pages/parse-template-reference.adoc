...
* After starting an expression is stated (by the sequence `#[`) it could contain nested expressions. Inside an expression, the character `[` can define the start of an expression without the need to add the character `#`, altough it can be used as well. So, the `[` character is considered as a special character inside an expression.
* Inside an expression single quotes are used to quote text and treat it as literal. So, the expression `#['[']` is a valid expression and will evaluate to `[`. Outside expressions, single quotes are used as literals. 
