...
Assume that you start the flow with a URI that uses a query parameter based on the ID of an employee, such as `+http://localhost:8081/getEmployee?id=1234+`, and that the Select operation returns the following input to Parse Template:
...
The output payload of the <<template2, template example>> is HTML with the dynamically produced rows and data from the input array.
...
The following template uses a DataWeave script to iterate over the input array and generates an HTML row for each element.
