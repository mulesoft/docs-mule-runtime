...
The examples use Parse Templates to display data dynamically in HTML.
...
that accepts queries by employee ID to acquire data about an employee. The flow returns the template-built payload to the caller.

image::parse-template-flow.png[width="55%"]

==== Template

The template inserts values for fields (name, department, job title, start date, and employee type) drawn from the message payload.
...
==== Configuration XML for the Example

The Mule configuration XML uses a file external to the flow as a template into which it inserts employee values from the payload. The flow then returns the template-built payload.
...
The following example demonstrates the use of a Parse Template on employee records that are represented as elements in a JSON array.
...
The following template uses a DataWeave script to iterate over the input array and generates an HTML row for each element. It uses the DataWeave xref:dw-core-functions-map.adoc[map] function with xref:dw-core-functions-joinby.adoc[joinBy].
  ...
  <body>
    <table>
      ...
      <tbody>
        ...
        <tr>
          <td><span>Annie</span></td><td><span>Easley</span></td>
