...
* Escape character (`\`):
+
Expressions can contain strings. String data is surrounded by quotation marks, for example, `'this is a string'`. To escape any special characters within a string that is embedded inside an expression, append `\` to the character. Examples of special characters include the sequence `\#[`, quotations marks (`'` or `"`), apostrophes (`'`), and `$`. It is not necessary to escape `\#` or `[` unless they adjacent to one another in the string, with the `\#` preceding the `[`.
+
.Parse Template Snippet:
[souce,XML,linenums]
----
