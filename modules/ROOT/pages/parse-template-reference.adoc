...
that accepts queries employee records to acquire data about an employee. The records are represented as elements in a JSON array.

image::parse-template-flow.png[width=75%]
...
[[template_1]]
The following template extracts the values of the key-value pairs in the first element of the array of employee records:
...
[[template_2]]
For a more dynamic template that iterates over the input array and generates an HTML row for each element, you can use the DataWeave xref:dw-core-functions-map.adoc[map] function with xref:dw-core-functions-joinby.adoc[joinBy], for example:
...
The configuration uses a file external to the flow as a template into which it inserts employee values from the payload. The flow then returns the template-built payload to the caller.
...
The output payload of the <<template_1, first template>> looks like this:
	...
	<body>
		<table>
			...
			<tbody>
				<tr>
					<td>[Melba, Annie]</td>
					<td>[Roy Mouton, Easley]</td>
					<td>[Computers, Software Development]</td>
					<td>[Assistant Chief of Research Programs, Technical Lead]</td>
					<td>[01/01/2021, 06/02/2020]</td>
					<td>[mathematician, Rocket Scientist]</td>
				</tr>
			</tbody>
		</table>
	</body>
...
The output payload of the <<template_2, dynamic example>> looks like this (reformatted for readability):
	...
	<body>
		<table>
          ...
          <tr>
              ...
              <td><span>01/01/2021</span></td><td><span>mathematician</span></td>
            </tr>
            <tr>
              <td><span>Annie</span></td><td><span>Easley</span></td>
              <td><span>Software Development</span></td>
              <td><span>Technical Lead</span></td><td><span>06/02/2020</span></td>
