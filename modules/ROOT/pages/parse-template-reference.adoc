...
Parse Template supports the use of nested expression and literals inside expressions. Characters for special uses: 
...
* Inside an expression, single quotes serve to quote text and to treat text as a literal. So, the expression `#['[']` is a valid expression that evaluates to `[`. Outside expressions, single quotes are used as literals. 
