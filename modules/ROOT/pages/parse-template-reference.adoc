...
Parse Template supports the use of expressions within the template and of literals within these expressions. The template recognizes the syntax `+\#[ the-expression-to-resolve ]+. Notice that the expression is surrounded by the characters `+#[]+`. For example, `<td>#[payload]</td>` and
`<td>#[upper("mulesoft")]</td>` contain expressions.

Any strings within the expression must be surrounded by quotation marks. String interpolation of an expression (from within the quotation marks around a string) is supported when the expression
is surrouned by `+#[]+`. Otherwise, the expression must reside outside the quotation marks. For example, the first two expressions within a template snippet return the same results, but the last one returns a string, without processing the embedded DataWeave function.
...
To escape an `#[`, quotations marks (`'` and `"`) apostrophes (`'`), and other special characters (such as `$`) within a string that is embedded inside an expression, append the `\`. Note that you do not need escape a `#` or `[` from a string unless the characters are adjacent to one another, with the `#` preceding the `[]`.

.Escape Characters From With a Template:
[souce,XML,linenums]
----
