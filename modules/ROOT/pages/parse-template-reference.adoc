...
The following template uses a DataWeave script to iterate over the input array and construct an HTML for each object in the array:
...
The output payload of the <<template2, template example>> is HTML with the dynamically produced rows and data from the objects in the array.
