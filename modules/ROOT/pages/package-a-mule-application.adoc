...
When this happens, the deployment doesn't complete and the runtime logs a fatal error similar to this one:
...
* Deploy the application to a runtime whose version is equal to or later than `minMuleVersion`.

[NOTE]
The required Mule version can be a later one than the version you set if a connector or other application dependency requires a later Mule runtime version. In that case, the error message reports the version required by the dependency. Verify the required version in the error message, and then either upgrade the target Mule runtime version or use a dependency version that's compatible with your runtime.
