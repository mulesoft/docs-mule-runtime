...
The `minMuleVersion` field declares the minimum Mule runtime version that the application requires. Deployment fails if you deploy the application to a runtime whose version is later than `minMuleVersion`, because the runtime can't guarantee that the features the application depends on are available.
