...
.. On *Windows*, this step is required. If `JAVA_HOME` isn't set, the license installation fails.
...
.. On *Windows*, copy the `license.lic` file into the `\bin` folder, then run `mule.bat -installLicense license.lic`.
