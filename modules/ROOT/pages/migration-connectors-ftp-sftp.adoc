...
only archives to another director on the SFTP server, not to a local drive. In
fact, the attribute is not intended for archiving to a local drive in Mule 3.

However, you can achieve this use case in Mule 4 with the File connector,
using its Write operation.
