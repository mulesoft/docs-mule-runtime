...
[start=2]
. Next, add a File Write component to save the output of the request for lorem ipsum text to a file on your file system:
...
. In *Connector Configuration*, click image:mruntime-add-plus.png[], and set up the *Host* and *Port*.
. Click *OK*.
...
. In *Connector Configuration*, click image:mruntime-add-plus.png[], and set up the *Host*, *Port*, and *Base Path*. This configuration sends requests to `http://baconipsum.com/api`.
. Click *OK*.
...
. In Studio, within your project, click image:mruntime-run-button.png[] to run the example as a Mule application.
