...
|IMAP |[JavaDoc Schema Doc] |image:check.png[] |image:error.png[] |image:error.png[] |image:error.png[] |image:error.png[] |image:error.png[] |one-way |one-way |org.mule.transport:mule-transport-imap
|IMAPS |[JavaDoc SchemaDoc] |image:check.png[] |image:error.png[] |image:error.png[] |image:error.png[] |image:error.png[] |image:error.png[] |one-way |one-way |org.mule.transport:mule-transport-imaps
|===
...
|deleteReadMessages |Whether to delete messages from the server when they have been downloaded. If set to false, the message are set to defaultProcessMessageAction attribute value. |True |No
...
|===
|Element |Description
|tis-client a|Configures the client key store with the following attributes:
...
The email transports are implemented by the mule-transport-email module. You can find the source for the email transport under transport/email.
