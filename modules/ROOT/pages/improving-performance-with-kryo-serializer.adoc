...
*Community Edition* - The open source Serialization API available in xref:https://github.com/mulesoft/mule-api/blob/master/src/main/java/org/mule/runtime/api/serialization/ObjectSerializer.java[GitHub in the ObjectSerializer.java interface] lets you serialize or deserialize objects into a byte array.
...
* If you are going to serialize and deserialize in a Mule Runtime, then you must define `getInternalProtocol()`. 
* On the other side, if you are going to use the serialized object in a external system, you must define that in `getExternalProtocol()` method. A xref:https://github.com/mulesoft/mule-api/blob/master/src/main/java/org/mule/runtime/api/serialization/SerializationProtocol.java[Serialization Protocol] is the Object that will be used to serialize and deserialize. 


=== Configuring a Custom Serializer

Configure the default ObjectSerializer for the Mule application using the Mule `<configuration>` tag:
...
<beans xmlns="http://www.springframework.org/schema/beans"
	...
	<bean id="customSerializer" name="customSerializer"
		class="com.my.CustomSerializer" scope="singleton">
		<!-- ... --> 
	</bean>
</beans>
----

If `defaultObjectSerializer` is not configured, then the default implementation uses plain old Java serialization. This means that from a user perspective, there’s no behavioral change unless you specifically set a different serializer.

[NOTE]
The Batch module is the only component that does not have its behavior affected, because the module always uses serialization.

=== Obtaining a Configured ObjectSerializer

There are many ways to obtain an `ObjectSerializer` inside Java code. Generally, the recommended approach is through dependency injection. The following code example shows you how to get the currently configured `ObjectSerializer`:
...
The Kryo without compression is significantly faster than the plain old Java serializer in all cases. However, the compression mode only provides an actual improvement on the high availability (HA) cases.
