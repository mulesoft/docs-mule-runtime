...
[[ex03]]
The following example uses an index to select the second child element from the input XML. Notice that DataWeave treats child elements as indices of an array, so it can select the second child by using the index `language[1]`. 
