...
[NOTE]
====
This section deals with starting and stoping Mule and deploying applications. To take full advantage of managing and monitoring features, you can deploy to the same Mule via the xref:runtime-manager::index.adoc[Runtime Manager]. See xref:runtime-manager::deployment-strategies.adoc[Deployment Strategies] for a deeper look at the different deployment alternatives offered by the platform.
====
