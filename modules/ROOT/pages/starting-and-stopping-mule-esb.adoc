...
For more information about the `wrapper.conf` file, consult the Java Service Wrapper  http://wrapper.tanukisoftware.com/doc/english/properties.html[online documentation] for the file, and to configure Mule 3 wrapper properties, see xref:configurable-wrapper-properties.adoc[Configurable Wrapper Properties] documentation.
