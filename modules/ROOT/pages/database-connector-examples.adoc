...
This document presents two usage examples of the database connector. This document assumes you are familiar with the xref:database-connector.adoc[database connector] as well as with xref:5@studio::datasense.adoc[DataSense]; to understand the second example, the reader should also be familiar with the
xref:5@studio::datamapper-visual-reference.adoc[Anypoint DataMapper Visual Reference] and Mule xref:batch-processing.adoc[batch processing]. You should also see xref:dataweave.adoc[DataWeave], which replaces DataMapper.
...
In this example, a database connector retrieves data from two tables. A xref:5@studio::datamapper-visual-reference.adoc[DataMapper transformer] maps this data to the CSV format. A xref:file-connector.adoc[file connector] writes the resulting CSV file to disk, and a xref:logger-component-reference.adoc[logger component] logs processing details to the console.
