...
This document offers examples of applications which use the Database Connector. For full documentation of the connector, access the xref:3.6@database-connector.adoc[Database Connector] documentation and xref:3.6@database-connector-reference.adoc[Database Connector Reference].

== Assumptions

This document presents two usage examples of the database connector. This document assumes you are familiar with the xref:3.6@database-connector.adoc[database connector] as well as with xref:3.6@datasense.adoc[DataSense]; to understand the second example, the reader should also be familiar with the xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] and Mule xref:3.6@batch-processing.adoc[batch processing].
...
In the Mule application, an inbound xref:3.6@http-connector.adoc[HTTP connector] listens for HTTP GET requests with the form: `http://<host>:8081/?lastname=<parameter>`. The HTTP connector passes the value of `<parameter>` as one of the message properties to a database connector. The database connector is configured to extract this value and use it for the SQL query listed below.
...
In this example, the database connector references the `MySQL_Configuration` xref:3.6@global-elements.adoc[global element] to obtain connection parameters. `MySQL_Configuration` is configured with the parameters listed below.
...
In this example, a database connector retrieves data from two tables. A xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] maps this data to the CSV format. A xref:3.7@file-connector.adoc[file connector] writes the resulting CSV file to disk, and a link:/mule-user-guide/v/3.3/logger-component-reference[logger component] logs processing details to the console.

This example uses xref:3.6@batch-processing.adoc[batch processing], which means that within this Mule app, messages are divided into records and processed asynchronously. Batch processing, in conjunction with *streaming* enabled on the database connector, can be useful to avoid system overload when dealing with large volumes of data.

image::ex-2-batchjob-flow.png[ex.2-batchjob.flow]

The database connector is inside a xref:3.6@poll-reference.adoc[poll scope] which, every ten minutes, requests the database connector to perform its configured operation on the database. The database connector performs a database query which returns 100 rows. Mule feeds this data into the DataMapper transformer inside the `Batch Commit` batch scope. DataMapper transforms the input into a CSV file; then, a xref:3.7@file-connector.adoc[file connector] writes the resulting CSV file to disk. A link:/mule-user-guide/v/3.3/logger-component-reference[logger component] logs processed records to the console.
...
* Learn more about xref:3.6@datasense.adoc[DataSense] and how you can use it to retrieve metadata on remote applications.

* Delve deeper into xref:3.6@batch-processing.adoc[batch processing] to gain a deeper understanding of how Mule handles messages in batch jobs.