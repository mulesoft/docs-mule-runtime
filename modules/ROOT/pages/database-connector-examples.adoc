...
In this example, a database connector retrieves data from two tables. A xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] maps this data to the CSV format. A xref:3.7@file-connector.adoc[file connector] writes the resulting CSV file to disk, and a https://docs.archive.mulesoft.com/mule-user-guide/v/3.3/logger-component-reference[logger component] logs processing details to the console.
