...
1. Error handling: since the first mentioned difference, For Each execution will be interrupted when an error is raised (and the Error Handler will be invoke), while Split Aggregate will work as defined before, processing every route before invoking the Error Handler with a `MULE:COMPOSITE_ROUTE` error type.
