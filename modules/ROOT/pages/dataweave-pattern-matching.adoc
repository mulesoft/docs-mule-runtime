= Pattern Matching in DataWeave using a match statement
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

The *match* statement behaves like a match or switch statement in other languages, like Java or C++, and routes an input expression to a particular output expression based on some conditions. The syntax for a match statement is:
...
Like in other languages, the DataWeave match statement is an alternative and more compact way to organize multiple chained together if/else statments. A match statement consists of a list of case statements and perhaps a final else statement. Each case statement consists of a conditional selector expression that must evaluate to true or false, which corresponds to the expression you would write in an if statement's condition, followed by a correponding DataWeave routing expression that can include the match operator's input expression. 

Each case statement can be an if statement, or can use other pattern matching shortcuts to define the case statement's condition. If a case statement is false, its routing expression is ignored. 
...
Here are a few examples of case statements in a match statement. Other more detailed examples follow. 
...
Notice how the input expression can be more compactly referred to by each case's local variable name, except the else statement that can refer to the input expression using the default parameter name $. 

If you want to name the input expression in the else statement, you can replace the else statement with an always true case statement: 
...
Matches when the evaluated value fits a given Java regular expression. In this example, the input payload includes an array of strings. The script uses the `map` function to iterate through the array. It evaluates each element against a Java regular expression and outputs an object based on matches to the input.
