...
For use cases in which the input is a String, you can also use the xref:dw-core-functions-match.adoc[match] _function_ in the `dw::Core` module to test the string against a regex pattern. The function returns an array with the string that matches the entire regex and its capture groups.

For use cases that require a Boolean result based on whether a String  matches a regex pattern, you can use the xref:dw-core-functions-matches.adoc[matches] _function_ in the `dw::Core` module instead of using pattern matching or the dw::Core module's `match` function.
