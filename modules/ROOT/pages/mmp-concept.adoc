...
== Skipping files from being packaged

You can exclude certain files or directories from being packaged. To do so, you need to create a `_muleExclude` file at the project root directory.

Each line in a `_muleExclude` file specifies a https://docs.oracle.com/javase/tutorial/essential/io/fileOps.html#glob[glob pattern].
...
The first line indicates that every file named `fileToBeExcluded.json`, no matter where it is placed, will be excluded when packaging. +
The second line indicates that the directory named `local_directory` placed under `src/main/resources` will be excluded when packaging. If the pattern were `local_directory` instead of `**/src/main/resources/local_directory` it would exclude all directories named `local_directory` no matter where they were placed.
