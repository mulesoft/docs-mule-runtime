...
[NOTE]
Currently, Mule Maven plugin does not support the new Mule runtime version schema introduced with Mule 4.5. For Mule runtime 4.5, Mule Maven plugin deployment selection is done on the xref:release-notes::mule-runtime/lts-edge-release-cadence.adoc#edge_channel[Edge release channel] and automatically defaults to the latest patch version. See xref:release-notes::mule-runtime/lts-edge-release-cadence.adoc[].
