...
`xref:mmp-concept.adoc#package-goal[package]` generates the jar file for your project.

`xref:mmp-concept.adoc#deploy-goal[deploy]` is used to automatically upload and start your application into any of the Anypoint Platform or Standalone environments.

`xref:mmp-concept.adoc#undeploy-goal[mule:undeploy]` is used to automatically remove your application from any of the Anypoint Platform or Standalone environments.
...
|onlyMuleSources | boolean | Generates the application jar file with only the source sode. This property is just for sharing purpose.
|attachMuleSources | boolean | Attachs the source code inside the generated jar file.
|lightweightPackage | boolean | Doesn't generates the `repository` with all the application dependencies inside the jar file. This property is just for sharing purpose.
|===

=== Deploy Goal

Uploads and deploy the application JAR file to any of the Anypoint Platform or Standalone environments.
...
|muleDeploy | boolean | If it's set to true, the plugin will perform a deploy to any of the Anypoint Platform or Standalone environments defined on the plugin configuration.
In the othe case, it will upload the artifacts to the repository defined on the `distributionManagement` section on the apps pom.xml.
|===

=== Undeploy Goal

Removes the application from any of the Anypoint Platform or Standalone environments.
Will get the information from the plugin configuration in order to get and remove the application from any of the environments defined.
