...
* A `maxConcurrency` setting on the Flow component affects the flow's Scheduler. The setting affects performance because it limits the number of instances of the flow that can run at the same time.
+
Setting the value to `1` prevents the Scheduler from triggering more than one execution of the flow at a time. When an instance is running, the application skips and avoids queuing any new events that are triggered by the Scheduler. Once that instance of the flow terminates and the Scheduler triggers an event, a new instance of the flow executes, and the same blocking and event-skipping pattern occurs.
+
