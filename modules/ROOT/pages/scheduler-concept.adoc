...
The Scheduler component is a Mule message source (trigger) that enables you to trigger a flow based on a time-based condition. You can schedule the flow to start at a regular interval or based on a cron expression. For example, a Scheduler might trigger the flow to start every 5 seconds. When starting the flow, the component generates a Mule message with an unset `payload` and `attributes`, which becomes available the downstream processor.
