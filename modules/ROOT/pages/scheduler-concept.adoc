...
A `maxConcurrency` setting on the Flow component affects the flow's Scheduler. The setting limits the number of instances of the flow that can run at the same time. Setting the value to `1` prevents the Scheduler from triggering more than one execution of the flow at a time.
