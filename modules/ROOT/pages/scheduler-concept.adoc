...
| *Expression*
| `expression`
| A a cron expression for triggering the Scheduler. There is no default cron expression. For guidance with expressions, see <<cron_expressions>>. Example: `expression="1 1 1 1,7 * ?"`
...
In Anypoint Studio xref:studio::to-run-debug-mode.adoc[debug mode], the Mule application disables the triggering of events. You can manually execute the Scheduler component when debugging by clicking the green arrow icon in Studio that appears at the bottom of the component, for example:
