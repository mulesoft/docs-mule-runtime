...
The Scheduler component is an event source in a flow that initiates execution of that flow at configured times. You can configure a fixed and repeating frequency (such as every second) or a cron expression. A cron expression can specify one or more specific dates and times, but more commonly, the expression specifies recurring dates, times, or intervals. For example, the cron expression `* * * ? * *` configures a Scheduler to start its flow every second.

A `maxConcurrency` setting on the Flow component affects the flow's Scheduler. The setting affects performance because it limits the number of instances of the flow that can run at the same time. Setting the value to `1` prevents the Scheduler from triggering more than one execution of the flow at a time.
...
* * If you set the `maxConcurrency` attribute of a Flow component to `1`, only one instance of the flow can execute at a time. When an instance is running, the application skips and avoids queuing any new events that are triggered by the Scheduler. Once that instance of the flow terminates and the Scheduler triggers an event, a new instance of the flow executes, and the same blocking and event-skipping pattern occurs.
+
See also, `disallowConcurrentExecution` in xref:scheduler-xml-reference.adoc#scheduler-properties[<scheduler> Properties].
