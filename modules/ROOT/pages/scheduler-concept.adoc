...
// TODO: MULE-14438
* If xref:execution-engine.adoc#backpressure[back-pressure] is triggered because no resources are available at the time of the trigger, that trigger is skipped. In this case a *MULE:FLOW_BACK_PRESSURE* error is thrown indicating the flow cannot accept new events at this time.  
...
In the Studio *Debug Mode* view, the Mule app disables triggering of Schedulers. When debugging, you can
instead trigger a Scheduler component when debugging by clicking the green arrow icon
