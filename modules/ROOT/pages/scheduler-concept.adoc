...
* The fixed frequency setting in this Scheduler triggers the generation of a Mule message in a new execution of the flow every fifteen seconds:
+
[source, xml]
----
...
By default, the Scheduler does not wait for one execution of a given flow to complete before executing again at the configured cadence. You can change this behavior by setting the `disallowConcurrentExecution="true"` attribute directly in the configuration XML.
...
| When this attribute is set to `true`, the Scheduler skips its scheduled execution if a prior execution of the flow is still processing. Once processing the flow finishes processing, the Scheduler executes at its next scheduled interval. The default value is `false`.
|===
