...
The Scheduler component is a Mule message source that triggers the execution of a flow based on a time-based condition and creates a new Mule message for each execution of the flow. For example, a Scheduler might start a new instance of the flow and create a new Mule message for that flow every 5 seconds or every day at 12:00 in a given time zone.
