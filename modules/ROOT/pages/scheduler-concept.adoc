...
The Scheduler component is a Mule message source that triggers the execution of a flow based on a time-based condition and creates a new Mule message for each execution of the flow. For example, a Scheduler might execute every 5 seconds.
...
Like other Mule message source components, a Scheduler is first the component in a Mule flow.
...
At least one processor must follow the Scheduler. Processors include Mule components and connector operations, such as an HTTP Request operation or Transform Message component. Failure to provide a processor produces a `MuleRuntimeException` and causes the deployment of the Mule application to fail.
...
The Scheduler elements accepts properties for configuring the execution interval and concurrency:
