...
Support for specifying both a day of the month and a day of the week simultaneously in a cron expression isn't complete. One of these values must be set to `?`. See https://www.quartz-scheduler.org/api/2.3.0/org/quartz/CronExpression.html[CronExpression].
