...
The Scheduler component is a Mule message source that triggers the execution of a flow based on a time-based condition and creates a new Mule message for each execution of the flow. For example, a Scheduler might execute every 5 seconds, starting a new instance of the flow and creating a new Mule message with each execution.
...
+
To set a time zone for the Scheduler, see `timeZone` in <<props_cron_expressions>>.
* The `disallowConcurrentExecution` property enables you to prevent the Scheduler from triggering the next execution of a flow while the previously executed flow is still running.
...
| When this attribute is set to `true`, the Scheduler skips its scheduled execution if the flow is still running. Once the flow is done, the Scheduler executes at the next configured interval. The default value is `false`.
|===
...
Several examples that you can find in https://www.mulesoft.com/exchange/[Anypoint Exchange^] show use cases for a Scheduler component:
