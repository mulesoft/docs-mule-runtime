...
The expression consists of six required settings and one optional setting
that represent the following date and time settings, in order:
...
Note that Quartz Scheduler special characters are allowed: 
...
Java `timeZone` values are supported. You should avoid the Java abbreviations, such as `PST` and `AGT`,
and instead use the full-name Java equivalents, such as `America/Los_Angeles` and 
...
In debug mode in Studio, the Mule app disables the triggering of events. You can instead
execute the Scheduler by clicking the green arrow icon that appears at the
bottom of the component when it is in the Studio *Debug Mode* view.
