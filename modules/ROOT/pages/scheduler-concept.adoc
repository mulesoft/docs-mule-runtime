...
* In a multi-worker CloudHub deployment, or in a custoemr-hosted Mule runtime cluster, the Scheduler runs (and then executes the flow) only on the primary node (that is, only in one particular Mule runtime).
...
This example logs the message "hello" every second, and only allows one flow instance to execute at a time:
