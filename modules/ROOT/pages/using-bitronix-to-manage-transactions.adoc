...
|`max-idle-time` |Defines the number of seconds an idle connection can stay in the pool before it is closed. | No |60
...
|`prepared-statement-cache-size` |Determines how many statements are cached per pooled connection. 0 means caching is disabled. | No | 5
|===
