...
There is only one instance of the Bitronix Transaction Manager running in Mule. All the applications that have Bitronix defined as their transaction manager use this instance. Because of this, the Bitronix Transaction Manager cannot be configured at the _application_ level, only at the _domain_ level.
...
Mule includes a transaction recovery procedure, no configuration is needed. When Mule restarts after a system crash, it connects to all of the resources involved in XA transactions and starts the transaction recovery process, leaving all systems in a consistent state.
