...
When using Mule Maven plugin versions 3.8.0 and 4.0.0 for deployments, the `muleVersion` property doesn't allow you to specify the release channel and Java version which may cause unexpected behavior. To specify the release channel and Java version, upgrade the plugin to version 4.1.1. | Yes 
