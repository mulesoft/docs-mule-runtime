...
To deploy the artifact without rebuilding it, run:
...
| `provider` | Provider MC (MuleSoft Control Plane) indicates that the deployment is managed through Anypoint Runtime Manager. Set to `MC` for CloudHub 2.0. | Yes
...
.5+! `inbound`
  ...
  ! `endpoint` ! Defines application endpoints by specifying the `url`, `pathRewrite`, and `access` attributes. Accepted values for `access` are `internal` and `external`.
  ...
  ! `lastMileSecurity` ! Enable Last-Mile security to perform TLS termination and decryption for forwarded HTTPS connections directly within the Mule application. This requires the Mule application to include an SSL certificate and consumes additional CPU resources. The default value is `false`.
  ! `forwardSslSession` ! Enables SSL forwarding during a session. The default value is `false`.
!===
[NOTE]
====
Mule Maven Plugin supports both `publicUrl` and `endpoint` for configuring application endpoints. Configure either `publicUrl` or `endpoints`, but not both in the same deployment configuration.
====
Configuration example:
[source,xml,linenums]
----
...
<deploymentSettings>
  <http>
    <inbound>
      <endpoints>
        <endpoint>
          <url>https://test1.com/ch2testapp1</url>
...
| `tracingEnabled` | Exposes tracing information in OpenTelemetry standard from Mule runtime into all APM solutions. See xref:cloudhub-2::ch2-deploy-private-space.adoc#enable-trace-data-collection[Enable Trace Data Collection].
...
| `instanceType` | Specifies the capacity of the replica for application deployment. This flag is available only for organizations using a usage-based pricing (UBP) package. Non-UBP organizations use the `vCores` parameter instead. See xref:general::pricing.adoc[].
