...
//CONFIRM IF THIS REQUIREMENT IS NECESSARY
//* You understand and have available the number of resources required to deploy to CloudHub 2.0 +
...
! `scope` ! The package name of the logging library to use.
! `logLevel` ! The log level control. Accepted values: `NONE`, `ERROR`, `WARN`, `INFO`, `DEBUG`, `TRACE`.
!===
| No
| `target` | The CloudHub 2.0 target name where to deploy the app. | Yes
