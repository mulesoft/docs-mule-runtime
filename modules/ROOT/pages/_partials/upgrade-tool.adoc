...
* Before running the upgrade tool, rename the `$MULE_HOME` directory to use either a generic name or one that matches the version you are upgrading to. For example, `mule-enterprise-4.9.8`. If you rename the directory after running the upgrade tool, Anypoint Monitoring stops working. To restore Anypoint Monitoring, uninstall and reinstall it.
// end::BeforeYouBegin[]
