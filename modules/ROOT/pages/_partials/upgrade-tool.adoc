...
* Mule distribution paths (`$MULE_HOME` and the new distribution path) that don't contain blank spaces. The upgrade tool doesn't support paths that contain blank spaces and exits with an error if it detects one.
