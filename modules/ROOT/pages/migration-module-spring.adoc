...
Because the class-loading mechanism implemented in Mule 4 prevents connectors from accessing classes from other connectors, you need to declare connector dependencies as shared libraries.

The Spring module doesn't include any particular Spring dependency. You can provide whatever ones you need by adding the following to your application's `pom.xml`:
...
* xref:mmp-concept.adoc#configure-shared-libraries[Configure Shared Libraries in Mule 4]
* xref:about-classloading-isolation.adoc[Class-loading Isolation in Mule 4]
