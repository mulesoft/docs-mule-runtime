...
There are several xref:mule-agents.adoc[agents] provided with Mule for JMX support. The easiest way to configure JMX is to use the default JMX support agent.

NOTE: This version of Mule uses log4j2, which is configured in the `$MULE_HOME/conf/log4j2.xml` file. References in this document to log4j no longer are applicable. For more information, see xref:logging-in-mule.adoc[Logging in Mule].
