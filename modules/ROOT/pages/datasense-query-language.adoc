...
In Anypoint Connectors which support the query functionality, the xref:5.x@studio::datasense-query-editor.adoc[DataSense Query Editor] allows you to easily query and retrieve complex data stored in a remote application. To define your query, you can proceed in one of two ways:
...
This document assumes that you are familiar with the xref:5.x@studio::datasense-query-editor.adoc[DataSense Query Editor]. The query language described in this document is used in conjunction with the Query Editor in the editor's properties panel.
...
* Learn about the xref:5.x@studio::datasense-query-editor.adoc[DataSense Query Editor] and how you can build your queries using a graphical interface

* Learn about xref:3.6@datasense.adoc[DataSense], which allows you to perform queries even if you do not know which objects and fields are available