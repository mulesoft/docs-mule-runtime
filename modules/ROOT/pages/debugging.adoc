= Debugging
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:keywords: debugging, debug, troubleshoot, stack, debugger, using logs, source code, source files, libraries

This section describes how to debug your Mule application. It also introduces a number of techniques useful in troubleshooting Mule problems and in gathering information for technical support.


* xref:configuring-mule-stacktraces.adoc[Configuring Mule Stacktraces]
* xref:debugging-outside-studio.adoc[Debugging Outside Studio]
* xref:logging.adoc[Logging]
* xref:logging-in-mule.adoc[Logging Configurations in Mule]

== See Also

* xref:6.x@studio::studio-visual-debugger.adoc[Debugging in Anypoint Studio's Visual Debugger]
* xref:6.x@studio::adding-source-attachments-to-studio.adoc[Adding Source Attachments to Studio]