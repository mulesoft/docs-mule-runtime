...
You can use the Mule Maven Plugin to package your application into a deployable ZIP file that you can later deploy to a running Mule Runtime.
...
The packager then packages your application and creates the deployable ZIP file into the target directory within your project’s folder.

If you want to import your generated ZIP into Anypoint Studio, you need to package your application using the `-DattachMuleSources` option.
...
The packager tool sends all files inside src/main/app and src/main/resources to the root directory of your binary package. +
...
== Deploy a Mule Application (Mule Maven Plugin)
...
----
. In your configuration deployment, reference the credentials id injecting the server id:
...
== Deploy a Mule App to a Standalone Mule Runtime (Mule Maven Plugin)
...
== Deploy a Mule App Using ARM API (Mule Maven Plugin)
