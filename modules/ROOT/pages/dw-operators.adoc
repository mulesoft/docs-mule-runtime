...
Before Mule 4.3, the following syntax is necessary to increase the `age` value by one without using the `update` operator or recreating the `Object` value of `myInput`:
...
In Mule 4.3 and later, the `update` operator is available to simplify the syntax:
...
As the `update` example shows, casting or iterating through all the key-value pairs is not necessary. 
...
* `update_expression` is the selector expression that matches a value in the `value_to_update`. 
...
* `[if(<conditional_expression>)]?` If the `conditional_expression` returns `true`, the script updates the value. This condition is required only for an optional update.
* `[!]` marks the selector as an upsert. If the expression doesn't match anything, marking an expression with `!` makes the `update` operator create all the required elements and insert the new value.
...
Any selector can incorporate an expression that makes the selection dynamic. 

The following example selects and dynamically updates a value of a DataWeave variable. The field is not hardcoded. The example uses `update` to change the value of the variable `theFieldName`, `name`, with a new value, `Shoki`:
...
The following example appends the specified key-value pair to the first object in the `myInput` array:
...
The `update` operator enables you to use a default variable `$` that avoids the need to use the "boilerplate" equivalent, which is longer.

The following example uses `$` to select the `age` value instead of the longer equivalent shown in comments:
