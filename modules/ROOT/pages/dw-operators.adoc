...
.Example: Using Prepend, Append, and Remove Operators on Arrays
[source,dataweave,linenums]
----
%dw 2.0
output application/json
...
DataWeave 2.0 supports the update operator that enables you to update a given field on a data structure with a new value. This use case is very common and was not simple to do in DataWeave.

_Introduced in DataWeave 2.3.0. Supported by Mule 4.3 and later._

.Example: Using Update Operator, let's say that you have an object:
[source,json,linenums]
----
{
    "name": "Ken",
...
And you want to update the age by increasing it with one, without the need of recreating all the object again. Using the current DataWeave syntax is:
...
Using the update operator will be:
...
* `[!]` marks then selector as an upsert, which means that if the selector is not matched then it is going to create it.
...
The update operator allows to insert a field if it is not present using the `!` symbol at the end of selector expression. When the field is not present the value `null` is going to be bind to the variable. +
...
payload  map ((user) ->
    user update {
        case name at .name! -> if(name == null) "JHON" else upper(name)
    }
)
----
...
The update operator allows to write a shorter version syntax to avoid some code boilerplate and this is by binding a default variable name `$`. +

Example: Same Value Selector example but using simple syntax.
