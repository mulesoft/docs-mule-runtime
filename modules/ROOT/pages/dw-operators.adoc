...
| `>>` | Prepends an array on the right side to data on the left side of the
         ...
         `1` to `2`.
| `<<` | Appends an array on the left side to data on the right-hand side of the
         operator. For example, `[1] << 2` results in `[ 1, 2 ]`, appending
         `2` to `1`.
| `+` | Appends the item on the right-hand side to the item in the array on the 
        left-hand side, for example, `[1] + 2` results in `[1, 2]`. To avoid errors, 
        do not use `+` with the array on the right-hand side of the operator. Use
        `>>` or `<<` instead. 
|===
