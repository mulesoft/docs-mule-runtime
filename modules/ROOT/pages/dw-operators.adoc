...
DataWeave supports a few operators for appending and prepending items within
an array.

[%header,cols="1,2"]
|===
| Operator | Description
| `>>` | Prepends an array on the right side to data on the left side of the
         operator. For example, `1 >> [2]` results in `[ 1, 2 ]`, prepending
         `1` to `2`.
| `<<` | Appends an array on the left side to data on the right side of the
         operator. For example, `[1] << 2` results in `[ 1, 2 ]`, appending
         `2` to `1`.
| `+` | Prepends the item on the left to the item on the right so long as on
        of the items is an array.
|===
...
Note that if you uncomment the last two examples, you can produce the error
