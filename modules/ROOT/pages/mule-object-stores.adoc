...
MuleSoft provides 3 different object stores:

* Object Store on-premises - This object store persists key-value pairs in memory for an application when Mule is used on premises. This object store is only limited by available memory, works with any on-premises Mule version, and can be set to be persistent or to only exist as long as Mule is running.
* Object Store v1 - This cloud-only object store runs in a single datacenter, supports up to 100,000 keys per application, 768 byte key size, 1 MB value size, and 1 GB of data per application. Object Store v1 is supported for use in Mule 3 applications.
* Object Store v2 - This cloud-only object store is available from multiple datacenters around the world, can have an unlimited number of keys, up to 10 MB values, and an unlimited amount of data. Data can persist for up to 30 days in Object Store v2. Object Store v2 is supported for Mule 3.8.5 and later, and all Mule 4 applications. Mule 4 cloud applications can only use Object Store v2.

To learn about Object Store v1 management in Runtime Manager, see xref:runtime-manager::managing-application-data-with-object-stores.adoc[Managing Application Data with Object Stores] or more generally, xref:cache-scope.adoc#configobjstore[Configuring an Object Store for Cache].

See also xref:object-store::index.adoc[Object Store v2].

NOTE: To access and manipulate object stores in Mule applications, download the https://www.anypoint.mulesoft.com/exchange/org.mule.modules/mule-module-objectstore/[Object Store connector] in Anypoint Studio. The Object Store connector works with both Object Store v1 and Object Store v2.
