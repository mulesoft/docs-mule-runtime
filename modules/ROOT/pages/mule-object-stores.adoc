...
To use the Object Store module in Anypoint Studio, you must first download the *Object Store Connector* from  https://www.mulesoft.com/exchange/[Anypoint Exchange].
...
* *In-memory store* – Prior to Mule 3.5.0, in-memory store was the default. As of Mule 3.5.0, the persistent store is the default. For more information, see "Object Stores and Clustering" in the xref:cache-scope.adoc[Cache Scope] document.
...
== Add Object Store Components

. Install the https://www.mulesoft.com/exchange/org.mule.modules/mule-module-objectstore/[Object Store component] available in Anypoint Exchange.
+
image::install-object-store-connector.png[]
+
...
. After installing the Object Store Connector in Studio, the connector is available in the Studio Palette.
. Drag the connector into a flow in your canvas. This creates a generic Object Store element.
+
...
. Select one of the available operations from of the drop-down menu.
+
[%header,cols="30s,70a"]
|===
...
|===
+
. Depending on the operation you select, different fields are available.

== XML Example

The following example demonstrates how to configure object stores in the following situations:
...
<!-- Global listable object store definition, used in Flow 3 below. -->
...
<!--  Idempotent Filter with In Memory Object Store -->
    ...
    doc:name="Flow1_idempotentWithInMemoryStore">
        <http:listener config-ref="HTTP_Listener_Configuration" 
        path="idempotentInMemory" doc:name="HTTP"/>
        <idempotent-message-filter 
        idExpression="#[message.payload]" 
        throwOnUnaccepted="true" 
        storePrefix="Idempotent_Message" doc:name="Idempotent Message">
            <in-memory-store name="myInMemoryObjectStore" 
            entryTTL="120" 
...
<!--  Idempotent Filter with Persistent File Store -->
    ...
    doc:name="Flow2_idempotentWithTextFileStore">
        <http:listener config-ref="HTTP_Listener_Configuration" 
        path="idempotentTextFile" doc:name="HTTP"/>
        <idempotent-message-filter 
        idExpression="#[message.payload]" 
        throwOnUnaccepted="true" 
        storePrefix="Idempotent_Message" doc:name="Idempotent Message">
...
<!--  Until Successful Scope with In Memory Object Store -->
    ...
    doc:name="UntilSuccessfulWithListableObjectStore">
        <http:listener config-ref="HTTP_Listener_Configuration" 
        path="hey" doc:name="HTTP"/>
        <until-successful objectStore-ref="myListableObjectStore" 
        maxRetries="15" 
        secondsBetweenRetries="1" 
        doc:name="Until Successful">
            <processor-chain doc:name="Processor Chain">
                <message-filter throwOnUnaccepted="true">
                    <expression-filter expression="return Math.random() &lt; 0.1" 
                    doc:name="Expression" />
                </message-filter>
