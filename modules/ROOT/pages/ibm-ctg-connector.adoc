...
To enable logging of the interactions of the connector with IBM CTG (request and response), a logger must be configured in the log4j2.xml file of the Mule application as follows (inside the Loggers tag in log4j2.xml):
