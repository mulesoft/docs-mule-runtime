...
Anypoint Connector for IBM CICS Transaction Gateway (IBM CTG Connector) provides integration with back-end CICS applications using CICS Transaction Gateway (CTG). For Java platforms, CTG implements the JCA resource adapter to connect Java applications to the CICS system. This connector provides access between Mule applications and CTG.

Release Notes: xref:release-notes::connector/ibm-ctg-connector-release-notes.adoc[IBM CTG Connector Release Notes] +
Reference: https://mulesoft.github.io/mule-ibm-ctg-connector/[IBM CTG Connector Reference] +
Exchange: https://www.anypoint.mulesoft.com/exchange/org.mule.modules/mule-ibmctg-connector/[IBM CTG Connector]
...
For information on pooling, see xref:tuning-performance.adoc[Tuning Performance].

== Enable Logging for Requests and Responses

To enable logging of request and response interaction with the IBM CTG Connector, configure a logger in the Loggers tag in your Mule app's log4j2.xml file:
