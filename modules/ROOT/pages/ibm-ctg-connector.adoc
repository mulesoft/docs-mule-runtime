...
|Response Timeout |Enter the response timeout value for a connection to CTG. This timeout represents the maximum amount of time (in milliseconds) a managed connection will attempt wait to response for a request. A value of 0 represents no timeout. Default value is 30000 (30 seconds).
