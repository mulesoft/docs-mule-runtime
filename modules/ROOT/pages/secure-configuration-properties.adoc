...
You can download the https://docs.mulesoft.com/downloads/mule-runtime/4.1/secure-properties-tool.jar[JAR file] for this tool to perform encryption or decryption operations.

This tool supports encrypting a text string or the values inside a properties file.
...
=== Encrypt Properties Files

In addition to processing strings, the encryption tool can encrypt or decrypt the values inside `.yaml` or `.properties` files.
...
|`<input file>`
|Specifies the properties file that is encrypted or decrypted during the `file` operation.

|`<output file>`
|Specifies the encrypted or decrypted properties file that is created after the `file` operation.
...
[NOTE]
====
The `key`, `algorithm`, and `mode` values set in `<secure-properties:config>` must match the values used to encrypt the properties files.
====
...
[NOTE]
====
The `key`, `algorithm`, and `mode` values set in the Secure Configuration Properties module must match the values used to encrypt the properties files.
====
