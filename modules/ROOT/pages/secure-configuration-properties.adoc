...
You can download the https://docs.mulesoft.com/downloads/mule-runtime/4.2/secure-properties-tool.jar[JAR file] for this tool so you can encrypt or decrypt single values, and complete files (both, YAML and Properties files).
This tool supports three encryption modes: `string`, `file`, and `file-level`

=== Encrypt or Decrypt Single values

Use the `string` mode to encrypt or decrypt a single value provided as a parameter.
...
In addition to encrypting strings, the encryption tool is able to encrypt complete `.yaml` or `.properties` files.

* Use the `file` mode to encrypt all values inside a properties file:
+
[source,console.linenums]
----
...
----
+
The output is a file with the same properties, but its values are encrypted. For example, assume that this is the input file:
+
`example_in.yaml`
+
...
* Use the `file-level` mode to completely encrypt the properties file.
+
In `file-level` mode, the output is a file that is entirely encrypted. Using as input the `example_in.yaml` file from the previous example, if you run:
+
----
...
|`string`
|Sets decryption mode to `string`. +
Encrypts a single string value provided as argument `<value>`.
|
* `string`
* `file`
* `file-level`

Only one of these can be set per operation.

|`file`
|Sets decryption mode to `file`. +
Encrypts the value of each property set in the properties file provided as argument `<input file>`.
|
* `string`
* `file`
* `file-level`
...
|`file-level`
|Sets decryption mode to `file-level`. +
Encrypts the whole content of the properties file provided as argument `<input file>`.
|
* `string`
* `file`
* `file-level`
...
|`<algorithm>`
|The algorithm to use during the operation.
| See <<supported_algorithms>> for a complete list.

|`<mode>`
|The mode to use during the operation.
| See <<supported_modes>> for a full list.

|`<key>`
|The word used to lock or unlock the file or value during the operation. +
|Any string value.

|`<value>`
|String value that is encrypted or decrypted during the `string` operation.
|Any string value.

|`<input file>`
|The properties file that is encrypted or decrypted during the `file` or `file-level` operation.
|A `.yaml` or `.properties` file.

|`<output file>`
|The encrypted or decrypted properties file that is created after the `file` or `file-level` operation.
|A `.yaml` or `.properties` file.
...
[NOTE]
====
* Key sizes might depend on the Java Cryptography Extension (JCE) version. For example, since version 1.8.0_161, the AES key size needs to be of at least 32 characters (256 bytes), instead of 16 for previous versions.
* If you use the `$` character in your key, it needs to be escaped with `\`. 
For example, to use key `myKey#$%123` you need to pass the `<key>` argument as `myKey#\$%123`.
====
