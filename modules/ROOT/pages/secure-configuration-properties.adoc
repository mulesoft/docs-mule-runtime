...
In `file-level` mode, the output is a file that is entirely encrypted. For the
example above, the output is:
...
Note that key sizes might depend on the Java Cryptography Extension (JCE)
version. For example, since version 1.8.0_161, the AES key size needs to be
of at least 32 characters (256 bytes), instead of 16 for previous versions.
