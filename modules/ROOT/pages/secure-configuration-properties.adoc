...
[NOTE]
For *Encoding* the value *- -Empty- -* corresponds to the default value of UTF-8.
...
The following example shows a Salesforce connector configuration where its properties (username, password and token) are encrypted in a `.yaml` file. The `secure::` prefix is added before the property name definition.
