...
Typically, a message source comes in the form of an xref:3.7@anypoint-connectors.adoc[*Anypoint Connector*].
...
Under certain conditions, flows do not need to be triggered by message sources. For instance, a xref:3.6@flow-reference-component-reference.adoc[Flow Reference Component] can trigger a different flow in the same application. Similarly, the xref:3.6@async-scope-reference.adoc[Async Scope] can trigger a another flow that executes asynchronously, (i.e. in parallel with the originating flow).
...
For further details, see xref:3.7@flow-processing-strategies.adoc[Flow Processing Strategies].
...
* call various other xref:3.7@flows-and-subflows.adoc[flows]  +

== See Also

* *NEXT STEP:* Learn about how xref:3.7@flows-and-subflows.adoc[flows and subflows] work together in an application.
* Examine an xref:3.6@flow-architecture-advanced-use-case.adoc[advanced use case] showing a more complex flow architecture.