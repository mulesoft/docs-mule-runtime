...
This document assumes that you are familiar with Mule, the xref:6@studio::index.adoc[Anypoint Studio interface], xref:global-elements.adoc[Global Elements], xref:6@studio::datasense.adoc[DataSense], and the xref:dataweave.adoc[DataWeave] or its predecessor, xref:6@studio::datamapper-user-guide-and-reference.adoc[Datamapper]. Further, it is assumed that you are familiar with Salesforce and have a http://developer.force.com/[developerforce] account. Learn more about xref:flows-and-subflows.adoc[flows and subflows] to gain more insight into the design of this example.
