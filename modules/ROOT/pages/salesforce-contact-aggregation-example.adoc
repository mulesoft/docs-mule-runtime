...
This example application uses the xref:salesforce-connector.adoc[Salesforce connector] to query contacts from two separate Salesforce instances and determine which contacts are present in both instances. The section immediately below describes the functions of each of the flows in the application. The section further below describes the configuration of the Salesforce Global Connector Configuration through which the Mule application establishes a connection to Salesforce.

Beyond this example, access xref:salesforce-connector.adoc[introductory] and https://mulesoft.github.io/mule3-salesforce-connector/[reference] documentation for the Salesforce Connector.

== Prerequisites

This document assumes that you are familiar with Mule, the xref:6.x@studio::index.adoc[Anypoint Studio interface], xref:global-elements.adoc[Global Elements], xref:6.x@studio::datasense.adoc[DataSense], and the xref:dataweave.adoc[DataWeave] or its predecessor, xref:6.x@studio::datamapper-user-guide-and-reference.adoc[Datamapper]. Further, it is assumed that you are familiar with Salesforce and have a http://developer.force.com/[developerforce] account. Learn more about xref:flows-and-subflows.adoc[flows and subflows] to gain more insight into the design of this example.
...
Invoked by the *triggerFlow* (configured in a separate mflow file in the application), the mainFlow begins processing the message triggerFlow accepted via HTTP request. The mainFlow consists of four *Flow Ref* scopes, which successively call the four subflows in the application. The flow also references a xref:catch-exception-strategy.adoc[catch exception strategy] to handle exceptions thrown. At runtime, the mainFlow begins by calling the *gatherDataFlow*, explained below the example.
...
This flow contains two xref:message-enricher.adoc[message enricher] scopes with a Salesforce connector in each. Each Salesforce connector connects to a Salesforce instance to retrieve all fields of type *Contact*. The message enricher for each connector stores this information in two xref:mule-message-structure.adoc[flow variables], one for each Salesforce organization.
...
* Access the introductory material for the xref:salesforce-connector.adoc[Salesforce Connector].

* Access https://mulesoft.github.io/mule3-salesforce-connector/[full reference documentation] for the Salesforce Connector.
...
* Learn more about xref:anypoint-connectors.adoc[Anypoint Connectors] in general.