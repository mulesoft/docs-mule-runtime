...
Beyond this example, access xref:salesforce-connector.adoc[introductory] and xref:salesforce-connector-reference.adoc[reference] documentation for the Salesforce Connector.

== Assumptions


This document assumes that you are familiar with Mule, the xref:5@studio::index.adoc#the-visual-editor[Anypoint Studio interface], xref:global-elements.adoc[Global Elements], xref:datasense.adoc[DataSense], and the xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer]. Further, it is assumed that you are familiar with Salesforce and have a http://developer.force.com/[developerforce] account. Learn more about xref:flows-and-subflows.adoc[flows and subflows] to gain more insight into the design of this example.
