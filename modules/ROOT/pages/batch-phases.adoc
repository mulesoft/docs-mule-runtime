...
* <<phase_on_complete>>: The Batch Job component issues a report object with the result of processing the batch job instance instance.
...
Note that changes to records that take place within a Batch Aggregator component _are not propagated_ outside of Batch Aggregator component. So the changes are not visible to subsequent Batch Step components in the flow.
