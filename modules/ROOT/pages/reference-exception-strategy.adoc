...
+
image::ref-global-1.png[ref_global-1]
...
[NOTE]
====
You can create a global exception strategy (i.e. access the Choose Global Type panel) from the reference exception strategy’s pattern properties panel. Click the *image:add.png[(plus)]* button next to the *Global Exception Strategy* drop-down and follow the steps to create a global xref:catch-exception-strategy.adoc[catch], xref:rollback-exception-strategy.adoc[rollback], or xref:choice-exception-strategy.adoc[choice] exception strategy.
