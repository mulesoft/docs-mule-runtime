...
For more detail see xref:about-the-xml-configuration-file.adoc[About the XML Configuration File].
...
<2> Shows the namespace for Mule's xref:stdio-transport-reference.adoc[stdio transport], which allows communication using standard I/O. It has the prefix "stdio". The convention for a Mule module or transport's schema is to use its name as the prefix. +
...
Filters once again can be configured as global elements and referred to where they are used, or configured at their point of use.  For more information, see xref:filters.adoc[Filters].

==== Expressions

For a current reference to using expressions in Mule, see xref:mule-expression-language-mel.adoc[Mule Expression Language MEL].
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see xref:routers.adoc[Routing Message Processors].
...
<1> and <2> are Java components, specified by each's class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see xref:entry-point-resolver-configuration-reference.adoc[Entry Point Resolver Configuration Reference].

==== Anypoint Connectors

An Anypoint connector calls a cloud service.

In #9 in <<Flows>>, the example calls `authorize.net` to authorize a credit card purchase, passing it information from the message.  For more about connectors, see xref:anypoint-connectors.adoc[Anypoint Connectors].
...
Mule allows you to define Agents to extend the functionality of Mule. Mule manages the agents' lifecycle (initializes them and starts them on startup, and stops them and disposes of them on shutdown). These agents can do virtually anything; the only requirement is that they implement `org.mule.api.agent.Agent`, which allows Mule to manage them.  For more about Mule agents, see xref:mule-agents.adoc[Mule Agents].
