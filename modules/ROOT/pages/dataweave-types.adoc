...
DataWeave represents data using values where each value has a type associated. There are many types, such as strings, arrays, Booleans, numbers, objects, dates, and times among others. Each of these types has several ways of creating their values that we are going to explore in this topic.
...
This topic describes the possible values for each DataWeave type, how these can be created, and also descriptions of common patterns, including:
