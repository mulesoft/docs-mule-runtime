...
DataWeave defines a `Null` data type. The value `null` belongs to the `Null`
type. It is important to note that `null` is the value, while `Null` is the type.

You can determine whether a DataWeave function can handle `null` values by
looking in the DataWeave reference documentation for a function signature
that handles a `Null` type. For example, the
...
`flatten` can handle `null` values. On the other
hand, xref:dw-core-functions-plusplus.adoc[++] (concatenate), which lacks
a signature for handling `Null`, cannot handle `null` values.
