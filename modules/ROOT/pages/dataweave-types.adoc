...
* `DatePeriod` +
 A date-based amount of time, for example 1 year, 2 months, 3 days. Because these expressions are date-based they cannot be converted to milliseconds, for example a month doesn't specify if it is a month of 31 days or not. Similarly applies to year as it can be a leap year. The data express just an amount of date. In DataWeave the syntax is `|P1Y|`.

* `Duration` +
A time-based amount of time, for example 1 second, 2 hours, 2 days. These expressions can be represented in seconds or milliseconds. In DataWeave the syntax is `|PT1H|`.
