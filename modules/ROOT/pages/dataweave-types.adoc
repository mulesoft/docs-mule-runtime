...
----
Notice the expression between the _parentheses_ (such as `(x)` where `x` is the variable shown in the header) should return a single object or an array of objects. In this example, `(x)` returns and array of objects, and `(y)` returns an object with just one key/value. All of the key/values from each object in that array get flattened "in place" by the object constructor curly braces into the outer object. The object constructor curly braces also operate on the `(y)` expression to flatten the single key/value "in place" from that object into the outer object. So the output looks like this:
