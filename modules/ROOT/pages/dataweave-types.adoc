...
DataWeave provides a mechanism for formatting numeric values and coercing dates and strings to numbers. The language provides operators that act on numeric values and includes many functions that take numeric values as arguments.

* xref:dataweave-types-coercion.adoc[Type Coercion with DataWeave] shows how to format a number.
