...
A `Period` represents an amount of time. The type takes the following form:
...
`DatePeriod` is useful for operate date manipulation, such as addition or subtraction. +
...
As `DatePeriod` is date-based it can be coerced to number with different date based units such as `years` or `months`. +
The following example shows how to coerce a `DatePeriod` to a `Number` by using different units:
...
A `Duration` can be deconstructed into parts: `hours`, `minutes` and `secs`.
