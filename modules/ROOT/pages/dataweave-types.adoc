...
* `isAbsolute`: Boolean value. `true` if the URI contains a scheme,
                `false` if the URI is relative (for example,
                `/path/to/somewhere`).
* `isOpaque`: Boolean value. `true` if the scheme of the URI begins with a
