...
`flatten` can accept `null` values, whereas xref:dw-core-functions-plusplus.adoc[++] 
(concatenate), which lacks a signature for handling value so the Null type, does 
