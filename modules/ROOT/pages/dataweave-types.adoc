...
A period represents an amount of time and has the form:
...
* `DatePeriod` +
 A date-based amount of time, for example 1 year, 2 months, 3 days. Because these expressions are date-based they cannot be converted to milliseconds, for example a month doesn't specify if it is a 31 days month or not. Similarly applies to year as it can be a leap year. The data express just an amount of date. +

 In DataWeave the syntax is `|P1Y|`

* `Duration` +
A time-based amount of time, for example 1 second, 2 hours, 2 days. These expressions can be represented in seconds or milliseconds. +

In DataWeave the syntax is `|PT1H|`
...
`DatePeriod` is useful to operate date manipulation such as addition or subtraction. +
The following example shows how to subtract one year from a date using `DatePeriod`. The return type is a new Date:
...
`Duration` is useful as a result of a Date to Date subtraction so it can be inferred in the amount of time between these two dates. +
The following example shows how the result of subtracting two Date returns a `Duration`:
...
As `DatePeriod` is date-based it can be coerced to number with different date based units such as: "years" or "month". +
The following example shows how to coerce a `DatePeriod` to a Number using different units:
...
As `Duration` is time-based it can be coerced to a number with different time based units such as: "nanos", "milliseconds", "seconds", "hours" and "days".

The following example shows how to coerce a `Duration` to a Number using different units:
...
A `Duration` can be deconstructed in each parts: `hours`, `minutes` and `secs`.

The following example shows how to extract different parts of a `Duration`:
