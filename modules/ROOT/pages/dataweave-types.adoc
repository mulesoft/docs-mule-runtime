...
Represents any object as a collection of `Key:value` pairs. Where a `Key` is composed by a `Name` and `Attributes`.
The `Name` type is composed by a `String` as the local name and the `Namespace`. The `Attributes` is composed of an Array of `Name:value` pairs. Take into account that the `Key` is not a `String` and so they can not be compared, but you can easily get the local name by doing `as String` on any value of type `Key`.
