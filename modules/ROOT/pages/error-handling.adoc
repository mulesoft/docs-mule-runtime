...
image::mruntime-error-handling-default.png["An error message display from an HTTP request failure in a Mule application"]
...
image::mruntime-on-error-continue.png["A flow showing error handling configurations with components like Listener, Request, and Logger"]
...
+
image::error-handling-global-config.png["A configuration dialog general fields including settings"]
+
