...
** *Element*: The XML element that failed execution and caused the error.
** *Element DSL*: The XML element that failed execution and caused the error, expressed in DSL.
** *Error type*: The type of Mule error raised by the Mule app.
** *Flowstack*: This field lists an entry for each flow that processed the Mule event and the processor that sent the event to the next flow. The last entry shows the flow and the processor that failed during the processing of the event.
+
Starting with Mule runtime engine 4.3.0, the Flowstack entry is enabled by default. You can disable Flowstack logging by setting the `mule.flowTrace` configuration property to `false`.
...
for the 404 associated with the file not found.
