...
Errors that occur in Mule fall into one of two major categories:
messaging errors and system errors.
...
built-in Error Handler component.
* Outside a flow, you can set up a global Error Handler component and
reference it from other Error Handler configurations. The global
...
as the message body). Because the page was not found, there is not content to
...
precisely as you want. The Error Handler component routes an error to the
first On Error scope that matches the error.
...
global error handler that you configure. The global error handler can be
useful if you want more than one flow to handle errors in the same way.
