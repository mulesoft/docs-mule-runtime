...
TIP: Instead of building a proxy application from scratch, consider xref:1.x@api-manager::setting-up-an-api-proxy.adoc[Setting Up and Deploying a Proxy].

== Deciding How to Proxy Your Web Service

While many times you can proxy web services without using CXF (see xref:proxying-web-services.adoc[Proxying Web Services]), there are several cases where you might want to use CXF proxies:
...
[TIP]
To send outbound requests to external web services, it's highly recommended that you use the xref:web-service-consumer.adoc[Web Service Consumer] instead.
