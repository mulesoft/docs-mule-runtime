...
when a Message source in a Mule flow is triggered, as when an HTTP listener 
receives a request from a client or each time the Scheduler component executes. 
For example, when an HTTP listener in a Mule app receives a request from a 
client, it creates a Mule event with a Mule message that contains the content 
of the response as its payload and the metadata associated with that content, 
such as HTTP headers, as it attributes. This request also starts (or triggers) 
the Mule flow. Message processors in the flow (such as Core components, File 
read operations, or the HTTP request operations) can then retrieve, set, and 
