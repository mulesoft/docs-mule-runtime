...
A Mule message is created as part of a xref:about-mule-event.adoc[Mule event] when a Mule event source in a Mule flow triggers a flow to start, as when an HTTP listener receives a response or each time the Scheduler component triggers an execution of the flow.
