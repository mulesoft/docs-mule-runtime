...
Triggering a Mule event source initiates a process that generates a Mule message, places the message in a Mule event, and executes the Mule flow that contains the event source. For example, this process begins each time an  xref:connectors::http/http-listener-ref.adoc[HTTP Listener] source receives a request or the xref:scheduler-concept.adoc[Scheduler] executes. 
