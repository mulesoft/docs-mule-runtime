...
when a Message source in a Mule flow (such as an HTTP listener or Scheduler
component) receives a request from a client or, as with the Scheduler, set 
to trigger at regular intervals. For example, when an 
HTTP listener in a Mule app receives a request from a client, it creates a
Mule event with a Mule message that contains the content of the response as
its payload and the metadata associated with that content, such as 
HTTP headers, as it attributes. This request also starts (or triggers) the 
Mule flow. Message processors in the flow (such as Core components, File read 
operations, or the HTTP request operations) can then retrieve, set, and 
process Mule message data that resides in the Mule event according to 
...
message. The specific attributes of a message depend on the protocol (such as 
