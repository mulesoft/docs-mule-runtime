...
A Mule message is created as part of a xref:about-mule-event.adoc[Mule event] during the process in which a Mule event source in a Mule flow triggers a flow to start, as when an HTTP Listener component receives a response or each time the Scheduler component triggers an execution of the flow.
