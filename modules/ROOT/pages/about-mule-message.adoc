...
A Mule event source triggers a process that generates Mule message, incorporates the message into a Mule event, and executes the Mule flow that contains the event source. For example, this process begins each time an xref:connectors::http/http-listener-ref.adoc[HTTP Listener] source receives a request or the xref:scheduler-concept.adoc[Scheduler] executes. 
