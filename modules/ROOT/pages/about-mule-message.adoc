...
Attribute names that follow the rules described in xref:dataweave-language-introduction.adoc#valid-identifiers[Rules for Declaring Valid Identifiers] can be accessed and declared without using any quotation marks.
...
Because attribute name `123 abc !@#` is not a valid identifier, you need to use quotation marks or backticks to declare it.

To access the value from attribute `123 abc !@#`, which is `"some_value"`, use quotation marks or backticks. For example:
...
It is not required to use quotation marks or backticks to declare an attribute or to select a value from an attribute when the attribute name is a valid identifier, but it can be done.
