...
Quotes are required around the inner attribute values such as `content-type` but not around inner values `date` because DataWeave applies the rules described in <<valid_identifiers>> to inner values.  To understand why you use `statusCode` instead of `Status Code` or `"Status Code"` to select the status code value, see the <<note_top_level_selectors, note>> in the next section.
