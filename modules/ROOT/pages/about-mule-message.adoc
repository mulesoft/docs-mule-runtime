...
+
Quotes are required to select inner attribute values such as `content-type` but not to select values like `date` because DataWeave applies the rules described in <<valid_identifiers>> to inner values.  To understand why you use `statusCode` instead of `'Status Code'` or `"Status Code"` to select the status code value, see the <<note_top_level_selectors, note>> in that section.
