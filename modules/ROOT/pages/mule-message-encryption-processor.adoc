...
As Mule’s default encryption strategy, the Java Cryptography Extension (JCE), as part of the Java Cryptography Architecture (JCA) encodes a message payload, or part of a payload.
...
|*Connector Configuration* |x |Use the drop down to select the global Encryption element you created.
|*Operation* |x |Encrypt
...
|*Stream Name* | | Name of the decrypted payload in a PGP-encrypted file. If a file is encrypted using Mule, this setting helps you keep the original filename so that it is recovered when decrypting the resulting PGP file.
|*Select encrypter* |x |JCE_ENCRYPTER
...
|*streamName* |  |Name of the decrypted payload in a JCE-encrypted file. If a file is encrypted using Mule, this setting helps you keep the original filename so that it is recovered when decrypting the resulting JCE file.
|===
+
[%header%autowidth.spread]
|===
...
|*keyPassword†* |  |If the key is stored in a keystore, specify the password to the keystore.
|===
+
†mutually exclusive
...
|*Operation* |x |Encrypt
|*Input reference* |  |Enter a Mule expression to define the part(s) of the message Mule should encrypt. If no value is entered, Mule encrypts the entire message payload.
|*Select encrypter* |x |XML_ENCRYPTER
|*Stream Name* | | Name of the decrypted payload in a XML-encrypted file. If a file is encrypted using Mule, this setting helps you keep the original filename so that it is recovered when decrypting the resulting XML file.
...
|*input-ref* |  |Enter a Mule expression to define the part(s) of the message Mule should encrypt. If no value is entered, Mule encrypts the entire message payload.
| *streamName* |  |  Name of the decrypted payload in a XML-encrypted file. If a file is encrypted using Mule, this setting helps you keep the original filename so that it is recovered when decrypting the resulting XML file.
|===
+
[%header%autowidth.spread]
|===
...
[NOTE]
Because of its increased complexity, the topic has earned its own page: refer to the xref:pgp-encrypter.adoc[PGP Encrypter] document.
