...
object. More precisely, a DataWeave selector operates within a context, which can
...
In DataWeave, selectors extract values from within DataWeave object
(such as `{ myKey : "myValue" }`) or array (such as `[1,2,3,4]` or
`[ { "myKey" : "myValue", "myKey" : "myValue2" } ]`).

The following DataWeave script uses the single-value selector (`.`) to retrieve
values from the object and arrays defined by the variables `myObject` and
`myArray`.
...
`*myKey`. For values of all duplicate keys at lower levels in the hierarchy,
...
object below the given key, regardless of their location in the hierarchy.
The syntax is `..myKey` For more examples, see <<descendants>>.
...
length (`.^contentLength`), (`.^encoding`), MIME type (`.^mimeType`),
...
`.myKey` selectors work over an object or array.
...
---
{ address: myData.people.person.address }
----
...
keys and the values of all matching keys pairs in the current context. These
are returned as an object, containing these retrieved keys and values.
