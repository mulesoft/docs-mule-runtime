...
In DataWeave, quotes around the name of an object's key are required in some cases but optional in others. Though a key is not an identifier, if the key name meets the criteria of a xref:dataweave-language-introduction#valid-identifiers[valid identifier name], quotes around the key are optional. If the key does not meet this criteria, you _must_ surround the key in quotes. For example, to correct the key syntax in `{ some-name: "somebody}`, you can use `{ "some-name" : "somebody" }` or `{ 'some-name' : "somebody" }`. Without quotes, `{ some-name: "somebody}` produces the error `Invalid input '-', expected Namespace`.
