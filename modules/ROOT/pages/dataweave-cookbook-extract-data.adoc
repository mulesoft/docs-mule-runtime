...
a| [[selector_metadata]]
Returns metadata for the value of a , which a Mule `payload`, `variable`, or
`attribute`. The selector can return class (`.^class`), encoding (`.^encoding`),
mime type (`.^mimeType`), media type  (`.^mediaType`), raw  (`.^raw`), and
custom (`.^myCustomMetadata`) metadata. See <<metadata_selector>>.
...
result from calling a method in a Java class or have a data type that
...
Returns the underlying binary value of a POJO. The output This selector is
