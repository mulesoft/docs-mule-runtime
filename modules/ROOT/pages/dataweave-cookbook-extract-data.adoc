...
To validate, a key name in a DataWeave object must start with a letter that is
optionally followed by any combination of numbers, letters, or underscores
(`&#95;`), and the key name cannot contain reserved key words (see 
xref:dataweave-language-introduction#valid-identifiers[Rules for Declaring Valid Identifiers]). 
DataWeave throws an error (such as, `Invalid input -`) on keys that 
do not follow this requirement. 

To correct invalid key syntax such as `some-name`, 
surround the key in quotes, for example, `{ "some-name" : "somebody" }`. 

If you are using a selector on a key that contains invalid syntax,  
you must surround both the key _and the selector_ in single-quotes
or double-quotes. For example, for a key like `some-name`,
`{ "some-name" : "somebody" }."some-name"` works, but
`{ "some-name" : "somebody" }.some-name` produces the error 
`Unable to resolve reference of: name` because the selector is not
