...
In DataWeave, you can surround the name of an object's key in quotes. If the key name meets  
the criteria of a xref:dataweave-language-introduction#valid-identifiers[valid identifier name], 
you can write the name of the key name without quotes, even though a key is not an identifier. 
If the key does not meet this criteria, you must surround the key in quotes. For example, to 
correct key syntax in `{ some-name: "somebody}`,  you can use `{ "some-name" : "somebody" }` or 
`{ 'some-name' : "somebody" }`.

If you are using a selector on a key that requires quotes, you must also surround the 
selector in quotes. For example, the selector `"some-name" in `{ "some-name" : "somebody" }."some-name"`
works, but the selector `some-name` in `{ "some-name" : "somebody" }.some-name` produces the error 
`Unable to resolve reference of: name` because the selector is not surrounded in quotes.

If you are using a selector on a key that does not require the quotes, then quotes around the
selector are optional. For example, all of these examples work:
