...
==== Convert Descendants to XML

This example uses `reduce` to iterate over the array of objects 
returned by `myVar.people..&name` to apply the expression
`(value, aggregator) -> aggregator ++ value` to each iteration,
and it transforms JSON input to XML output. 
...
---
{
  names: (myVar.people..&name) reduce (value, aggregator) -> aggregator ++ value
  //Alternative methods
...
`mapObject` to iterate over the entire input object returns matching
