...
The following example uses the Set Payload component (`set-payload`) to get the XML payload received via the HTTP Listener POST request, then uses the Transform Message component (`ee:transform`) to get the encoding value of the XML payload, by returning the raw data (`(payload.^raw as String) scan /encoding='([A-z0-9-]+)'/)`). The Logger component (`logger`) returns the `payload` in a JAVA format.
...
Send a POST request with the XML body:
[source,XML,linenums]
----
