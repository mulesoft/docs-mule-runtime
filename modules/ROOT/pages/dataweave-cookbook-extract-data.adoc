...
DataWeave can select data from objects, arrays, and variables that store that
data. You can use selectors in Mule modules, connectors, and
components that accept DataWeave expressions.
...
DataWeave function. For examples, see <<single_value>>.
...
```
+
Returns `"me"` when the `output` directive is set to `application/json`.
+
...
```
+
Each example returns `[ "me" ]` when the `output` directive is set to
`application/json`.
+
...
```
+
Only returns the first `user` value, `"a"`  when the `output` directive is set
to `application/json`.
...
```
+
Returns `["a", "b"]` when the `output` directive is set to `application/json`.
...
```
+
Returns `[ { "user": "ale" }, { "user": "joe" } ]` when the `output` directive
...
This selector retrieves both the keys and the values of all matching keys pairs
...
As selectors only return the value of a key-value pair, in order to get both
the key and value, you can use a type conversion to object.


Note that in this example, `as Object` transforms the value into an object that
contains the key as well as the value. Without this conversion to an object, the
returned XML body would simply be `<user>Mariano</user>`.
...
output application/xml
---
user: payload.nameFirst as Object
----
