...
You can use the Mule upgrade tool to update or upgrade Mule 4 instances located in different folders.
...
The Mule upgrade tool compares configuration files from the _current_ Mule distribution and the _new_ distribution to detect changes and prevent losing them. When the Mule upgrade tool detects changes in the _new_ Mule distribution configuration files, it will try to resolve conflicts automatically and continue with the upgrade. If conflicts cannot be resolved automatically, it will perform the following actions:
