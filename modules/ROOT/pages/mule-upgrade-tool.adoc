...
You can use the Mule upgrade tool to update or upgrade Mule instances located in different folders.
...
* A currently operational Mule 4.x instance, in _stopped_ status to prepare for the upgrade
* The Mule upgrade tool inside your current Mule instance's `$MULE_HOME/tools` directory
+
Download the tool from the https://help.mulesoft.com/s/[Help Center^] and copy it into the `$MULE_HOME/tools` directory of your current Mule instance.
...
+
You can download Mule runtime distributions from the https://help.mulesoft.com/s/[Help Center^].
...
. Run the `upgrade -n` subcommand, specifying as an argument the directory where the latest Mule runtime distribution is downloaded and unpacked, for example:
+
...
Before starting an upgrade or update process, the Mule upgrade tool generates a backup of your existing Mule instance, which enables you to restore the upgraded or updated Mule instance to its previous state. The backup is stored in the `$MULE_HOME/backup` folder.
...
The Mule upgrade tool compares configuration files from the _current_ Mule distribution and the _new_ distribution to detect changes and prevent losing them. When the Mule upgrade tool detects changes in the _new_ Mule distribution configuration files, the tool performs the following actions:
...
* Copies the configuration files with conflicts from the `${MULE_HOME}/conf` directory of the current Mule distribution to `${MULE_HOME}/upgrade-tool/mule-config-conflicts/to-review`.
...
* If you decide to accept the changes, merge the content of the file `${MULE_HOME}/upgrade-tool/mule-config-conflicts/to_review/fileWithConflicts.conf` with the changes in the `${MULE_HOME}/upgrade-tool/mule-config-conflicts/new-${MULE_HOME_NEW_FOLDER_NAME}/fileWithConflicts.conf` file.
* If you decide to discard the changes, move the file `${MULE_HOME}/upgrade-tool/mule-config-conflicts/to_review/fileWithConflicts.conf` to the `${MULE_HOME}/upgrade-tool/mule-config-conflicts/ready` folder.
+
...
Reading permissions in the new Mule distribution are needed, and the file referenced in the stack trace is not readable by the user performing the upgrade.

==== Solution

The upgrade tool helps on identifying which files are not readable, yet, assistance from the system administrator could be needed to correct permissions.
...
Writting permissions in the old Mule distribution are needed, and the file referenced in the stack trace is not writtable by the user performing the upgrade.

==== Solution

The upgrade tool helps on identifying which files are not writtable, yet, assistance from the system administrator could be needed to correct permissions.
...
The upgrade tool relies on the information returned by the Mule Runtime to determine if it is running or not. For the purpose of the upgrade tool, it should not be running.

==== Solution

The Mule Runtime should be stopped before starting the upgrade process. To check the current status use the command `${MULE_HOME}/bin/mule status`.
