...
include::release-notes::partial$mule-upgrade-tool/mule-upgrade-tool-1.1.1.adoc[tag=intro]
...
The Mule upgrade tool support upgrading clustered Mule instances by manually upgrading each node using the tool. For further information on how to proceed with the upgrade, please refer to section <<upgrade_clustered_mule_instances>>.
The manual procedure you need to follow to do it manually is described in xref:release-notes::mule-runtime/updating-mule-4-versions.adoc#mulerunvers[Upgrading an On-Premises Mule Instance Managed Through Runtime Manager].
...
Currently, the upgrade tool does not automatically support upgrading Mule clustered instances, and because of that you can find here a description of what to do to upgrade it using the Mule upgrade tool.

=== Before You Begin

To ensure less downtime, first copy the new instance of the Mule runtime and the Mule upgrade tool to each instance that will be upgraded. That way you can have everything you need to do the upgrade.
Follow the same procedures as are described in the <<before_you_begin>> section.

=== Upgrade or Update Mule

The procedure you need to follow is the same as in the stand-alone instance upgrade.
Follow the same procedure described above, starting from the server node, and continuing with every other node till each of those are upgraded to the same version.
This way your system will be properly updated and working as a cluster with the proper new version.

=== Rollback Mule

The same thing that happens with the upgrade happen with the rollback procedure, every node must be rolled back and ends up having the same version. Because of that, you will need to roll back every node in the same order as the upgrade, first the server node, and then each individual node that is part of the cluster, making sure that, at the end, every node has the same version.
