...
The Mule upgrade tool runs in any environment that is capable of running a standalone Mule instance. This tool can update only standalone Mule instances that are running any Mule 4 version. You can upgrade local Mule instances to
...
!===
  ...
  ! Upgrades the local Mule instance to a specific distribution that is downloaded and unpacked in your file system. Parameter `<newMuleDistribution>` specifies the path to the new Mule distribution, for example:
  +
...
!===
  ...
  +
  `$ ./upgrade-tool upgrade -o /mule-home-of-the-instace-to-restore`
