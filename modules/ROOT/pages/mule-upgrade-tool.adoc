...
//confirm if we want to clarify here which is the latest version
...
* Enough disk space on the file system and access privileges to install the new Mule distribution.
* The Mule instance you want to update in stopped status.
...
+
If the Mule upgrade tool is not bundled with your current Mule distribution, download the tool from the https://help.mulesoft.com/s/[Support Portal^] and copy it into the `$MULE_HOME/tools` directory.

////
Alternative table version for compatibility
...
Mule 4.x
Deployment targets
Standalone Mule instances
////

////
OS? Windows / MacOs (cualquier os donde corra mule)
Mule versions? Mule 4.
JDK version?
Is this only for standalone or also RTF? Solo para standalone
Other…,
////

== Upgrade a Mule Instance

You can upgrade your current Mule instance to the latest available version or to a particular Mule distribution that you have previously downloaded and unpacked. You can also use the Mule upgrade tool to update Mule instances located in different folders.
...
. Navigate to the Mule upgrade tool directory.
. Run the `upgrade -n` command, specifying the directory of the Mule distribution as an argument:
+
```
