...
A *Global Element* is a reusable object containing parameters that any number of elements in a flow can share. You typically reference a global element from connectors and components (such as xref:filters.adoc#bundled-filters[filters]) in your Mule project.
...
Before venturing further into this topic, review the features of the xref:6.x@studio::index.adoc[Anypoint Studio Visual Editor] to learn more about developing Mule applications using the Anypoint Studio graphical user interface and to understand the context in which we use these "global elements".
...
You can configure a global element just once, and *_share that configuration among processors of the same type_*. This is particularly useful when you have to define the connection details and login credentials for an external source, such as a SaaS application or database.
...
* *NEXT STEP:* Use what you've learned to follow the xref:6.x@studio::basic-studio-tutorial.adoc[Basic Studio Tutorial].
