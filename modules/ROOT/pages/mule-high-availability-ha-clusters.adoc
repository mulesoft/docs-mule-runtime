...
[NOTE]
====
Have in mind that Mule only coordinates that different nodes in the cluster don’t try to access the same resource.
If you have other applications accessing a file (non-Mule applications, or Mule applications outside the cluster), then you need to implement logic to lock that file.
====
...
* In-memory messaging is not suported, which affects:
  - VM transport
  - SEDA queues
* The Quorum feature can only be configured manually. See xref:creating-and-managing-a-cluster-manually.adoc[Creating and Managing a Cluster Manually]
...
[NOTE]
High performance is implemented differently on xref:runtime-manager::index.adoc[CloudHub] and xref:runtime-manager::deploying-to-pcf.adoc[Pivotal Cloud Foundry], so this section applies only for xref:runtime-manager::deploying-to-your-own-servers.adoc[on-premises deployments].
...
If you plan to use many nodes in your cluster, or you plan to store large message payloads, it is recommended that you research Hazelcast cluster scaling in the https://hazelcast.org/documentation/[Hazelcast documentation], or to contact http://support.mulesoft.com[MuleSoft Support]
...
* xref:installing-an-enterprise-license.adoc[Install an Enterprise License] to start managing clusters in production.
