...
You can encrypt traffic between cluster nodes by defining the `mule.cluster.network.encryption.key` property. You can define this property in the `wrapper.conf` file, or as a system property.
...
* If you don't define the encryption key, the application fails to deploy with message: `Cluster key must be defined in FIPS mode.`.
* Use the same encryption key for all the clustered nodes, otherwise this feature won't work.
* Running a clustered environment in FIPS mode causes a negative impact in performance, because all the communication between nodes will be encrypted (and decrypted).
