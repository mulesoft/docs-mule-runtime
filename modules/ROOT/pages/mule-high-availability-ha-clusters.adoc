...
For example, if multiple programs process files in the same shared directory by reading, modifying, and then deleting them, these programs must use an explicit, application-level locking strategy to prevent the same file from being processed more than once.
====
