...
Note that this does not imply that the memory is not shared. In cluster mode, the object stores are always distributed and shared between nodes. The only difference is that when one node is down in performance mode, the data associated to it will be lost.
