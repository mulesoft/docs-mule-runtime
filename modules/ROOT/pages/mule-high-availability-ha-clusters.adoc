...
[NOTE]
Note that high performance is implemented differently on xref:runtime-manager::index.adoc[CloudHub], so this section applies only for xref:runtime-manager::deploying-to-your-own-servers.adoc[on-premises deployments].
