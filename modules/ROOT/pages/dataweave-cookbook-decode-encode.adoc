...
In Anypoint Studio, you can create and run a Mule flow that encodes and decodes a PDF file by using the following procedure. The example reads the file from the specified local directory using the Read operation from the File module. It writes the PDF to the specified directory by using the Write operation in the File module. It uses the Transform Message operation to encode and decode Base64.
