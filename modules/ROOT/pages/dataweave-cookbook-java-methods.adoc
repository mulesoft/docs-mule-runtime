...
. From *my-app*, create the `utils` package by right-clicking
*src/main/java* from Studio's *Package Explorer*, selecting *New* -> *Package*,
and naming the package `utils`.
. Create the `MyClass` class in the your new `utils` package by right-clicking
*utils*, selecting *New* -> *Class*, naming that class `MyClass`, and clicking
*Finish*.
...
for DataWeave to call the object's `getFoo()` method.)
...
component into the project's canvas, and click to open the component.
...
. Click *Preview* (located farthest right on the tab) to view the following
output:
+
[source,json,linenums]
----
