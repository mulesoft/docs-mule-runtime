...
A negative value removes the idle limit, but the pool is still bounded by `maxActive` unless `exhaustedAction` is `WHEN_EXHAUSTED_GROW` (in which case idle connections are truly unlimited). Setting `maxIdle=0` disables caching entirely — every returned component is destroyed.
