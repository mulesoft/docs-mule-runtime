...
. The catch exception strategy replaces the message payload with new content. Mule uses the xref:3.4@mule-expression-language-mel.adoc[Mule Expression Language MEL] `exception` variable to include an error number in the message payload. The error number helps the caller understand the exception.
