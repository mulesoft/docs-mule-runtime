...
* The xref:3.6@tcp-transport-reference.adoc[TCP Transport], which uses the basic TCP transport.
...
* Communicating with an external service that uses low-level unsecured TCP connections? If so, use the xref:3.6@tcp-transport-reference.adoc[TCP] protocol.
...
* Is it important that messages be persisted until they can be processed? If so, consider using a persistent transport like xref:jms-transport-reference.adoc[JMS] or xref:3.6@file-transport-reference.adoc[File].
...
* Is performance the primary concern and it is not important that messages be delivered in the proper order or that the sender is notified if any are lost? If so, use the lighter-weight xref:3.6@udp-transport-reference.adoc[UDP transport] instead.
...
|class |The name of the class that implements the custom protocol |  |See xref:3.6@tcp-transport-reference.adoc[below] for an example of writing a custom protocol
