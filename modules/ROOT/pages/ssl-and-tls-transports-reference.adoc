...
* Are there advantages to a higher-level protocol built on top of TCP, for instance, the request-response features of xref:https-transport-reference.adoc[HTTP], or the store-and-forward features of xref:email-transport-reference.adoc[Email]? If so, use the transport for that protocol instead.
...
|rethrowExceptionOnRead |Whether to rethrow an exception that occurred while trying to read from the socket |false |Setting this to "false" avoids logging stack traces when the remote socket closes unexpectedly
|===
