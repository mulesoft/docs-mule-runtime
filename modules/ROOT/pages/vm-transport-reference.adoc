...
You can use the Java Virtual Machine (VM) transport for intra-JVM communication between Mule xref:flows-and-subflows.adoc[flows]. This transport by default uses in-memory queues but can optionally be configured to use persistent queues.

*Notes:*

* VM file persistency does not work on clusters.
* VM queue persistence behaves differently on CloudHub. See the xref:runtime-manager::cloudhub-fabric.adoc[CloudHub Fabric] documentation for more details.
* For Mule 4 VM, see the xref:connectors::vm/vm-connector.adoc[VM Connector].
...
|Doc |https://www.mulesoft.org/docs/site/3.9.0/apidocs/org/mule/transport/vm/package-summary.html[JavaDoc] http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_vm/namespace-overview.html[SchemaDoc] |Links to the JavaDoc and SchemaDoc for the transport.
...
The VM transport has often been used to implement complex integrations made up of multiple applications. You can use xref:flow-reference-component-reference.adoc[Flow References] to directly reference one flow from another without a transport in the middle.
...
https://www.mulesoft.org/docs/site/3.9.0/apidocs/org/mule/transport/vm/package-summary.html[VM Javadoc]
