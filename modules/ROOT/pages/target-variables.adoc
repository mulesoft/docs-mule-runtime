...
[source,xml,linenums]
----
<db:select config-ref="dbConfig" target="customerName" targetValue="#[payload[0].name]">
    <db:parameterized-query>select * from CUSTOMER order by ID</db:parameterized-query>
</db:select>
----
