...
Mule also supports the SFTP protocol for secure file transfer. The xref:3.7@sftp-transport-reference.adoc[SFTP Transport] is included in the Mule distribution.
...
* Support for xref:3.6@configuring-reconnection-strategies.adoc[reconnection strategies]
...
If xref:3.6@configuring-reconnection-strategies.adoc[reconnection strategies] are configured, the FTP connection can be re-established automatically by Mule based on the policy you have configured.
...
|file:abstract-filenameParser |0..1 |The filenameParser is used when writing files to an FTP server. The parser will convert the outputPattern attribute to a string using the parser and the current message. To add a parser to your configuration, you import the "file" namespace into your XML configuration. For more information about filenameParsers, see the xref:3.6@file-transport-reference.adoc[File Transport Reference].
|===
...
Which allows to use xref:3.6@non-mel-expressions-configuration-reference.adoc[arbitrary expressions] to compose the filename that is used when storing files on the FTP server.
