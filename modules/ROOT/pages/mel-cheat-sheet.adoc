...
|dateTime |clusterId |encoding |ID
|env |home |name |rootId
|fileSeparator |nodeId |standalone |correlationId
|host |version |workDir |correlationSequence
|ip | |registry |correlationGroupSize
|locale | | |replyTo
|javaVersion | | |dataType
|javaVendor | | |payload
|osName | | |inboundProperties
|osArch | | |inboundAttachments
|osVersion | | |outboundProperties
|systemProperties | | |outboundAttachments
|timeZone | | |exception
|tmpDir | | |
|userName | | |
|userHome | | |
