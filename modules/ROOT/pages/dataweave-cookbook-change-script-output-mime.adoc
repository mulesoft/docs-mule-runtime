...
This DataWeave example shows how to differentiate the output MIME type for a script from the MIME type in which the output data is formatted. Such differentiation can help
you build Mule applications that use MIME types to affect the way data is rendered
by the clients that use the application. An example is HTTP, which uses the MIME type to generate its `Content-Type` header.

//LINK TO DW 1.0 EXAMPLES:
include::partial$dataweave1-links.adoc[tag=dataweave1Examples]

The following example uses the "problem details" standard for APIs. The standard requires
