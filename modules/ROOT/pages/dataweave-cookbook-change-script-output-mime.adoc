...
//STUDIO FLOW: ONLY DW IS AUTO-VALIDATED
.Mule Flow:
[source,xml,linenums]
----
<flow name="problemDetailFlow">
		...
		<error-handler >
			<on-error-propagate>
				...
				<ee:transform>
					<ee:message>
						<ee:set-payload><![CDATA[include::partial$cookbook-dw/change-script-output-mime-ex1/transform.dwl[]]]></ee:set-payload>
					</ee:message>
				</ee:transform>
			</on-error-propagate>
		</error-handler>
...
include::partial$cookbook-dw/change-script-output-mime-ex1/transform.dwl[]
----
...
//STUDIO FLOW: MANUAL VALIDATION REQUIRED (MD FIX NEEDED)
.Mule Flow:
[source,xml,linenums]
----
<flow name="listFlow">
...
//STUDIO CONSOLE OUTPUT: MANUAL VALIDATION REQUIRED
.Response:
[source,text,linenums]
----
HTTP/1.1 200
