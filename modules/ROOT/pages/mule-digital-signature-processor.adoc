...
. Optionally, select *Fail On Verification* option if an exception must be thrown when the signature mismatch the expected one. Defaults to false.

The following image shows the *Verify User Signature* fields:

image::mruntime-example-signature-1.png[Expected Signature]

In the XML editor, the *Verify User Signature* shows like this:
