...
In this tutorial, you run DataWeave scripts on sample data, without relying
...
examples here and elsewhere in the docs, you can try out your own DataWeave scripts on sample data in the playground.
...
pairs within curly braces. Learn more about
xref:dataweave-language-introduction.adoc[DataWeave Scripts]
and xref:dataweave-variables.adoc[DataWeave Variables] when you are ready.
+
...
`src/main/resources` folder so you can provide that contents as sample data for
a DataWeave script.
+
...
tab, replace the current script with one that uses `readUrl` to get the JSON
...
----
+
Note that you can also load the contents of a file through a custom data type in
...
xref:dw-core-functions-filter.adoc[filter] function (used to get the `author` values from the array in the `myVar2` variable),
xref:dataweave-types-coercion.adoc[Type Coercion with DataWeave] (for using
`as Number` to coerce input strings like `"19.99"` into numbers like
`19.99`), and
 ...
 * xref:dw-functions.adoc[DataWeave Reference]: Docs on the DataWeave function
 modules provide many examples using DataWeave functions.
...
data type through Transform Message. Data types can accept a
...
*Preview* button:
+
image::studio_source_only.png[Source Graphic and Other View Selectors]
+
...
payload.
.. Click the Preview button in the tab to see the JSON object from the file
in the DataWeave output.
.. Now change the body of the script in the source code area to `payload.hello`, and retain the existing DataWeave header content.
+
