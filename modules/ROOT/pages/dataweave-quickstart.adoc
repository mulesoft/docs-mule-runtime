...
+
The source code area on the left is the place where you add DataWeave scripts,
and the Preview pane on the right is where you view the output of
that script.
+
...
variable `myJson`:
.. Add this script to the source code area of the *Transform Message* tab:
+
[source,dw,linenums]
----
%dw 2.0
...
key-value pairs within curly braces. When you are ready, learn more about
