...
Now that you've succeeded in transforming data from Mule object to JSON, and from one data structure to another, you're ready to get started with scripting in the DataWeave language (see xref:dataweave-language-quickstart.adoc[DataWeave Language Quickstart]) and explore more xref:about-components.adoc[Core components] and more xref:connectors::index.adoc[connectors and modules].
