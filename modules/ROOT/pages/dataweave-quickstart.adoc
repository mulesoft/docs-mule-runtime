...
two strings ("hello" and "World") together into a single string, and then
perform a simple JSON to XML transformation on the output:
.. Replace the current script in the source code area of the *Transform Message*
tab with this one:
+
[source,dw,linenums]
...
output format), in this case `application/json`. You can learn more about
...
key-value pairs within curly braces. You can learn more about
...
----
+
You can learn more about the DataWeave
xref:dw-core-functions-plusplus.adoc[++] function when you are ready.
+
...
are xref:dataweave-types#dw_type_array[arrays]. You can learn more about
