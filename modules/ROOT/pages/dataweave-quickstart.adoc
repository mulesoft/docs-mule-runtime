...
. Begin with a DataWeave script that introduces a variety of data types and
shows how to add comments to a script.
...
----
+
As you can see, DataWeave supports multi-line comments within `/* */` markup
and single-line comments after forward slashes (`//`). It also supports many
data types, including strings (surrounded by quotation marks, `""`), numbers,
date and time structures (within pipes, `||`), arrays (within square brackets,
`[]`), JSON-like objects (key-value structures within curly braces, `{}`),
`null`, and binaries. When you are ready for more on this topic, you can review
...
----
+
. Now try a simple DataWeave script that uses the `++` function to concatenate
two strings ("hello" and "World"), and then perform a simple JSON to XML
transformation on the output:
.. Replace the current script in the source code area of the *Transform Message*
tab with this one:
+
[source,dw,linenums]
...
----
+
* The body of the DataWeave script contains a JSON object
(`{ myString: ("hello" &#43;&#43; "World") }`). The value of this input object
is a DataWeave expression (`("hello" &#43;&#43; "World")`) for concatenating
the strings "hello" and "World" into a single string.
* The header of the script is all the content above the three dashes `---`.
It includes important directives, some of which are required (such as the
output format), in this case `application/json`. When you are ready for more
on this topic, you can review
...
----
+
.. Now change the output format to XML by changing the `output application/json`
found in the header of the DataWeave script to `output application/xml`:
+
[source,dw,linenums]
----
%dw 2.0
...
----
+
It is important to notice that the `myString` key of the input JSON object
`{ myString: ("hello" ++ "World") }` is converted to the root element of the
XML output and that the concatenated string becomes the value of the that
object to become `<myString>helloWorld</myString>`.
+
Without a _single_ key to serve as a root node for the XML output (for example,
...
+
DataWeave supports many output and input formats. When you are ready for
more on this topic, you can review
...
----
+
A JSON object like `{ "hello" : "world" }`, defined as the `myJson` variable
in the script's header, is a data structure that can be made up of one or more
key-value pairs within curly braces. When you are ready for more on this topic,
you can review xref:dataweave-variables.adoc[DataWeave Variables].
+
...
----
+
Here is this what this example looks like in Studio:
+
