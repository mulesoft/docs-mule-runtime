...
on external data sources. The examples introduce some key DataWeave 2.0 concepts
that you can explore further whenever you are ready, and they show how to turn
the Transform Message component into a DataWeave 2.0 playground.
...
your DataWeave 2.0 script can act on it. Once you feel comfortable with DataWeave 2.0
examples here and elsewhere in the docs, you can use the DataWeave 2.0 playground
to practice writing your own DataWeave 2.0 scripts on sample data.
...
includes a DataWeave 2.0 script in the source code area with output in the
*Preview* pane.
...
Set up a Mule project that serves as a DataWeave 2.0 playground:
...
DataWeave 2.0 scripts, and the Preview pane on the right is where you view the
...
Begin with a simple DataWeave 2.0 script that concatenates two strings ("hello" and
...
----
+
* The body of the DataWeave 2.0 script contains a key-value pair
(`{ myString: ("hello" &#43;&#43; "World") }`). The value of this input object
is a DataWeave 2.0 expression (`("hello" &#43;&#43; "World")`) for concatenating
the strings "hello" and "World" into a single string, "helloWorld".
+
When you are ready, you can learn about the DataWeave 2.0
...
xref:dataweave-language-introduction.adoc[DataWeave 2.0 Scripts] when you are ready.
+
...
+
DataWeave 2.0 supports many output and input formats. You can learn more about
xref:dataweave-formats.adoc[Formats Supported by DataWeave 2.0] when you are ready.
+
. Proceed to <<step_3>>
...
Now provide a DataWeave 2.0 script that simply introduces you to a variety of
...
----
+
DataWeave 2.0 supports multi-line comments within `/* */` markup and single-line
...
xref:dataweave-types.adoc[DataWeave 2.0 types].
+
. See the JSON output in the *Preview* pane:
+
[source,json,linenums]
...
[[step_4]]
=== Define and Use a DataWeave 2.0 Variable as Input

Now try a simple DataWeave 2.0 script that outputs the value of the DataWeave 2.0
...
xref:dataweave-variables.adoc[DataWeave 2.0 Variables] when you are ready.
+
...
[[step_5]]
=== Use a DataWeave 2.0 Function in a DataWeave 2.0 Variable

Now try a script that uses the DataWeave 2.0 `avg` function in a DataWeave 2.0
...
function, xref:dataweave-formats.adoc[Formats Supported by DataWeave 2.0], and
xref:dataweave-selectors.adoc[DataWeave 2.0 Selectors] when you are ready.
+
. Preview the output:
+
[source,json,linenums]
...
[[step_7]]
=== Read File Contents with a DataWeave 2.0 Function
...
image::dw-tut-sample-file.png[DataWeave 2.0 Sample File]
+
...
Now use the DataWeave 2.0 `pluck` function to iterate over values in a JSON
...
separate arrays. The point here is simply to provide a taste of DataWeave 2.0's
...
xref:dataweave-types-coercion.adoc[type coercion with DataWeave 2.0] to see how
...
xref:dataweave-variables#example_local_variable[local DataWeave 2.0 variables] in a
script.
+
. Preview the output:
+
...
[[more_examples]]
== Try Out More DataWeave 2.0 Examples

Now you are ready to run DataWeave 2.0 examples on your own. In your DataWeave 2.0
playground, you can run examples from the docs whenever you want to discover
more about the DataWeave 2.0 language. You can play with the examples and use them
...
+
 * xref:dw-operators.adoc[DataWeave 2.0 Operators]
 * xref:dw-functions.adoc[DataWeave 2.0 Reference]: Docs on the DataWeave 2.0 function
 modules provide many examples that use DataWeave 2.0 functions.
 * xref:dataweave-flow-control.adoc[Flow Control in DataWeave 2.0]
 * xref:dataweave-pattern-matching.adoc[Pattern Matching in DataWeave 2.0]
 * xref:dataweave-cookbook.adoc[DataWeave 2.0 Cookbook]
 * xref:dataweave-functions.adoc[Define DataWeave 2.0 Functions]
+
...
In DataWeave 2.0, `payload` is a built-in Mule Runtime variable that holds the
...
DataWeave 2.0 script. The docs often refer to this content as "the payload."

To try out DataWeave 2.0 examples that use `payload` to get the contents of a
Mule message, you have some options:

* <<small_payload>>: You can use a DataWeave 2.0 variable.
...
payload to a variable. You simply copy the payload's content into a DataWeave 2.0
variable (`var`) in the header of a DataWeave 2.0 script. Then, in the body of the
...
does not exist:
+
.. Add a `myInput` DataWeave 2.0 variable supplying some input data the script can
read, and change the Mule `payload` variable in the body of the script to the
DataWeave 2.0 `myInput` variable.
+
[source,DataWeave, linenums]
----
%dw 2.0
...
For DataWeave 2.0 examples with many lines of sample data, consider creating a
...
retaining the existing DataWeave 2.0 header content.
+
...
+
Learn more about xref:dataweave-selectors.adoc[DataWeave 2.0 Selectors]
when you are ready.
...
Core components accept DataWeave 2.0 selectors and short DataWeave 2.0 expressions
in their *fx* fields.
+
