...
the *myJson.json* tab (or withing the *Source* sub-tab of the *myJson.json* tab):
+
[source,json,linenums]
----
{
...
----
+
.. Now replace the content of the source code area with a script that uses
...
+
You can skip the step if you still have the file in Studio. The next steps
show how to use the contents of this file as your payload.
+
. Now replace the current content from the source code area of the Transform
...
+
.. Back in the *Create new type* dialog that opens, select *JSON* from
the *Type* drop-down menu.
+
image::studio_select_metadata_type.png[Select Metadata Type]
+
.. Below the new type, change *Schema* to *Example*.
...
Core components accept short DataWeave selectors and short DataWeave
expressions in their *fx* fields.
+
To learn about the components, you can start from
xref:mule-application-about.adoc[Mule Applications].
+
To try out sample data in running a Mule app, you can use these
Core components:
+
...
Mule event variable.
** xref:scheduler-concept.adoc[Scheduler] to trigger the regular generation of Mule events to hold any data set by those components.
