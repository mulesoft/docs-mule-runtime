...
Now that you've succeeded in transforming data from Mule object to JSON, and from one data structure to another, you're ready to explore more xref:dataweave.adoc[DataWeave features].
To get started with scripting in the DataWeave language, see the xref:dataweave-language-quickstart.adoc[DataWeave Language Quickstart].
