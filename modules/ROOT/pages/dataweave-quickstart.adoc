...
. Change the JSON output format (from <<step_1>>) to XML by replacing
`output application/json` in the header with `output application/xml`:
+
[source,dw,linenums]
----
%dw 2.0
...
[[step_4]]
=== Define and Use a DataWeave Variable
...
object and output those values in an array.
