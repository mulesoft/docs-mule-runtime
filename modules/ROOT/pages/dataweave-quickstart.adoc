...
This quickstart shows you an easy way to test DataWeave scripts on sample data,
without relying on external data sources. It shows how to turn the Transform
Message component into your own DataWeave playground.

Here, you copy DataWeave scripts into the *Output* pane of a Transform Message
component in Studio, then view the results in the component's *Preview* pane.
...
For an overview of the language, see xref:dataweave[DataWeave Language].

[[example1]]
.Transform Message Component in the Studio UI
image::dw-tut-script-studio-basic.png[Simple Script]

IMPORTANT: The quickstart procedure shows a _limited use_ of the xref:transform-component-about.adoc[Transform Message]
component to help you get started with the language (as opposed to
xref:mule-app-tutorial.adoc[Mule app development]) and does not attempt
...
create and run a Mule app to generate the payload within a
xref:about-mule-event.adoc[Mule event]. It is
...
xref:mule-app-tutorial.adoc[Mule App Tutorial]. Also note that when you
are learning to create your own Mule apps, some useful Core components for trying out sample data without depending on external data sources are
...
to create content in a Mule event variable, and
