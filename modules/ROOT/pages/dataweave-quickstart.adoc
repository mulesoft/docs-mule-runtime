...
. Begin with a simple DataWeave script that uses the `++` function to concatenate
...
It includes important directives, one of which specifies the output format
...
+
.. Now change the output format to XML by replacing `output application/json`
in the header with `output application/xml`.
+
Many integrations require transformations from one format (such as JSON) to
another (such as XML).
+
[source,dw,linenums]
----
...
+
. Now try a DataWeave script that simply introduces you to a variety of data
types and shows how to add comments to a script.
.. Replace the current script in the source code area of the *Transform Message*
tab with this one:
+
[source,dw,linenums]
...
*Package Explorer* tab, then navigating to *New* -> *File*, providing the file name `myJson.json` for that file, and clicking *Finish*.
