...
. *Understand how to see the contents of the Mule message.* In order to be able to act upon it, you need to recognize the type of data the message contains. Is the payload an array? Does the message contain a flow variable? What inbound properties exist?
...
* xref:mule-expression-language-examples.adoc[Example apps that use MEL]