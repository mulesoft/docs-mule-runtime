...
DataWeave represents the Newline Delimited JSON format (ndjson) is as an array of objects. Each line of the ndjson format is mapped to one `Object` element in the `Array`.
...
The DataWeave (dw) format outputs the ndjson input into an array of comma-separated object.
...
This example shows that ndjson reader ignores all lines of ndjson data that are invalid if `skipInvalid=true`.

==== Input

The input to the DataWeave script includes valid and invalid lines of ndjson data. Assume that the input is from a file `myInput.ndjson`.
...
The ndjson reader accepts optional parameters that provide instructions for reading input data.
...
The ndjson writer accepts optional parameters that provide instructions for writing output data.

//TODO: SOME EMPTY DEFAULT VALUES
...
| `encoding` | `String` | | Encoding for the ndjson reader to use.
