...
block level. To control the number or threads to use and the rate at which these processing tasks execute,
the Mule runtime engine automatically makes tuning decisions based on a set of processing types that are also configurable
(see xref:execution-engine.adoc[Execution Engine]).  
...
A Mule application can contain several batch job definitions. Each 
definition has its own scheduling strategy.
