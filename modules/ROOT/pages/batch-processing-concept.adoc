...
+
In this scenario, the Batch Job scope sends the processed records to its Batch Aggregator scope. The Batch Aggregator scope loads an array with all records for an instance from the record stepping queue until all records from the current batch step are aggregated. A processor within the scope must be able to accept an array of records as input.
+
