...
+
<1> The batch job builds blocks of records limited by the configured block size and sends them to their corresponding batch step for processing. If fewer records are available than the configured size, the batch job will also send them to the corresponding batch step instead of waiting for more records.
