...
Mule batch processing components are designed for reliable, asynchronous processing of larger-than-memory data sets. Within an application, a batch job splits incoming messages into individual records, processes the records asynchronously from the main flow within blocks that run in parallel, and then reports on the results. When processing takes place, the batch job can access and modify records, and it can push the processed output to other systems or queues. For details about Mule batch processing scopes and batch processing behavior, see <<batch_phases>>.
...
* TODO: OTHER IMPORTANT USE CASES?
...
* <<phase_load_dispatch>> Splits the input into record, and creates and starts execution of a batch job instance for that input. This phase takes place within the Batch Job scope.
...
+
During this phase only, you can access and modify records using the Mule variables `payload`, `attributes`, and `vars`. The records are consumed during this phase, and there are some special propagation rules for variables (see <<variable_propagation>>).
...
+
In this scenario, the Batch Job scope sends the processed records to its Batch Aggregator scope. The Batch Aggregator scope loads and processes all records for an instance from the stepping queue and continues  until all records from the current batch step are aggregated and processed.
+
