...
+
[calloutlist]
. The batch job builds blocks of records limited by the configured block size and sends them to their corresponding batch step for processing. If fewer records are available than the configured size, the batch job sends them to the corresponding batch step instead of waiting for more records.
...
. The aggregator sends all the aggregated records to the stepping queue.
...
[calloutlist]
. The batch job builds record blocks of the configured block size and sends them to their corresponding batch step for processing.
. Each batch step receives one or more record blocks and starts processing them in parallel.
. After the batch step processes a record, the batch step sends the record to the aggregator for further processing.
. The aggregator continues processing records until there are no more records in the stepping queue that are waiting to be processed by the current batch step.
. The aggregator sends all the aggregated records to the stepping queue.
