...
Mule batch processing components are designed for reliable asynchronous processing of larger-than-memory data sets. Mule splits messages into individual records automatically, processes the records, and reports results. Batch processing can also push the output to other systems or queues. If your Mule application is redeployed or crashes, job execution resumes at the point it stopped. Mule batch processing components also provide semantics for record-level variables, aggregation, and error handling. 
