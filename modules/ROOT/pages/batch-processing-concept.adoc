...
Within a Mule application, batch jobs automatically split source data into individual records and group the records into blocks containing a configurable number of records. Batch jobs process the records according to instructions you configure through Mule components and operations within one or more steps of the each job. You can also aggregate processed records, for example, when upserting them to other systems or queues. 

When batch processing is complete, the batch job reports results, including any failures. Batch components provide mechanisms for handling errors that occur during the batch job. Internally, persistent queues that store unprocessed records enable batch jobs to resume processing at a point of failure if your Mule application crashes or redeploys while processing records. 
