...
[NOTE]
Batch processing is exclusive to Mule Enterprise runtimes. Batch processing is not available in Mule Kernel (formerly Community Edition).

Mule batch processing components are designed for reliable asynchronous processing of larger-than-memory data sets. Mule splits messages into individual records automatically, groups a configurable number of records into record blocks, processes the records in each block, and reports results. Batch processing can aggregate processed records and push the processed output to other systems or queues. Mule batch processing components also provide semantics for record-level variables, aggregation, and error handling. If your Mule application is redeployed or crashes, job execution resumes at the point it stopped. 

//TODO: ARE THERE RECORD-LEVEL VARIABLES IN MULE 4?

Important use cses include the following:
...
The batch job performs an implicit split operation on the Mule message that recognizes the following payloads:
...
The batch job cannot split any other data format. If you are working with a data format that is not compatible with the splitting process, transform the payload to a supported format before it enters the batch job.

//image::batch-main3.png[]

If you are familiar with batch processing in Mule 3.x, you can review the differences in Mule 4.x in the xref:migration-core-batch.adoc[Migrating the Batch Module].

== Basic Architecture

Mule provides a few components for batch processing within a Mule application:

* Batch Job (`batch:job`)
+
Batch processing takes place within this component, which splits large messages into records that Mule processes asynchronously. In the same way flows process messages, batch jobs process records. By default, the runtime stores 100 records in each batch step. You can customize this size according to the performance you require. See xref:batch-filters-and-batch-aggregator.adoc[Refining Batch Steps Processing] for more information.

* Batch Step (`batch:step`)
+
A batch step processes records within each batch job using one or more of the many the Mule components that you add to the step. For example, you can use a Transform Message component within a step to modify the data in each record. 

* Batch Aggregator
+
During the <<on_complete>> phase, this component set the number of records within a batch to process together before sending the records to the next batch step. 
...
block level. Mule uses its autotuning capabilities to determine how many threads to use and the level of parallelism to apply (see xref:execution-engine.adoc[Execution Engine]).
...
After all the records pass through all batch steps, the runtime ends the batch job instance and reports the batch job result to indicate which records succeeded and which failed during processing.
...
Note that by default, a batch job instance does not wait for all its queued records to finish processing in one batch step before pushing any of them to the next batch step. Configuring an aggregator can change that behavior:
...
+
<1> The batch job builds blocks of records limited by the configured block size and sends them to their corresponding batch step for processing. If fewer records are available than the configured size, the batch job sends them to the corresponding batch step instead of waiting for more records.
...
//TODO: IS <batch:record-variable-transformer/> A MULE 3 ELEMENT?
