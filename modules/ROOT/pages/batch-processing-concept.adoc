...
In the On Complete phase, none of this variables (and not even the original ones) are visible. Only the final result is available in this phase. Moreover, since the Batch Job Instance executes asynchronously from the rest of the flow, then no variable set neither in a Batch Step, nor in the On Complete phase will be visible outside the Batch Scope.
