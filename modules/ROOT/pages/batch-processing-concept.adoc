...
* Extracting, transforming and loading (ETL) information into a target system, such as uploading data from a flat file (CSV) to Hadoop. //TODO: WE NEED AN EXAMPLE THAT SHOWS HOW TO DO THIS
* Handling large quantities of incoming data from an API into a legacy system. //TODO: WE NEED AN EXAMPLE THAT SHOWS HOW TO DO THIS
* Routing records to the appropriate processors: Accept Policies and Accept Expression on Batch Step scopes are xref:batch-filters-and-batch-aggregator.adoc#batch_filters[filters] that enable you to route successful and failed records to different processors, or to provide a DataWeave expression that determines which records to accept or skip.
* Recovering from record processing failures and handling the failed records. //TODO: ADD LINK

== Architecture

A set of Mule batch processing components prepare records for processing in batches, run processors on those records, and issue a report on the results of the processing. Record preparation and reporting take place within the Batch Job scope. Processing takes place within one or more Batch Step scopes and, optionally, a Batch Aggregator scope within a Batch Step scope.

image:mruntime-batch-job-overview.png[Flow with Batch Scopes and Processors]

The processors you place within the scopes act on records. Each record has the structure of a Mule event, so the processors can access, modify, and route record payloads. Processors can also work on other parts of the record, such their variables or attributes. The Batch Job scope consumes the records, so record processing outside of the Batch Step scope is not possible.

The following example outlines the XML structure of a Mule flow that performs batch processing and returns a report with the results of that processing. In the example, the `<processor placeholder />` entries within the flow and batch processing scopes are placeholders that represent processors, such as connector operations, Mule core components, and so on, which process Mule messages, records, and the batch processing report.
...
<1> The message source triggers the Mule flow. Common message sources are listeners, such as an HTTP listener, the Scheduler, and connector operations that poll for new files.
<2> Processors located upstream of the Batch Job scope retrieve and, if necessary, prepare a message for the Batch Job scope to consume. For example, an HTTP request operation might retrieve data to process, and a DataWeave script in a Transform Message component might transform the data into a <<valid_input, valid format>> for the Batch Job scope to receive.
<3> When the Batch Job scope receives a message from an upstream processor in the flow, the <<phase_load_dispatch, Load and Dispatch phase>> begins. In this phase, the scope prepares the input for processing as records, which includes creating a batch job instance in which processing takes place.
<4> The batch job instance executes when the flow reaches `<process-records/>`. At this point, the <<phase_process, Process phase>> begins. This phase processes records.
<5> Each Batch Step scope contains one or more processors that act upon a record to transform, route, enrich data, or otherwise process the records. For example, you might configure a connector operation to pass processed records one-by-one to an external server. The only unsupported processors are those that perform a request-response operation, such as an HTTP request, which returns a response.
<6> A Batch Aggregator scope is optional. You can add only one to a Batch Step scope. Processors within an aggregator must be able to accept an array of record objects as input. The aggregator is useful for loading an array of processed records to an external server. It is also possible to use scopes, such as For Each, that iterate over an array so that other processors can process the records individually. The Batch Aggregator scope requires a <<batch_aggregator_processing, `streaming` or `size`>> setting to indicate how to process records.
<7> Additional Batch Step scopes are optional. This example does not contain a Batch Aggregator scope.
<8> After records pass through all Batch Step scopes, Mule completes the batch job instance and reports the results, indicating which records succeed and which failed during processing. You can retrieve or log the result with a Logger or other processor within `<batch:on-complete />`.

[[valid_input]]
== Valid Input to the Batch Job Scope

When triggered by an upstream event in the flow, the Batch Job scope performs an implicit split operation on the Mule message input. The operation accepts any Java Iterables, Iterators, or Arrays, as well as JSON and XML payloads. The scope cannot split any other data format.

If you are working with a data format that is not compatible with the splitting process, transform the payload to a supported format before it enters the Batch Job scope.
...
* <<phase_load_dispatch>>: Splits <<valid_input, valid input>> into records and prepares the records for processing. This phase takes place within the Batch Job scope.
+
* <<phase_process>>: Processes records in batches using one or more Batch Step and, optionally, Batch Aggregator scopes. To process records within these scopes, you need to configure other processors, such as connector operations and Mule components, within these batch scopes.
* <<phase_on_complete>>: Issues a report object with the result of processing the instance. In Studio, a processor for this phase, such as a Logger, goes within the On Complete area (`<batch:on-complete />` ) of the Batch Job scope. 

[[phase_load_dispatch]]
=== Load and Dispatch

When an upstream event in the flow triggers the Batch Job scope, the Load and Dispatch phase begins. In this phase:
...
+
. Mule uses DataWeave to split input to the scope into records and stores the records in a persistent queue. The input is a serialized message payload that Mule splits into a collection of records. During the Process phase, Batch Step scopes pull records from this queue, which is called a _record stepping queue_.
+
...
The Process phase begins after the Load and Dispatch phase finishes loading records to the queue and starts execution of a batch job instance. In this phase, Mule pulls records from the queue to build record blocks of the configured batch block size (100 records per block, by default). Then Mule sends the blocks to the Batch Step scope.

Within the Batch Step scope, processing takes place parallel at the block level, but records within each block process sequentially by default. Mule uses its auto-tuning capabilities to determine how many threads to use and the level of parallelism to apply (see xref:execution-engine.adoc[Execution Engine]).

During this phase, processors within the scopes can access and modify records using the Mule variables `payload`, `attributes`, and `vars`. See propagation rules for variables (see <<variable_propagation>>).

After processing all the records in a block, a Batch Step scope sends the records back to the record stepping queue, where the records await processing by any Aggregator scope within the Batch Step scope or by the next Batch Step scope, if another is present. This process continues until every record passes through every Batch Step scope. At the end of this phase, all records are consumed. They are not available for further processing by downstream processors in the Mule flow.

image::mruntime-batch-step-process.png[Record Stepping Queue for Batch Steps]

As a record is processed, it keeps track of each Batch Step scope it completes.

[[batch_aggregator_processing]]
By default, a batch job instance does not wait for all its queued records to finish processing in one Batch Step scope before making them available for processing by the next. Configuring a Batch Aggregator scope (aggregator) changes this behavior:

* Behavior when the Batch Aggregator scope sets a fixed size:
+
In this scenario, the aggregator pulls records from the record stepping queue into arrays a fixed size. After processing records in an array, the aggregator sends the records back to the record stepping queue for further downstream processing. A processor within the aggregator must be able to receive the array of records as input.
+
...
/////////
<1> Each Batch Step scope within a Batch Job scope receives one or more record blocks and starts processing them in parallel.
<2> After processing a record, the Batch Step scope sends the record to the Batch Aggregator scope for further processing.
<3> The Batch Aggregator scope places records one or more arrays according to its `size` configuration. If fewer records are available from the queue than the configured size, the scope pulls them into the scope for processing. This scenario can occur, for example, when the size setting does not divide evenly into the number of records. The last set of records is smaller than the other sets.
<4> After processing records in a given array, the Batch Aggregator scope sends all the records back to the record stepping queue.

* Behavior when the Batch Aggregator scope is configured to stream records:
+
In this scenario, the Batch Job scope sends the processed records to its Batch Aggregator scope. The Batch Aggregator scope loads and processes all records for an instance from the record stepping queue and continues  until all records from the current batch step are aggregated and processed.
+
image:mruntime-batch-job-process-aggregator-streaming.png[Batch job process with Aggregator configured for Streaming]
+
<1> Each Batch Step scopes receives one or more record blocks and starts processing them in parallel.
<2> After the batch step processes a record, the batch step sends the record to the aggregator for further processing.
<3> The Batch Aggregator scope continues processing records until there are no more records in the record stepping queue that are waiting to be processed by the current batch step.
...
Each record in a batch job instance inherits event variables from the input to the Batch Job scope. You can access and change the value of these variables in the Batch Step and Batch Aggregator scopes, which run during the Process phase, and you can create new variables within these scopes. For each record within this phase, the variables (and modifications to them) are propagated through the different Batch Step and Aggregator scopes within a Batch Job scope. For example, if record R1 sets a variable `varName: "hello"`, record R2 sets `varName: "world"`, and record R3 does not set this variable, then in the next step, R1 has the value `"hello"`, R2 the value `"world"`, and R3 has no value for that variable.
//TODO: does the variable _exist_ (without a value set) in R3 though? Or does it not exist?

Though new and updated variables retain any changes that take place during the Process phase, the variables do not propagate to the On Complete phase, nor do any changes to the values of variables during the Process phase. Only variables inherited from the triggering event to the Batch Job scope propagate to the On Complete phase, and standard variables that are part of the batch job report are also present in this phase. It is also possible to create a variable in the On Complete phase that persists through the phase but not after the phase completes. In the On Complete report, you find the final set variables for a batch job instance, for example:
...
* `myVariable` is an example of a variable created before reaching the Batch Job scope. This variable retains the value it had when it entered the scope. Any changes to the value during the Process phase are not retained in the On Complete phase.
...
Downstream from the Batch Job scope, only `myVariable` persists. Like the records and their attributes, the variables of the records are completely consumed within the Batch Job scope. The batch job instance executes asynchronously from the rest of the flow, so no variable created within the Process or On Complete phases persists outside of the Batch Job scope.
