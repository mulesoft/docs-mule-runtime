...
Mule batch processing designed to process large data sets while providing reliability. Batch processing enables asynchronous processing of larger-than-memory data sets. It automatically splits and stores source data into persistent queues. If the Mule application is redeployed or crashes, job execution resumes at the point it stopped.
