...
Within an application, you can initiate a Batch Job scope, which splits messages into individual records, performs actions upon each record, then reports on the results and potentially pushes the processed output to other systems or queues.
...
When the job instance becomes executable, the batch engine submits a task per each record block to the
I/O pool to process each record. Parallelism occurs automatically, at the record
block level. Auto-tuning logic controls the number or threads to use and the
rate at which these processing tasks execute.
...
The Batch Job scope provides Scheduling Strategy options for controlling the
execution of job instances.
...
creation timestamp.
+
Note that there is no guarantee that the records will be executed in order.
This setting does not control the execution order of records in the batch job,
nor does it take into account the number of records each instance contains.
+
* ROUND_ROBIN: This setting attempts to execute all available instances of a
batch job using a round-robin algorithm to assign the available resources. It
works for the use case in which jobs are triggered manually and you want them to
run concurrently, such as one-way synchronization. This setting was introduced
before the current default, ORDERED_SQUENTIAL.

A Mule application can contain more than one batch job configuration. These
separate batch jobs can run at the same time because the scope of each
configuration is local to instances of the same job and is independent
of other jobs in the application. Instances of the same job cannot run
simultaneously, unlike instances of different jobs.
