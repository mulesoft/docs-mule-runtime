...
Note that by default, a batch job instance does not wait for all its queued records to finish processing in one batch step before pushing any of them to the next batch step. However, you can configure the batch job or the block size of the aggregator to change the behavior of the batch job step:
...
+
<1> The batch job builds blocks of records limited by the configured block size and sends them to their corresponding batch step for processing. The block size is an upper limit. If fewer records are available than the configured size, the batch job processes the records. 
