...
Mule enables you to process messages in batches. Within a Mule application, batch jobs split messages into individual records, process the records, and report results. Jobs can also push the processed output to other systems or queues. 
...
Mule batch processing designed to process large data sets while providing reliability. Batch processing enables asynchronous processing of larger-than-memory data sets, and it automatically splits and stores source data into persistent queues. If the Mule application is redeployed or crashes, job execution resumes at the point it stopped.
