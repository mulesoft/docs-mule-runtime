...
further event processing. In this way, you can think of a flow as a function call that accepts an event as input and then returns the modified event.
...
A subflow is a scope that enables you to group event processors, similar to a flow, but with certain differences and limitations:

* Subflows do not have event sources.
* Subflows do not have an error handling scope.
* During design, subflows work as a macro that replaces the Flow Reference components that call the subflow. When you build the application, all flow-reference components that call a subflow are replaced by the contents of the referenced subflow.
* Referencing a subflow has higher performance than referencing a flow.

=== Considerations

Because the contents of the sub-flow replace each Flow Reference component that references the subflow, several instances of the event processors inside the subflow exist in the application at runtime. This behavior can cause issues with event processors that depend on unique IDs or instances to execute like Batch processors.

For example, configuring a Batch Job inside a subflow causes the application to fail during the build phase if the subflow is referenced from more than one Flow Reference component. The application fails to build because multiple instances of a Batch Job with the same Job Instance ID exist, which is not allowed.

== Error Handling

Each flow (but not subflow) can have its own error handling. One reason for
calling a flow through a Flow Reference component is to separate out the error
