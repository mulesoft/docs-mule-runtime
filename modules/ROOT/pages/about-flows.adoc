...
further event processing. In this way, you can think of a Flow Reference as a function call that accepts an event as input and then returns the modified event.
...
Because the contents of the subflow replace each Flow Reference component that references the subflow, several instances of the event processors inside the subflow exist in the application at runtime. This behavior can cause issues with event processors that depend on unique IDs or instances to execute like Batch processors.
