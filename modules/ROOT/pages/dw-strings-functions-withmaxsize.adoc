...
Checks that the string length is a large as the specified `maxLength`.
If the string is larger, the function cuts the text, starting from left.
...
Helper function to make `withMaxSize` work with a `null` value.
