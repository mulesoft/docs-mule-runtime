...
:page-aliases: general:getting-started:content-based-routing.adoc, choice-router-xml-reference.adoc
...
To configure the choice router you specify one or more conditions to evaluate, and the message processors that execute when any condition evaluates to `true`. Additionally, you configure a default route that executes when none of the defined conditions are `true`.
...
. Click on *When*, inside the Choice router. Configure the *Expression* value in the properties window to specify the condition to evaluate.
...
* XML Config
+
The following is the basic XML structure for a Choice router:
+
[source,xml,linenums]
----
...
<2> Add any number of message processors to execute if the `expression` evaluates to true.
<2> Add any number of message processors to execute none of the conditions evaluate to true.
