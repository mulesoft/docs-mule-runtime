...
* In the window that opens, log into Anypoint Exchange and search project: _Content-Based Routing_.
* Click Open (image:mruntime-open-exchange-icon.png[6%, 6%]).
...
Following is the XML code of the previous Choice router example:
...
| Business Events a| For activating the Business Events feature. For example: +
[source,xml,linenums]
----
...
|===
|Property |Description
| Expression (`expression`) a| Expression in DataWeave language to evaluate input. If the expression evaluates to `true`, this routing option is used. For example: +
[source,xml,linenums]
----
<when expression="#[vars.language == 'Spanish']" >
----
