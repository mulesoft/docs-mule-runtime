...
To configure the choice router you specify a condition to evaluate, and the message processors that execute when the condition evaluates to `true`. Additionally, you configure a default route that executes when none of the defined conditions are `true`.
