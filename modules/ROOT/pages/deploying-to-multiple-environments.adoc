...
Consider externalizing other aspects of your configuration, like time-out values, polling frequencies, etc. Even if they don't vary between environments. This configuration facilitates tuning and experimenting as the whole Mule application  becomes configurable through a single properties file.
...
+
This practice ensures that files are easy to re-use across teams. A good strategy is to add the application name or ID to the properties file names.
...
+
Doing this can cause conflicts between the various apps that share a domain. Instead, you can set environment variables over the scope of the deployed app, its domain and other apps under that domain.
+
In Studio, you can create environment variables through the *Environment* tab of the *Run Configurations* menu, reachable via the dropdown menu next to the Play button.

== Basic Architecture of an Environment Property Configuration

Review the following Mule configuration fragment that defines the global property `env`, a dynamic configuration properties file based on the `env` property, and an HTTP connector with variable configuration:
...
This example uses Spring's property placeholder resolution mechanism. The variable bits are clearly visible: the *base path*, *host*, and *port* can vary for each environment where this connector gets deployed in.

When deploying your application, the `env` property can be dynamically replaced by the particular environment you're deploying to, such as `qa` or `prod`, causing Mule to load a different `.yaml` file with properties specific to the configured environment.
