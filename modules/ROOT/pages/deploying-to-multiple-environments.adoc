...
You can configure your Mule application to facilitate deployment to one of many different environments, both on-premises and in the cloud, by using configuration files for your development environments (prod, qa, etc) so your application dynamically loads property files based on the environment you are working on. To do so, you must complete the following macro steps:
...
== Configuring Mule Apps to Deploy in Multiple Environments
...
As a prerequisite you need to xref:runtime-manager::servers-create.adoc[create a server in Runtime Manager] and link it to Mule. Then you can xref:runtime-manager::servers-properties-create.adoc[create a custom server property].
...
If you deploy your application to CloudHub via Runtime Manager, you can xref:configuring-properties#setting-properties-values-in-runtime-manager.adoc[set properties values] using the Runtime Manager Console.
