...
* Perceptive Flow Design only functions with Anypoint connectors which support xref:5.x@studio::testing-connections.adoc[connection testing]. See xref:5.x@studio::datasense-enabled-connectors.adoc[the list].
...
. Configure the Salesforce connector, and its corresponding global connector, then test the connection to confirm communication. Refer to xref:5.x@studio::testing-connections.adoc[Testing Connections] for details.
...
* Learn more about Anypoint DataMapper through the xref:5.x@studio::datamapper-user-guide-and-reference.adoc[Datamapper User Guide and Reference]. Specifically on the subject of metadata, see xref:5.x@studio::defining-datamapper-input-and-output-metadata.adoc[Defining DataMapper Input and Output Metadata].
