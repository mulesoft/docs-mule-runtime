...
== Repeatable vs Nonrepeatable Streaming
Understand which streaming strategy resultsresults in the best flow performance for your use case: +

* A repeatable stream (read payload more than once)
* A nonrepeatable stream (read payload only once)
...
Following certain practices at the design phase helps you to achieve better performance from your Mule apps.
