...
logic, the aggregation may be performed with a DataWeave transformation.

The payload that comes out of the Scatter-Gather is a collection of messages, 
each being the result of each route. Those messages are accessible with 
DataWeave.
...
For Mule 4, the Java class is removed, and the flow elements would become:
...
Notice the `ee:transform` element that replicates the logic of the `CustomSGAggregationStrategy`, but in DataWeave rather than Java.
