...
. Export your certificate in the standard PEM format. First verify keystore type, then export certificates:
----
...
Replace `<keystore-type>` with the value of your choice verified in keystore type for example pkcs12. +
Replace `<keystore-name>` with the name of the keystore you wish. +
Replace `<pem-name> with the name of the pem you wish
...
<tls:context name="customContext">
    <tls:trust-store path="trustStore" password="mulepassword" type="pkcs12"/>
