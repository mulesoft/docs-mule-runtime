...
|1.3|Supported when running Mule on a JDK that supports TLS 1.3 but _not_ enabled by default|Not supported|Supported and enabled when running on a JDK that supports TLS 1.3
...
Use Java 8 to generate your keystores. Using a different JDK version generates an incompatible keystore that causes an invalid keystore format runtime error when the Mule application tries to read the keystore.
