...
+
You can change the encryption algorithm to ECDSA by specifying `-keyalg EC` instead of `-keyalg RSA`.
+
[IMPORTANT]
If you do not specify the algorithm, the tool uses DSA by default which is not compatible with TLS 1.2 and causes the SSL handshake to fail when the Mule app receives an HTTPS request.
