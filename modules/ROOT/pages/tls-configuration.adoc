...
If the `tls:context` has an empty truststore defined:
```
<tls:context name>
    <tls:trust-store />
</tls:context>
...
```
in the global case, then the default values of the JVM are used, which usually include a truststore with certificates for all the major certifying authorities.
