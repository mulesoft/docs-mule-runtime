...
Mule uses the default Java truststore for the `tls:trust-store` and `tls:key-store` elements. Java updates the default trust store when you update Java, so getting regular updates is recommended to keep well-known CA certificates up-to-date.

[IMPORTANT]
A custom Java Trust Store is not a standard solution for MuleSoft and does not update automatically. Use the default Trust Store for automatic updates with new Java versions.
