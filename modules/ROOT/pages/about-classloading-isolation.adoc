...
When the connectors in your application need access to a dependency in your project, you need to configure the dependency to be visible by the connector's classloader. There are two different ways to do this configuration:
...
All dependencies (`.jar` files for example) declared in the application's `pom.xml` file are visible to the application's classloader but not visible to the classloader of each connector used in the application. The Mule runtime engine classloading mechanism isolates each connector to prevent it from accessing classes from other connectors. If a connector's classloader needs access to an application dependency, you need to declare this dependency as a shared library.
...
Instead of configuring shared libraries, which makes application dependencies visible to all connectors in the application,
you can configure additional plugin dependencies to enrich a specific connector's classpath. This configuration enables you to effectively add new dependencies to a connector, as if the dependencies were declared in the connector's `pom.xml` file.
