...
You can also add custom xref:enabling-ws-security.adoc[*Token Validators*] to the SOAP Component.
...
. Click the image:add.png[] icon in the *Add Configuration Element* pane to create a new key-value pair.
+
...
+
[TIP]
If you have not yet created any beans, click the image:add.png[] button to open a new properties panel in which you can create and configure a new bean. The bean imports the Java class you have built to specify the custom validator's override behavior.
+
