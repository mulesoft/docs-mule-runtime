...
For further details on MEL syntax, see the full xref:3.7@mule-expression-language-reference.adoc[Mule Expression Language Reference] material.

== Assumptions

This document assumes that you have read xref:3.7@mule-expression-language-mel.adoc[Mule Expression Language MEL] and have studied the xref:3.7@mule-message-structure.adoc[Mule message structure].
...
For a full list of syntax rules, see full xref:3.7@mule-expression-language-reference.adoc[MEL reference] material. +
...
|*Wildcards* |`wildcard("Hello*")` |Matches a value (the message payload, by default) against a wildcard pattern, these use the metacharacters '?' to represent any single character and '*' for a repetition of any character. It's case sensitive by default.
|===

Further, you can use expressions inline to create lists, maps and arrays. Learn more about xref:3.7@mule-expression-language-reference.adoc[accessing maps, lists and arrays]from within a MEL expression.
...
There is really no such thing as a single _typical_ MEL expression.** **That said, a few example expressions can help illustrate how MEL expressions resolve. As the following table of examples demonstrates, the values that MEL expressions return can be numerical values, logical values (`true` or `false`), strings, or virtually any data type. MEL expressions can also perform operations, invoke methods, and execute functions. Explore all the possibilities by consulting the complete xref:3.6@mule-expression-language-basic-syntax.adoc[syntax reference]. Access full xref:3.7@mule-expression-language-examples.adoc[examples] that illustrate how to use MEL expressions in applications.
...
[TIP]
For the complete reference on date and time functions, see xref:3.6@mule-expression-language-date-and-time-functions.adoc[Mule Expression Language Date and Time Functions].
...
* Access full *xref:3.7@mule-expression-language-examples.adoc[examples]* that illustrate how to use MEL expressions in applications.
* Access a full set of *xref:3.7@mule-expression-language-reference.adoc[reference material]* for MEL. +
* Access a complete list of *xref:3.7@mule-expression-language-tips.adoc[tips and gotchas]* when using MEL.