...
a| Defines where the Annotation can be used. If not defined, it can be used in any possible target.
...
field type is only going to be validated at design but at runtime only minimal
type validation is going to be done. This is useful for performance reasons
specially with complex Object types.
