...
Mule runtime engine provides the ability to create locks for synchronizing access to resources within Mule components. To manage concurrent access to resources, Mule provides a lock factory that you can access programmatically by scripts or in custom extensions built with the xref:mule-sdk::getting-started.adoc[Java SDK].
...
* xref:on-premises::mule-high-availability-ha-clusters.adoc[Mule High Availability (HA) Clustering]

*  xref:on-premises::creating-and-managing-a-cluster-manually.adoc[Creating and Managing a Cluster Manually]
