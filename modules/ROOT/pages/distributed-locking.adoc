...
Mule provides the ability to create locks for synchronizing access to resources within Mule components. To manage concurrent access to resources, Mule provides a lock factory that you can access programmatically by scripts or in custom extensions built with the xref:1.1@mule-sdk::getting-started.adoc[Java SDK].

Any locks you create with Mule's lock factory work seamlessly on deployment models which use a single server or a cluster of servers. This means that if you have a server running the same flow in multiple threads or a cluster environment running the same app, you can guarantee resource synchronization with Mule locks. Additionally, Mule's locking system offers a simple API to access shared locks. Learn how to create locks and sharing them between operations in the following sections.


== Creating a Lock

To create a lock, follow these tasks:

. Gain access to a `LockFactory` through injection. +
If you are building an extension with the Java SDK and need locking for one of your  operations, you need to inject the `LockFactory` into your class. +
. Use the `LockFactory` to start creating locks. +
...
After obtaining the lock, the critical code section can be executed and, no matter what the result is, we want to release the obtained lock (to prevent deadlocks on other threads waiting for this lock), so we wrap the `lock.unlock` call in a `finally` block.
...
Locking is not only useful to create locks for many threads running the same operation, but it can also be used for different operations that may be running in parallel. Using the same lock ID to create a lock in different operations, will make them obtain access to the same lock. The following code sample describes two custom operations which create locks using the same lock ID.
