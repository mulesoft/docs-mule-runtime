...
With XA Transaction, you are able to nest transactions. This means, when executing within a transaction, you can create a new Transaction (using the xref:try-scope-concept.adoc[Try Scope]). Therefore, operations that run within the new transaction will be accountable to it. If the nested transaction is rollback, it does not mean that the original one will rollback. After the nested transaction has finished (committed or rolled back), the original transaction continues its execution. 
...
In the example above, there is an original transaction created by the first `try` Scope. Then, there is a second one created by the `try` Scope within it. So, the `vm:publish` operation runs within the first transaction, the `jms:consume` and the `db:insert` will run within the second transaction. In the example, after the error is raised, the second transaction is rolled back (and so, the two mentioned operations), but since the first try handles the error with an `on-error-propagate`, the first transaction commits (therefore, the `vm:publish` operation). Another example could be:
...
This time, the second `try` scope does not create a transaction. It is there only to provide error handling, so we can continue with the next operations. The third `try` does create a transaction. Now, both `vm:publish` and `db:insert` run within the first transaction, and `jms:consume` operation does it in the second one. When the error is raised, the second operation is rolled back. Then, the error propagated is handled by the `on-error-continue` of the second `try` scope. After that, the first `try` scope continues with its execution (the `db:insert` operation), and the first transaction is committed. 
