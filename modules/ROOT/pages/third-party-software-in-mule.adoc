...
Mule runtime engine includes third-party software as internal dependencies. Mule has dependencies on a subset of libraries from the included third-party software. Mule applications might also include additional third party dependencies managed through Maven, which are not included in the third-party software list shared below.
