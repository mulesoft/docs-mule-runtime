...
+
image::mruntime-business-event-flow.png["A flow configuration interface for JMS processing with options to enable event tracking"]
+
...
* In the UI, select *Enable default events tracking*:
+
image::mruntime-business-event-component.png["A business event tracking within a choice component"]
+
