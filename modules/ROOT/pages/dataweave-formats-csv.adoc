...
|`escape`  |`String`|`\`|Character used to escape invalid characters, such as separators or quotes within values of fields.
...
* If `header=false`, you must access the fields by index, referencing the entry first and the field next, for example, `payload[107][2]`. Valid Options: `true` or `false`.
...
|`separator`  |`String`|`,`|Character that separates one field from another.
