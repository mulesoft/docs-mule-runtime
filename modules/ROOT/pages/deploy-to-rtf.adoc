...
== Deploy to Runtime Fabric Using Mule Maven Plugin
...
== Redeploy to Runtime Fabric Using Mule Maven Plugin
...
| `memoryMax` | Amount of max memory to be allocated for each replica of the application +
If a memoryReserved configuration is present, ensure that this value is equal or higher. +
[source,xml,linenums]
----
<deploymentSettings>
...
| `cpuReserved` | Amount of cores to be allocated for each replica of the application. +
If not present, the default value is 0.5 vCores. +
[source,xml,linenums]
----
<deploymentSettings>
    <cpuReserved>500m</cpuReserved>
...
* xref:runtime-manager::deploy-to-runtime-fabric.adoc[Deploy to Runtime Fabric Using Runtime Manager]
