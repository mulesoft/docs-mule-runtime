...
* Create a separate XML for global elements, for example configuration elements.
...
* Add indentation and format to all your XML (Mule XMLs, `pom.xml`, `log4j2.xml`, etc) files before committing to the source code repository.
...
To avoid the need to create variables per environment, Mule 4 contains a built-in mechanism that enables you to set default values. The recommended approach is to manage the properties via Anypoint Runtime Manager. For details see xref:intro-configuration.adoc[Introduction to Mule 4: Configuring Mule Apps] and xref:configuring-properties.adoc[Configuring Properties].

== Tip while Building Apps

As you build your Mule app, avoid packaging the configuration files for all the environments inside the application. Instead, use a `.yaml` file to provide defaults properties, and use Anypoint Runtime Manager to override each of these properties at deployment time.
...
* Use inline scripts on the development phase.
...
* Use Flow reference to separate and reuse common logic.
...
As you build your app and if you use Anypoint Connector for Apache Kafka (Apache Kafka Connector) consider the following points:

* Use the `.properties` file to add bootstrap server configuration.
* Add property placeholders either in `.yaml` or `.properties` files, and use the file in the configuration.
...
* Apache Kafka Connector provides various settings for producer and consumer, add your own settings in `consumer.properties` and `producer.properties` under `src/main/resource`.
