...
To avoid creating a separate set of variables for each environment, Mule 4 contains a built-in mechanism that enables you to set default values, which you manage by using Anypoint Runtime Manager. For details, see xref:about-mule-configuration.adoc[Mule Configuration File] and xref:configuring-properties.adoc[Configuring Properties].
