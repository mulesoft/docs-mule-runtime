...
Property placeholders are reserved for HTTP and HTTPS ports in CloudHub deployments, and the defaults for these properties are used by the shared load balancer in CloudHub. Use of the same placeholders is recommended for on-premises deployments.
