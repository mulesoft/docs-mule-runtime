...
This example shows the password configuration for Amazon S3 Connector connection to get an image from Amazon S3. The `p` function returns a string that identifies the value of `secure::accessKey` and `secure::secretKey` properties. The `secure::` prefix is used to access Mule secure configuration properties.
