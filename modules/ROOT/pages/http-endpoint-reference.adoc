...
To learn more about endpoints, refer to the xref:3.4@studio-endpoints.adoc[Studio Endpoints] document.
...
. In the *Connector Reference* field, use the combo-box to select a *Global Connector* for the HTTP endpoint to reference. The endpoint uses the xref:3.4@http-transport-reference.adoc[connector] configurations you define within the global connector.  +
