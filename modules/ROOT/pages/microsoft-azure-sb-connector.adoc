...
. Select the *Microsoft Azure Service Bus* connector from palette.
+
...
The following example configures a Mule app to send a message to an Amazon Azure Service Bus queue or topic. The following diagram shows the flow for this example:

image::amazon/amazon-azure-use-case-flow.png[Sending a message to a queue or topic]
...
. Drag the *Microsoft Azure Service Bus* connector next to the HTTP connector in the flow.
...
. Because the *Publish message* operation has no return value, use another *Set Payload* to return a message: `#["message sent"]`.
...
** Set the *Queue* or *Topic* field to the desired destination, and set the *Receive mode* to `MANUAL`.
. Drag a *Record Variable*, *Session Variable*, or *Variable* transformer next to the *Microsoft Azure Service Bus* in the flow.
...
. Add another *Microsoft Azure Service Bus* connector next to the variable component in the flow, and select the *Acknowledge message* operation. For the *Lock Token* field value, retrieve the stored *Lock Token* value.
...
+
To split a string of many messages separated by commas into separate messages, drag a *Transform message* transformer to the flow and use the following script:
```
%dw 1.0
...
. Create a Mule app to receive messages from a queue or topic, as described in <<basic>>.
. Fill in the "Scheduled enqueue time UTC" value of the *Publish message* operation with the date or time when you want the message to be dispatched. For example: `2019-06-27T21:16:46.866Z`.
...
When there are network connection problems, you can direct the source components for the Microsoft Azure Service Bus connector to automatically execute reconnection attempts by specifying a reconnection strategy. You can specify a reconnection strategy when you create or edit the connector configuration:
