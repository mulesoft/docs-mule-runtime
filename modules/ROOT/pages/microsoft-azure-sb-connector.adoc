...
Simple Mule application that sends a message to a Queue. Sending to Topic can be done in a similar way.

. Create a new Mule application in Studio.
. Add a new *HTTP Connector* as the source of the new flow. Make it listen to the port 8081, path `/send`.
. Add a *Set Payload* or *Transform Message* and craft a message to be enqueued.
. Drag a new *Microsoft Azure Service Bus* connector to the flow. Create a configuration for it.
...
When you call `http://localhost:8081/send`, a message will be uploaded to the queue.

=== Receive messages from a queue with manual ACK

. Create a new Mule application in Studio.
. Add a new *Microsoft Azure Service Bus* as the source of the new flow and create a configuration for it.
...
If you wish to send a message to the queue or topic, but you wish it to be dispatched to its listeners at a certain time:name: value
...
In some circumstances, the underlying library used by the connector may regularly log complete stacktraces with level WARN. These messages do not represent an issue but may clutter the logs.
To reduce the noise in said logs, there are two alternatives. Both require changes in the `src/main/resources/log4j2.xml` file:

1) You can specify an *AsyncLogger* for the library's package, raising the level of the log:
...
2) You can add a *RegexFilter* to the existing appender (E.g. RollingFile):
...
In case of network connection problems, if you want the Listeners (Sources) components in the Azure connector to automatically execute reconnection attempts, you must specify a reconnection strategy. The reconnection strategy is specified when creating or editing the *'Connector Configuration'* in the Azure connector (parameter group: *'Basic Settings'*). In the *'Connector Configuration'* pop-up screen, you must access the *'Reconnection'* tab and indicate that a *'Standard Reconnection'* strategy will be used. There we could indicate the *'Frequency (ms)'* of the check and the number of *'Reconnection Attempts'*.
