...
. Create a Mule app to receive messages from a queue or topic, as described in <<basic>>.
. Replace the call to the *Publish message* operation with ta call to the *Publish batch of messages* Operation. Make sure the *Destination type* and *Destination name* are properly set.
