...
The following example uses a Mule app to send a message to an Amazon Azure Service Bus queue. You can create a similar app to send a message to an Amazon Azure Service Bus topic:
...
. Drag the *Microsoft Azure Service Bus* connector to the flow, and then configure the connector:

/// I see Queue receive and Topic receive as operations. Not Publish ///////message.

.. Set the "Publish message" operation, "Destination Type" (`QUEUE` or `TOPIC`) and the "Destination name".
. Since the "Publish message" operation has no return value, use another *Set Payload* to return a message: `#["message sent"]`.


When you call `http://localhost:8081/send`, Mule uploads the message to the queue.

=== Receive Messages from a Queue or Topic with a Manual ACK

. Create a new Mule app in Studio.
. Add a *Microsoft Azure Service Bus* connector as the flow source, and configure the connector:
.. In the *Operation field*, select *Queue receive* or *Topic receive*.
.. Set the *Queue* field or *Topic* field to the desired destination, and "Receive mode" to `MANUAL`.

/// Not finding the Receive mode field

. Drag a *Variable* component to the flow.
...
. Add another *Microsoft Azure Service Bus* connector to the flow and set it to call the *Ack message* operation. For the *Lock Token* field value, retrieve the *Lock Token* stored before.

Mule processes every message received and acknowledges successful message processing.

/// Ask about if Mule acks all received messages.

To have Mule automatically acknowledge messages, set the *Receive Mode"* field to `AUTOMATIC`.

/// where is the receive mode field?

=== Scheduled Reception of Messages

The following Mule app...

. Create a Mule app to receive messages from a queue or topic as described above. When you create the app:
.. Set the *Unix CRON Expression* field to a CRON expression that follows the UNIX standard. For example, to connect and receive messages everyday at 8 AM, use `0 8 * * *`.
...
The following example creates a Mule app to send a message to all listeners at a specified time:
...
When handling a very high number of messages (thousands per second), especially if publishing to and reading from the same destination within the same Mule app, you might see messages like this in the log:
...
```
When the connector performs a "prefetch", the lockToken's validity time is fixed in relation to that moment. The problem arises when the lockToken's validity time is not long enough to process the entire batch of records. In cases like this, Mule might throw an error because you intend to do an ACK of a lockToken that is expired. To prevent this, reduce the size of the prefetch (default to 100), and increase the validity time of the lockToken, or both.  (this can be done when creating the queue/subscription from the connector, in the lock duration property or from Azure portal for existing queues). The maximum duration value for the lock token in Azure is 5 minutes (300 seconds).
...
In situations were there are network connection problems, you can direct the sources components for the Azure connector to automatically execute reconnection attempts by specifying a reconnection strategy. You can specify a reconnection strategy when you create or edit the connector configuration:
