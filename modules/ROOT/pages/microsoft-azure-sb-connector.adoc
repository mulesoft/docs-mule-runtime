...
// Reviewers: I see a *Lock* field but not a *Lock Token* field.

The connector processes every message received and acknowledges successful message processing.
...
The following example shows how configure a Mule app to send multiple messages to a queue:

. Create a Mule app to receive messages from a queue or topic, as described in <<basic-example>>.
. Replace the call to the *Publish message*" operation with the *Publish batch of messages*. Make sure the *Destination type* and *Destination name* are properly set.
...
In situations where you have restricted access to your resources (to only send/receive amqp messages), enable the option *AMQP-only credentials* inside the *Advanced* tab.

=== Handling a Very High Number of Messages

When handling a very high number of messages, such as thousands per second, you might see messages like the following in the log.() :
