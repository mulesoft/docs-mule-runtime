...
. Click *+* next to the *Connector Configuration* field to configure global elements for the connector.
...
+
With *Basic authentication*, applications gain access to Azure Service Bus resources by providing a shared access key on the Azure Service bus namespace. The shared access key enables user access to access Azure Service Bus resources without having to store user account access keys in the application.
+
...
|*Key name* | Name of the shared access key configured on the Azure Service Bus namespace. An access key created at a lower level, such as a topic-level shared key, does not work with this option unless you disable the connectivity test at startup.
...
|*Service namespace* | Name of the service namespace to address Service Bus resources within your application.
|*Connection string* | Link provided by the Microsoft Azure portal that contains values for the Azure `Endpoint`, `SharedAccessKeyName`, and `SharedAccessKey` parameters. For example:

[source,code,linenums]
----
Endpoint=sb://mulesbft.servicebus.windows.net;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=VqhjhWH3u7Ffw9xnP7MU5FQcbJPksBNrsgutTQ2BHdc=
----
|===
...
Use the *Queue listener* input source when you want the app to receive messages from an Azure queue. Configure the input source as follows:
...
|===
|Field |Description
|*Queue name* | Queue to receive events. To receive events from the dead-letter queue, specify `QueueName/$deadletterqueue`. The dead-letter queue is a queue that holds messages that cannot be processed or delivered.
|*Receive mode*
a|
...
Use the *subscriptionlistener listener* input source when you want the app to receive messages from an Azure subscription. Configure the input source as follows:
...
This example configures a Mule app to send a message to an Azure Service Bus queue. The following diagram shows the flow for this example:
...
. Add a *Set Payload* transformer and enter a message value for the queue.
. Drag the *Microsoft Azure Service Bus* connector next to the *Set Payload* transformer on the canvas.
. Click *+* next to the *Connector Configuration* field to configure a global element for the connector. For more information about configuring a global element , see <<install-studio>>.
. Configure the connector as follows:
** Select the *Publish message* operation and *Destination type* `QUEUE`.
...
This example configures a Mule app to send an acknowledgement after receiving a message from a queue. The following diagram shows the flow for this example:
...
** In the *Operation field*, select *Queue listener*.
...
. In the *Choose Global Type* dialog, select *Microsoft Azure Service Bus: Basic authentication* and set values for the required fields. For more information about the Basic authentication fields, see <<install-studio>>.
. Drag a *Record Variable* transformer next to the *Microsoft Azure Service Bus* on the canvas.
