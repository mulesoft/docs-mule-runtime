...
The Microsoft Azure Service Bus connector has two input sources:
...
For more information, see https://docs.mulesoft.com/mule-runtime/3.9/flow-processing-strategies#queued-asynchronous-flow-processing-strategy.
...
In case of possible micro-cuts in the network connection, asynchronous message sending flows may experience exceptions and message losses. This is due in principle to two factors on the one hand Azure TimeoutException exceptions (*com.microsoft.azure.servicebus.primitives.TimeoutException*) do not fall within the retry mechanism (RetryPolicy, in particular RetryExponential - Configurable from Azure Connector Configuration) and on the other hand as the exception is generated within a thread pool, that executes the calls asynchronously, the exception does not populate towards a "catch exception strategy". For this, a programmatic solution must be implemented by the client in the flow according to the business needs.
