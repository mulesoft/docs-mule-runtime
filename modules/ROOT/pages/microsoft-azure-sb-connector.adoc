...
. Drag the *Microsoft Azure Service Bus* connector next to the HTTP connector on the canvas.
...
** To send messages to a topic, enter the name of the topic int he *Topic* field and the name of the subscription in the *Subscription* field.
...
To prevent this issue, reduce the size of the prefetch (default 100), and increase the validity time of the lockToken, or both. You can do this when creating the queue or subscription from the connector, in the lock duration property or from the Azure portal (for existing queues). The maximum duration value for the lock token in Azure is 5 minutes (300 seconds).
