...
To start configuring the connector, you first need to select and configure an input source. The Amazon Azure Service Bus connector supports three input sources:

* HTTP connector, which receives messages from a web server. Using an HTTP connector enables you to test the connector.
...
+
. To configure the *HTTP* connector, accept the defaults by clicking *OK*.
+
...
The Azure Service Bus connector has two input source operations:
