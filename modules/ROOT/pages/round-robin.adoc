...
The Round Robin router accepts an array as the input payload. The router can contain two or more routes, but it routes the Mule event containing that payload to only _one_ of the routes each time the flow executes the router. Each route within Round Robin is a scope that can contain one or more processors. Starting with the first route, Round Robin executes processors within the route over the Mule event. The router keeps track of the previously selected route and never selects the same route consecutively. For example, the first time Round Robin executes, it selects the first route. The next time, it selects the second route. If the previously selected route is the last route, Round Robin executes the first route.

The payload, variables, and attributes from the Mule event that reaches Round Robin are available for processing within one of it routes. Any changes to the Mule event within a route propagate to the processor after Round Robin. If a route does not change some part of the Mule event, the unchanged value is propagates to that processor.
