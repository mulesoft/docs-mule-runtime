...
. The application first uses a xref:file-connector.adoc[File connector] to upload a CSV file, then uses a link:/mule-user-guide/v/3.6/datamapper-concepts[DataMapper] to convert the data format into a collection (see mapping below). Each item in the collection represents a lead. Each lead contains a company name, a first name, a last name, a birthday and an email address.
+
...
* xref:batch-processing-reference.adoc[reference details]

* xref:batch-processing-reference.adoc[attributes]

* xref:batch-filters-and-batch-commit.adoc[filters]

* xref:batch-filters-and-batch-commit.adoc[batch commit]
* xref:record-variable.adoc[record-level variables]
* xref:batch-processing-reference.adoc[MEL expressions]

* xref:runtime-manager::cloudhub-fabric.adoc[CloudHub support for batch processing]

* xref:anypoint-connectors.adoc[Anypoint Connectors]

* xref:poll-reference.adoc[Polling and Watermarks]

* xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper]
