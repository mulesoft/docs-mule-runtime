...
To build applications with Batch jobs and run them in a test or production environment, you must build the application with an Enterprise runtime. Learn more about xref:6.x@studio::changing-runtimes-in-studio.adoc[Changing Runtimes in Studio].
====
...
* Anypoint Studio. See xref:6.x@studio::index.adoc[Anypoint Studio Essentials] chapter to learn more about developing with Mule's graphical user interface.
* xref:anypoint-connectors.adoc[Anypoint Connectors].
* xref:6.x@studio::datamapper-user-guide-and-reference.adoc[Anypoint DataMapper User Guide and Reference]
...
* xref:poll-reference.adoc[Polls].
...
Access xref:batch-processing-reference.adoc[Batch Processing Reference] for more information.
====
...
* A *batch job instance* is an occurrence in a Mule application whenever a Mule flow executes a batch job. Mule creates the batch job instance in the <<Load and Dispatch>> phase. Every _batch job instance_ is identified internally using a unique String known as xref:batch-job-instance-id.adoc[batch job instance id].
====
...
. Mule sends the message payload through a xref:splitter-flow-control-reference.adoc[collection splitter]. This first step triggers the creation of a new batch job instance.
...
* You can set *record variables* on records and pass them from step to step (xref:record-variable.adoc[read more])

* You can apply filters by adding *accept expressions* within each batch step to prevent the step from processing certain records; for example, you can set a filter to prevent a step from processing any records which failed processing in the preceding step (xref:batch-filters-and-batch-commit.adoc[read more])

* You can *commit* records in groups, sending them as bulk upserts to external sources or services. (xref:batch-filters-and-batch-commit.adoc[read more])
...
Read more about xref:batch-filters-and-batch-commit.adoc[fine-tuning filters] on batch steps to manage failed records at a more granular level.


Also check xref:using-mel-with-batch-processing.adoc#mel-functions-that-involve-batch-jobs[MEL expressions for Batch Processing] for a detailed description of the MEL expressions you can use to monitor
...
If a batch job _does not_ accumulate enough failed records to cross the `max-failed-records` threshold, _all_ records – successes and failures – continue to flow from batch step to batch step; use xref:batch-filters-and-batch-commit.adoc[filters] to control which records each batch step processes.
...
link:{attachmentsdir}/new_batch_example_app.zip[Download example app]

[TIP]
Note that this example introduces features not discussed in great detail in this document. Consult xref:batch-filters-and-batch-commit.adoc[Batch Filters and Batch Commit] and xref:record-variable.adoc[Record Variable] for more information.
...
Batch flow example in Anypoint Studio:

image::mruntime-batch-example-1.png[Batch flow example]
...
[TIP]
====
If you copy and paste the XML code into your instance of Studio, be sure to enter your own values
...
http://www.mulesoft.org/schema/mule/ee/dw http://www.mulesoft.org/schema/mule/ee/dw/current/dw.xsd">
    ...
    <sfdc:config doc:name="Salesforce" name="Salesforce" password="password"  username="username">
        <sfdc:connection-pooling-profile exhaustedAction="WHEN_EXHAUSTED_GROW" initialisationPolicy="INITIALISE_ONE"/>
    </sfdc:config>

    <batch:job max-failed-records="1000" name="CreateLeads">
        <batch:threading-profile poolExhaustedAction="WAIT"/>
        <batch:input>
            <file:inbound-endpoint doc:name="File" moveToDirectory="src/main/resources/output" path="src/main/resources/input" pollingFrequency="10000" responseTimeout="10000"/>
...
payload map {
	...
	LastName   : $.LastName
        ...
        <batch:process-records>
            ...
            <batch:step accept-expression="#[!recordVars['exists'] == false]" name="insert-lead">
            ...
            <batch:step accept-policy="ONLY_FAILURES" name="log-failures">
...
. The application first uses a xref:file-connector.adoc[File connector] to upload a CSV file, then uses xref:dataweave.adoc[DataWeave] to convert the data format into a collection (see mapping below). Each item in the collection represents a lead. Each lead contains a company name, a first name, a last name, a birthday and an email address.
+

image::mruntime-batch-example-2.png[Transform component mapping]
...
[start=3]
. Mule begins processing each lead as a record. The first batch step, `lead-check`, uses a *Salesforce Connector* wrapped with a xref:message-enricher.adoc[Message Enricher] to:
...
image::mruntime-batch-example-3.png[example_query3]

** The first item shows the MEL expression that evaluates against the message payload, which in batch processing is the payload of the record.


image::mruntime-batch-example-4.png[Message enricher scope]


** The second item shows the Message Enricher component view. This component enriches the payload of the record.
** The third item shows how to set a record variable on those records for which a Salesforce lead already exists.
...
----
<enricher doc:name="Message Enricher" source="#[payload.size() > 0]" target="#[recordVars['exists']]">
                    <sfdc:query config-ref="Salesforce" doc:name="Find Lead" query="dsql:SELECT Id FROM Lead WHERE Email = '#[payload[&quot;Email&quot;]]'" fetchSize="100"/>
                </enricher>
----
...
image::mruntime-batch-example-6.png[Insert Lead view]

** The first item instructs the batch step not to process any record enriched with an `exists` record variable that equal to false.
...
image::mruntime-batch-example-8.png[Batch Commit]

** The first item shows the commit size that defines the number of records to commit in chunks, to Salesforce.

image::mruntime-batch-example-9.png[Insert Lead]

** The second item instructs Mule to create a new Lead using the payload of the record.
...
----
INFO 2019-10-31 16:30:00,856 [[import-leads-to-salesforce].connector.file.mule.default.receiver.01] org.mule.api.processor.LoggerMessageProcessor: 2 Loaded Records 1 Failed Records
----

== Limitations

* Batch processing does not support the use of xref:business-events.adoc[Business Events].
...
* Learn more about xref:batch-processing-reference.adoc[MEL expressions] you can use to access batch processing data.
* Read more about xref:runtime-manager::cloudhub-fabric.adoc[Runtime Manager support for batch processing].
* Learn more about xref:anypoint-connectors.adoc[Anypoint Connectors].
* Learn more about xref:poll-reference.adoc[Polling and Watermarks].
* Learn more about xref:dataweave.adoc[DataWeave].
