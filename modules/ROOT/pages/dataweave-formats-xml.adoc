...
| `indexedReader` | `Boolean` | `true` | Indicates whether to use the indexed XML reader when the threshold is reached. Valid values are `true` or `false`. It only supports the encodings US-ASCII, UTF-8 and ISO-8859-1. For other encodings the in-memory reader will be used.
