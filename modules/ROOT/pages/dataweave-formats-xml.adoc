...
|`streaming`  |`Boolean`|`false`|Streams input when set to `true`. Use only if entries are accessed sequentially. The input must be a top-level array. For more on streaming in DataWeave, see https://docs.mulesoft.com/mule-runtime/4.3/dataweave-formats#dw_readers_writers[DataWeave Readers].
Valid values are `true` or `false`.
