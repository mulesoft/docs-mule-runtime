...
This example iterates over an XML file that contains details of employees such as the `Id`, `Name` and `Address`, and you convert the file into JSON format. The DataWeave script uses the `replace` function to replace in the `Address` element the characters `-` and `/` with blank space.
...
The DataWeave script iterates over the payload elements, and the `replace` function `/([\-\,\/])/ with " "` replaces the `-` and `/` characters with blank space.
