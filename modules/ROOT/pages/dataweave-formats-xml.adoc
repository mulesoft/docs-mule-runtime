...
|`maxEntityCount`  |`Number`|`1`|The maximum number of entity expansions. The limit is in place to avoid Billion Laughs attacks, which are a denial-of-service attacks.
...
|`streaming`  |`Boolean`|`false`|Property for streaming input. Use only if entries are accessed sequentially. Valid values are `true` or `false`. The input must be a top-level array. For more on streaming in DataWeave, see https://docs.mulesoft.com/mule-runtime/4.3/dataweave-formats#dw_readers_writers[DataWeave Readers]. Valid Options: `true` or `false`.
...
|`doubleQuoteInDeclaration`  |`Boolean`|`false`|When set to `true`, escapes double quotation marks in XML declaration. Valid Options: `true` or `false`.
|`encoding`  |`String`|`null`|The character set to use for the output, such as UTF-8.
|`escapeCR`  |`Boolean`|`false`|When set to `true`, escapes `CR` characters. Valid Options: `true` or `false`.
|`escapeGT`  |`Boolean`|`false`|When set to `true`, escapes '>' characters. Valid Options: `true` or `false`.
|`indent`  |`Boolean`|`true`|Indicates whether to indent the JSON code for better readability or to compress the JSON into a single line. Valid values are `true` or `false`. Valid Options: `true` or `false`.
...
|`skipNullOn`  |`String`|`null`|Skips `null` values in the specified data structure. By default, it does not skip the values. Valid values are `arrays`, `objects`, or `everywhere`. Valid Options: `elements` or `attributes` or `everywhere`.
|`writeDeclaration`  |`Boolean`|`true`|Indicates whether to write the XML header declaration. Valid Options: `true` or `false`.
...
|`writeNilOnNull`  |`Boolean`|`false`|When set to `true`, writes the `nil` attribute when the value is `null`. Valid Options: `true` or `false`.
|===
