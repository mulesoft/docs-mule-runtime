...
|`externalEntities`  |`Boolean`|`false`|Indicates whether to process external entities. Disabled by default to avoid XML External Entity (XXE) attacks. Valid values are `true` or `false`.

 Valid values are `true` or `false`.
|`indexedReader`  |`Boolean`|`true`|If the indexed reader should be used when the threshold is reached.
...
|`streaming`  |`Boolean`|`false`|Property for streaming input. Use only if entries are accessed sequentially. Valid values are `true` or `false`. The input must be a top-level array. For more on streaming in DataWeave, see https://docs.mulesoft.com/mule-runtime/4.3/dataweave-formats#dw_readers_writers[DataWeave Readers].
...
|`deferred`  |`Boolean`|`false`|When set to `true`, DataWeave generates the output as a data stream, and the script's execution is deferred until it is consumed.
 ...
 Valid values are `true` or `false`.
|`doubleQuoteInDeclaration`  |`Boolean`|`false`|When set to `true`, escapes double quotation marks in XML declaration.

 Valid values are `true` or `false`.
|`encoding`  |`String`|`null`|The character set to use for the output, such as UTF-8.
|`escapeCR`  |`Boolean`|`false`|When set to `true`, escapes `CR` characters.

 Valid values are `true` or `false`.
|`escapeGT`  |`Boolean`|`false`|When set to `true`, escapes '>' characters.

 Valid values are `true` or `false`.
|`indent`  |`Boolean`|`true`|Indicates whether to indent the JSON code for better readability or to compress the JSON into a single line. Valid values are `true` or `false`.

 Valid values are `true` or `false`.
|`inlineCloseOn`  |`String`|`'empty'`|Indicates when the writer should use inline close tag or use open close tags.

 Valid values are `empty` or `none`.
|`onInvalidChar`  |`String`|`null`|

 Valid values are `base64` or `ignore` or `none`.
|`skipNullOn`  |`String`|`null`|Skips `null` values in the specified data structure. By default, it does not skip the values. Valid values are `arrays`, `objects`, or `everywhere`.

 Valid values are `elements` or `attributes` or `everywhere`.
|`writeDeclaration`  |`Boolean`|`true`|Indicates whether to write the XML header declaration.
...
|`writeNilOnNull`  |`Boolean`|`false`|When set to `true`, writes the `nil` attribute when the value is `null`.
