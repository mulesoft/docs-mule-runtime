...
The XML reader accepts parameters that provide instructions for reading input data.

//TODO: collectionPath (Default and Description)
//TODO: optimizeFor: What are the valid values for this one? And what are the tradeoffs? Is it like onlyData, for ex?

[cols="1,1,1,3a", options="header"]
|===
| Parameter | Type | Default | Description
|`collectionPath`  |`String`|  | Path to the document where the collection is located. It specifies a path expression that identifies the location of the elements to stream.
...
| `nullValueOn` | `String` | `blank` | Indicates whether to read an element with empty or blank text
as a `null` value. Valid values are `empty`, `none`, or `blank`. See <<example3>> and <<example4>>.
|`optimizeFor` |`String`| `'speed'` | Indicates how to configure the XML parser.
...
The XML writer accepts parameters that provide instructions for writing output data.
