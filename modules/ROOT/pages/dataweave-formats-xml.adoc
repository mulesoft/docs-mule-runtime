...
The example maps the value of the `title` element to a `String` and value of the `author` element to `null`, so the XML reader property `nullValueOn` is set to `empty`.
...
in the `application/dw` format and MIME type.
...
The DataWeave uses the `is CData` expression to determine whether the `String` value is CDATA.
...
This example uses the `inlineCloseOn` writer property with the value `none` acts on the key-value pairs from in the input.
