...
|`collectionPath`  |`String`|`null`|Sets the path to the location in the document where the collection is located.
|`externalEntities`  |`Boolean`|`false`|Indicates whether to process external entities. Disabled by default to avoid XML External Entity (XXE) attacks. Valid values are `true` or `false`. Valid Options: `true` or `false`.
...
|`optimizeFor`  |`String`|`'speed'`|Configures the XML parser. Valid Options: `speed` or `memory`.
