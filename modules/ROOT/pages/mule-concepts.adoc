...
When you configure the individual elements that link together to make your flow or batch job, you can use xref:3.7@mule-expression-language-mel.adoc[Mule Expression Language] to extract information about the message or its environment and instruct Mule to make processing decisions based on that information.

== See Also

* *NEXT STEP:* Learn more about the xref:3.7@elements-in-a-mule-flow.adoc[elements in a Mule flow].
* Find out more about xref:3.6@batch-processing.adoc[Batch Processing].