...
A lightweight, Mule-specific expression language that you can use to access and evaluate the data in the payload, properties and variables of a Mule message. See xref:mule-expression-language-mel.adoc[Mule Expression Language (MEL)], xref:mule-expression-language-examples.adoc[Mule Expression Language Examples], and xref:mule-expression-language-reference.adoc[Mule Expression Language Reference].
...
A building block in service orchestration that transforms message payloads (data) to and from different types. All of these transformations can also be carried out by xref:dataweave.adoc[DataWeave].
