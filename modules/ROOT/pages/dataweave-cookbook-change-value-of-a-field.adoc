...
* the `update` operator to update specified fields of the payload with new values.
* the single `.` and multi `.*` value selectors to navigate the payload and select the fields to update.

Given this script, DataWeave will create a new users list, updating each `user` in the payload, with a new `user` where the `ssn` and `password` values are now `\****`.
...
The `update` operator is the most general and easy way of changing the value of a field. However, there is also a simpler way of masking values as the example above required and doing so without focusing on the path to each sensitive field.

This example uses:

* the `mask` function to update all simple elements matching the selected name throughout the payload with the desired mask.
