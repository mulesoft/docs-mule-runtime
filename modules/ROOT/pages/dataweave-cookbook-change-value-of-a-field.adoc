...
The following DataWeave examples use `update` and `mask` to change the values of some XML elements that contain sensitive data, and converts them to a set of asterisks (`\****`).
...
The following script shows how the `update` operator works. The example creates a new `users` list by updating each `user` in the payload with a new `user`. After the transformation, the `ssn` and `password` values within each `user` become `\****`.
...
DataWeave provides a simple way to mask values, without specifying the path to each sensitive field:
