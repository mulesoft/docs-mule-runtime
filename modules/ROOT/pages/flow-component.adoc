...
| Max Concurrency (`maxConcurrency`) a| Optional. Sets the maximum number of concurrent messages that a flow can process. If not set, the container thread pool determines this value. While the flow is processing the maximum number of concurrent messages, it cannot receive additional requests.
