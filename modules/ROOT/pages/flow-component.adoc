...
subflows, see xref:about-flows.adoc[About Flows and Subflows].
...
Subflow components provide a way to edit the name of the subflow and to add metadata. Unlike flows, subflows lack a built-in mechanism for setting up error handling, though you can use error handling provided by the xref:try-scope-concept.adoc[Try scope] within a subflow if you need to perform error handling.
...
For details about these components, see xref:on-error-scope-concept.adoc[About On-Error Scopes].
...
image::about-flows-ef7ca.png[]
image:about-flows-98a17.png[]
////