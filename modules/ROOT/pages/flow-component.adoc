...
| Initial State (`initialState`) a| Values: Started, Stopped. These are Mule runtime settings for the flow. The default, Started (`initialState="started"`, also called Empty in Studio), indicates that the flow is active when you start the app. So it can be triggered by an internal or external event source within the flow (such as an HTTP listener or Scheduler), by a Flow Reference from another flow or subflow, or through a call to the xref:dataweave-runtime-functions.adoc#functions_runtime[lookup] function.
...
| Max Concurrency (`maxConcurrency`) a| Optional. The default is limited only by the availability of threads. The container thread pool determines the maximum level of parallelism that the flow can use to optimize the performance when processing messages. You can use Max Concurrency to prevent the flow from accepting new messages while the configured number of maximum request is being processed.

Set `maxConcurrency="1"` to force the flow to process requests sequentially, one at a time.

See xref:execution-engine.adoc#backpressure[Back-pressure] for details about how Mule handles reaching Max Concurrency limits.
