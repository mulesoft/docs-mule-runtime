...
+
. If needed, set the OAuth tab in Global Element Properties as described in xref:using-a-connector-to-access-an-oauth-api.adoc[Using a Connector to Access an OAuth API].
...
For information on setting the Pooling Profile tab, see xref:tuning-performance.adoc#about-pooling-profiles[About Pooling Profiles].

For information on setting the Reconnection tab, see xref:configuring-reconnection-strategies.adoc[Configuring Reconnection Strategies].
...
* xref:microsoft-dynamics-365-operations-connector.adoc[Microsoft Dynamics 365 Operations connector]
