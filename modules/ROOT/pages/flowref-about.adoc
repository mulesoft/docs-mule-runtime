...
image::flowref-about-8b5d1.png["A main flow and its referenced subflow with multiple steps"]
...
+
image:mruntime-flow-ref-create.png["A flow showing main flow and secondary flow with variable and payload settings"]
. Open the Flow Reference properties and specify the flow you want to reference in *Flow name*:
+
image:mruntime-flow-ref-config.png["A configuration panel for a secondary flow showing settings for flow name and target"]
