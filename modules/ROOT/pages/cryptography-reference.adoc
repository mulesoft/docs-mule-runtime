...
You can enable the *Use random IVs* field to use random initialization vectors (IVs). If you enable this field, the decryption algorithm assumes IVs are prepended to the ciphertext during decryption operation. To configure this field in Anypoint Studio, follow these steps:
...
In the *XML editor* window, the `useRandomIVs="true"` field, looks like this:
