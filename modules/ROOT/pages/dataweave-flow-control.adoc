...
An `if else` statement can act on any valid DataWeave input and return the data in any valid output format. For example, the following script transforms the input to XML and uses `if else` statements with the result of the following conditions:

* A mathematical operation in `ex1` +
The `if else` statement returns the Boolean value `true` or `false`.
...
The `if else` statement returns the string `"ID is empty"` or `"ID is not empty"`depending on whether the `aRecord.bookId` contains a value.
* A mapping in `ex4` that iterates over `firstInput` + 
The `if else` statement returns the `bookId` as a Number if the value is equal to `101` or it returns the specified string if the value is not equal to `101`.
...
The following example is similar but takes an array as input instead of an object. The body of the script uses `if else` statements within a `do` operation to populate the value of the `hello` variable. 
...
["Argentina", "USA", "Brazil"] map (country) -> do {
  var hello = if(country == "Argentina") "Hola"
   ...
   else "Don't speek $(country) language."
   ---
   "$(hello) DataWeave"
}
----
