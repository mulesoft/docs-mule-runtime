...
Any other router or scope continues with the transaction and its behavior is not modified.
...
Consider an example in which the message source (`jms:listener`) starts a transaction, which is continued by the Try scope (`transactionalAction` is set to default, `INDIFFERENT`).
