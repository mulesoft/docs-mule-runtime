...
* On Error Continue: the error is threated as managed and the Transaction remains active (and is able to commit). The inner processors of the On Error Continue run within the Transaction.
...
If the `flow` had an `on-error-propagate` instead of `on-error-continue`, then the transactional is would have been rolled back, and the logger within that `on-error-propagate` will not run within the Transaction. 
...
Since the `someFlowContinuesTx` Flow is not the one that started the transaction, then its Error Handler will not rollback it. 
