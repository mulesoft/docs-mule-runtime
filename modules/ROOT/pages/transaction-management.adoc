...
While xref:xa-transactions.adoc[XA Transactions] offer similar functionality, xref:multiple-resource-transactions.adoc[Multiple Resource transactions] use less overhead.  XA transactions are more reliable. Both can include JDBC resources.
====

== Assumption

This document assumes that you are familiar with Mule and/or the xref:6.x@studio::index.adoc[Anypoint Studio interface]. Further, this document assumes you have some knowledge of transaction processing and are familiar with xref:anypoint-connectors.adoc[Anypoint Connectors].
...
|XA Transaction |Apply a transaction to a flow that involves multiple resources: JMS, VM or JDBC. See xref:xa-transactions.adoc[XA Transaction] for details.
