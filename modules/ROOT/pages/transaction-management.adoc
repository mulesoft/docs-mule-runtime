...
This document assumes that you are familiar with Mule and/or the xref:6.x@studio::index.adoc[Anypoint Studio interface]. You should also have some knowledge of transaction processing and be familiar with xref:anypoint-connectors.adoc[Anypoint Connectors].
...
* Apply a transaction as a wrapper (known as a scope in Studio) to apply a transaction to elements within a flow that _do not_ begin with an inbound connector configured as a transaction.
...
|BEGIN_OR_JOIN |JMS VM |When it receives a message, Mule joins a transaction if one is already in progress. Otherwise, Mule begins a new transaction.
...
|BEGIN_OR_JOIN |When it receives a message, Mule joins a transaction if one is already in progress. Otherwise, Mule begins a new transaction.
|===
+
