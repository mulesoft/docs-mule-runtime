...
, which is the default receiver on inbound JMS endpoints that use XA transactions. If you are using JBoss transactions, please read xref:jboss-transaction-manager-reference.adoc[here] for information on how to configure the `timeout` value.
...
|JRun
|no |yes |java:/TransactionManager |org.mule.transaction.lookup.JRunTransactionManagerLookupFactory
