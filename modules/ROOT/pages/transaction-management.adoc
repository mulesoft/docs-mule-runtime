...
| `ALWAYS_BEGIN`
| Always start a new transaction when receiving a message. If a Single Resource transaction exists, an error is raised. If an XA transaction exists, a nested transaction is created. 
|
...
Add the `transactionalAction` element and the `transactionType` element (if necessary), and set their values.
+
In the following XML example the Try scope demarcates a transaction that includes a Database operation (`db:insert`) and a VM operation (`vm:publish`). Either both will be applied if the entire process succeeds, or none will be applied if an error is raised within the Try scope:
+
[source,xml,linenums]
----
...
** If the `on-error-propagate` is inside an element that did not start the transaction:
+
The transaction is not rolled back and the processors inside the `on-error-propagate` run within the transaction. Remember that some scopes and routers behave differently when executing within a transaction.

* On Error Continue
+
The error is considered managed, the transaction remains active and is able to commit. The processors inside the `on-error-continue` run within the transaction.
...
The `jms:publish` inside the `on-error-propagate` joins the transaction. The error is then propagated to the flow's `error-handler`, which handles it with an `on-error-continue`. The `jms:consume` inside the `on-error-continue` runs within the transaction. Finally, the transaction is committed.
...
If the `on-error-continue` is replaced with an `on-error-propagate` in the previous example, then the transaction is rolled back before the execution of the processors. In this scenario, the configuration is not correct because the `jms:consume` cannot join any transaction.

=== Example: Error Raised in a Referenced Flow
...
In `flow2` the `on-error-propagate` rolls back the transaction, then the processors inside the `on-error-propagate` are executed and a new transaction is created. This configuration is correct.
