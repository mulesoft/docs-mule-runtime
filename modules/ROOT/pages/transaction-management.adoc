...
If we replace the `on-error-continue` with an `on-error-propagate` in the example above, then the transaction is rolled back, and the logger inside this `on-error-propagate` does not run within the transaction.
