...
A transport is responsible for carrying messages from application to application in the Mule Runtime framework. In EIP terms, transports are used to implement message channels and provide connectivity to an underlying data source or message channel in a consistent way. For example, the xref:tcp-transport-reference.adoc[TCP transport] handles messages sent via the TCP protocol, whereas the xref:file-transport-reference.adoc[File transport] reads and writes files placed on the server's file system.
