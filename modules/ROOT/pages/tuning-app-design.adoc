...
The Mule runtime maintains a flow call stack for each event being processed. Each `flow-ref` invocation adds an entry to this stack. When the stack depth limit is exceeded, the runtime raises a `CRITICAL` error that cannot be handled by error handlers.

Additionally, extremely long flows with many processors generate deeply nested internal execution chains. When these flows are combined with recursive patterns or very deep call chains, the accumulated nesting can exhaust the JVM thread stack and produce a `StackOverflowError`, a `VirtualMachineError` that leaves the runtime in an inconsistent state (see xref:mule-error-concept.adoc#fatal_jvm_errors[What Happens After a FATAL_JVM_ERROR]).
