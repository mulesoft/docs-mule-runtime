...
| Use a source-based polling trigger (for example, Scheduler with watermark).

| Paginated API calls
| Use connector-native pagination or a `<foreach>` over page numbers.

| Complex iterative logic that cannot be expressed with built-in scopes
...
Beyond avoiding recursion, keep individual flows at a reasonable length. Flows with hundreds of processors generate very deep internal execution chains. If a flow grows too large, split it into separate smaller flows invoked via `flow-ref`. Unlike subflows (which are inlined at execution time and do not reduce internal nesting), separate flows introduce independent execution chains and reset the internal nesting depth. This not only helps prevent stack overflow conditions but also improves readability and maintainability.
