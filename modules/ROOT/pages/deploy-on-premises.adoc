...
There are also additional operations that enable you to deploy applications in parallel and update applications at runtime from the command line.
...
+
```
mule start -M-Dmule.deployment.parallel
```

After performing these steps, Mule starts and deploys in parallel all applications in the `/apps` directory.
