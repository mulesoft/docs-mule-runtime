...
| `muleVersion` | The release version of Mule runtime engine running in your local machine instance.  +
...
The Mule Maven Plugin does not download a new Mule version if these values don't match.
...
include::partial$mmp-concept.adoc[tag=applicationNameParameterDescription]
| `muleVersion` | The release version of Mule runtime engine required for your application to run in your deployment target. +
Example values: `4.2.0`, `4.2.2-hf4`. +
If this value does not match the Mule version running in your deployment target, the plugin raises an exception. +
The Mule Maven Plugin does not download a new Mule version if these values don't match.
| `muleHome` | The location of the Mule instance in your local machine.
