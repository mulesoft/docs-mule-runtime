...
|`enforceRequires`  |`Boolean`|`false`|Produces an error if required value is missing. Valid values are `true` or `false`. Valid Options: `true` or `false`.
...
|`notTruncateDependingOnSubjectNotPresent`  |`Boolean`|`false`|Fill the whole group when the Depending On Subject is not present Valid Options: `true` or `false`.
