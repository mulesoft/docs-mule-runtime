= Getting Started with Mule Apps
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

To demonstrate a simple HTTP request-response flow, the following example
responds with the message "Hello World" to requests submitted from a Web browser.
The example demonstrates how a Mule application interacts with a user over an
HTTP request. You set up and run this application as a project in Anypoint Studio.

In a more complex application, it is possible to connect to external servers
(such as a Salesforce, SAP, for example) or import API specifications directly
into a Mule project. This example does not explore such tasks.
...
in the Studio overview, xref:docs-studio::index.adoc[Anypoint Studio].

== How the Example Works

The Hello World example consists of one simple Mule flow. This flow accepts an
...
. Create a project in Studio named "HelloWorld" by selecting
...
operation into the the *Source* section of the *helloworldFlow*.
...
+
. In the listener's *Path* field, type the path `/helloworld`.
+
...
. In the component's *Value* field, type the string `Hello World!`.
...
== Run and Execute the Hello World Example

After setting up Hello World in Studio, you can run and test it.

. Run your *helloworld* app by right-clicking within the Studio canvas and
selecting *Run project helloworld*.
+
...
the URL to the listener, for example, +`http://0.0.0.0:8081/helloworld`+.
+
You must use the same host and port that is configured in the HTTP listener.
+
. Check for the following message in the browser: `Hello World!`
. In the Studio, notice that the message in the console includes the
request path (`/helloworld`) in the first `INFO` line, which ends
`org.mule.runtime.core.internal.processor.LoggerMessageProcessor: /helloworld`.


== Configuration XML for the Hello World Example
