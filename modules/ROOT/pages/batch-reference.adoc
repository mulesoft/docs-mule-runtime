...
Mule batch components are designed to manage bulk processing of records with minimal adjustments to their default settings. However, the components are highly configurable to meet specific processing and performance requirements.
...
////
TODO:
It appears that processors within the batch step and aggregator cannot access attributes of the input to the batch job.
////
...
The Batch Job component provides a number of configurable properties, including several that determine how processing takes place.

Anypoint Studio provides the following fields properties in its *General* tab:
...
* Steaming limits access to the items in the output. A streaming commit is a one-read, forward-only iterator, which means that only one processor within the component can read and process the input to it. However, you can use a component like For Each with multiple processors within it because For Each maintains its own list of records.
//TODO: YOU can use For Each either way, streaming or not, right? Not clear why
// we're saying this.
