...
This reference assumes that you are familiar with the xref:batch-processing-concept.adoc#batch-job-processing-phases[batch processing phases] of a batch job instance.

[[ref_batch_job_component]]
== Batch Job Component (<batch:job />)

This component prepares its input for processing and outputs a report with results of that processing. Preparation takes place during the xref:batch-processing-concept.adoc#load-and-dispatch[Load and Dispatch] phase of a batch job instance, and the report is available in the xref:batch-processing-concept.adoc#on-complete[On Complete] phase.
...
The Batch Step component runs during the xref:batch-processing-concept.adoc#process[Process phase] of a batch job instance, not during the Load and Dispatch or On Complete phases.
...
| Accept Expression | `acceptExpression` | Optional DataWeave expression for a filter that determines whether to process a record in the component. If _both_ the `acceptExpression` _and_ the `acceptPolicy` values for a record evaluates to `true`, then the component accepts the record for processing. If not, the component skips the record, and the record becomes available to any downstream Batch Step components. Examples:
...
The Batch Aggregator component is an optional component that acts on an array of records. Aggregation takes place during the xref:batch-processing-concept.adoc#process[Process phase] of a batch job instance.
