...
[NOTE]
====
When you deploy an application to an on-premises Mule cluster, deployment artifacts are staged to cluster nodes sequentially. Sequential staging doesn't guarantee that the application starts on one node before deployment begins on another. Startup and readiness across cluster nodes aren't ordered and can overlap.

Build Mule applications so that each node can start independently in a clustered environment.
====
