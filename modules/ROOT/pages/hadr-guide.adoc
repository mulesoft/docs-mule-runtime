...
The following table shows how even small amounts of downtime add up and hurt an organization:
...
An effective SLA does the following:
...
|You install, configure, and fully run the Mule environment, but it does not process requests until you detect an outage.
...
Sequential staging does not guarantee that the application starts on one node before startup begins on another. Application startup and readiness across cluster nodes follow no fixed order and can overlap.
====
...
Fault tolerance means a failure within the system does not affect the service at all. It differs from high availability, which tolerates some service impact and downtime.
