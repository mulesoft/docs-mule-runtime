...
functioning, that is, properly processing requests and responses. This doesn't
...
Vary depending on SLA requirements. This model needs 2 nodes to satisfy SLA. If the SLA's acceptable service impact changes to the terms stated in the normal operations then at a minimum the environment needs 3 nodes to accommodate 1 node failure. More nodes may be required depending on the probability of not having at least 2 nodes running.

=== Active-Active Clustering Fault Tolerance Deployment Model

The basic definition of fault tolerance is a failure within the system doesn't impact the service at all. This differs from high availability as service impact and downtime is tolerated.
...
This example is assuming each environment is using the same number of Mule runtimes and cores. It's plausible that the new environment may have more or less number than the baseline environment.
