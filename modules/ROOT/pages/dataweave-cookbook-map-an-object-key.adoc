...
Then the DataWeave function `mapObject` iterates every sheet in the root, and if the `extractNumber` equals to the `append.Id` value, then adds the `append` object to the matched sheet object.
