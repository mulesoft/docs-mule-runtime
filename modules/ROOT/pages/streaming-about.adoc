...
[[non_repeatable_iterable]]
=== Non-Repeatable Iterable

This non-repeatable strategy disables repeatable streams, which enables you to write a stream of output objects only once. Use this strategy when your use case does not require the extra memory or performance overhead that comes with repeatable streaming. Because the stream is not saved to memory, the non-repeatable strategy performs better than the other strategies.
