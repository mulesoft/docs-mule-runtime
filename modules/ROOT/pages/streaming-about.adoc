...
image::mruntime-streaming-about-1.png["A flow showing HTTP and file operations with error handling"]
+
...
image::mruntime-streaming-about-2.png["A flow showing components including HTTP, Logger, and error handling in a streaming application"]
...
image::mruntime-streaming-about-3.png["A flow showing asynchronous file processing with scatter-gather and error handling"]
...
When a connector operation returns a stream (such as reading a file, querying a database, or receiving an HTTP response body), the stream holds underlying resources open (file handles, database cursors, network connections) until it is fully consumed or the event that originated the stream completes processing.

=== Why Prompt Consumption Matters

* *Resource exhaustion*: Unconsumed streams keep resources (connections, file descriptors, cursors) allocated. If many streams accumulate without being consumed, the application can run out of available resources, eventually leading to `OutOfMemoryError` or connection pool exhaustion.
...
* *Connection starvation*: For HTTP and database operations, the underlying connection cannot be returned to the pool until the response stream is consumed or the event finishes.
...
| Use a DataWeave transformation, `<logger>` with payload access, or write to a target to ensure the stream is read to completion.
...
| When using `target` to store operation results, ensure the stored value is eventually consumed or that `targetValue` forces consumption at store time (for example, by applying a DataWeave expression that reads the stream).

| Prefer non-repeatable streaming for single-read scenarios
| If you only need to read the stream once, use `<non-repeatable-stream/>` to avoid buffering overhead and release resources immediately after consumption. See <<disable_repeatable_streaming>>.
|===
...
For information on what happens when unconsumed streams lead to memory exhaustion, see xref:mule-error-concept.adoc#fatal_jvm_errors[What Happens After a FATAL_JVM_ERROR].
