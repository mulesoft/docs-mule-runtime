...
* Disabling repeatable streaming means that the stream is only read once. Keep in mind that your flow might contain components that require consumption of the full stream even if you do not explicitly specify that requirement. Examples include the Cache component (`<ee:cache>`), some transformations through the Transform Message component (`<ee:transform>`, the For Each component (`<foreach>`) when iterating through a JSON array, and any other component that is using an expression that accesses the stream. 
