...
When a connector operation returns a stream (for example, when reading a file, querying a database, or receiving an HTTP response body), the stream keeps underlying resources open (file handles, database cursors, and network connections) until it is fully consumed or until processing of the originating event completes.
...
* Connection starvation: For HTTP and database operations, the underlying connection cannot return to the pool until the response stream is consumed or the event finishes.
...
| When using `target` to store operation results, ensure the stored value is eventually consumed, or use `targetValue` to force consumption at store time (for example, with a DataWeave expression that reads the stream).
