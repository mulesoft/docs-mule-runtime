...
|`%dw 1.0`    |`%dw 2.0`|
|`%input`     |`input`  | In a Mule app flow, DataWeave 2.0 recognizes the input type based on the `output` directive specified in the preceding component. The input type is not defined using the `input` directive in the header anymore.
However, the `input` directive is still used in specific cases like xref:dataweave-streaming.adoc#validate-a-script-for-streamed-data-experimental-feature[Validate a Script for Streamed Data].
|`%output`    |`output` |
|`%var`       | `var`   |
|`%function`  | `fun`   |
|`%namespace` | `ns`    |
