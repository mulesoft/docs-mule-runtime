...
|File-based VM (standalone Mule) |Provided out of the box with Mule. There is no need to purchase additional software. |Applies only to non-clustered environments.
|JDBC |Depending on resources available, JDBC can hold a very large amount of information. |Requires additional software (database server). It can be complex to use, since proper DBMS schemas are required.
