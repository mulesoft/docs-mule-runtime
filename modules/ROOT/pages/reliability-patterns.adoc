...
|
* Only applies to non-clustered environments.
|JDBC
|
...
|
* It is the most reliable choice because messages are persisted to disk. It applies to clustered environments.
|
* Requires additional software (message queue server).
|===

[NOTE]
In memory persistence setups for both VM and JMS are excluded from the comparison since they can lose messages; therefore, they are not considered reliable.
