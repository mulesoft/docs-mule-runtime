...
The application logic flow (the right-hand side of the diagram) delivers the message from the Message Source that uses a transactional connector to the business logic for the application.
...
* Provided ready to use with Mule. There is no need to purchase additional software.
|
...
Here is a code example that shows what a complete reliability pattern would look like, including the reliable acquisition flow and the application logic flow. In this example, the reliable acquisition flow part of the reliability pattern is HTTP-to-VM. Note that this example shows VM file persistency, which does not work on clusters.
...
<2> The message is read from the VM queue transactionally. This ensures that if an error occurs, the reading will be rolled back and the message reprocessed.


== Implementing a Reliable Acquisition Flow

The example provided in Implementing a Reliability Pattern showed a reliable acquisition flow for an HTTP listener to a VM publish-operation. Now, let's focus on a reliable acquisition flow that has a non-transactional Message Source: FTP-to-VM (Or SFTP, FTPS).

In this scenario, we are working with a resource-based connection. When using resource-based connections, you need to ensure to read the resource only once. This is done by setting `watermarkEnabled` to `true`.

You must not set the `auto-delete` parameter to `true` because doing so will delete the file as soon as the flow finishes, even if the processing is done by another flow. The same applies to `moveToDirectory`.
...
<2> If the file is not intended to be deleted, do not use the Delete Operation. The same applies if the intended operation is Move.

A similar implementation is done for any other of the resourced-based connectors.

== General Considerations

Here are a number of points to consider when implementing the reliability pattern:
