...
* If the outbound operation in the reliable acquisition flow is not transactional (for example, a flow from file-to-FTP), the only way to ensure message delivery ###this doesn't ensure delivery. It makes it more likely, but at best can ensure completion-errors are logged. Important not to give relatively inexperienced users (who will take this at face value) false security### is to use a xref:try-scope-concept.adoc[Try Scope].
