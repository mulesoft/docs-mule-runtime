...
|File-based VM (standalone Mule) |Provided ready to use with Mule. There is no need for additional software. |Applies only to non-clustered environments.
|JDBC |Depending on resources available, JDBC can hold a large amount of information. |Requires additional software (database server). It can be complex to use since proper DBMS schemas are required.
|JMS (enabled for persistence) |Applies to both clustered and non-clustered environments. |Requires additional software (message queue server).
|===
...
❶ The flow specifies a synchronous processing strategy (`processingStrategy="synchronous"`). In a synchronous processing strategy, the entire flow is processed in the receiver thread. This ensures that the transfer to the VM endpoint happens in the same thread. See xref:3.7@flow-processing-strategies.adoc[Flow Processing Strategies] for further details about the synchronous processing strategy. +
