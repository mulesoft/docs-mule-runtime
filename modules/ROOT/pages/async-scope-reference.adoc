...
The following XML snippet shows an example configuration of an Async scope inside an application flow. The Async scope contains a `file:read` operation that executes asynchronously once triggered, while the flow continues processing the next operation in the flow which is `http:request`:
...
<flow name="myMainFlow" >
  ...
  <ee:transform doc:name="Transform Message" >
    ...
    <async doc:name="Async" >
      ...
      <try doc:name="Try" >
        <!-- After this Write operation finishes executing, the next processors in the Async scope starts its execution -->
  ...
  <!-- This HTTP Request operation starts executing right after the Async scope starts its execution -->
