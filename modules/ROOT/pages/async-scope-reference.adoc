...
The Async scope provides a mechanism for processing the Mule event simultaneously with the main flow. Processing within the scope does not block the execution of downstream components outside of Async. The flow does not wait for the last processor within the asynchronous flow to completes its tasks.
