...
This document assumes that you are familiar with the xref:6.x@studio::index.adoc[Visual Editor in Anypoint Studio] that you can use to create applications in Studio. Further, this document references xref:global-elements.adoc[Global Elements] and the category of message processors known as xref:scopes.adoc[scopes].  Review xref:6.x@studio::index.adoc[Anypoint Studio Essentials] to learn more about developing with Mule's graphical user interface. Also review xref:exchange::index.adoc[Anypoint Exchange] for how to locate and install a connector using Exchange.
...
. Run your project in Debug Mode. (Refer to the xref:6.x@studio::studio-visual-debugger.adoc[Visual Debugger] document for greater detail.)
