...
. Add the Twitter connector as child element of the poll element.
+
[%header%autowidth.spread]
|===
...
|*Default Expression* |`default-expression` ^|X |- |If Mule cannot locate the object store key if uses the default expression to generate a value. This is useful for the first run of the flow.
