...
The XQuery Module gives users the ability to perform XQuery transformations on XML messages in Mule. This works in a very similar way to the xref:3.6@xslt-transformer.adoc[XSLT Transformer] shipped with Mule.
...
If you convert to Mule 3.6 and later, more information on XML can be found in xref:3.8@xpath.adoc[XPath3] guide.

For examples on using the XQuery transformer, including the use of features and improvements in version 3.0, see xref:3.7@xquery-support.adoc[XQuery Support].
...
These properties are pulled from the current message and made available in the XQuery context so that they can be referenced in your XQuery statements. These can be object references or you can use xref:3.7@mule-expression-language-mel.adoc[Mule Expressions] to get information from the current message.
