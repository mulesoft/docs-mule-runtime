...
The previous example uses a variable to show how to pass parameters into the script. MuleSoft recommends that as a best practice to keep the script decoupled from the rest of the flow, so that changes to the Mule applications don't require changes to the script.
