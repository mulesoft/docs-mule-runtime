...
The First Successful router iterates through a list of configured processing routes until one of them executes successfully. If any processing route fails its execution (throws an exception), the router executes the next configured route.

If none of the configured routes execute successfully, the First Successful router throws an exception.

== Firs Successful Router Configuration Example
...
The behavior after the execution is as follows:
