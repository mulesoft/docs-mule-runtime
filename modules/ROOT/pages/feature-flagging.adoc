...
[TIP]
Set `minMuleVersion` to the minor version whose features your application needs, with patch `.0`, for example `4.9.0`. Do not set it to the exact patch of the runtime you built or scaffolded with. Declaring a higher version than your application needs can prevent it from being deployed.
...
The most visible case is deployment. The runtime refuses to deploy an application whose `minMuleVersion` is newer than the running runtime, and reports that the application requires a newer runtime version. For example, an application with `minMuleVersion=4.12.2` does not deploy on a `4.12.1` runtime, even though the two only differ in patch. To check the version of the target runtime: in CloudHub or Runtime Fabric, check the environment or instance details in Runtime Manager; on-premises, run `./mule -version` or check the startup log.

The same comparison also happens outside the deployment cycle. Tooling that reads the artifact without deploying it also enforces it, for example when the platform inspects an application's schedulers, or when Anypoint Exchange extracts metadata from a published asset. An over-declared `minMuleVersion` can cause an artifact to be rejected by that tooling even when you aren't deploying it at all.

This comparison is patch-sensitive by design, so that the backported bug fixes described below work reliably.

=== Adopting a backported bug fix (advanced)

The *patch* component of `minMuleVersion` (the third number, for example the `2` in `4.12.2`) only matters for the corner case where a bug fix was backported into a patch release instead of shipping with the next minor version, typically as the result of an escalated product request. The <<Feature Flags Reference>> table below documents these cases: most flags become available at a minor release, but a few become available at a patch release, for example `mule.tx.error.when.timeout`, available since Mule 4.6.1.
