...
|When enabled, all flows are stopped in parallel rather than sequentially when a Mule application stops. This prevents delays caused by in-flight events in one flow from delaying the shutdown of other flows.
