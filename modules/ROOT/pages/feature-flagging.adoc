...
`minMuleVersion` isn't only a feature-flag selector. The runtime, and other tools that work with a Mule artifact, compare its `minMuleVersion` against a target version using the full `major.minor.patch`. If the artifact's `minMuleVersion` is newer than the target, it's rejected.
...
This comparison is patch-sensitive by design. That's what makes the backported bug fixes described in <<Adopting a Backported Bug Fix (Advanced)>> possible: raising the patch of `minMuleVersion` is how an application opts into one.
