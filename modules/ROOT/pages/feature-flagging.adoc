...
|When enabled, the default error handler added by the Runtime doesn't rollback a transaction if it doesn't correspond.
...
|When enabled, the Runtime trims whitespaces from CDTA text parameter of pojos in the DSL.
...
* MULE-20048
<.^|`mule.https.forceWorkerThreadIoStrategy`
|When enabled, `optional` attribute entries in a `registry-bootstrap.properties` are ignored.
...
|When enabled, DataWeave retains a mixed-content structure instead of grouping text with mixed content into a single text field.
...
|When enabled, the Objects factories are created with Byte Buddy instead of CGLIB.
...
|When enabled, expression validations are enforced on all DataWeave expressions.
...
|When enable, if the items to iterate over on a parallel-foreach scope are messages (such as the output of an operation that returns Result objects), they are flattened in a way that is consistent with what the foreach scope does.
