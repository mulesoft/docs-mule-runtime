...
* W-20038933
<.^|`mule.forcefulShutdown.dynamicConfig.disableStoppingCheck`
|When enabled, dynamic configurations are created during forceful shutdown. This can be useful in some scenarios, but it isn't recommended because it can disrupt the normal shutdown process by establishing new connections after shutdown is requested.
...
|When enabled, all flows in a Mule application stop in parallel during application shutdown instead of stopping sequentially. This prevents delays in stopping one flow, such as delays caused by inflight events, from delaying the shutdown of other flows.
...
* W-
<.^|`mule.enableLogEventDumpOnShutdown`
|When enabled, Mule logs an event dump when an application is shut down to help troubleshoot graceful shutdown.
...
* W-
<.^|`mule.componentBuildingDefinitionDisambiguationMode`
|When enabled, controls component-building definition disambiguation mode. Supported values are `enabled`, `disabled`, and `strict`. The `strict` value prevents duplicates and is intended only for development or debugging environments.
