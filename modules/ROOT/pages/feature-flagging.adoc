...
[TIP]
If your deployment was rejected because of `minMuleVersion`: lower it to at or below the target runtime's version, keeping the patch at `.0` unless you specifically need a backported fix (see <<Adopting a backported bug fix (advanced)>>). Do not set it to the exact patch of the runtime you built or scaffolded with.

`minMuleVersion` declares the minimum runtime *feature set* that a Mule application requires. Conceptually, this corresponds to the *MINOR* number in https://semver.org/[Semantic Versioning]. Mule 4 follows Semantic Versioning 2.0.0; see xref:about-classloading-isolation.adoc[Class-loading Isolation] for more on this. Anypoint Studio and Anypoint Code Builder use this value when scaffolding a new application.

=== How the runtime and tools check minMuleVersion

`minMuleVersion` is not only a feature-flag selector. The runtime, and other tools that work with a Mule artifact, compare its `minMuleVersion` against a target version using the full `major.minor.patch`. If the artifact's `minMuleVersion` is newer than the target, it is rejected.

The most visible case is deployment. The runtime refuses to deploy an application whose `minMuleVersion` is newer than the running runtime, and reports that the application requires a newer runtime version. For example, an application with `minMuleVersion=4.12.2` does not deploy on a `4.12.1` runtime, even though the two only differ in patch. To check the version of the target runtime: in CloudHub, CloudHub 2.0, Runtime Fabric, or a Runtime Manager-managed on-premises server, check the environment or instance details in Runtime Manager; on a standalone on-premises server, run `./mule -version` or check the startup log.

The same comparison also happens outside the deployment cycle. Tooling that reads the artifact without deploying it enforces it too. For example, a scheduler that introspects an application's flows to list its triggers, or Anypoint Exchange extracting metadata when you publish an asset, both compare the artifact's `minMuleVersion` the same way deployment does. An over-declared `minMuleVersion` can cause an artifact to be rejected by that tooling even when you aren't deploying it at all.

This comparison is patch-sensitive by design. That is what makes the backported bug fixes in the next section possible: raising the patch of `minMuleVersion` is how an application opts into one.

=== Choosing the value

Set `minMuleVersion` to the lowest Mule version whose feature set your application actually needs, not the latest patch of whatever runtime you happened to build or scaffold with. Declaring a higher version than necessary is called over-declaring, and, as explained above, it can prevent an otherwise-compatible runtime or tool from accepting the application.

Lowering `minMuleVersion` has a trade-off too: it disables any feature or bug fix that is enabled by default between the new value and the version the application previously declared. Pick the lowest version whose behavior your application actually relies on, not an arbitrarily old one.

=== Adopting a backported bug fix (advanced)

The *patch* component of `minMuleVersion` (the third number, for example the `2` in `4.12.2`) only matters for one corner case: a bug fix backported into a patch release instead of shipping with the next minor version. This typically happens as the result of an escalated product request. The <<Feature Flags Reference>> table below documents these cases. Most flags become available at a minor release, but a few become available at a patch release, for example `mule.tx.error.when.timeout`, available since Mule 4.6.1.

Like every entry in that table, a backported bug fix is *disabled by default*, so that an application relying on the preexisting behavior isn't affected by upgrading the runtime. Enabling it is a deliberate opt-in.

On an on-premises Mule instance, the recommended way to opt in to a backported fix (or any flag) is to configure it explicitly through a system property, as described in <<Configure Feature Flags for the Mule Instance>>, rather than by bumping the patch component of `minMuleVersion`. Raising the patch changes every flag available since that patch at once, including ones you may not have intended to adopt, and it also tightens the compatibility check described above.

Only set a patch-level `minMuleVersion` when all of the following are true: the application depends on a specific backported fix, the deployment target doesn't support a system-property override, and MuleSoft Support has directed you to do so. Otherwise, leave the patch at `.0`.

For the full list of fields in `mule-artifact.json`, including `minMuleVersion`, see xref:mule-artifact-json.adoc[Application Descriptor Reference (mule-artifact.json)].
...
[NOTE]
This example intentionally uses an older version, `4.1.0`, to illustrate selecting an earlier feature set. It is not a recommended value for a new application; see <<Understanding minMuleVersion>>.
