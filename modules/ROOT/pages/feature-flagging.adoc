...
|When set to true, the encrypted payload is encoded in hexadecimal format when no Expression ID is specified.
...
|When set to true, events cancelled during a forceful shutdown raise a `MULE:CANCELLED` error instead of `MULE:UNKNOWN`. Dynamic configuration creation attempts during shutdown also raise a `MULE:CANCELLED` error instead of `MULE:UNKNOWN`.
...
|When set to true, events aren't retried if a `MULE:CRITICAL` error occurs within an until-successful scope.
...
|When set to true, batch processing uses batch errors instead of step exceptions. The DataWeave exception-based bindings `Batch::getFirstException()`, `Batch::getLastException()`, `Batch::failureExceptionForStep(String)`, and `Batch::getStepExceptions()` are replaced by their error-based equivalents `Batch::getFirstError()`, `Batch::getLastError()`, `Batch::failureErrorForStep(String)`, and `Batch::getStepErrors()`.
...
|When set to true, the `Authorization`, `Proxy-Authorization`, and `Expect` headers are removed from redirected requests to a different host. This applies only when the HTTP Service implementation is Netty.
...
|When set to true, policies use the object store implementation configured for the application. This enables alternative implementations, such as Object Store v2, to be used by policies.
...
|When enabled, ee:cache scope will persist only the Message instead of the full Event. Java Customizations of ee:cache will be disallowed: custom implementations of cachingStrategy-ref, keyGenerator, responseGenerator or eventCopyStrategy.
