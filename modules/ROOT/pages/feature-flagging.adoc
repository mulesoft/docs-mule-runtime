...
|When set to true, cancelled events raise a `MULE:CANCELLED` error instead of `MULE:UNKNOWN` when a forceful shutdown occurs after the graceful shutdown period expires.
...
|When set to true, a policy will be configured with the object store implementation defined for the application that it is being applied to, allowing alternative object store implementations such as Object Store v2 to be also used for policies.
