...
By default, features released for later Mule runtime versions are disabled for Mule applications that require an earlier Mule version. However, it is possible to change this behavior manually for each Mule application or for all apps running in the same Mule runtime instance.
