...
|`anypoint.platform.platform_base_uri` | Represents the URL to which the agent should report its status, for example `+https://qa.anypoint.mulesoft.com/apiplatform+` |
|`anypoint.platform.coreservice_base_uri` | Represents the URL to which the agent should report its status, for example `+https://qa.anypoint.mulesoft.com/accounts+` |
|`anypoint.platform.analytics_base_uri` | Represents the url where the gateway should post analytics status and metrics, for example `+https://qa.analytics-ingest.anypoint.mulesoft.com+` |
|`anypoint.platform.proxy_host` | The host of your actual proxy, for example `+http://myorg.proxy.net+` |
|`anypoint.platform.proxy_port` | The port of the proxy that listens for incoming requests, for example `8080` |
...
|`anypoint.platform.poll_policies_freq` | Interval between each poll for new policies from API platform. Time is expressed in seconds. | 5 seconds
...
|`anypoint.platform.policycache` | Enable policy caching to be used by gatekeeper `flexible` state. Also enables S3 backup in CH | true
...
|`anypoint.platform.discover_ch_endpoint` | This flags enables or disables verifying if the runtime is running on CloudHub.| true
