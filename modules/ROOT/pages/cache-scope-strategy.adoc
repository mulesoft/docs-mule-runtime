...
+
image:mruntime-caching-strategy.png["A Caching Strategy setup with options for object store and event key generation"]
+
...
+
. Select a mechanism for generating a key for storing messages within the caching strategy:
+
* *Default*
* *Key Expression*
+
...
+
For two requests that are the same ("equal"), generate the same key. Otherwise, you can get wrong results.
+
* *Key Generator* (deprecated)
+
include::_partials/cache-scope-eol-warning.adoc[tag=cacheKeyGeneratorDeprecated]
+
. (Optional) Open the *Advanced* tab in the properties window to configure advanced settings (deprecated):
+
include::_partials/cache-scope-eol-warning.adoc[tag=cacheAdvancedSettingsDeprecated]
+
image:mruntime-caching-strategy-advanced.png["A configuration panel for a caching strategy showing settings for cache behavior and response generator"]
+
* Select or create a *Response Generator*.
+
This step requires implementing the
...
The Cache scope persists only the message (payload and attributes) in the caching strategy's object store, not the full Mule event. This reduces the amount of data the Cache scope serializes and avoids issues caused by internal event objects, such as completion callbacks and locks, that aren't meant to be serialized.

The `mule.serialize.message.in.cache` feature flag controls this behavior. See xref:feature-flagging.adoc[Feature Flagging Mechanism].

include::_partials/cache-scope-eol-warning.adoc[tag=cacheObjectStoreMigration]
