...
The following example shows a `jms:listener` configured to initiate a transaction at flow level, a `try` scope that tries to initiate/join the transaction (depending on its configuration), and a `jms:publish` operation configured to run outside of the transaction.
...
==== An Error Handler is Added at Flow Level
...
==== An Error Handler is Added in the Try Scope
...
In all these cases the transaction created at Flow level is rolled back, and the message remains available to be read again. The behavior in the `try` Scope is:
