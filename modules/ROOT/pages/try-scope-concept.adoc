...
In addition to Transactional Action, you can configure the Transaction Type to Local (single Resource) or XA Transaction. See xref:transaction-management.adoc[Transaction Management] for more information about transactions types. 

When setting the Transaction Type you must consider that a Transaction cannot change it's type in the middle of its execution. So, for the different Transactional Action you get different behaviors:
...
* `BEGIN_OR_JOIN`: If there is a Transaction already created, then the Transactional Type makes no difference, as in the case of `INDIFFERENT`. If there is no Transaction, it creates one of the Type configured in Transactional Type, as in the case of `ALWAYS_BEGIN`.
