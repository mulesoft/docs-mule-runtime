...
* The scopes root element `<try />` has configurable attributes described in <<reference>>.
...
* Within the error handler, define the strategies within one or more On Error Continue (`<on-error-continue />`) and On Error Propagate (`<on-error-propagate />`) error handlers. On Error Propagate (`<on-error-propagate />`) is the default for any (`ANY`) error.
...
The following example shows a JMS Listener `<jms:listener />` operation that starts a transaction at flow level, a Try scope that sets a `transactionalAction` value based on a predefined xref:configuring-properties.adoc[configuration property] `${action}`, and a `<jms:publish />` operation configured to run outside of the transaction:
...
+
If the Try scope's `transactionalAction` is set to this value in the example, the scope joins the active transaction started by `<jms:listener ... transactionalAction="ALWAYS_BEGIN"/>`. The result is the same as in `INDIFFERENT`.
...
//TODO: FIX IMAGE, MUST MATCH EXAMPLE
image::error-handling-try-scope.png[Error Handling with the Try Scope,400]
