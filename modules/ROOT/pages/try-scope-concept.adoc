...
* An On Error Continue executes ands sends the result of the execution to its container Try scope, which uses that result to complete the execution successfully. Any transactions at this point are also committed.
* An On Error Propagate rolls back any transactions, executes, and uses that result to re-throw the existing error, meaning that its container Try Scope is considered as failing.
...
* Ignore (`INDIFFERENT`): The transaction is _continued_, but since the error is not handled by an `on-error-continue` at the flow level, the transaction is rolled back, and the message is not sent. 
* Always Begin (`ALWAYS_BEGIN`): Once again, this configuration is wrong, for trying to create a new Transaction even when there is already one active. 
* Begin or Join (`BEGIN_OR_JOIN`): Once again, same scenario as `INDIFFERENT`. 
