...
Notice that `fallbackAddresses` is only supported from 1.3.0 onwards in Mule 4 AMQP Connector. The preferred strategy is to use an HAProxy instead of using `fallbackAddresses` to maintain consistency with other connectors. The AMQP clients also tend to deprecate the use of fallback addresses in the AMQP connection definition. 
