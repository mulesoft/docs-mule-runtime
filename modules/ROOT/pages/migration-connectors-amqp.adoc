...
<amqp:config name="AMQP_Config">
	<amqp:connection host="localhost" port="5672"
		...
		<amqp:fallback-addresses >
			<amqp:amqp-address host="192.168.150.11" port="5672" />
		</amqp:fallback-addresses>
	</amqp:connection>
...
Notice that `fallbackAddresses` is only supported from 1.3.0 onwards in Mule 4 AMQP Connector. The preferred strategy is to use an HAProxy instead of using `fallbackAddresses` in order to maintain consistency with other connectors. AMQP clients also tend to deprecate the use of fallback addresses in the AMQP connection definition. 
