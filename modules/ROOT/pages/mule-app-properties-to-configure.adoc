...
When the application is deployed, the property values will be computed from the different possible sources and will statically replace the placeholders as if they have been written directly in the source code.

Following the previous example, it would be equivalent to have written it like this:
...
There is no special escaping based on the context, so the resulting values need to be valid in the context from which they are referenced.

Consider, for example, the following scenario which contains a Dataweave script.
...
And the contents of the `.properties` file look like this:

----
message=I love $
----

After property replacements take place, the application would look like this:
...
This is an invalid Dataweave script because the dollar sign has a special meaning and needs to be escaped. See xref:dataweave::dataweave-types.adoc#escape-chars-string[Dataweave - Escaping Special Characters].
