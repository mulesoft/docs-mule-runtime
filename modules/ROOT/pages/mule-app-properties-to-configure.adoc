...
In order to fix that, one option is to escape the value in the properties file, like this:

----
message=I love \$
----

Another option would be to use DataWeave's xref:dataweave::dw-mule-functions-p[p function] instead of a property placeholder:
...
In that case, the property is evaluated when the script is executed. The script remains syntactically valid at the moment of parsing.
