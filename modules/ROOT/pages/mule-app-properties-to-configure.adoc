...
MuleSoft recommends using `.yaml` files because it supports the addition of type validations and autocompletion.

== Create and Configure your Properties File

. In Studio, go to the *Package Explorer* view and right click the `/src/main/resources` folder in your project. Then, select *New* > *File*:
+
image:mruntime-properties-new-file.png[new-file-properties]
. Choose a name for your file and set the extension to either `.yaml` or `.properties`, depending on which format you prefer to use.
