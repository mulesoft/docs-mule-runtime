...
This document assumes that you are familiar with Mule, the xref:5@studio::index.adoc[Anypoint Studio interface], and xref:global-elements.adoc[Global Elements]. Further, it is assumed that you are familiar with the Microsoft .NET Framework.
...
* Anypoint Studio, the graphical interface to Mule ESB. See xref:6@studio::download-and-launch-anypoint-studio.adoc[Download and Launch Anypoint Studio] for details
