...
[CAUTION]
In most cases, the `error.cause` selector returns a `Throwable` instance that is part of the Mule API. Accessing the error's internal structure to define error handling logic is not recommended. All necessary information to handle errors is exposed by the other error selectors. Also, serializing the `error.cause` might result in redundant information.
