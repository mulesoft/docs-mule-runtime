...
* `#[error.cause]` returns `org.mule.extension.http.api.request.validator.ResponseValidatorTypedException`. Note that if you print the return instance of `ResponseValidatorTypedException`, Mule will print the result of `toString`. When the error has an associated known HTTP status code (such as 404), the string representation of the `ResponseValidatorTypedException` instance is a human-readable description of the error.
