...
When the JVM throws a `VirtualMachineError` (for example, `OutOfMemoryError` or `StackOverflowError`), Mule runtime wraps it as a `MULE:FATAL` error and raises a `FATAL_ERROR_RAISED` alert. Because these errors are in the `CRITICAL` hierarchy, On-Error components (`on-error-continue`, `on-error-propagate`) cannot catch them.
