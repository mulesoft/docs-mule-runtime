...
xref:connectors::database/database-documentation.adoc[Database Connector]
...
----
+
Note that you can return the payload or attributes (metadata)
associated with the error message by using the following constructions:
`&#35;[error.errorMessage.payload]`, `&#35;[error.errorMessage.attributes]`.
Depending on the metadata supported by a given connector, you can
also select values of fields for attributes, such as the status code from
an HTTP request (for example, `&#35;[error.errorMessage.attributes.statusCode]`).
However, because `statusCode` is a runtime field that is not available at
design time, the Studio UI does not list it in its autocompletion menu,
described in xref:studio::datasense-concept.adoc[DataSense]. Instead, you can
find the metadata in Studio using the *Input* and *Output* tabs in the
DataSense Explorer once the Mule message. To see those runtime fields, the
application must be running, and the message must be present.
+
