...
Each component in a Mule app declares the type of errors it throws, so you can
easily identify potential errors when designing a Mule flow.

Mule execution failures result in a Mule Error, which has the following components:
...
|A description regarding the problem, which can be the same or more extensive than the description
...
Error types consist of a namespace and an identifier, which enables you to
distinguish the types according to their domain. In the example above, the error
type is `HTTP:NOT_FOUND`, not simply `NOT_FOUND`.


For example, there are `HTTP:NOT_FOUND` and `FILE:NOT_FOUND` error types. While
connectors define their namespace, core runtime errors have the implicit `MULE`
namespace, so `MULE:EXPRESSION` and `EXPRESSION` are interpreted as one.

Another important characteristic of error types is that they may have a parent
...
The hierarchies enables error routing to be more general or more specific. For
example, a handler for `MULE:SECURITY` catches `HTTP:UNAUTHORIZED` errors and
OAuth errors. The follow figure shows the core runtime hierarchy:

image::error-hierarchy.png[Error Hierarchy]

All errors belong to one of these two main types: `ANY` or `CRITICAL`. All types
under `ANY` are matched by its parent and can be handled, while error types under
`CRITICAL` are so severe that cannot be handled and are only logged.

Whenever there is no clear reason for the failure, a component can use the
...
raising the errors. These errors require a specific custom namespace to
