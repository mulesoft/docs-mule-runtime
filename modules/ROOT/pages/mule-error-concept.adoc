...
module and connector operations in a Mule app declare the type of errors they
throw. xref:on-error-scope-concept.adoc[On-Error components] use this information
to list the Mule errors in the UI and enable you to select the errors you want
to handle.

The operations can throw many different types of Mule errors. Examples include
HTTP:NOT_FOUND (a 404 error) and HTTP:CONNECTIVITY errors from the HTTP listener
in the HTTP connector. DB:BAD_SYNTAX and DB:QUERY_EXPRESSION are errors that the
Select operation in the Database connector can throw.
...
Mule execution failures result in a Mule error. Mule errors are complex data
types with several fields, such as a description and type. When logging and
handling Mule errors, can select the values from any number of the error
fields.
