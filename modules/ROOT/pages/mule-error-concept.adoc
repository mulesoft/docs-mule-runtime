...
are associated with a Mule event. Mule apps also log ERROR, WARN, and INFO messages, which are not error objects. For information about logs, see xref:logging-and-debugging.adoc[Logging and Debugging].

Mule error objects are grouped into <<error_types, error types>> that follow a hierarchical
