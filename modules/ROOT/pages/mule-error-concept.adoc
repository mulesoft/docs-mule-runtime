...
However, because `statusCode` is a runtime value and not a field that
is available at design time, the Studio UI does not offer the
field in its xref:datasense-concept#using-datasense-for-autocomplete[autocompletion] menu.
+
