...
<<<<<<< HEAD
in the app can throw errors that are represented by a Mule error object, which
is associated with a Mule event. Mule apps also log ERROR, WARN, and INFO messages, which are not error objects. For information about logs, see xref:logging-and-debugging.adoc[Logging and Debugging].
=======
in the app can throw errors that are encapsulated within a Mule error object, which
are associated with a Mule event. Mule applications log these errors and enable you
to handle them with On-Error components. 
>>>>>>> 97c79c50bbabb1307bed5aa79725066cb3f3a735
