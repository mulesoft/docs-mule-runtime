...
** `TRANSFORMATION`: indicates an error ocurred while transforming a value. This involves Mule Runtime transformation and not DataWeave transformations. For example, using the Expression Transformer. 
...
** `CONNECTIVITY`: indicates that there was a problem establishing a connection. This could occur while using a connector, for example a HTTP requester. 
...
** `SECURITY`: indicates a security error ocurred, like invalid credentials being received or expired token being used. 
