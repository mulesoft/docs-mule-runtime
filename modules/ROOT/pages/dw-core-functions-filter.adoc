...
This example reads a JSON array that contains objects with `user` and `error` keys, and uses the `filter` function to return the objects that have their key `error` set with the value `null`.
...
filter ((item, index) -> item.error != null)
----
