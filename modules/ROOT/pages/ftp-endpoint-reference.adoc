...
|===
|image:check.png[] |The title at the top of the Properties pane, which will says either “Inbound Endpoint” or “Outbound Endpoint” reveals whether your endpoint is set up to send or receive files.
...
See the xref:ftp-transport-reference.adoc[FTP Transport Reference] for details on setting the properties for an FTP endpoint using an XML editor.