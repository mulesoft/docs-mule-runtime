...
The argument of the function call `toUpper("hi!")` matches with the types String and Any (so it could be called with the first two definitions) but DataWeave will execute the function `fun toUpper(a: String) = upper(a)` because is defined before the one that uses the type Any.
