...
The following example shows how to append writer properties to the DataWeave `output` directive. The script uses set `indent = false` to compress the JSON output into a single line.
...
[source,json,linenums]
----
{"hello": "world","jello": "world","mello": "world"}
----
...
Connector operations that read input data provide an `outpuMimeType` field that you can use to identify input MIME type. This field also accepts one or more reader properties, which is useful for helping DataWeave read the input correctly.

By default, the DataWeave reader treats a comma (`,`) as a separator. However, assume that a CSV input is separated by the pipe (`|`) but that the content also contains commas. To identify the appropriate separator to read, you can set the set the CSV reader property to recognize the pipe.

.Pipe-separated CSV Input
[source,csv,linenums]
----
id | name | role
1234 | Shokida,Mr. | lead
2345 | Achaval,Mr. | arch
3456 | Fellisati,Ms. | mngr
----

The following example uses the Read operation in the File connector to read the
pipe-separated (`|`) CSV input, and it uses a DataWeave script in the Transform Message component to output a row of the input in comma-separated format.
...
The Read operation (`<file:read/>`) identifies the input MIME type, the CSV separator (`|`), and the default header setting (`true`).

The script in the Transform Message component (`<ee:transform-message/>`) returns the second row from the input CSV (`2345 , Achaval\,Mr. , arch`). The script selects that row with `payload[1]` because DataWeave treats each row in the CSV as an index of an array. The output is comma-separated because the default CSV separator is a comma. The script omits the CSV header from the output by using the `output` directive (`output application/csv header = false`). To return pipe-separated output instead, the directive can add append the `separator` property: `output application/csv header = false,separator = "|"`.
