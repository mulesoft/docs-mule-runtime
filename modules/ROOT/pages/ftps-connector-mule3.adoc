...
// image is missing from repo

image:ftps_basic_config.png[Configuration]

In this example, the placeholder values refer to values saved in a `.properties` file, which exists by default in the `src/main/resources` folder of your project. See xref:configuring-properties.adoc[Configuring Properties].

For ease of maintenance and reusability, we recommend that you use a `.properties` file for your FTPS connector properties, rather than hardcoding these values in the global element. 
With the `.properties` file, you can deploy the app to different environments, such as production, development, and QA, changing the credentials, if needed.
See xref:deploying-to-multiple-environments.adoc[Deploying to Multiple Environments].
...
|===
|Field |Description
|Username | Username on the FTPS server.
