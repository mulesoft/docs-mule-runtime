...
Message Processors are used within flows to control how messages are sent and received within that flow. This is further described in xref:using-flows-for-service-orchestration.adoc[Using Flows for Service Orchestration]. (Within services, the same job is performed by message routers – see xref:using-mule-services.adoc[Using Mule Services] and xref:using-message-routers.adoc[Using Message Routers] for further details.)
