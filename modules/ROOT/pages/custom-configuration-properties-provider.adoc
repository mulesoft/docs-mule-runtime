...
== Custom Configuration Properties Provider vs Connector

Properties Provider approach is the correct one in case properties are static. This means, they would not change in runtime. In case your properties could change in runtime, then you will need to create a connector that could provide the value as one of its operations. 
