...
== Mule SDK example module

To create a configuration element that allows to configure the new custom configuration properties provider within Studio you must create a Mule SDK module. 
...
The example project is a Mule SDK module. To customize it:
...
Once your module is ready you can install it locally using *mvn clean install* so we can access it from Studio.


=== Using the custom properties provider in a Mule application

Now let's use the custom properties provider: 

. First of all create an application in Studio
. Add the dependency to you new module
.. Open the pom.xml file
.. Within _<dependencies>_ tag add a new dependency using the GAV that you put in your module.
.. Don't forget to add the _<classifier>mule-plugin</classifier>_ since it's a mule module.
.. Save your changes

Now open the application XML file and in the *Global Elements* tab click *Create*. Under *Connector Configuration* you should see an option to select the config from your custom module:

image::custom-configuration-properties-provider.png[]
