...
A Caching Strategy defines the actions a Cache scope takes when a message enters its sub-flow. By default, the Cache scope uses a Caching Strategy that stores data in an InMemoryObjectStore instead of an ObjectStore. For production environments, set up a Caching Strategy that uses an ObjectStore. Use the default strategy only for testing purposes.

You can configure the cache size, expire time, and maximum allowed entries by setting these values in the object store that you define or reference from the caching strategy.
