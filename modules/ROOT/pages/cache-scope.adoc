...
+
By default, Mule uses an `SHA256KeyGenerator` and a SHA256 digest to generate a unique key for the message payload. However, you can define a custom key by setting a Key Expression in your caching strategy.
+
...
+
* If there is no cached response (a _cache miss_), the Cache scope processes the new message and produces a response.
+
It also saves the resulting response in the object store (if the response is repeatable).
+
* If there is a cached response (a _cache hit_), the caching strategy generates a response that combines data from both the new request and the cached response.
+
...
Your caching strategy can also reference a Key Expression for the object store, which is a DataWeave expression that creates the key for storing the payload in the object store.

The Cache scope persists only the message (payload and attributes) in the object store, not the full Mule event. Because these Java-based customizations require the full Mule event, they're deprecated:

include::_partials/cache-scope-eol-warning.adoc[tag=cacheExtensionPointsDeprecated]

For details, see xref:cache-scope-strategy.adoc[Set Up a Caching Strategy].
////
REMOVED:
