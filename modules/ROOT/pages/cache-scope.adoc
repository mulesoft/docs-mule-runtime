...
. Executes, and returns the answer to, the Fibonacci equation (see below) using a number (_n_) provided by the caller as a request header. The images below show it as a query parameter, but you must pass it as a request header. Use a tool like Postman to send the following equation text:  +
