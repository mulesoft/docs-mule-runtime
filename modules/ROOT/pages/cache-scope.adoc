...
The Cache scope only caches repeatable streams, it does not cache non-repeatable streams which can only be read once before they are lost. By default, all streams are repeatable in Mule unless a component's streaming strategy is configured to non-repeatable.

In general, the caching process follows this sequence:

. A message enters the Cache scope.
. Cache scope determines whether the message payload is non-repeatable.
...
+
Note that if the generated response is a non-repeatable stream, the scope does not cache the response.
+
