...
=== Example: Invalidating Cache Usage

To invalidate a cache, you can create a Flow with two components:

* An `http:listener` component that will be the entry point of the Flow.
* An `ee:invalidate-cache` component that will invalidate the referenced cache.

Once this Flow gets executed, it invalidates the referenced cache clearing all its stored entries.
