...
[NOTE]
The default caching strategy uses an `InMemoryObjectStore` and should only be used for testing. On actual implementations, you need to create and configure a caching strategy.
...
* `doc:name` - Customize to display a unique name for the cache scope in your application.
+
...
* `cachingStrategy-ref` - (Optional) Specify the name of the global caching strategy that you have defined as a global element. If no `cachingStrategy-ref` is defined, Mule uses the default caching strategy.
...
+
* *Object Store* - (Optional) Configure an object store in which Mule stores all of the scope’s cached responses. By default, Mule stores all cached responses in an `InMemoryObjectStore`.
+
Example:
+
[source,xml,linenums]
...
| In Memory Object Store. +
(non-persistent)
| The data is stored in a distributed hazelcast map but is not shared accross the cluster nodes.
| Persistent Object Store.
...
|Persistent |`persistent="true"` |Check to ensure that the object store saves cached responses in persistent storage. Default is `false`.
...
|Directory |`directory` |(Only valid for `simple-text-file-store`) Enter the file path of the file where object store saves cached responses.
|===
...
[IMPORTANT]
====
Studio does not have the Invalidate Cache component. You need to manually add the component by modifying the configuration XML, adding an `ee:invalidate-cache` element to your flow.
====

=== Examples: Invalidating Cache Usage

==== Invalidate a Complete Cache

To invalidate a cache, create a flow with two components:

* An `http:listener` component that is the entry point of the flow.
* An `ee:invalidate-cache` component that invalidates the referenced cache.

Once this flow is executed, it invalidates the referenced cache by clearing all its stored entries.

* XML Configuration file:
+
[source,xml]
----
<flow name="invalidateCache">
        <http:listener config-ref="HTTP_Listener_Configuration" path="/invalidate" doc:name="HTTP"/>
        <ee:invalidate-cache cachingStrategy-ref="InvalidatableCachingStrategy" />
</flow>
----

==== Invalidate a Cache Key
To invalidate a key, create a flow with two components:

* An `http:listener` component that is the entry point of the flow.
* An `ee:invalidate-key` component that calculates a cache key from the current event, and invalidates that key.

Once this flow is executed, it calculates a key from the current event using the specified `keyGenerator-ref` or `keyGenerationExpression`, and invalidates that key.

* XML Configuration file (`keyGenerator-ref` defined):
+
[source,xml]
----
<spring:beans>
       <spring:bean id="keyGenerator"
                    class="com.mulesoft.mule.cache.config.CachingStrategyConfigTestCase.TestMuleEventKeyGenerator"/>
</spring:beans>

<flow name="invalidateKey">
        <http:listener config-ref="HTTP_Listener_Configuration" path="/invalidate" doc:name="HTTP"/>
        <ee:invalidate-key cachingStrategy-ref="InvalidatableCachingStrategy" keyGenerator-ref="keyGenerator"/>
</flow>
----

* XML Configuration file (`keyGenerationExpression` defined):
+
[source,xml]
----
<flow name="invalidateKey">
        <http:listener config-ref="HTTP_Listener_Configuration" path="/invalidate" doc:name="HTTP"/>
        <ee:invalidate-key cachingStrategy-ref="InvalidatableCachingStrategy" keyGenerationExpression="#[payload]"/>
</flow>
----

== Example Cache Scope Usage
...
. Executes, and returns the answer to, the Fibonacci equation (see below) using a number (_n_) provided by the caller as a request header. The images below show it as a query parameter, but it should be passed as a request header. Use a tool like Postman to pass it the following equation text:  +
...
=== Example in Anypoint Studio
Review the flow example in Studio:

image::mruntime-cache-flow-2.png[cache+flow+2]

==== Create a New Project in Studio
. In Anypoint Studio, click *File > New > Mule Project*.
. In the New Mule Project window add a *Project Name*, for example: `cachescopeexample`.
. Click *Finish*.

==== Add Fibonacci Response Generator Java Class
This example project requires `FibonacciResponseGenerator.java` To create a java class follow these steps:

. In the `src/main/app` folder of the example project, do right click *> New > Package*.
. In the Package window, write `com.mulesoft.mule.cache` for *Name*.
. Click *Finish*
. In the `com.mulesoft.mule.cache` package file of your project, do right click *> New > Class*.
. In the Java Class window, write `FibonacciResponseGenerator` for *Name*.
. Click *Finish*
. Copy the code from link:{attachmentsdir}/FibonacciResponseGenerator.java[FibonacciResponseGenerator.java] and paste it in the `FibonacciResponseGenerator.java` file of your example project.

==== Example in XML
After you create your Mule project and add the Fibonacci Response Generator Java class, review and add the example XML code in your Studio project:

. Copy and paste the following XML code into your `cachescopeexample.xml` file of your example project.
. Save all your changes and run the application.
...
http://www.mulesoft.org/schema/mule/vm http://www.mulesoft.org/schema/mule/vm/current/mule-vm.xsd">
    ...
    <ee:object-store-caching-strategy name="Caching_Strategy" keyGenerationExpression="#[message.inboundProperties['n']]" responseGenerator-ref="responseGenerator"/>
    ...
    <flow name="cache-exampleFlow1" >
        <http:listener config-ref="listener-config" path="fibonacci" />
        <expression-filter expression="#[message.inboundProperties.'http.request.path' != '/favicon.ico']" />
 <choice>
            <when expression="message.inboundProperties.'http.query.params'.['n'] &lt; 20">
                <message-properties-transformer>
                    <add-message-property key="n" value="#[message.inboundProperties.'http.query.params'.['n']]"/>
                    <add-message-property key="originalN" value="#[message.inboundProperties.'http.query.params'.['n']]"/>
                </message-properties-transformer>
                <vm:outbound-endpoint exchange-pattern="request-response" path="fibonacci" connector-ref="vmConnector" />
                <expression-component><![CDATA[payload= "Fibonacci(" + message.inboundProperties['originalN'] + ") = " + payload +"\nCOST: " + message.inboundProperties['cost']]]></expression-component>
            </when>
            <otherwise>
                <expression-component><![CDATA[payload= "ERROR: n must be less than 20"]]></expression-component>
            </otherwise>
        </choice>
    </flow>
 ...
 <ee:cache cachingStrategy-ref="Caching_Strategy"
     filterExpression="#[groovy:message.getInboundProperty('nocache') == null]">
            <expression-component><![CDATA[
			    ...
			    message.outboundProperties["originalN"]   =  message.inboundProperties["originalN"];
                if (n < 2)
                {
                    payload = n;
