...
Instead of processing all message payloads that it receives, the Cache scope can exclude specific payloads from the Cache scope flow based on an DataWeave expression. If the given expression evaluates to false, the current message will not use the cache.
