...
By default, Mule stores all cached responses in an InMemoryObjectStore. <<createcachingstrategy,Creating a Caching Strategy>> and define a new xref:mule-object-stores.adoc[object store] if you want to customize the way Mule stores cached responses.
