...
When using an HTTP Listener Connector, whenever there are outbound properties included in the Mule Message that arrives to the end of the flow, each outbound property is transformed into an HTTP response header. It's easy to add properies to the Mule Message by using the xref:property-transformer-reference.adoc[Property Transformer].
...
In the example above, two headers are added to every response to a request to the app. Both headers take their values from xref:mule-expression-language-date-and-time-functions.adoc[dateTime MEL expressions.]
...
. Set the variable's name to `headersOut` and its value to a map through the following xref:mule-expression-language-mel.adoc[MEL expression]:  `#[['date': server.dateTime, 'expires' : server.dateTime.plusHours(1)]]`
...
See xref:configuring-the-spring-security-manager.adoc[Configuring the Spring Security Manager] for more details.
...
A detailed reference can be found in the xref:tuning-performance.adoc#connector-level[tuning performance at the connector level] documentation.

The HTTP connector can also be configured to use a xref:http-connector.adoc#non-blocking-processing[non-blocking processing strategy].
...
* Consult a reference to the deprecated predecessor of this element, xref:deprecated-http-transport-reference.adoc[HTTP Transport Reference]