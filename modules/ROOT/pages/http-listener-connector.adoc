...
In the example above, two headers will be added to every response to a request to the app. Both headers take their values from xref:mule-expression-language-date-and-time-functions.adoc[dateTime MEL expressions].
...
* See a xref:3.7@http-connector-reference.adoc[full Reference] of the available XML configurable options in this connector
...
* Consult a reference to the deprecated predecessor of this element,  xref:http-transport-reference.adoc[HTTP Transport Reference]