...
This document assumes that you have xref:5.x@studio::index.adoc[downloaded and installed Anypoint Studio]. If you do not have any previous experience with Eclipse or an Eclipse-based IDE, review the brief introduction to the xref:6.x@studio::index.adoc[Anypoint Studio interface]. The document also assumes you're familiar with http://maven.apache.org/[Apache Maven] and know what it's used for.
