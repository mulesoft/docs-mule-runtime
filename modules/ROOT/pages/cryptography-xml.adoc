...
If the document has multiple signatures, you need to set the `elementPath` attribute to select the signature to validate. Specify a signed element using an XPath expression to validate the signature used to sign that element.

== Targeting a Custom Namespace With elementPath

To sign or validate an XML element that is inside a custom `namespace`, you have to specify the namespace. To define a namespace, use the XPath functions: https://developer.mozilla.org/en-US/docs/Web/XPath/Functions/namespace-uri[namespace-uri] and https://developer.mozilla.org/en-US/docs/Web/XPath/Functions/local-name[local-name].
...
----
To target the content of `FirstElement` inside `soap:Envelope`, you need to specify the `xmlns:soap` namespace. The XML Schema for the `xmlns:soap` namespace is defined in `http://www.w3.org/2003/05/soap-envelope/`.
...
| Use inline certificate if present a| Boolean |  +++Choose whether or not to validate the signature against a certificate contained in the +++`ds:Signature`+++ element, if the certificate is present.+++ | `"false"` | {nbsp}
| Key Id a| String |  +++The key ID, as defined in the JCE configuration.+++ |  | {nbsp}
| Jce Key Info a| One of:
