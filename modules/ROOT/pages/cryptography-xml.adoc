...
If the document being validated has multiple signatures, the `elementPath` attribute is needed selecting one of them. The XPath expression should target the element whose signature we want to validate.

== Targeting custom *namespaces* with elementPath
There are some cases in wich a XML element with a custom `namespace` has to signed / validated, for example consider the document below.

.Example: XML document with custom namespaces
[source,xml,linenums]
----
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope/">
    ...
    <AnotherAwesomeElement>
        Billy said that
    </AnotherAwesomeElement>
</soap:Envelope>
----

Let's say we want to target whatsoever the `SomeAwesomeElement` inside the `soap:Envelope`. Since we need to define the namespace in which the custom element lives, we need to use two XPath special functions called: https://developer.mozilla.org/en-US/docs/Web/XPath/Functions/namespace-uri[namespace-uri] and 
https://developer.mozilla.org/en-US/docs/Web/XPath/Functions/local-name[local-name]. For example, in our case the `elementPath` expression will be the following `xml-sign` operation:

.Example: Custom namespaces XPath expression
[source,xml,linenums]
----
<crypto:xml-sign 
    ...
    elementPath="/*[namespace-uri()='http://www.w3.org/2003/05/soap-envelope/' and local-name()='Envelope']/SomeAwesomeElement"/>
----
...
| Use inline certificate if present a| Boolean |  +++choose whether or not to validate the signature against a certificate contained in the +++`ds:Signature`+++ element itself, if it's present+++ | `"false"` | {nbsp}
| Key Id a| String |  +++The key ID, as defined in the JCE configuration.+++ |  | {nbsp}
| Jce Key Info a| One of:
