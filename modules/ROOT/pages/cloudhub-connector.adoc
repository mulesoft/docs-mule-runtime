...
+
[IMPORTANT]
*Inherited Token Authentication* is a configuration option that works only when your CloudHub connector's application is run inside a CloudHub instance. With this approach the connector retrieves the API Token based on the user that created first the application. If the user who created the application is deleted, this authentication method fails.
+
. Enter your Anypoint Platform credentials into the configuration fields and click *OK*. The *Url* field specifies the path to CloudHub in an Anypoint Platform control plane. For example, you can change the *Url* field to access other control planes such as EU. The EU control plane is at `+https://eu1.anypoint.mulesoft.com/+`.
+
