...
This example iterates over an XML file that contains details of employees such as the id, name and address and you convert the file into JSON. The DataWeave script uses the `replace` function to replace in the address field the characters `-` and `/` with blank space.
