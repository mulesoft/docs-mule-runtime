...
Other namespaces can be introduced via `<spring:beans>` or by adding a dedicated element in a module (see the xref:3.6@scripting-module-reference.adoc[Scripting module's] `<lang>` element).
...
Mule defines most of what you need out of the box, so it's fairly straightforward to jump in and write your own. Following are a few key concepts:
