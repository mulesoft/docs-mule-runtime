...
This procedure assumes that you already have a Mule instance installed on your host machine. If not, follow the instructions for xref:3.5@downloading-and-starting-mule-esb.adoc[Downloading and Starting Mule].
...
To use the SAP connector in your Mule application, you must first configure a global SAP element. Read more about xref:global-elements.adoc[Global Elements].
...
[TIP]
As a best practice, use property placeholder syntax to load the credentials in a more simple and reusable way. Read more about property placeholders at xref:configuring-properties.adoc[Configuring Properties].
...
|*Extended Properties* |`jcoClientExtendedProperties-ref` |A reference to `java.util.Map` containing additional JCo connection parameters. Additional information and a complete list of parameters can be found xref:sap-jco-extended-properties.adoc[here]. |
...
*Note*: With DataSense support, the recommended way to generate the XML definitions is using xref:dataweave.adoc[DataWeave]. However, if you are using a Mule 3.3 application, see  xref:5@studio::datamapper-user-guide-and-reference.adoc[DataMapper].
...
If you intend to employ an SAP endpoint in conjunction with a xref:5@studio::datamapper-user-guide-and-reference.adoc[DataMapper] or xref:dataweave-reference-documentation.adoc[DataWeave] transformer to map and transform data, you can make use of Anypoint Studio's xref:5@studio::datasense.adoc[DataSense] functionality.
...
http://mulesoft.github.io/mule-sap-connector/[Download examples].
