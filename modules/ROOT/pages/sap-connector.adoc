...
Mule ESB supports SAP integration through our xref:anypoint-connectors.adoc[Anypoint Connector] for SAP, which is an SAP-certified Java connector that leverages the SAP Java Connector (JCo) libraries.  The SAP connector enables Mule applications to:
...
. Follow the detailed xref:datasense.adoc[DataSense] instructions to drop the endpoint into your flow, then xref:5@studio::testing-connections.adoc[test the connection] to SAP using the SAP Connector.
...
For more information about using XML definitions without DataMapper and DataSense, see xref:3.6@xml-definitions.adoc[XML Definitions].
