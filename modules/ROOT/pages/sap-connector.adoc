...
Mule supports SAP integration through our xref:anypoint-connectors.adoc[Anypoint Connector] for SAP, which is an SAP-certified Java connector that leverages the SAP Java Connector (JCo) libraries.  The SAP connector enables Mule applications to:
...
If you intend to employ an SAP endpoint in conjunction with a xref:6@studio::datamapper-user-guide-and-reference.adoc[DataMapper transformer] to map and transform data, you can make use of Anypoint Studio's xref:datasense.adoc[DataSense] functionality.

. Follow the detailed xref:datasense.adoc[DataSense] instructions to drop the endpoint into your flow, then xref:testing-connections.adoc[test the connection] to SAP using the SAP Connector.
...
Port 3300 is predefined by SAP, so if you need to validate other port numbers based on your SAP instance number, you can check the complete list of xref:sap-jco-server-services-configuration.adoc[service-to-port mappings].
...
In this case, you must know the configuration property _as defined by SAP_. You can check xref:sap-jco-extended-properties.adoc[the configuration properties list].
...
For more information about using XML definitions without DataMapper and DataSense, see xref:xml-definitions.adoc[XML Definitions].
...
|*Extended Properties* |jcoClientExtendedProperties-ref |Advanced |A Reference to `java.util.Map`, which contains additional JCo connection parameters. As necessary, consult the xref:sap-jco-extended-properties.adoc[complete list of parameters]. |
|===
...
For additional information, consult the xref:3.8@mulesoft-enterprise-java-connector-for-sap-reference.adoc[Enterprise Java Connector for SAP Reference].
