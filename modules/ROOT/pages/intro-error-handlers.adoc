...
In the above example, an on error propagate handler is configured to propagate database connection errors, causing the `Try` to fail and the flow's error handler to execute. 
An on error continue handler is configured to deal with any other error. The Try scope’s execution is considered successful for that case, which means that the flow’s execution continues with the next processor, an HTTP request.
