...
The mapping customizes error handling by routing errors from the first Request operation to error handler `APP:API_1` and routing errors from the second Request operation to `APP:API_2`. To apply different handling policies if the APIs go down, the following example maps `HTTP:INTERNAL_SERVER_ERROR` to different error handlers. One propagates the error if it comes from the first API. The other handles the error if it comes from the second API.
