...
In the above example, an on error propagate handler is configured to propagate database connection errors, causing the `Try` to fail and the flow's error handler to execute. An on error continue handler is configured to deal with any other error. Therefore, the Try scope execution is considered successful, meaning that the flow execution continues with the next processor, an HTTP request.
