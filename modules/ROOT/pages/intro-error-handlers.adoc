...
By mapping errors from the first request to a custom `APP:API_1` error and errors in the second request to `APP:API_2`, you route those errors to different handlers. The next example maps `HTTP:INTERNAL_SERVER_ERROR` to apply different handling policies if the APIs go down (propagating the error in the first API and handling it in the second API).
