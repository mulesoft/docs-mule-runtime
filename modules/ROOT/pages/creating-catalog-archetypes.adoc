...
The module archetype will xref:creating-module-archetypes.adoc[ask various questions] and then generate the files. You can also use this plug-in without user prompts by entering all the arguments at the command line. For a full list of arguments that can be passed in, see the xref:creating-catalog-archetypes.adoc[Command Line Options].
...
* *siso*: The pattern receives messages from a single inbound endpoint and dispatches to a single outbound endpoint. The xref:bridge-pattern.adoc[Bridge Validator] and xref:proxying-web-services.adoc[Web Service Proxy] patterns are of this kind.
