...
The module archetype will xref:3.4@creating-module-archetypes.adoc[ask various questions] and then generate the files. You can also use this plug-in without user prompts by entering all the arguments at the command line. For a full list of arguments that can be passed in, see the xref:3.4@creating-catalog-archetypes.adoc[Command Line Options].
...
* *siso*: The pattern receives messages from a single inbound endpoint and dispatches to a single outbound endpoint. The xref:3.4@bridge-pattern.adoc[Bridge Validator] and xref:3.4@proxying-web-services.adoc[Web Service Proxy] patterns are of this kind.
