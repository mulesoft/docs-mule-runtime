...
The `flatMap` is useful for refactoring uses of `flatten` on the results of a
...
objects. It also defines a function with a set of if-else statements that
act on name-value pairs with the key `name`.
...
in the subarrays into a single array, removing the parent array, and
