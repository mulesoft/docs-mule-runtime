...
Flows are valuable for manhy use cases, including:
...
A Flow is configured using the XML <flow> element. Each flow has a name attribute, message source (unless it's a private flow), one or more components, and an optional error handler.
...
A private flow differs from a subflow in that a private flow (like all flows) has it's own context and error handler.
