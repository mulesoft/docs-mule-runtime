...
By default, Mule does not display all stack traces. When Mule does show stack traces, it filters out some internal class references from to produce a more readable output.

You can control Mule stacktrace behavior in two ways:
...
Use `-M-DpropertyName` with the following command-line arguments to control the behavior of stacktraces:
...
* `mule.stacktrace.filter` - A comma-separated list of packages and/or classes to
 ...
 Note: These arguments are available for Mule developers and administrators to use while troubleshooting. They are not intended for use by end users.

== Set Properties in Anypoint Studio

In Studio, you can set the properties specified above to instruct Mule to
provide stack traces or to filter out packages or classes to remove. To adjust these settings, open the `Run Configurations...`
...
Follow these steps to troubleshoot Mule if stops and you need
...
To identify the Mule process ID, use the Unix/Linux `ps` command, or execute the `jps`
  ...
  For example, a line containing `MuleContainerBootstrap`, `ApplicationDeployer`,
...
Perform this procedure to retrieve a heap dump if the Mule server throws
 an Out Of Memory error: `java.lang.OutOfMemoryError: Java heap space`

The Oracle JVM provides a https://docs.oracle.com/javase/7/docs/webnotes/tsg/TSG-VM/html/clopts.html#gbzrr[JVM modifier]
...
To apply this modifier to the JVM that Mule uses, set a parameter in Mule's
