...
| Pass XML attributes from the input source payload to the output XML. It uses the dynamic attribute expression `@((dynamicAttributes))` to create the attributes of the new output tag selecting the attributes dynamically from the input.
