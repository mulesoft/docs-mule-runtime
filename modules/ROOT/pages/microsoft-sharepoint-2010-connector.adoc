...
If you choose for a query to return either SharepointListReference or SharepointListMultiValueReference, the returned value of the field depends on the value of the *Retrieve full objects for reference fields* parameter (set in Query Builder):
...
----
+
* Checked: Retrieves the full object graph. In case there is a cycle, the summary reference object is shown:
+
[source,json,linenums]
----
"Title": "A title",
