...
You can change the serialization mechanism to improve functionality and performance when Mule runtime engine executes any of the following processes:
...
The open source Serialization API provides the `ObjectSerializer.java` which enables you to serialize or deserialize objects into a byte array. See https://github.com/mulesoft/mule-api/blob/master/src/main/java/org/mule/runtime/api/serialization/ObjectSerializer.java[ObjectSerializer.java in GitHub].
...
In addition, if you are going to serialize and deserialize in Mule, then you must define the `getInternalProtocol()` method.
If you are going to use the serialized object in a external system, define that in the `getExternalProtocol()` method. +
...
The Batch module is the only component that does not have its behavior affected, because this module always uses serialization.
