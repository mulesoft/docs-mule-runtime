...
Starting with version 4.6, Mule runtime supports Java 17, which enforces JPMS. To achieve this capability, Mulesoft introduces several custom serializers that replace the default Kryo frameworks, because those default frameworks rely on mechanisms that break JPMS encapsulation.
If you're using Mule runtime 4.6 and Java 17, we recommend that you use a clean slate.
