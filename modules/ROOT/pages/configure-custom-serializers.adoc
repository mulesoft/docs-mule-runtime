...
By default, Mule uses plain old Java serialization. You can configure `defaultObjectSerializer` in your Mule application to specify a different serialization mechanism such as the Kryo serializer or any other custom serializer.

The Batch module always uses Kryo serialization, so the module's behavior is not affected by the configured serializer.
