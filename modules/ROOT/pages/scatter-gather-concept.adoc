...
Assume a scheduled input (`<<scheduler />`) to Scatter Gather with the message `"hello scatter gather"` from Set Payload (`<set-payload />`).
...
* The second `<route />` within Scatter-Gather adds the string `" second route"` to the input `payload`.

The first Logger after Scatter gather prints the output `payload` of Scatter-Gather, which is an objection containing key-value pairs for each route. The key provides the index of the route (0 or 1), and the value provides a set of key-value pairs not only for the payload but for attributes and other properties of the event, excluding Mule variables, for example:
...
INFO  ...LoggerMessageProcessor:
{
  ...
  "2": {
    ...
    "payload": "hello scatter gather third route",
    "outboundPropertyNames": [
...
To access and manipulate such a payload, you can use a DataWeave function such as `mapObject`. The second Logger in the example uses `valuesOf(payload mapObject { ($$) : $}).payload` to print the payloads of each key in the output object:
