...
* `simple` (default). With this option, the LDAP server authenticates the user by using the user's fully qualified DN (Principal DN parameter) and password (Password parameter).
+
* `none`. With this option, the LDAP server allows the client to read and update data configured to be accessible by an unauthenticated client. For more information, see https://docs.oracle.com/javase/jndi/tutorial/ldap/security/ldap.html.
|===
+
...
Inside the `<version>` tags, put the desired version number, the word `RELEASE` for the latest release, or `SNAPSHOT` for the latest available version. To specify a specific release version, refer to the xref:release-notes::connector/ldap-connector-release-notes.adoc[LDAP Connector Release Notes] for possible values.
...
Using Anonymus authentication in the LDAP config

    config.principal.dn = <DN>
    config.password=<Password>
    config.authentication = NONE or ANONYMUS
    config.bindUrl = <>
----
+
