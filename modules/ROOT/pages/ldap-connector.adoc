...
|Authentication |Specifies the authentication mechanism to use: SIMPLE,
"none" if no authentication environment properties have been set.
This means that the server does not know or care who the client is and will allow the client to access (read and update) any data that has been configured to be accessible by any unauthenticated client. For more details please check this link: https://docs.oracle.com/javase/jndi/tutorial/ldap/security/ldap.html
|===
+
