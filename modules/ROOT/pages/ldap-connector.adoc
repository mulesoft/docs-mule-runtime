...
|Authentication |Specifies the authentication mechanism to use. Default: the `Simple` value.
|===
+
...
|===
+
. Create the organizational unit entry using a Groovy component. Drag the Groovy component next to the Variable Transformer and use the script below.
+
...
updates the Use Case 1 XML file (above) with it.
