...
Access the documentation for the xref:3.8@database-connector.adoc[Database Connector] to take advantage of the newly-designed functionality.
====
...
NOTE: The JDBC connector supports multi-resource transactions. Multi-resource transactions are also supported by the xref:database-connector.adoc[Database Connector]. Consider also the use of xref:xa-transactions.adoc[XA Transactions] to handle this use case.
...
For details on setting the properties for a JDBC endpoint using an XML editor, consult the xref:jdbc-transport-reference.adoc[JDBC Transport Reference].