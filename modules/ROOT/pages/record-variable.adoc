...
During the xref:3.6@batch-processing.adoc[Process] phase of batch processing, use the *Record Variable message processor* to remove an existing `recordVars` from records, setting the *Operation* to *Remove Record Variable*.
...
To access a record variable within the xref:3.6@batch-processing.adoc[Process] and xref:3.6@batch-processing.adoc[On Complete] phases of batch processing, use the following expression:
