...
A redelivery policy is a filter that is added to Mule Sources in order to prevent dispatching messages that are known to cause errors. +
By adding a redelivery policy in a flow's source, the data received will be first evaluated prior to executing the flow's components. If that data caused failures multiple times in the past, the redelivery policy will prevent the flow from processing it and a `REDELIVERY_EXHAUSTED` error will be raised instead.  +
This will prevent the runtime from wasting resources by executing a flow with poison messages (Messages that caused errors in previous executions).


[IMPORTANT]
Some connectors have their own logic for handling an error thrown by a redelivery policy. In those cases, check the documentation for the connector which that source belongs to for more information. +
E.g: The http-connector may want to return 429: TOO MANY REQUESTS.
...
If the flow processing causes an exception, a counter associated with it will be incremented. Once the counter reaches a value bigger than the configured maxRedeliveryCount, an `MULE:REDELIVERY_EXHAUSTED` error will be thrown. +
If the flow processing does not cause an exception, it's counter will be reset.

==== Throws

* MULE:REDELIVERY_EXHAUSTED
+
When the number of executions that raise an error is bigger than the configured `maxRedeliveryCount`.
