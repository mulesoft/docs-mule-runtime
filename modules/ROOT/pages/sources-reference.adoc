...
A redelivery policy can be considered as a filter that is added to Mule Sources in order to prevent dispatching messages that are known to cause errors. +
By adding a redelivery policy, each source will add logic for pre-processing the data received and avoid dispatching it if it's known to fail. +
This will prevent the runtime from wasting resources by executing a flow with messages that caused errors in previous executions.
