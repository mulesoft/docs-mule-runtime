...
A redelivery policy is a way of avoiding processing messages that are known to cause errors and prevent the runtime from wasting resources. +
...
If the flow processing, a counter associated with it will be incremented. Once the counter reaches a value bigger than the configured maxRedeliveryCount, an `MULE:REDELIVERY_EXHAUSTED` error will be thrown. +
