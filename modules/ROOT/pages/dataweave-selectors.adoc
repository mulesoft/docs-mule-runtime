...
//TODO: NEED AN EXAMPLE. TEXTED Shoki about it on 01/07/2020. MDA on PTO.
...
The following input payload is an object with the key "name" and the
value "DataWeave".
...
The following DataWeave script uses `payload.name` to select the
value of the object in input payload that has the key "name".
...
The following input payload is an array that contains two objects with the same
key, "name".
...
The following example selects values from XML elements with different namespaces.

==== Input

The following input payload contains XML with two `table` elements under the
...
The following example selects XML values using a local name (`payload.root.table`
instead of `payload.root.furniture#table` in the
<<namespace_variable, previous example>>).

==== Input

The following input payload contains XML with a `table` element that contains
...
The following DataWeave script selects values of children in the `table` element
...
The following input payload is an XML `user` element that contains the attribute
`name="Weave"`.
...
The following DataWeave script selects the value of the `name` attribute
...
The following example returns an Array of all the matched keys value from
and array of XML elements. If no key matches, the selection returns the value
`null`.

==== Input

The following input payload contains an XML array of `user` elements.
...
(for example, with `payload..user`) to select the values of each `user` key and of its
descendants, or you can use `payload..*name` to select the values
of all `name` descendants.

The following example selects each descendant of the input payload.

==== Input

The input payload contains a set of `user` elements that are nested
at different levels.
...
The following DataWeave script uses `..` to select the values of
all `user` elements found at all levels of the input payload.
