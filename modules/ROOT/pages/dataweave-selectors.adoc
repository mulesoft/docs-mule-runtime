...
Notice that it passes `payload.ref` within the dynamic selector `[()]`.
...
a namespace. It uses the value on the `"ref"` property to select the value
with that key.

==== Input Payload

The input payload contains a `<root/> element with two child elements, one of
which has the namespace `http://www.w3.org/TR/html4/`:
