...
* Single Value with namespace: `payload.ns0#"$(nameExpression)"`


=== Example 
This example shows how to dynamically select a single value. It uses the value on the "ref" property to select the value with that key.
...
=== Example 
This example shows how to dynamically select a single value with namesapce. It uses the value on the "ref" property to select the value with that key.
...
selector will match all namespaces with that local part, regardless of the
namespace.
