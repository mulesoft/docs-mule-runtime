...
The output shows how the DataWeave (dw) format represents the Multipart input. Note that the `raw` and `content` values are artificially shortened in the output example.
...
The Multipart reader accepts optional parameters that provide instructions for reading input data.
...
The Multipart writer accepts optional parameters that provide instructions for writing output data.
