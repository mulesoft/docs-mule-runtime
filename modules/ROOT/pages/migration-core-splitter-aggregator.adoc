...
xref:connectors::aggregator/aggregator-module.adoc[Aggregators] remain in Mule 4 but as a separate module with very different behavior.
...
* A variable `collection-splitter0-group-size` should be created to store the size of the collection. This is needed because inside the `foreach` scope, the payload should correspond to the element from the collection being processed.
