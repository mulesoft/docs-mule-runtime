...
* If you using Mule 3.9.3 or earlier and you are _not_ using xref:mule-management-console::index.adoc[Mule Management Console (MMC)], which reached its end of life in 2019, disable the MMC agent by adding `mule.agent.enabled=false` to `wrapper.conf`. 
