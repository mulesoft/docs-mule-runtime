...
In addition to declaring local variables through the `do` function (see xref:dataweave-flow-control#control_flow_do[Flow Control in DataWeave]), you can initialize local variables using literal expressions, variable reference expressions, or functional expressions. These expressions can reference any other local variables within their scope or any input or global variables.
