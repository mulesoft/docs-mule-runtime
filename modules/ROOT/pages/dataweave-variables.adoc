...
DataWeave is a functional programming language in which variables behave just like functions. DataWeave eagerly evaluates variables, the same way it evaluates arguments to functions. In addition, DataWeave variables are immutable. 
...
An internal annotation, `@Lazy()`, can appear in autocompletion suggestions when you are writing a DataWeave script in Studio. This annotation is not intended for general use and does not improve the performance of your scripts. 
