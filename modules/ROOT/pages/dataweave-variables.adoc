...
The following example passes the string `HELLO` to the `test` function defined in the header. The `do` scope accepts the string, converts it to lower case, and then concatenates that string to the `suffix` variable, which is also defined in the header of the scope.
