...
The following example produces an error because the `fromDoScope` is referenced from outside the scope of the `do`. As a consequence, the concatenation operation (`++`) cannot append the name-value pair to the collection.
