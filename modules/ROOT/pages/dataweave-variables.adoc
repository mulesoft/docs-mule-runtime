...
MuleSoft recommends that you declare local variables within `do` scopes (see xref:dataweave-flow-control#control_flow_do[Flow Control in DataWeave]). The alternative of declaring a scope with the `using` keyword is no longer recommended and only available for compatibility reasons.
...
The references to all the variables are valid. `fn` and `ln` are defined and called
within the `do` scope. The global `myVar` variable is also accessible from that scope.
...
This example produces an error because the `fromDoScope` is referenced from outside the
scope of the `do`. As a consequence, the concatenation operation (`++`) cannot append the name-value pair to the collection.
...
---
{ person :
	myDo ++ { "outsideDoScope" : fromDoScope }
}
----
...
This example passes the string `HELLO` to the `test` function defined in the header. The `do` scope accepts the string and converts it to lower case and then concatenates that string to the `suffix` variable also defined in the header of the scope.
