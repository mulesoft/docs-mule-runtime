...
The http://en.wikipedia.org/wiki/Multicast[Multicast] transport sends and receives messages from groups of multicast sockets. It is implemented on top of xref:udp-transport-reference.adoc[UDP] and is highly scalable since knowledge of the receivers is not required.
...
|Multicast |https://www.mulesoft.org/docs/site/current3/apidocs/org/mule/transport/multicast/package-summary.html[JavaDoc SchemaDoc] |image:check.png[] |image:check.png[] |image:check.png[] | - | - | - |one-way, request-response |request-response |org.mule.transport:mule-transport-multicast
|===
