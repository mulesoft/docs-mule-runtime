...
[TIP]
====
Using the Query Editor in conjunction DataSense's xref:5.x@studio::using-perceptive-flow-design.adoc[perceptive flow design] greatly facilitates designing and building a SaaS integration flow. Perceptive flow design allows you to create complex mappings by simply placing an xref:5.x@studio::datamapper-user-guide-and-reference.adoc[Anypoint DataMapper transformer] before or after a connector in your flow.
====

== Assumptions

This document assumes that you are familiar with the xref:6.x@studio::index.adoc[Visual Editor in Anypoint Studio], as well as the Anypoint Connectors, in particular the Salesforce cloud connector used in the example queries. This document also references xref:3.7@anypoint-connectors.adoc[Connectors], xref:3.6@global-elements.adoc[Global Elements] and xref:3.6@datasense.adoc[DataSense]. To learn more about developing with Mule ESB's graphical user interface, review xref:6.x@studio::index.adoc[Getting Started with Anypoint Studio].
...
Learn how to xref:5.x@studio::installing-extensions.adoc[add connectors] to your instance of Anypoint Studio.
...
* *xref:5.x@studio::datasense-query-language.adoc[DataSense Query Language]*  allows you to perform queries using SQL elements and syntax. Using the DataSense query language, you can use the Query Editor's graphical interface for almost all operations.
...
* Learn more about the Query Editor's xref:5.x@studio::datasense-query-language.adoc[DataSense Query Language].
