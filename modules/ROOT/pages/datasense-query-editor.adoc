...
[TIP]
====
Using the Query Editor in conjunction DataSense's xref:using-perceptive-flow-design.adoc[perceptive flow design] greatly facilitates designing and building a SaaS integration flow. Perceptive flow design allows you to create complex mappings by simply placing an xref:6@studio::datamapper-user-guide-and-reference.adoc[Anypoint DataMapper transformer] before or after a connector in your flow.
====

== Using the Query Editor

The table below lists the xref:anypoint-connectors.adoc[Anypoint Connectors] which support Query Editor functionality. (Additionally, you can use the Query Editor with any custom-built connector which supports the query functionality.)
...
. To access the Query Editor, add a connector to your flow, then access its xref:the-properties-editor.adoc[Properties Editor.]
...
* *xref:datasense-query-language.adoc[DataSense Query Language]*  allows you to perform queries using http://en.wikipedia.org/en/wiki/SQL[SQL] elements and syntax. Using the DataSense query language, you can use the Query Editor's graphical interface for almost all operations.
...
. Create a Salesforce xref:global-elements.adoc[Global Element].
