...
The Mule 4 message model makes it easier to work with data in a consistent way, by using the Dataweave expression language.

The Mule message and its data are part of a context called the xref:about-mule-event.adoc[Mule event]. The Mule event has its own Mule message, composed of a payload and attributes. 

Attributes are metadata like file size, and can be accessed through xref:dataweave-variables.adoc[Predefined Variables] or stored in user-defined xref:about-mule-variables.adoc[Variables] to retrieve information such as operation results, auxiliary info and even lambda functions.

== Inbound Properties Are Now Attributes

In Mule 3, inbound properties stored additional information about a payload obtained through a Message Source, such as the query parameters coming through an HTTP listener. In Mule 4 "attributes" replace inbound properties and have these advantages:
...
Attributes can be easily accessed through DataWeave expressions, just as inbound properties were in Mule 3, for example:
...
When any Source or Operation produces a new Message as a result of its execution, *both parts of the Message (the payload and attributes) are updated with the new values*, thus updating the previous attributes. If you need to preserve any information from the attributes across operation invocations, you can either store the previous attributes in a variable or xref:target-variables.adoc[set the `target` parameter in the invoked operation].
