...
Mule 4 includes a simplified Mule message model in which each Mule event has a message and variables associated with it. A Mule message is composed of: +

* A payload and its attributes (metadata, such as file size).
* Variables (`vars`) hold arbitrary user information such as operation results, auxiliary values, and so on. In general, operations do not automatically propagate variables to other components, like a listener. For example, if you want variables of a flow 1 available in a flow 2 you would need to explicitly send them as part of the message.
...
Above, `http:headers` settings like `vars.httpStatus default 200` set a
...
Instead, store data in target variables (sometimes called target parameters).
...
For examples, see
xref:migration-core-enricher.adoc[Migrating the Enricher to a Target Parameters]
