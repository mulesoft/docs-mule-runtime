...
You can combine <<ex03, formatting characters>> to write supported date and time formats. The example uses the `as` operator to write the dates and times as a string.
...
For multiple, similar conversions in your script, you can define a custom type as a directive in the header and set each date to that type. Note that type names are case-sensitive.
