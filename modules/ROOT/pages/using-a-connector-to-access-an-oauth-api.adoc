...
[NOTE]
If you need to deploy to different environments (e.g., production, development) where these parameters need to have different values, please see xref:3.7@deploying-to-multiple-environments.adoc[Deploying to Multiple Environments].
...
+
image::connectorconfiguration-1.png[]
...
* Learn how to access an OAuth API using the xref:3.6@http-request-connector.adoc[HTTP Outbound Endpoint]