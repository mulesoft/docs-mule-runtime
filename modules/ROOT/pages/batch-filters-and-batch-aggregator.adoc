...
A Batch Aggregator scope enables you to pull records from the stepping queue and then process and return them as an array of records. This functionality is useful, for example, for bulk uploading records to an external server, such as a database server. Within the scope, you can add an operation, such as a bulk update, insert, or upsert opertion, to load records in bulk, instead of running an operation separately on each record individually. 

The Batch Aggregator scope pulls records from the stepping queue into arrays. You can process arrays of a fixed size or stream a single array of records from the batch job instance. For example, you can configure a Bulk Update operation in the Anypoint Connector for Database (Database Connector) to update 200 processed records on the server. Alternatively, you can stream all the records in the instance to the server. For information about configuring bulk operations for DataBase Connector, see xref:TODO.adoc. 

The following example updates a database with an array of 200 records:
...
The following example streams records to the server:
...
This example does not change the records. However, it is possible to modify records in within the Batch Aggregator scope, just as you can modify them with processors in the Batch Step scope. Records are mutable, meaning that you can access the payloads and variables of the records. When aggregating a fixed number of records, access to each record can take place sequentially or through random access. When streaming records, Mule allows only sequential access to records. See <<modify_records_fixed_size>> and <<modify_records_streaming_aggregator>>.
...
[[modify_records_fixed_size]]
=== Modifying Records of a Fixed Size within the Batch Aggregator 
...
[[modify_records_streaming_aggregator]]
=== Modifying Records while Streaming within the Batch Aggregator 

You can configure a Batch Aggregator scope to stream its content. Setting aggregator to stream the records (`streaming="true"`) enables you to process an array of all the records in the job instance without running out of memory, no matter how many or how large the records are. 
