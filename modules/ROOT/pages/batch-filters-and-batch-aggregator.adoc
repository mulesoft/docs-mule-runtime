...
Aggregation is useful for bulk uploading one or more arrays of records to an external server, such as a database server. Within the Batch Aggregator scope, you can add an operation, such as a bulk update, insert, or upsert operation, to load one or more arrays of records to a server, instead of running an operation separately on each record.
