...
Aggregation is useful, for example, for bulk uploading one or more arrays of records to an external server, such as a database server. Within the scope, you can add an operation, such as a bulk update, insert, or upsert operation, to load the records in bulk, instead of running an operation separately on each record. See <<aggregator_bulk_update>> for an example.
