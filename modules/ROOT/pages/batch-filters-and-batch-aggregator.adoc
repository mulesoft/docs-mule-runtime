...
As the following example shows, only sequential access to records is possible when streaming within the Batch Aggregator scope. Random access is not supported when streaming because streaming requires access to the entire set of records. Without a fixed size, Mule cannot guarantee that all records will fit in memory.
