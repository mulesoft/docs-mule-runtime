...
Imagine a Batch Job configuration in which the first Batch Step checks for a Salesforce contact in a record, and a second Batch Step updates each existing Salesforce contact with new information. You can apply a filter to the second Batch Step to ensure that it only processes records that succeed during the first Batch Step process (because they have a Salesforce contact).

By making Batch Step components accept only a portion of the records for processing, you streamline the batch job process, which allows the Mule runtime engine to focus on the relevant data for a particular Batch Step.
...
A Batch Step can accept one `acceptExpression` and one `acceptPolicy` attribute to filter records.

You use the `acceptExpression` attribute to process only those records that evaluate to `true`. If the record evaluates to `false`, the Batch Step skips the record without processing it and sends it to the next component. In other words, Mule filters _out_ any records with an `acceptExpression` that resolves to `false`.
...
[%header,cols="25a,75a"]
|===
|Accept Policy Values |When it evaluates to `true`
| `NO_FAILURES` |_Default_ +
Batch Step processes only those records that _succeeded_ in all preceding Batch Steps.
| `ONLY_FAILURES` |Batch Step processes only those records that _failed_ in a preceding Batch Step.
| `ALL` |Batch Step processes all records, regardless of whether they _failed_ in a preceding Batch Step.
|===

If you do not apply filters to a Batch Step, the batch processes only those records that _succeeded_ in all preceding Batch Steps, which is the default `acceptPolicy` (`NO_FAILURES`).

The following example illustrates the second Batch Step in a Batch Job component. This Batch Step processes only those records that failed in the preceding Batch Step. In the first Batch Step, Mule checks each record to see if it has an existing Salesforce contact. The second Batch Step, which creates a contact for each record, processes only the failed records (that is, records that failed to have an existing account).
...
* If you apply no filters to a Batch Step, the batch processes only those records that _succeeded_ in all preceding steps. In other words, the default Accept Policy applied to all Batch Steps is `NO_FAILURES`.
...
* When you apply both types of filters, Mule evaluates them in the following order:
+
. *Accept Expression* (`acceptExpression`)
. *Accept Policy* (`acceptPolicy`)
...
You can use the Batch Aggregator scope to accumulate a subset of records from a Batch Step, and bulk upsert them to an external source or service. For example, rather than upserting each lead (a record) in a batch to Salesforce, you can configure a Batch Commit to accumulate, for example, 200 records and then upsert all of them to Salesforce in one chunk.
