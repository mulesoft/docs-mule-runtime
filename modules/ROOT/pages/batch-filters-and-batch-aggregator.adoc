= Using Batch Processing Components
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:page-aliases: batch-aggregator-concept.adoc, filter-records-batch-faq.adoc

You can set up and configure batch scopes to perform common batch processing use cases and to enhance the performance of batch jobs for cases in which the defaults require adjustment.
...
Aggregation is useful for bulk uploading one or more arrays of records to an external server, such as a database server. Within the Batch Aggregator scope, you can add an operation, such as a bulk update, insert, or upsert operation, to load the records in bulk, instead of running an operation separately on each record.
...
=== Tips on Streaming within the Batch Aggregator
