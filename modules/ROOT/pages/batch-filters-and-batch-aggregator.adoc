...
and improves an operation's load. Batch jobs use Mule runtime's thread pools, so
...
* Consider a batch job that needs to process images, and an average image size of 3 MB. You then have 100-record blocks with payloads of 3 MB, being processed in your threads. Hence, your default threading-profile setting would require a large amount of working memory just to keep the blocks in the queue. In this case, set a lower block size to distribute each payload through more jobs and lessen the load on your available memory.
