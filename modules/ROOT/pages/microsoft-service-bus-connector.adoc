...
|*Skip Connectivity Test* |In case you have limited access to Azure Service Bus resources and you want to skip the connectivity test performed at startup (as this works at the root level of your namespace) you need to set this setting to true. For example, you might need to receive or send on a specific queue or topic and your *Shared Access Key* has a security policy that allows to perform limited set of operations only on the specific target resource. For example, if the credentials used to connect to Azure Service Bus only have 'Send' and 'Listen' privileges, by checking this you will be able to send and receive messages but the other operations will not work as you are missing 'Manage' privileges.
