...
+
This behavior is relevant when there are different flows (for example, `out` and `in`). Assume that `out` has a Flow reference (`flow-ref`) to `in` and an exception strategy, but `in` has _no_ exception strategy. If an exception occurs in `in`, the exception handler addresses the exception in `out`, but the payload is `NullPayload`. To address this behavior:
