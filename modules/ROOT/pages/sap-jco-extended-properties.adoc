...
|
|jco.client.snc_mode
|Secure network connection (SNC) mode, 0 image:lightbulb.png[] or 1 image:lightbulb-on.png[lightbulb_on]
|
|
...
|1 force usage of RFC_METADATA_GET in SAP Server, 0 deactivate it. If not set the destination will made initially a remote call to check if RFC_METADATA_GET is available.
|
|
|===
