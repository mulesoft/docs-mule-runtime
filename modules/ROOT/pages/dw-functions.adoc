...
Each DataWeave function in the DataWeave Reference is identified by its function signature.

Basic syntax of a two-parameter function signature:

`function(parameterType,parameterType): returnType`

For example, the signature xref:dw-core-functions-contains.adoc#contains2[contains(String, String): Boolean] for the `contains` function has two parameters, each of which accepts a value of the String type. The function returns a Boolean value.
