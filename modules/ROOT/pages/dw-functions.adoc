...
Each DataWeave function in the DataWeave Reference is identified by its function signature. A function specifies a function name, zero or more parameters of a given type, and a return type. 
...
Some parameters within function signatures are lambdas that take one or more parameters of the form `variable:Type` with a transformation expression (`-> expression`). For example, the lambda `(item: T, index: Number) -> R)` is a parameter of the xref:dw-core-functions-map.adoc[map] function (`map(Array<T>, (item: T, index: Number) -> R): Array<R>`. The lambda accepts a value of any type `T` and a value of type Number, which serve as parameters to the expression `R`.
