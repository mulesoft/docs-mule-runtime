...
Each DataWeave function in the reference is identified by its function signature, such as   `contains(String, String): Boolean` for the `contains` function. This function has two parameters, each of which accepts a value of the String type. The function returns as Boolean value. The parameters in the function signature correspond, _in order_, to its named parameters, in this case, `text` for the first parameter and `toSearch` for the second, as shown in xref:dw-core-functions-contains.adoc#contains2[contains(String, String): Boolean].
