...
. Copy the Authentication Token that displays for use when you configure the PowerShell connector in Anypoint Studio. For more information, see the xref:windows-powershell-connector-guide.adoc[Windows PowerShell Connector Guide.] After you copy the token, click *Install*.
