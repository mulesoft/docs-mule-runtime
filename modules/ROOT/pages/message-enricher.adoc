...
<6> Set Variable creates an inner 'greeting' variable that appends a string to the original 'greeting' variable: `flowVars.greeting + ' World'`.
...
<11> Logger prints a message with the value of the _outer_ payload, `null`. This payload is outside of the scope of Processor Chain.
