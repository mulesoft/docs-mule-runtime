...
You can make the source a namespace by simply populating the field with a xref:3.7@mule-expression-language-mel.adoc[Mule Expression Language MEL] expression, for example with an xpath expression that takes arguments from the Mule message.

You can find information on using a variable with any enricher listed on these pages:

* xref:3.6@non-mel-expressions-configuration-reference.adoc[Default enrichers] are loaded at runtime
