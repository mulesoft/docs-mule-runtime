...
"Oh, " ++ flowVars.greeting ++ " Traveler"]]></dw:set-variable>
	...
	<logger message="Value of payload: [#[payload]]" level="INFO" doc:name="Log Final Payload Value"/> <!--15-->
</flow>
----
<1> HTTP listener triggers the flow from a running application with `localhost:8081/hello`.
...
<4> Message Enricher adds exclamation points to the `greeting` variable by setting the source to `#[flowVars.greeting + '!!!']`.
<5> Components within Processor Chain modify the value of the `greeting` variable and set an arbitrary payload value.
<6> Set Variable creates an inner 'greeting' variable that assigns and modifies the outer 'greeting' variable to `greeting + ' World'`.
<7> Transform Message adds text around the 'greeting' variable: `'Oh, ' + greeting + ' Traveler'`.
<8> Transform Message adds a MIME type for the input payload, which avoids the message `MimeType was not resolved '*/*' delegating to Java.`
<9> Set Payload sets an arbitrary payload value, `some arbitrary value`, which applies only within the context of the enricher.
<10> Logger prints a message with the inner payload: `some arbitrary payload`.
<11> Logger prints a message with the outer payload, `null`.
<12> Logger prints a message with the final `greeting` variable, `Oh, Hello World Traveler!!!`.
<13> Set Payload sets the value of the final `greeting` variable (`Oh, Hello World Traveler!!!`) as the payload.
<14> Logger prints a message with the final payload (`Oh, Hello World Traveler!!!`).
<15> Logger prints a message that includes text around payload value: `"Value of payload: [#[payload]]"`.
