...
The Mule flow in the following example shows how to use a Message Enricher (<enricher/>) to modify content of a Mule variable. It uses Set Variable to set the initial value of the Mule variable called `greeting`. It also shows how to use Set Variable and Transform Message to enrich the value of `greeting` and how to pass the value of the Mule variable to the payload. It also demonstrate differences between the scope of a payload and variable that are defined or modified within the scope of the Processor Chain.
...
<9> Set Payload sets an arbitrary payload value, `some arbitrary value`, which applies within the context of the enricher. Unlike a Mule variable, the scope of this Mule payload is limited to the Processor Chain.
