...
The <<Available Transformers>> section of this page describes all the standard transformers provided with Mule. Additionally, many xref:transports-reference.adoc[transports] and xref:3.7@modules-reference.adoc[modules] have their own transformers, such as the ObjectToJMSMessage transformer for the JMS transport.
...
The basic transformers are in the https://www.mulesoft.org/docs/site/3.5.0/apidocs/org/mule/transformer/simple/package-summary.html[org.mule.transformer.simple] package. They do not require any special configuration. For details on these transformers, see xref:3.7@transformers-configuration-reference.adoc[Transformers Configuration Reference].
...
|ExpressionTransformer |Evaluates one or more expressions on the current message and return the results as an Array. For details, see xref:3.7@mule-expression-language-mel.adoc[Mule Expression Language MEL].
...
https://www.mulesoft.org/docs/site/3.5.0/apidocs/org/mule/module/json/transformers/package-summary.html[org.mule.module.json.transformers] package. They provide the ability to work with JSON documents and bind them automatically to Java objects. For information, see xref:native-support-for-json.adoc[Native Support for JSON].
...
The xref:jms-transport-reference.adoc[JMS Transport Reference] and xref:3.6@mule-wmq-transport-reference.adoc[Mule WMQ Transport Reference] (enterprise only) both provide transformers for converting between JMS messages and several different data types.
