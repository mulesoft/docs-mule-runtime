...
This page serves as a reference for all of the supported attributes and child elements of the xref:http-connector.adoc[HTTP Connector].
...
Required: no +
Default: none
|clientSocketProperties-ref |Reference to a xref:tcp-transport-reference.adoc#client-socket-properties[TCP Client Socket properties element]. +
Type: string +
Required: no +
Default: none
...
[NOTE]
Take into account that the NTLM domain is also case sensitive. Verify that the case is correct to avoid authentication problems.
...
* Learn how to xref:tuning-performance.adoc#connector-level[tune performance at the connector level].
