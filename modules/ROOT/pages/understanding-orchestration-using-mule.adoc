...
* Event processing, where multiple services must be composed simultaneously and must operate simultaneously
...
Use orchestration when you need to combine existing services to meet business requirements. For example, you might use orchestration when you're adding a new employee who needs to be entered in both the payroll system and HR system. You might also use orchestration for order processing. In this scenario, you could use orchestration to do the following:

. Accept an order.
. Make sure the order is in the inventory system.
. Use the credit check system to ensure that you trust the customer's credit.
. Enter the order into the billing system.
. Enter the order into the shipping system.
