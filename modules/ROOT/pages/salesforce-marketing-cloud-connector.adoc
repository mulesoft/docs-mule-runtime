...
This document assumes that you are familiar with Mule, the xref:6@studio::index.adoc[Anypoint Studio interface], xref:global-elements.adoc[Global Elements], xref:6@studio::datasense.adoc[DataSense], and xref:batch-processing.adoc[batch processing] in Mule. Further, it assumes you are familiar with Salesforce Marketing Cloud.
