...
This document assumes that you are familiar with Mule, the xref:5@studio::index.adoc[Anypoint Studio interface], xref:global-elements.adoc[Global Elements], xref:5@studio::datasense.adoc[DataSense], and xref:batch-processing.adoc[batch processing] in Mule. Further, it assumes you are familiar with Salesforce Marketing Cloud.
...
To use the Salesforce Marketing Cloud connector in your Mule application, configure a global Salesforce Marketing Cloud connector element that can be used by all the Salesforce Marketing Cloud connectors in the application (read more about xref:global-elements.adoc[global elements]).
