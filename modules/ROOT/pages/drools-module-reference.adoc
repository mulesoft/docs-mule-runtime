...
[NOTE]
====
Starting in Mule 3.9.1, Drools libraries are no longer bundled with the Mule distribution. Drools 5.0 is the latest supported version. To add the Drools module along with its libraries to 3.9.1 or later, add the Maven dependency to your Mule application's `pom.xml` file. See <<maven, Maven>>. 
====
