...
This page describes how to configure an endpoint. For details on the various attributes and elements you can configure on an endpoint, see xref:3.4@endpoint-configuration-reference.adoc[Endpoint Configuration Reference].

== Basic Configuration

In its most basic form, an endpoint consists of a xref:3.4@connecting-using-transports.adoc[transport] and a transport-specific channel/destination/resource used to identify the channel and location where two flows can exchange information. For example:
...
A transaction can begin or commit when an event is received or sent via an endpoint. The endpoint must be synchronous, and transaction support depends largely on the particular transport being used. For more information see xref:3.4@transaction-management.adoc[Transaction Management].
