...
The correlation id is a unique string identification for each event.
...
The default way the mule runtime generates correlation ids is by generating a java Universally Unique IDentifier (UUID).

As a simple example, we can see the following flow:
...
This is very useful, for example, to understand the entire history of an event that had some problem, unhandled
...
We will get a log like the following:
...
In this case, after performing the request we get the following logs:
...
As stated before, a real scenario in which you might want to modify correlation id could be while processing the results of a database query:
...
In this case, each record obtained from the DB will be processed within the foreach scope, and the correlation id for each execution will correspond to the `orderId` of said record. This way, if there is a problem in HTTP request operation, this allows to easily track which record was responsible for it.

==== Correlation ID Scope and Error Handling

In case of an error, the correlation id within the error handler will be the same to the one corresponding to the beginning of the Flow / Try scope to which this error handler is related to. For example: 
...
In this scenario, the correlation Id logged will be the original one at the start of the flow, since after the error is raised the execution of the correlation id scope is finished. 
...
Now the correaltion id to be logged is the correlation ID set by the correlation id scope (i.e. with the "-EXAMPLE" suffix). This is because the error handling is still executing within the correlation id scope. 
