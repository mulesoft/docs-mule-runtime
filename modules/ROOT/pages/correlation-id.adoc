...
image::mruntime-with-correlation-id-scope.png["A flow showing a correlation ID scope configuration"]
...
image:mruntime-correlation-id-db-example.png["A flow showing a correlation ID database query with components like Select, For Each, and Logger"]
