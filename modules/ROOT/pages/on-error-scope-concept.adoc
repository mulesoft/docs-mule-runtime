...
+
* Matching based on an condition (for more advanced use cases):
+
...
through the XML.
You can also configure notification listeners in your Mule application, so they can react to the notification events.
...
| A matching expression that defines the condition to trigger the execution of this on-error strategy. The expression has to evaluate to a boolean value (`true` or `false`).
