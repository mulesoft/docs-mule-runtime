...
errors a different way, you need to provide an On-Error component configuration
for that error before a component that captures the remaining errors
(identified as `ANY` by the second On-Error component). The following error
handlers are configured to produce the
`{ "MyError": "value was expected to be null" }` for a `VALIDATION:NOT_NULL`
error, rather than the `{ "messageANY" : "Some other error" }`. The string
`value was expected to be null` is the description of the
`VALIDATION:NOT_NULL` error. Such an error occurs if your Mule application
contains a Validation component Is Null that receives a payload that is _not_
`null`.
