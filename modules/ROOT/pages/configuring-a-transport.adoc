...
For example, consider a VM transport configured with `numberOfConcurrentTransactedReceivers` set to `3`. This endpoint launches three processes that concurrently consume messages from the queue and sends them to the flow. |  |No
