...
|createMultipleTransactedReceivers | Only used for inbound endpoints with a configured transaction. Creates multiple concurrent receivers for this connector. Inbound endpoints support having multiple receivers that pull messages concurrently. |false |No
|numberOfConcurrentTransactedReceivers a|If `createMultipleTransactedReceivers` is set to `true`, the number of concurrent receivers that will be launched. +
