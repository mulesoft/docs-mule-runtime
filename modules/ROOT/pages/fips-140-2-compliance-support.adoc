...
KeyStores or TrustStores in Mule Applications are typically formatted as PKCS12 or JKS.
These formats are not FIPS compliant and must be converted to BCFKS format for compliance.

The following steps outline how to convert a JKS keystore to BCFKS format.

NOTE: If the source keystore is instead a PKCS12 keystore, set the parameter `-srcstoretype` to `PKCS12` in the `keytool` command.

. Verify that you are using Java 8 or later and JAVA_HOME is set.
. Download the `bc-fips.jar` file from the https://www.bouncycastle.org/download/bouncy-castle-java-fips/[Bouncy Castle website].
. Use the following example command to convert a keystore to BCFKS format:
...
IMPORTANT: If you are using Java 17 or later, ensure to add the following parameter to the keytool command before the `-importkeystore` parameter:
