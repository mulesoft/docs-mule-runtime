...
The following examples demonstrates how to set up a FIPS 140-2 Java environment.

* <<example1>>
* <<example2>>

[[example1]]
==== Example: Setup Instructions Using Bouncy Castle Security Provider in Java 8

The following example demonstrates how to install and configure Bouncy Castle security provider in Java 8.
...
[[example2]]
==== Example: Setup Instructions Using Bouncy Castle Security Provider in Java 11 and Later

The following example demonstrates how to install and configure Bouncy Castle security provider in Java 11 and later.
...
. If you are using CloudHub, run the `amc_setup` parameter and register Mule runtime with CloudHub. For example, `/amc_setup -H d32c4bde-7184-444a-bd87-7a274623e14f---704247 node10  --fips`, where `d32c4bde-7184-444a-bd87-7a274623e14f---704247` is the key from Anypoint Runtime Manager -> Add server.
. To enable FIPS, update `$MULE_HOME/conf/wrapper.conf`. Run the commands `wrapper.java.additional.<n>=-Dmule.security.model=fips140-2` and `wrapper.java.additional.<n>=-Dmule.cluster.network.encryption.key=someKey`.
...
. Start Mule runtime. FIPS is now enabled. 
