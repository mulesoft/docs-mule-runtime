...
+
image::fips-register-jar-renumber.png["A list of security providers in sequential order"]
+
. Save your changes.
...
* Enabling FIPS at the OS level, such as on Red Hat, isn't supported because it causes cipher suite errors during license validation. If you're running Mule on RHEL with system-wide FIPS mode enabled, consider disabling the automatic FIPS alignment in the Red Hat build of OpenJDK by setting the JVM flags `Djava.security.disableSystemPropertiesFile=true`, `Dcom.redhat.fips=false`, or both. These properties disable the global policy that configures cryptographic behavior in Red Hat's FIPS mode, which may prevent runtime conflicts like cipher suite errors. For more details, see https://docs.redhat.com/en/documentation/red_hat_build_of_openjdk/8/html-single/configuring_red_hat_build_of_openjdk_8_on_rhel_with_fips/index[Red Hat's OpenJDK FIPS configuration guide].
