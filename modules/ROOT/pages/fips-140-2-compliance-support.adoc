...
. Run the `amc_setup` command and register Mule runtime. For example, in *Add Server*, you find the registration token `/amc_setup -H d32c4bde-7184-444a-bd87-7a274623e14f---704247 node10  --fips`, where `d32c4bde-7184-444a-bd87-7a274623e14f---704247` is the key from Anypoint Runtime Manager.
. To enable FIPS, update `$MULE_HOME/conf/wrapper.conf`. Add the properties `wrapper.java.additional.<n>=-Dmule.security.model=fips140-2` and `wrapper.java.additional.<n>=-Dmule.cluster.network.encryption.key=someKey`.
