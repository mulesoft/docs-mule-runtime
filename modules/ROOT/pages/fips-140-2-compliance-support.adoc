...
The Mule 4 Runtime can be configured to run in a FIPS 140-2 certified environment. This includes all Runtime connectors, such as HTTP connector. Note that Mule doesn't run in FIPS security mode by default. There are two requirements:
...
If you are running in FIPS mode with a Mule Runtime in a clustered environment, you should define an encryption key for the cluster. See xref:on-premises::mule-high-availability-ha-clusters.adoc#cluster_fips[Cluster reference] for more insight. 
