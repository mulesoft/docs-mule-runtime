...
[NOTE]
--
By default, Government Cloud is configured for FIPS 140-2, so you don't need to perform the following steps if you're using Government Cloud.

If you're using Runtime Fabric, see xref:runtime-fabric::enable-fips-140-2-compliance.adoc[Enabling FIPS 140-2 Compliance Mode for Runtime Fabric] instead of performing these steps.
--

== Assumptions

This document assumes that you're familiar with https://csrc.nist.gov/files/pubs/fips/140-2/upd2/final/docs/fips1402.pdf[FIPS 140-2], the US government security standard that requires that compliant parties use only cryptographic algorithms and techniques that have been certified by NIST. This document also assumes that you have selected and obtained a https://csrc.nist.gov/projects/cryptographic-module-validation-program/validated-modules[certified security provider].

[[set_up_environment]]
== Setting Up a FIPS 140-2 Java Environment

Mule relies on the Java runtime to provide a FIPS-compliant security module, which is why the first requirement is to have a FIPS 140-2 Java environment properly set up. If you're setting up your system for FIPS compliance for the first time and you haven't already configured a certified security provider, you must first https://csrc.nist.gov/projects/cryptographic-module-validation-program/validated-modules[select and obtain one], then set up your Java environment following the instructions specific to your selected provider.
...
. Verify that you're using Java 8 and `JAVA_HOME` is set.
...
. Verify that you're using Java 11 or later and `JAVA_HOME` is set.
. Download the provider files from https://www.bouncycastle.org/download/bouncy-castle-java-fips/[the BouncyCastle web page].
+
----
Provider:bc-fips-1.0.2.7  TLS: bctls-fips-1.0.24.jar  PKIX: bcpkix-fips-1.0.12.jar
...
. Copy the 3 files downloaded to `/lib/boot` (needed for Mule runtime) and `/mule-agent-plugin/lib` (needed for Runtime Manager agent) folders in Mule runtime.
...
+
When configuring Runtime Manager agent, include the `--fips` flag to enable FIPS compliance. If you're using Government Cloud, also include the `--region us_gov` flag:
+
----
./amc_setup -H <YOUR_TOKEN> <server-name> --fips --region us_gov
----
+
You must pass the `--fips` argument each time you register a Mule runtime in a FIPS-compliant environment.
+
...
+
image::fips-wrapper-conf-edit-modules.png["A code string suggests uncommenting for fips mode"]
+
. Set the approved-only Bouncy Castle property to enforce FIPS compliance. Be sure to replace the `<n>`.
+
image::fips-approved-only.png["A string of code showing Bouncy Castle approved only as true"]
+
. Save your changes.
...
If you're running in FIPS mode with a Mule runtime in a clustered environment, define an encryption key for the cluster. See xref:on-premises::mule-high-availability-ha-clusters.adoc#cluster_fips[Cluster reference] for more insight.

== FIPS 140-2 Compliant Cipher Suites

This list contains supported FIPS 140-2 compliant cipher suites.
...
. Use Java 8 or later and make sure that `JAVA_HOME` is set.
. Download the `bc-fips.jar` file from the https://www.bouncycastle.org/download/bouncy-castle-java-fips/[Bouncy Castle website].
...
* https://csrc.nist.gov/files/pubs/fips/140-2/upd2/final/docs/fips1402annexa.pdf[Approved Cryptographic Algorithms]
