...
These formats aren't FIPS compliant and must be converted to `BCFKS` format for compliance.

To convert a `JKS` keystore to `BCFKS` format, ensure that you: 
...
[IMPORTANT]
If you are using Java 17 or later, add the `-J--add-exports=java.base/sun.security.provider=ALL-UNNAMED` parameter to the keytool command before the `-importkeystore` parameter.
