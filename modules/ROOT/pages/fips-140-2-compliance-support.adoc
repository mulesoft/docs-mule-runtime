...
+
. For java 11 and later, uncomment also the following line:
+
image::fips-wrapper-conf-edit-modules.png["Modules configuration"]
+
. Set approved only bouncy castle property to enforce use of FIPS compliance. Be sure to replace the `<n>`.
+
image::fips-approved-only.png["BC Approved Only Mode"]
+
. Save your changes.
