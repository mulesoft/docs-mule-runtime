...
The Enterprise Integration Patterns are accepted solutions to recurring problems within a given context. They provide a framework for designing, building messaging and integration systems, as well as a common language for teams to use when architecting solutions.
...
|===
|Pattern |Mapping to a Mule Object
|Event Message | Mule works transmiting events from different Application or Processors.
...
|Splitter | xref:for-each-scope-concept.adoc[Foreach Scope], xref:parallel-foreach-scope.adoc[Parallel Foreach Scope] and xref:batch-processing-concept.adoc[Batch].
