...
PowerShell scripts may be executed on a remote Windows machine using the xref:windows-gateway-services-guide.adoc[Windows Gateway Services] and the result processed within Mule.
...
This guide draws on information from the xref:windows-gateway-services-guide.adoc[Windows Gateway Services Guide] and assumes you are using Windows and Anypoint Studio. You can download the Windows version of Studio from the https://www.mulesoft.com/platform/studio[Anypoint Studio site].
...
. Download and install the Windows Gateway Services Software. A link to the software is in the xref:windows-gateway-services-guide.adoc[Windows Gateway Services Guide].
...
Yes, the PowerShell connector passes the script contents to the xref:windows-gateway-services-guide.adoc[Windows Gateway Services] for execution so can be run on any platform.
