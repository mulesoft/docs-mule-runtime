...
=== Redirect the JNA Temporary Directory on Linux

Mule uses Java Native Access (JNA) to load native libraries. JNA unpacks these libraries to a temporary directory and then executes them from that location. On Linux hosts where the default JNA temporary directory is on a filesystem mounted with the `noexec` option, Mule can't execute the native libraries, and library loading fails.

If you run Mule on a host where the JNA temporary directory is mounted with `noexec`, redirect the JNA temporary directory to a path on a filesystem that permits execution, such as `/tmp`. Add this line to the `wrapper.conf` file, replacing `<n>` with an unused sequence number:

[source,text,linenums]
----
wrapper.java.additional.<n>=-Djna.tmpdir=/tmp
----

If `/tmp` is also mounted with `noexec` in your environment, set `jna.tmpdir` to another directory that is mounted without the `noexec` option.

After modifying the `wrapper.conf` file, restart your Mule instance to enable the change.
