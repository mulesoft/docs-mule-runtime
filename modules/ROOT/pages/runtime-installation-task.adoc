...
[NOTE]
====
On Linux hosts, Mule uses Java Native Access (JNA) to load native libraries from a temporary directory. If that directory is mounted with the `noexec` option, library loading fails. Add a line to the `wrapper.conf` file to redirect JNA to a directory that permits execution, replacing `<n>` with an unused sequence number: `wrapper.java.additional.<n>=-Djna.tmpdir=/tmp`.
====
