...
In order to avoid the sequences of the properties in `wrapper-additional.conf` from colliding with they ones you added, you can stop and start Mule (instead of restart), that way the `wrapper-additional.conf` is regenerated taking your new properties into account. Another option is to use `-additionalJavaProperties=numberOfExtraProperties` to have a gap between the last sequence in `wrapper.conf` and the first one in `wrapper-additional.conf` (Mule Enterprise Edition only).
====
