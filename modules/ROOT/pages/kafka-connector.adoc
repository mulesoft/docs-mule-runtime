...
http://mulesoft.github.io/mule-kafka-connector/[Technical Reference and Demo App Download] +
...
*Note:*
Track feature additions, compatibility, limitations and API version updates with each release of the connector using the xref:release-notes::connector/kafka-connector-release-notes.adoc[Connector Release Notes]. Review the connector operations and functionality using the http://mulesoft.github.io/mule-kafka-connector/[Technical Reference] and in https://www.anypoint.mulesoft.com/exchange/?search=kafka[Anypoint Exchange].

MuleSoft maintains this connector under the xref:anypoint-connectors.adoc#connector-categories[_Select_] support policy.


== Prerequisites

This document assumes you are familiar with Mule, xref:anypoint-connectors.adoc[Anypoint Connectors], and xref:6@studio::index.adoc[Anypoint Studio Essentials] To increase your familiarity with Studio, consider completing one or more xref:6@studio::basic-studio-tutorial.adoc[Anypoint Studio Tutorials]. Further, this page assumes that you have a basic understanding of xref:elements-in-a-mule-flow.adoc[Mule Flows] and xref:global-elements.adoc[Global Elements].


=== Hardware and Software Requirements

For hardware and software requirements, see xref:hardware-and-software-requirements.adoc[Mule Hardware and Software Requirements] page.
...
* This global element can be used by all the Kafka connectors in the application (read more about xref:global-elements.adoc[global elements]).
...
In the image above, the placeholder values refer to values saved in a `.properties` file, which exists by default in the `src/main/resources` folder of your project (See xref:configuring-properties.adoc[Configuring Properties]).

For easy maintenance and re-usability of your connector properties, we recommend that you use a `.properties` file. Avoid hardcoding these in the global element if you need to deploy the app to different environments, such as production, development, and QA, where your access credentials might differ. For more, see xref:deploying-to-multiple-environments.adoc[Deploying to Multiple Environments].
...
+
*Important*: Keep in mind that if you are going to use the Kafka Producer with `Reuse Producer` option you should configure the `useTicketCache=false`. Otherwise the connector would need a `kinit` module installed on the machine your application will be running to be able to refresh the kerberos tickets when they expire.
+
...
* Read more about xref:3.8@anypoint-connectors.adoc[Anypoint Connectors].
