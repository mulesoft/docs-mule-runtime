...
[[database_bulk_execute]]
=== Migrating Bulk Execute operation

The `<db:bulk-execute>` operation from the Mule 3 connector is replaced by the new `<db:execute-script>` operation.

It's important to note that even though the Mule 3 version used the word bulk, it was not an actual bulk operation as in the examples above. What it 
would do is execute a bunch of individual statements using the same connection, but it would not create bulk prepared statements. Because of that, the 
operation was renamed to `execute-script` to better describe what it's actually doing.
...
.Mule 4 Example: Using the execute-script operation using an external file
[source,xml,linenums]
----
