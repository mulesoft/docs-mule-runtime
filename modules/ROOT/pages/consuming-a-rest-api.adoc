...
The REST API you are consuming needs to have a http://raml.org[RAML] file that describes the API. If you reference a RAML file that describes your API, then the HTTP connector proactively presents you with smart suggestions. In the RAML file, you can define the following:
...
. From the Mule Palette in Studio, add an HTTP Request operation to your application.
...
+
If the API you are calling requires working with security requirements, involves redirects, or requires specifying an HTTP content-type and encoding header, the HTTP connector supports additional configuration options to manage these details.
...
You can add a property to set this header, as describe in the following section.
...
As security requirements for calling the API, you can configure a global HTTPS connector if your API requires incoming requests via HTTPS, and also set basic authentication credentials.
...
+
. Go to the HTTP Request configuration window. In TLS Configuration, select *Global Reference*, and select the TLS context created before. You can also create the TLS context inline.
