...
|xref:3.x@apikit::apikit-basic-anatomy.adoc#apikit-router-main-flow[APIkit Router] | Based on an API https://raml.org[RAML file], it routes arriving calls to the corresponding flow depending on the resource and method. See xref:apikit::index.adoc[APIkit documentation].
...
|xref:scatter-gather.adoc[Scatter Gather] |Sends a request message to multiple targets concurrently. It collects the responses from all routes, and aggregates them into a single message.
|xref:3.x@apikit::apikit-for-soap.adoc[SOAP Router]| Based on a WSDL file, it routes arriving calls to the corresponding flow depending on the resource and method. See xref:3.x@apikit::apikit-for-soap.adoc[APIkit for SOAP documentation].
...
See xref:creating-custom-routers.adoc[Creating Custom Routers] for more details.
...
xref:choice-flow-control-reference.adoc[Read More]
...
xref:splitter-flow-control-reference.adoc[Read More]
