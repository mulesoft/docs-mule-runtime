...
members of a team, including an assignee named `Fred M`. The following
DataWeave script reads an XLSX file and returns filtered data from it.
...
Using the same DataWeave script, the next example uses writes the results of
the same `filter` expression to a file, `fredBugs.json`. The example is a
