...
. In `<file: write />`, the expression `(payload.name replace " " with("-") ++ "-") ++ (now() as String {format: "uuuuMMddHHmmss"}) ++ ".json"` extracts the value of the `payload.name` from each company object and appends a unique date-time stamp to it. Names of files created by the expression are `Abernathy-Group-20221007120752.json` for the first company and `Yost-and-Sons-20221007120752.json` for the last.
+
