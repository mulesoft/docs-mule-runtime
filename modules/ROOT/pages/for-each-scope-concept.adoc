...
Within the scope, each element or batch of elements is treated as a separate `payload`. Instead of outputting processed input, the scope outputs the entire Mule event that it received as input, along with any additional variables created within the scope. The input event includes the un-split payload, attributes of the payload, and any variables associated with the event. By default, the scope stores that event in a variable (`rootMessage`) that you can access from within For Each. Outside of the scope, `vars.rootMessage` returns `null`. After all For Each processing is complete, continue using the typical variables (`payload`, `attributes`, `vars`) to access the parts of the event from outside For Each.

The scope's splitter ignores attributes of the input. Attributes are not accessible from the split elements, so `attributes` returns `null` from within For Each. However, you can use `vars.rootMessage.attributes` to access un-split attributes from within For Each.
...
----
. As For Each iterates over each object, the DataWeave expression (`payload.company`) in the xref:transform-component-about.adoc[Transform Message] component (`<ee:transform />`) uses DataWeave selectors to extract the value of each `"company"` key and transform the `payload` to the value each of those keys. As in a Mule flow, the next component within For Each receives the transformed payload as its input.
...
For configurable properties, see <<xml_reference>>.

== Variable Propagation

Each execution within For Each begins with the values of variables
from the previous execution. New variables or modifications to the values of
existing variables that take place when processing one element are accessible during the processing other elements. Changes to variables continue to be available outside the For Each scope.
...
As the xref:choice-router-concept.adoc[Choice router] (`<choice />`) processes each condition, the first logger (`Logger After Choice`) prints the following variable values:
...
The last values are propagated outside of For Each. Notice that the next logger (`Logger After For Each`) prints the same values as the ones printed after the final choice condition (`otherwise`).
...
For Each scopes provides a number of configurable properties you can configure within the `<foreach/>` tag. As shown in the <<example, example>>, processors within For Each are children of this tag.
...
<flow name="example-for-each-flow"  >
  ...
  <foreach doc:name="For Each" collection="payload.company">
    ...
    <!-- 4: File Write -->
    <file:write doc:name="Write"
          path='#[(payload.name ++ "_" ++ (now() as String  {format: "uuuuMMddHHmmss"}) ++ ".json")]'
          config-ref="File_Config" mode="CREATE_NEW">
    </file:write>
    <!-- 5: Logger 3 -->
...
After an HTTP request (`<http:request />`), the remaining components are configured to illustrate how some of the For Each properties work and how processing of the `payload` and `attributes` takes place within and outside of For Each:

. For Each collects the company objects (`collection="payload.company"`) within the JSON array returned from the HTTP request's `url`.
...
. In Logger 2, `sharedStatusCode : vars.rootMessage.attributes.statusCode` retrieves the value of the status code attribute that is stored in the `rootMessage` variable: `{sharedStatusCode=200}`. The status code comes from the original input to For Each and is not included in the split payload, so each iteration makes the Logger print the same status code.
. In `<file: write />`, `(payload.name ++ "_" ++ (now() as String  {format: "uuuuMMddHHmmss"}) ++ ".json")` extracts the value of the `payload.name` from the company object produced during the For Each splitting process with `collection="payload.company"` and appends a unique date-time stamp to it. Names of files created by expression are `Abernathy Group_20221007120752.json` for the first company and `Yost and Sons_20221007120752.json` for the last.
+
...
The next example illustrates the batch size (`batchsize`) functionality. Assume that flow collect the same input payload from the preceding flow through a Flow Reference (`<flow ref />`) component:
