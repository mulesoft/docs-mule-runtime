...
1. Error handling: since the first mentioned difference, For Each execution will be interrupted when an error is raised (and the Error Handler will be invoked), while Split Aggregate will process each route and then invoke the xref:intro-error-handlers.adoc[Error Handler], with the aggregation of the results and erros of all the routes.
