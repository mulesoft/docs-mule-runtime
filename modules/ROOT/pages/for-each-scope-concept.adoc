...
There is an obvious similarity between the For Each and the xref:split-aggregate-scope.adoc[Split Aggregate scope]: both split the defined collection and process each element in it with the components within the scope. Also, in both cases, each route runs with the same initial context.
The difference with these two scopes are:

1. For Each works sequentially, while the Split Aggregate does it in parallel.
1. The For Each does not modify the payload, while the Split Aggreate will output a collection of the output messages of each iteration.
1. Error handling: since the first mentioned difference, For Each execution will be interrupted when an error is raised (and the Error Handler will be invoke), while Split Aggregate will process each route and then invoke the xref:intro-error-handlers.adoc[Error Handler], with the aggregation of the results and erros of all the routes.
