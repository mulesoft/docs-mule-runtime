...
Every execution of the For Each scope starts with the variables and values from the previous execution of the block. New variables or modifications to existing variables that take place when processing one element are visible during the processing of another element. However, these changes to variables will not be available outside the For Each scope. In other words, the set of variables (and their values) after the execution of the For Each Scope remain the same as before its execution.
