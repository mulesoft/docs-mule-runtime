...
Each item in the collection is stored in the payload when processed through the For Each Scope. For example, in the scenario detailed above each split item is stored in `payload.topic` during each iteration.
////
