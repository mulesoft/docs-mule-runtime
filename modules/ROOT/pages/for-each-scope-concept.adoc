...
Unlike attributes, Mule variables are accessible and modifiable within For Each. See <<variable_propagation, Variable Propagation>> for details.
...
For Each scopes provides a number of properties you can configure within the `<foreach/>` tag. As shown in the <<example, example>>, processors within For Each are children of this tag.
...
| `batchSize`
| `1`
| Number that the scope uses to partition the collected input into arrays of a specified size. For example, if input to For Each is an array of 199 elements and you set the batch size to 50, the scope produces 3 arrays of 50 elements each and 1 array of 49 elements.
