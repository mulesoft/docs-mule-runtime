...
* Error handling: Because of the first-mentioned difference, For Each execution gets interrupted when an error is raised (and the Error Handler will be invoked), while Split Aggregate processes each route and then invoke the xref:intro-error-handlers.adoc[Error Handler] with the aggregation of the results and errors of all the routes.
