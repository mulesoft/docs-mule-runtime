...
Every execution of the ForEach scope will start with the variables and values from the previous execution of the block. New variables or modifications of already existing ones while processing one element will be visible while processing another element. However, all of those variable changes will not be avaiable outside the ForEach scope. In other words, the set of variables (and their values) after the execution of the ForEach Scope will remain the same as before its execution.
...
None of the modifications are propagated outside the ForEach Scope, including the creation of new variables.
