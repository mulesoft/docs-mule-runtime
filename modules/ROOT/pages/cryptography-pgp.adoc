...
A key can contain subkeys, according to the RFC-4880 standard specification. When working with subkeys, use the _fingerprint_ property to specify the key to use.

For example, if you use different keys for signing and encrypting operations, and each of these keys use different algorithms like _DSA_ and _ElGamal_, then you must reference each key's _fingerprint_ depending on the operation you want to perform.

In this case you reference the _ElGamal_ fingerprint to encrypt your messages, and the _DSA_ fingerprint to sign your messages.
...
| Principal a| String | A combination of name and email specified while generating the key. When you use this field you will be using the primary key.
Do not use this field when you are using keys with different algorithms to sign and encrypt (e.g: DSA and Elgamal), use _fingerprint_ instead to specify the proper key. |  |
|===
