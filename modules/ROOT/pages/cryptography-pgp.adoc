...
+++
Encrypt a stream using PGP, giving an ASCII-armored stream output as a result. The encryption is done with the public key of the recipient, so the private keyring and secret passphrase are not required.
+++
