...
== Key structures
According with https://tools.ietf.org/html/rfc4880#section-12[RFC-4880] a key can contains many subkeys.
In these cases, use *_fingerprint_* property to refer the correct key.

For example, if you are using keys with different algorithms for signing and encrypting like _DSA_ and _ElGamal_ both keys will have their own *fingerprint*. Reference the _ElGamal_ fingerprint to encrypt your messages and _DSA_ fingerprint to sign your messages.
...
This field is not recommended when you are using keys with different algorithms to signing and encrypting (e.g: DSA and Elgamal).
To prevent mistakes selecting the right key use _fingerprint_ field instead of _principal_ field. |  |
|===
