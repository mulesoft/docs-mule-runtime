...
In addition to encrypting, you can atomically encrypt and sign a message, which returns a message (in ASCII-armored format) similar to the encrypted one. However, the returned message also has a signature inside its encrypted contents. The signature provides an integrity check of the original message. 
...
You can encrypt and sign a stream using PGP, producing an ASCII-armored stream output as a result. The encryption requires the public key of the recipient, so the secret passphrase is not required. The signing is done with the private key of the signer (usually the sender), so the secret passphrase is required.
