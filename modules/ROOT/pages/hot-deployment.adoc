...
* Jar custom classes (if any), and put them under `$MULE_HOME/apps/foo/META-INF/mule-src/foo/src/main/java/`
...
As a bonus, application's master config file is monitored, so if there are any class changes you want to pick up or simply modify the config, save or touch any of the mule configuration files and Mule will hot-reload the application.
