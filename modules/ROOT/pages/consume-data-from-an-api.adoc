...
The REST API you are consuming must have a https://raml.org/[RAML] file that describes the API, so that the HTTP connector proactively presents you with smart suggestions. +
...
image::mruntime-consume-rest-api-configuration-1.png["An HTTP Request configuration with Host and Port"]
...
image::mruntime-consume-rest-api-configuration-2.png["An HTTP Request GET Method and Path"]
...
image::mruntime-consume-rest-api-configuration-3.png["A configure Dynamic Requests with DataWeave expressions in Path field"]
...
image::mruntime-consume-rest-api-configuration-4.png["A custom header configuration in Studio"]
...
. Click *OK*.
+
image::mruntime-consume-rest-api-configuration-7.png["An HTTP Request showing settings for connection timeout, response buffer size, and TLS configuration"]
...
image::mruntime-consume-rest-api-configuration-5.png["An HTTP Request showing settings for protocol, host, port, and persistent connections configuration"]
...
.Configure TLS.
image::mruntime-consume-rest-api-configuration-6.png["A TLS configuration showing settings for trust store and key store configurations"]
...
image::mruntime-consume-rest-api-configuration-8.png["An HTTP Request configuration including TLS and proxy details"]
