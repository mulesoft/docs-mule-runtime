...
=== Configuration HTTP Request Operation in Anypoint Studio
...
=== Configuration HTTP Request Operation in XML
...
Most use cases require that the call to the API change dynamically based on data in the message. You can configure these dynamic requests by using DataWeave expressions. +
...
For example, if the API you are consuming requires a developer key, then specify that key key as a header on your requests by using the header name `accessKey`. +
...
* Click *Add*.
* Define *Header Name* and its value. Both can be defined using expressions or properties placeholders. +
...
. In the Key Store Configuration section, select a type of key store configuration from the drop-down menu: for example, *JKS* (Java Key Store) which is the default type.
