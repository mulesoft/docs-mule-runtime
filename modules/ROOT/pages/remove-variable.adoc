...
| Name
| `variableName`
| Name of the variable to remove. If the variable does not exist in the flow, the component logs a warning (`WARN`). Attempts to access the value of a variable that does not exist return a `null` value. See the <<example>>.
|===
