...
. The Mule URL endpoint: any valid xref:3.6@mule-endpoint-uris.adoc[Mule Endpoint] used to determine the transport, endpoint, and other information about delivery of the message. This can also be an endpoint name stored in configuration.
...
For SOAP support, you can use the Mule xref:3.6@cxf-module-reference.adoc[CXF Module Reference].
...
If you do not set the wire format, the serialization format is used. For more information on transformers, see xref:3.6@using-transformers.adoc[Using Transformers].
