...
This topic provides details on the elements to configure for notifications.
...
|===
|Name |Description
|`dynamic` |If the notification manager is dynamic, you can set up the listeners to be registered dynamically at runtime. Otherwise, some parts of Mule will cache the notification configurations for efficiency and will not generate events for newly-enabled notifications or listeners.
//See https://docs.mulesoft.com/mule-runtime/3.9/mule-server-notifications#registering-listeners-programmatically for more information.// +
*Type*: `boolean` +
*Required*: no +
*Default*: `false`
|===
...
Registers a bean as a listener with the notification system. Events are dispatched by reflection. The listener receives all events associated with any interface it implements.
