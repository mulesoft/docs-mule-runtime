...
The following DataWeave script reads the input payload phone numbers, and uses the `filter` function to search the country code in the variable `country_code` that match with the calling code. Subsequently, uses the `map` function to get the country code, and returns the results in an array of JSON objects:
