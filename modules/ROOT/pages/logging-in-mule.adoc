...
Mule uses http:ç, which is a logging facade that discovers and uses a logging strategy from the classpath, such as https://logging.apache.org/log4j/2.x/[log4j 2] or the JDK Logger. By default, Mule includes Log4j 2, which is configured with a file called `log4j2.xml`.
...
For other Log4j 2 appender configurations, see https://logging.apache.org/log4j/2.x/manual/appenders.html.
...
Set the `log4j2.xml` at the root of your classpath. For more information about configuring Log4j 2, see https://logging.apache.org/log4j/2.x/[Apache Log4j 2].
