...
[NOTE]
Mule 4 introduced thread-local functionality to optimize the memory usage of log4j. To enhance the use of this feature, you can set the system property `AsyncLoggerConfig.RingBufferSize` in a xref:configuring-properties.adoc[configuration properties] file. `20000` is the suggested value, but you need to tune it to meet your needs.
