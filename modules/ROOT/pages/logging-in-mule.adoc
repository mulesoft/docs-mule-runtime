...
Mule uses http:ç, which is a logging facade that discovers and uses a logging strategy from the classpath, such as https://logging.apache.org/log4j/2.x/[log4j2] or the JDK Logger. By default, Mule includes log4j2, which is configured with a file called `log4j2.xml`.
...
When asynchronous logging is used and there is a system crash, some actions may not be logged. This occurs because log writing is performed on a separate thread that runs independently of other actions. See the <<Exception Handling With Asynchronous Logging>> to see how to mitigate these issues.
...
To log the Runtime Manager Agent state in a location other than the default `mule_agent.log` file, configure the `$MULE_HOME/conf/log4j2.xml` file to include a new Log4j2 Appender called `mule-agent-appender`. If included, the Runtime Manager Agent plugin uses this appender to log its state.
...
Mule runtime uses log4j2 as the framework for logging. To add or change the monitoring interval at which log4j checks for changes, add the `monitorInterval` to the `log4j2.xml` configuration file:
...
----
+
In this example, log4j2 checks for changes every 15 seconds.
+
. Save the file, and restart the Mule server.

For more information, see "Automatic Reconfiguration" in  https://logging.apache.org/log4j/2.x/manual/configuration.html[Log4j Configuration].
