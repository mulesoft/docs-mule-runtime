...
The minimum supported Mule runtime engine version is 4.4.x to use Cryptography Module 2.0.0 and make sure compatibility with FIPS compliance and supported runtime environments.  Upgrade your Mule runtime if you are using an older version.
...
After upgrading Cryptography Module, be aware that the module no longer bundles security providers.
If your environment already has the security providers (such as those in the MuleSoft Government Cloud environment), you don't need to do any additional configuration. The existing environment will provide ‌the security providers.
...
If you don’t add these dependencies, you may encounter runtime errors such as:
...
If you need to revert to the previous version of Cryptography Module, go to Anypoint Studio and do the following:
...
== Changes to Default Values for Connector Operations
To improve the security posture of the Cryptography Module, many default values for operations and configuration parameters have been updated in version 2.0.0. These changes reflect the adoption of more secure algorithm stacks and modern cryptographic practices.

When migrating from a previous version, declare the default values you were relying on in version 1.x. This ensures compatibility and avoids unexpected results due to default value changes in the new version.
...
[NOTE]
Create a backup of your keyrings and keys in case you need to perform a rollback.
...
. Create your PGP keys for signing or validating by running this command and follow the instructions returned. (Select RSA as your cipher, because DSA isn't FIPS-compliant.)
...
In GPG 2.1 and later, `gpg` is not directly responsible for encrypting secret keys at rest and options such as `--s2k-cipher-algo` have no effect. Instead, `gpg-agent` is solely responsible and it provides no interface to change the cipher or digest algorithms in use.  For details, refer to https://www.google.com/url?q=https://dev.gnupg.org/T1800&sa=D&source=docs&ust=1748032101600550&usg=AOvVaw1WU8BTGl-579opeNkbOlL0[Allow s2k options for gpg --export-secret-key] and  https://www.reddit.com/r/GnuPG/comments/1dj4vo8/s2k_do_not_work_help/?rdt=57819[S2K do not work HELP]

In practice:

* S2K parameters passed on the command line or stored in `gpg.conf` are not applied  when generating or editing keys in GPG 2.1+.
...
If your goal is to produce secret keys protected with specific algorithms, for example AES256 and SHA256, use Java and Bouncy Castle to generate them explicitly.
...
When operating in a FIPS-enabled environment, password-based encryption (PBE) algorithms starting with PBEWith are no longer supported due to security restrictions. For example:
...
You must define all four components. Defaults aren't assumed. If any component is missing or malformed, ‌the validation fails.
