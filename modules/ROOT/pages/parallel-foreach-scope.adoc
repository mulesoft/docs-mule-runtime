...
Every route starts with the same initial variable values. Modification to a variable within a specific route does not affect other routes. The set of variables (and their values) will stay the same as before the execution of the Parllel ForEach. 
