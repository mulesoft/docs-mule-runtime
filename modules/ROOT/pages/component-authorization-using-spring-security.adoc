...
* xref:3.4@component-authorization-using-spring-security.adoc[Securing Flow Components]
* xref:3.4@component-authorization-using-spring-security.adoc[Setting Security Properties on the Security Provider]
...
In addition to the `daoAuthenticationProvider` and `inMemoryDaoImpl` beans (see xref:3.4@configuring-security.adoc[Configuring Security]), the following beans must be configured:
