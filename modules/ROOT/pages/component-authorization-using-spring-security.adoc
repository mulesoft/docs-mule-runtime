...
In addition to the `daoAuthenticationProvider` and `inMemoryDaoImpl` beans (see xref:3.7@configuring-security.adoc[Configuring Security]), the following beans must be configured:
