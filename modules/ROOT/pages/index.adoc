...
Mule runtimes or the infrastructure on which your apps run. You use Runtime
Manager to deploy Mule apps to CloudHub, select the Mule runtime version,
set the number of vCores needed to run the app, and so on.
* Hybrid deployment models manage Mule apps and runtimes from the Cloud while
running them in a datacenter that is managed by your company. There are a couple
...
case, you provision the infrastructure in which the apps run. To guarantee
...
solution, xref:1.2@runtime-fabric::index.adoc[Runtime Fabric], which runs Mule
