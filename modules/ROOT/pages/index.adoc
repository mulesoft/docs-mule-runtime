...
xref:runtime-manager::index.adoc[Runtime Manager] in a number of deployment
models for Mule apps:

* xref:runtime-manager::cloudhub.adoc[CloudHub] is a fully managed Cloud PaaS
for Anypoint Platform that enables you to run your Mule apps without providing
Mule runtimes or infrastructure on which your apps run.
* Hybrid deployment models manage Mule apps and Mule runtime engines from the
Cloud while running them in a datacenter that is managed by your company. There
are a couple of hybrid deployment models:
...
Mule apps through in the Cloud, though Runtime Manager. In this model, you
need to provide the infrastructure and Mule runtime. See
xref:mule-standalone.adoc[Mule Installation].
** For a hybrid PaaS deployment, you set up and run the PaaS on your company's
datacenter and use Runtime Manager to run the Mule apps within the PaaS. In
this case, you provision the infrastructure in which the apps run. To guarantee
high availability of Mule apps, you use Runtime Manager to handle Mule runtimes.
xref:1.6@anypoint-platform-pcf::index.adoc[Anypoint Platform for PCF] is an
example of a hybrid PaaS solution. MuleSoft also provides
xref:1.2@runtime-fabric::index.adoc[Runtime Fabric], which runs Mule in
