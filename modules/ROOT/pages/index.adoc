...
//TODO? THIS BEGS FOR A DIAGRAM TO SHOW THE RELATIONSHIPS DESCRIBED HERE

For a Mule app to run, it must be deployed into an environment where the Mule
is installed.

Mule is packaged with the xref:7.3@studio::index.adoc[Studio] IDE and with
xref:design-center::index.adoc[Design Center] on Anypoint Platform so
that you can run a Mule app as you design it.

Mule is integrated into xref:runtime-manager::cloudhub.adoc[CloudHub], a
MuleSoft environment where you can deploy and run your Mule apps in a
production or pre-production environment, typically using Runtime Manager
to deploy the app and allocate resources to it.

In addition to the CloudHub environment, MuleSoft also supports deployments
to Pivotal Cloud Foundry through
xref:1.6@anypoint-platform-pcf::index.adoc[Anypoint Platform for PCF], and
the MuleSoft product xref:1.2@runtime-fabric::index.adoc[Runtime Fabric]
supports deployments within "containerized" environments to AWS, Azure,
virtual machines (VMs), and bare-metal servers.

On-premise installations are also supported. You can install Mule to servers
on your own hardware, rather than CloudHub, PCF, or some other Cloud-based
server. When Mule is installed on its own, and not bundled into another
Mule product, it is sometimes called a standalone or "naked" Mule installation.
See xref:mule-standalone.adoc[Mule Installation].

xref:runtime-manager::index.adoc[Runtime Manager] is an online
interface for deploying and managing Mule apps. Runtime Manager is available
from Anypoint Platform for CloudHub deployments. It is also available through
Anypoint Platform Private Cloud for on-premise deployments and
to Runtime Fabric and Anypoint Platform for PCF.

To perform deployments and manage Mule apps without Runtime Manager, you can
use xref:runtime-manager::anypoint-platform-cli-commands.adoc[Anypoint CLI 3.x],
...
bundled with and share the same version as Mule.
...
Mule 4.x runtimes, while DataWeave 2.0 and Studio 6.x are compatible with
Mule 3.x runtimes. Connectors and modules are either compatible with Mule 4.x
or Mule 3.x, but not both.
...
Mule Runtime includes an embedded API Gateway, which enables you to apply a
basic authentication policy on top of a Mule app, enrich an incoming or
outgoing message, add capabilities to an API without having to write any code.
See xref:2.x@runtime-agw-landing-page.adoc[API Gateway] for the Mule runtime
engine in the API Manager documentation.
