...
events, and schedulers for internally triggered events. This behavior reflects 
Mule's event-driven architecture (EDA).
...
are available.
+
* Core components. These components can route data, perform data
transformations, handle errors that might occur when processing the event, 
and perform other tasks in a Mule app.
+
...
data in the Mule event to a format that another app or service can read.
+
