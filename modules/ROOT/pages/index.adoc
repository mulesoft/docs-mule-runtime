...
resources. Mule supports a variety of components called Event sources, 
including listeners for external events, and schedulers for internally 
triggered events. This behavior reflects Mule's event-driven architecture 
(EDA).

* Mule apps accept and process events as messages through Event processors.
Mule apps can also handle high volume of messages in batches through Batch 
...
and forwards that event on for processing by next component in the flow.
...
Mule Event processors can indicate to the runtime whether they are CPU intensive,
