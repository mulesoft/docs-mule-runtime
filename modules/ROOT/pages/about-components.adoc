...
Mule flow. The xref:scheduler-concept.adoc[Scheduler] serves as an endpoint. It triggers a flow to start at a configurable interval.
...
A Flow Control takes the input event and routes it to one or more separate sequences of components. Flow Control components (or Routers) include:
...
DataWeave logic to pick one of several routes, where each route is a separate sequence of event processors.

A xref:scatter-gather-concept.adoc[Scatter-Gather router] router copies the
...
A Scope is a type of component that groups together a sequence of event processors (modules, components, or connectors) to apply some programming behavior to that isolated sequence of event processors.
