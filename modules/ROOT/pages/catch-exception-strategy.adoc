...
[NOTE]
You can define _only one_ exception for each flow. If you need to design a more complex error handling strategy that involves more than one way of handling exceptions, consider using a xref:choice-exception-strategy.adoc[Choice Exception Strategy]
...
|*when* a|A MEL expression to indicate the kind of exception the catch exception strategy handles when it is embedded within a xref:choice-exception-strategy.adoc[Choice Exception Strategy].
