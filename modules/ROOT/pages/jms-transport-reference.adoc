...
* Supports local (JMS), distributed (XA), and multi-resource xref:transaction-management.adoc[transactions] *Enterprise*
...
Mule Enterprise includes an xref:mule-wmq-transport-reference.adoc[enhanced transport for WebSphere MQ] which is recommended if you are using WebSphere MQ as your JMS provider.
====
...
For performance it is important to use the "Caching Connection Strategy" between you JMS Connector and the actual JMS ConnectionFactory implementation. For more information, see xref:jms-transport-reference.adoc[Caching connection factory] below.
====
