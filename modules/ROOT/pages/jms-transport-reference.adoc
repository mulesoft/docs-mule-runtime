...
* Mule 3.8 and newer supports non-blocking request-response when the non-blocking processing strategy is used. For more information on setting the strategy, see xref:flow-processing-strategies.adoc[Flow Processing Strategies].
...
* Supports local (JMS), distributed (XA), and multi-resource xref:transaction-management.adoc[transactions]  *_Enterprise_*
