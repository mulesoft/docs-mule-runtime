...
If you are upgrading from Mule 4.1.x or Mule 4.2.x you can use the DEDICATED strategy so that any customizations applied in the `scheduler-conf.properties` file can still be used. However, MuleSoft still recommends to try the new default setting first and then determine whether the optimizations are still required. Also, try first to customize the new UBER strategy before falling back to the legacy mode.
