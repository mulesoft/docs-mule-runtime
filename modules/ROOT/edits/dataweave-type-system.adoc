...
For example, the `repeat` function in the String module has the signature `repeat(String, Number): String`, which means that the function only accepts as a first parameter a value of type `String`, and as a second parameter a value of type `Number.` The function returns a value of type `String`.
...
The script assigns to the first parameter a value that is the type `Null` (the type of this value
is `Type&lt;Null&gt;`), and it returns `&quot;NullNullNull&quot;` because there is an autocoercion that coerces the value to the type `String` (`&quot;Null&quot;`).

Some other functions have an overloaded signature for the function (the function has multiples definitions for different parameter types) that let you call it with `null`.

For example, the `isNumeric` function from `dw::core::Strings` has the signature `isNumeric(String): Boolean` (receives a string and successfully returns a result if that string is numeric), but the function is also defined as `isNumeric(Null): Boolean`. +
...
A composite types contains other values. The composite types are `Array`, `Object` and `Function`.
...
The variable `idsList` only passes the type-checking phase when the array that is assigned to it only contains values of the `Number` type.
...
The syntax to define an `Object` type with a set of constraints for its key-value pairs is very similar to defining a value of an `Object` type:
...
You can apply some modifiers to each key-value pair to determine if a field is always present or if the field can be repeated.
...
A closed object means that the type only accepts an object value if there are no additional fields in the object,
just those key-value pairs specified by the type.
...
DataWeave is a functional language, and functions are first class citizens, which means that functions are values,
...
For example, you can define a function that receives another function as an argument:
...
In some cases, you cannot enforce a restriction because the type accepts all the values. +
...
In the following example, the `Intersection` concatenates the two `Object` types, resulting in the type `{name: String, lastName: String}`. The type is an open object that can have more key-value pairs. The variable accepts the value assigned to it:

----
var a: {name: String} & {lastName: String} = {name: "John", lastName: "Smith", age: 34}
----

In the case of closed objects, it returns the concatenation of the object types but resuls in a closed object. +
...
In the following example, the variable only accepts a value of the literal types `404` and `500`. The type system ensures that the variable is one of those literal values:
